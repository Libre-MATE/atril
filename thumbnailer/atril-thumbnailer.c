...
static const GOptionEntry goption_options[] = {
    ...
    {"no-limit", 'l', G_OPTION_FLAG_REVERSE, G_OPTION_ARG_NONE, &time_limit,
...
G_GNUC_NORETURN static gpointer time_monitor(gpointer data) {
  ...
  if (finished) g_thread_exit(NULL);
...
static void time_monitor_stop(void) { finished = TRUE; }
...
static EvDocument *atril_thumbnailer_get_document(GFile *file) {
  ...
  if (!g_file_is_native(file)) {
    ...
    g_file_copy(file, tmp_file, G_FILE_COPY_OVERWRITE, NULL, NULL, NULL,
                &error);
    if (error) {
      g_printerr("Error loading remote document: %s\n", error->message);
      g_error_free(error);
  ...
  if (tmp_file) {
    if (document) {
      g_object_weak_ref(G_OBJECT(document), (GWeakNotify)delete_temp_file,
                        tmp_file);
    } else {
...
static gboolean atril_thumbnail_pngenc_get(EvDocument *document,
                                           const char *thumbnail, int size) {
  EvRenderContext *rc;
  double width, height;
  GdkPixbuf *pixbuf;
  EvPage *page;
...
static gpointer atril_thumbnail_pngenc_get_async(struct AsyncData *data) {
  ev_document_doc_mutex_lock();
  data->success =
      atril_thumbnail_pngenc_get(data->document, data->output, data->size);
...
int main(int argc, char *argv[]) {
  ...
  if (!g_option_context_parse(context, &argc, &argv, &error)) {
    ...
    if (file_arguments) g_strfreev(file_arguments);
  ...
  if (time_limit) time_monitor_start(input);
  ...
  if (EV_IS_ASYNC_RENDERER(document)) {
    ...
    g_thread_new("EvThumbnailerAsyncRenderer",
                 (GThreadFunc)atril_thumbnail_pngenc_get_async, &data);
