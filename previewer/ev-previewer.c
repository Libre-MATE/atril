...
static const GOptionEntry goption_options[] = {
    ...
    {"print-settings", 'p', 0, G_OPTION_ARG_FILENAME, &print_settings,
     N_("Print settings file"), N_("FILE")},
...
static void ev_previewer_load_document(const gchar *filename,
  ...
  g_signal_connect(job, "finished", G_CALLBACK(ev_previewer_load_job_finished),
                   model);
...
gint main(gint argc, gchar **argv) {
  ...
  ev_previewer_window_set_print_settings(EV_PREVIEWER_WINDOW(window),
                                         print_settings);
  ...
  if (unlink_temp_file) ev_previewer_unlink_tempfile(filename);
  if (print_settings) ev_previewer_unlink_tempfile(print_settings);
