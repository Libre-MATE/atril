...
G_DEFINE_TYPE_WITH_CODE(
    EvImageAccessible, ev_image_accessible, ATK_TYPE_OBJECT,
    G_ADD_PRIVATE(EvImageAccessible)
        G_IMPLEMENT_INTERFACE(ATK_TYPE_COMPONENT,
                              ev_image_accessible_component_iface_init)
            G_IMPLEMENT_INTERFACE(ATK_TYPE_IMAGE,
                                  ev_image_accessible_image_iface_init))

static void ev_image_accessible_get_atk_rect(AtkObject *atk_object,
  ...
  _transform_doc_rect_to_atk_rect(view_accessible, page, &image->priv->area,
                                  atk_rect, coord_type);
}
...
static void ev_image_accessible_get_extents(AtkComponent *atk_component,
  ...
  ev_image_accessible_get_atk_rect(ATK_OBJECT(atk_component), coord_type,
                                   &atk_rect);
  *x = atk_rect.x1;
  *y = atk_rect.y1;
...
static void ev_image_accessible_get_image_size(AtkImage *atk_image, int *width,
                                               int *height) {
  EvRectangle atk_rect;

  ev_image_accessible_get_atk_rect(ATK_OBJECT(atk_image), ATK_XY_WINDOW,
                                   &atk_rect);
  *width = atk_rect.x2 - atk_rect.x1;
  *height = atk_rect.y2 - atk_rect.y1;
}

static void ev_image_accessible_get_image_position(AtkImage *atk_image, gint *x,
                                                   gint *y,
                                                   AtkCoordType coord_type) {
  EvRectangle atk_rect;
...
static AtkStateSet *ev_image_accessible_ref_state_set(AtkObject *atk_object) {
  ...
  state_set = ATK_OBJECT_CLASS(ev_image_accessible_parent_class)
...
EvImageAccessible *ev_image_accessible_new(EvPageAccessible *page,
                                           EvImage *image, EvRectangle *area) {
