...
#define EV_PAGE_DATA_FLAGS_DEFAULT                                  \
  (EV_PAGE_DATA_INCLUDE_LINKS | EV_PAGE_DATA_INCLUDE_TEXT_MAPPING | \
   EV_PAGE_DATA_INCLUDE_IMAGES | EV_PAGE_DATA_INCLUDE_FORMS |       \
    ...
    for (i = 0; i < cache->n_pages; i++) {
      ...
      if (data->job) {
        g_signal_handlers_disconnect_by_func(
            data->job, G_CALLBACK(job_page_data_finished_cb), cache);
        g_signal_handlers_disconnect_by_func(
            data->job, G_CALLBACK(job_page_data_cancelled_cb), data);
      }
...
static EvJobPageDataFlags ev_page_cache_get_flags_for_data(
    EvPageCache *cache, EvPageCacheData *data) {
  EvJobPageDataFlags flags = EV_PAGE_DATA_INCLUDE_NONE;

  if (data->flags == cache->flags && !data->dirty) return cache->flags;

  /* Flags changed or data is dirty */
  if (cache->flags & EV_PAGE_DATA_INCLUDE_LINKS) {
    flags = (data->link_mapping) ? flags & ~EV_PAGE_DATA_INCLUDE_LINKS
  ...
  if (cache->flags & EV_PAGE_DATA_INCLUDE_TEXT_ATTRS) {
    flags = (data->text_attrs) ? flags & ~EV_PAGE_DATA_INCLUDE_TEXT_ATTRS
...
static void job_page_data_finished_cb(EvJob *job, EvPageCache *cache) {
  ...
  if (job_data->flags & EV_PAGE_DATA_INCLUDE_TEXT) data->text = job_data->text;
...
static void ev_page_cache_schedule_job_if_needed(EvPageCache *cache,
                                                 gint page) {
  ...
  if (data->job) ev_job_cancel(data->job);
  ...
  g_signal_connect(data->job, "finished", G_CALLBACK(job_page_data_finished_cb),
                   cache);
  g_signal_connect(data->job, "cancelled",
                   G_CALLBACK(job_page_data_cancelled_cb), data);
...
void ev_page_cache_set_page_range(EvPageCache *cache, gint start, gint end) {
  ...
  if (cache->flags == EV_PAGE_DATA_INCLUDE_NONE) return;

  for (i = start; i <= end; i++) ev_page_cache_schedule_job_if_needed(cache, i);
...
void ev_page_cache_mark_dirty(EvPageCache *cache, gint page,
                              EvJobPageDataFlags flags) {
  EvPageCacheData *data;
  ...
  if (flags & EV_PAGE_DATA_INCLUDE_TEXT) g_clear_pointer(&data->text, g_free);
...
EvMappingList *ev_page_cache_get_link_mapping(EvPageCache *cache, gint page) {
  ...
  if (!(cache->flags & EV_PAGE_DATA_INCLUDE_LINKS)) return NULL;

  data = &cache->page_list[page];
  if (data->done) return data->link_mapping;

  if (data->job) return EV_JOB_PAGE_DATA(data->job)->link_mapping;
...
EvMappingList *ev_page_cache_get_image_mapping(EvPageCache *cache, gint page) {
  ...
  if (!(cache->flags & EV_PAGE_DATA_INCLUDE_IMAGES)) return NULL;

  data = &cache->page_list[page];
  if (data->done) return data->image_mapping;

  if (data->job) return EV_JOB_PAGE_DATA(data->job)->image_mapping;
...
EvMappingList *ev_page_cache_get_form_field_mapping(EvPageCache *cache,
  ...
  if (!(cache->flags & EV_PAGE_DATA_INCLUDE_FORMS)) return NULL;

  data = &cache->page_list[page];
  if (data->done) return data->form_field_mapping;

  if (data->job) return EV_JOB_PAGE_DATA(data->job)->form_field_mapping;
...
EvMappingList *ev_page_cache_get_annot_mapping(EvPageCache *cache, gint page) {
  ...
  if (!(cache->flags & EV_PAGE_DATA_INCLUDE_ANNOTS)) return NULL;

  data = &cache->page_list[page];
  if (data->done) return data->annot_mapping;

  if (data->job) return EV_JOB_PAGE_DATA(data->job)->annot_mapping;
...
cairo_region_t *ev_page_cache_get_text_mapping(EvPageCache *cache, gint page) {
  ...
  if (!(cache->flags & EV_PAGE_DATA_INCLUDE_TEXT_MAPPING)) return NULL;

  data = &cache->page_list[page];
  if (data->done) return data->text_mapping;

  if (data->job) return EV_JOB_PAGE_DATA(data->job)->text_mapping;
...
const gchar *ev_page_cache_get_text(EvPageCache *cache, gint page) {
  ...
  if (!(cache->flags & EV_PAGE_DATA_INCLUDE_TEXT)) return NULL;

  data = &cache->page_list[page];
  if (data->done) return data->text;

  if (data->job) return EV_JOB_PAGE_DATA(data->job)->text;

  return data->text;
}

gboolean ev_page_cache_get_text_layout(EvPageCache *cache, gint page,
  ...
  if (!(cache->flags & EV_PAGE_DATA_INCLUDE_TEXT_LAYOUT)) return FALSE;
...
PangoAttrList *ev_page_cache_get_text_attrs(EvPageCache *cache, gint page) {
  ...
  if (!(cache->flags & EV_PAGE_DATA_INCLUDE_TEXT_ATTRS)) return NULL;

  data = &cache->page_list[page];
  if (data->done) return data->text_attrs;

  if (data->job) return EV_JOB_PAGE_DATA(data->job)->text_attrs;

  return data->text_attrs;
}

gboolean ev_page_cache_get_text_log_attrs(EvPageCache *cache, gint page,
  ...
  if (!(cache->flags & EV_PAGE_DATA_INCLUDE_TEXT_LOG_ATTRS)) return FALSE;
