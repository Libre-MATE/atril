...
enum { TARGET_DND_URI, TARGET_DND_TEXT, TARGET_DND_IMAGE };
...
typedef enum { EV_VIEW_FIND_NEXT, EV_VIEW_FIND_PREV } EvViewFindDirection;
...
static void find_page_at_location(EvView *view, gdouble x, gdouble y,
                                  gint *page, gint *x_offset, gint *y_offset);
/*** Hyperrefs ***/
...
/*** Forms ***/
static EvFormField *ev_view_get_form_field_at_location(EvView *view, gdouble x,
                                                       gdouble y);

/*** Annotations ***/
static EvAnnotation *ev_view_get_annotation_at_location(EvView *view, gdouble x,
                                                        gdouble y);
static void show_annotation_windows(EvView *view, gint page);
static void hide_annotation_windows(EvView *view, gint page);
/*** GtkWidget implementation ***/
static void ev_view_size_request_continuous_dual_page(
    EvView *view, GtkRequisition *requisition);
...
#if ENABLE_SYNCTEX
static void highlight_forward_search_results(EvView *view, cairo_t *cr,
                                             int page);
#endif
static void draw_one_page(EvView *view, gint page, cairo_t *cr,
                          GdkRectangle *page_area, GtkBorder *border,
                          GdkRectangle *expose_area, gboolean *page_ready);
static void ev_view_reload_page(EvView *view, gint page,
                                cairo_region_t *region);

/*** Callbacks ***/
static void ev_view_change_page(EvView *view, gint new_page);
static void job_finished_cb(EvPixbufCache *pixbuf_cache, cairo_region_t *region,
                            EvView *view);
static void ev_view_page_changed_cb(EvDocumentModel *model, gint old_page,
                                    gint new_page, EvView *view);
...
/*** Zoom and sizing ***/
static double zoom_for_size_fit_width(gdouble doc_width, gdouble doc_height,
                                      int target_width, int target_height);
static double zoom_for_size_fit_height(gdouble doc_width, gdouble doc_height,
                                       int target_width, int target_height);
static double zoom_for_size_fit_page(gdouble doc_width, gdouble doc_height,
                                     int target_width, int target_height);
static double zoom_for_size_automatic(GdkScreen *screen, gdouble doc_width,
                                      gdouble doc_height, int target_width,
                                      int target_height);
...
static void ev_view_zoom_for_size_continuous_and_dual_page(EvView *view,
                                                           int width,
                                                           int height);
static void ev_view_zoom_for_size_continuous(EvView *view, int width,
                                             int height);
static void ev_view_zoom_for_size_dual_page(EvView *view, int width,
                                            int height);
static void ev_view_zoom_for_size_single_page(EvView *view, int width,
                                              int height);
...
static EvRectangle *ev_view_find_get_result(EvView *view, gint page,
                                            gint result);
static void jump_to_find_result(EvView *view);
static void jump_to_find_page(EvView *view, EvViewFindDirection direction,
                              gint shift);
/*** Selection ***/
static void compute_selections(EvView *view, EvSelectionStyle style,
                               GdkPoint *start, GdkPoint *stop);
...
static void ev_view_build_height_to_page_cache(EvView *view,
  ...
  if (uniform) ev_document_get_page_size(document, 0, &u_width, &u_height);
  ...
  for (i = cache->dual_even_left; i < n_pages + 2; i += 2) {
    if (uniform) {
      uniform_height = swap ? u_width : u_height;
      cache->dual_height_to_page[i] =
          ((i + cache->dual_even_left) / 2) * uniform_height;
      if (i + 1 < n_pages + 2)
        cache->dual_height_to_page[i + 1] =
            ((i + cache->dual_even_left) / 2) * uniform_height;
    } else {
      if (i + 1 < n_pages) {
        gdouble w, h;
...
static EvHeightToPageCache *ev_view_get_height_to_page_cache(EvView *view) {
  ...
  cache =
      g_object_get_data(G_OBJECT(view->document), EV_HEIGHT_TO_PAGE_CACHE_KEY);
  if (!cache) {
    ...
    g_object_set_data_full(G_OBJECT(view->document),
                           EV_HEIGHT_TO_PAGE_CACHE_KEY, cache,
...
static void ev_view_get_height_to_page(EvView *view, gint page, gint *height,
                                       gint *dual_height) {
  EvHeightToPageCache *cache = NULL;
  gdouble h, dh;

  if (!view->height_to_page_cache) return;
...
static gint ev_view_get_scrollbar_size(EvView *view,
  ...
  if (!GTK_IS_SCROLLED_WINDOW(swindow)) return 0;
...
static gboolean is_dual_page(EvView *view, gboolean *odd_left_out) {
  ...
  if (odd_left_out) *odd_left_out = odd_left;

  return dual;
}

static void scroll_to_point(EvView *view, gdouble x, gdouble y,
                            GtkOrientation orientation) {
  gdouble page_size;
  gdouble upper, lower;
  ...
  if (orientation == GTK_ORIENTATION_VERTICAL) {
    ...
    if (view->continuous) {
      gtk_adjustment_clamp_page(view->vadjustment, y - view->spacing / 2,
                                y + page_size);
    } else {
    ...
    if (is_dual_page(view, NULL)) {
      gtk_adjustment_clamp_page(view->hadjustment, x, x + page_size);
    } else {
...
static void ev_view_scroll_to_page_position(EvView *view,
  ...
  if ((orientation == GTK_ORIENTATION_VERTICAL && view->pending_point.y == 0) ||
    ...
    _ev_view_transform_doc_point_to_view_point(
        view, view->current_page, &view->pending_point, &view_point);
    x = view_point.x;
    y = view_point.y;
  }
...
static void view_set_adjustment_values(EvView *view,
                                       GtkOrientation orientation) {
  GtkWidget *widget = GTK_WIDGET(view);
  ...
  if (zoom_center < 0) zoom_center = page_size * 0.5;
  ...
  switch (view->pending_scroll) {
    ...
    case SCROLL_TO_CENTER:
      new_value =
          CLAMP(upper * factor - zoom_center + 0.5, 0, upper - page_size);
      if (orientation == GTK_ORIENTATION_HORIZONTAL)
...
static void view_update_range_and_current_page(EvView *view) {
  ...
  if (view->continuous) {
    ...
    if (!(view->vadjustment && view->hadjustment)) return;
    ...
    for (i = 0; i < ev_document_get_n_pages(view->document); i++) {
      ...
      if (gdk_rectangle_intersect(&current_area, &page_area, &unused)) {
        ...
        if (area > area_max) {
          best_current_page =
              (area == area_max) ? MIN(i, best_current_page) : i;
          area_max = area;
        }
  ...
  if (view->start_page == -1 || view->end_page == -1) return;
  ...
  ev_page_cache_set_page_range(view->page_cache, view->start_page,
                               view->end_page);
  ev_pixbuf_cache_set_page_range(view->pixbuf_cache, view->start_page,
                                 view->end_page,
                                 view->selection_info.selections);
  if (view->accessible)
    ev_view_accessible_set_page_range(EV_VIEW_ACCESSIBLE(view->accessible),
                                      view->start_page, view->end_page);

  if (ev_pixbuf_cache_get_surface(view->pixbuf_cache, view->current_page))
    gtk_widget_queue_draw(GTK_WIDGET(view));
}

static void set_scroll_adjustment(EvView *view, GtkOrientation orientation,
                                  GtkAdjustment *adjustment) {
  ...
  if (adjustment && adjustment == *to_set) return;

  if (*to_set) {
    g_signal_handlers_disconnect_by_func(
        *to_set, (gpointer)on_adjustment_value_changed, view);
...
static void add_scroll_binding_keypad(GtkBindingSet *binding_set, guint keyval,
                                      GdkModifierType modifiers,
                                      GtkScrollType scroll,
                                      GtkOrientation orientation) {
  guint keypad_keyval = keyval - GDK_KEY_Left + GDK_KEY_KP_Left;

  gtk_binding_entry_add_signal(binding_set, keyval, modifiers, "scroll", 2,
                               GTK_TYPE_SCROLL_TYPE, scroll,
                               GTK_TYPE_ORIENTATION, orientation);
  gtk_binding_entry_add_signal(binding_set, keypad_keyval, modifiers, "scroll",
                               2, GTK_TYPE_SCROLL_TYPE, scroll,
                               GTK_TYPE_ORIENTATION, orientation);
}
...
static gdouble compute_scroll_increment(EvView *view, GtkScrollType scroll) {
  ...
  rect.y = view->scroll_y +
           (scroll == GTK_SCROLL_PAGE_BACKWARD ? 5 : allocation.height - 5);
  ...
  if (cairo_region_num_rectangles(region)) {
    ...
    sel_region = ev_selection_get_selection_region(
        EV_SELECTION(view->document), rc, EV_SELECTION_STYLE_LINE, &doc_rect);
...
void ev_view_scroll(EvView *view, GtkScrollType scroll, gboolean horizontal) {
  ...
  if (view->key_binding_handled) return;
  ...
  if (view->current_page == 0) first_page = TRUE;
...
static void ev_view_scroll_internal(EvView *view, GtkScrollType scroll,
                                    GtkOrientation orientation) {
...
static void ensure_rectangle_is_visible(EvView *view, GdkRectangle *rect) {
  ...
  if (rect->y < adj_value) {
    ...
    value = MIN(gtk_adjustment_get_upper(adjustment),
                rect->y + rect->height - allocation.height + MARGIN);
    gtk_adjustment_set_value(view->vadjustment, value);
  }
  ...
  if (rect->x < adj_value) {
    ...
    value = MIN(gtk_adjustment_get_upper(adjustment),
                rect->x + rect->width - allocation.width + MARGIN);
    gtk_adjustment_set_value(view->hadjustment, value);
  }
}
...
static void ev_view_get_page_size(EvView *view, gint page, gint *page_width,
                                  gint *page_height) {
  _get_page_size_for_scale_and_rotation(view->document, page, view->scale,
                                        view->rotation, page_width,
                                        page_height);
}

static void ev_view_get_max_page_size(EvView *view, gint *max_width,
                                      gint *max_height) {
  double w, h;
  gint width, height;
  ...
  if (max_width)
    *max_width =
        (view->rotation == 0 || view->rotation == 180) ? width : height;
  if (max_height)
    *max_height =
        (view->rotation == 0 || view->rotation == 180) ? height : width;
}
...
gboolean ev_view_get_page_extents(EvView *view, gint page,
  ...
  if (view->continuous) {
    ...
    if (is_dual_page(view, &odd_left)) {
      x = view->spacing +
          ((page % 2 == !odd_left) ? 0 : 1) * (max_width + view->spacing);
      x = x +
          MAX(0, allocation.width - (max_width * 2 + view->spacing * 3)) / 2;
      if (page % 2 == !odd_left)
    ...
    if (is_dual_page(view, &odd_left)) {
      ...
      if (other_page < ev_document_get_n_pages(view->document) &&
          (0 <= other_page)) {
        ev_view_get_page_size(view, other_page, &width_2, &height_2);
        if (width_2 > width) max_width = width_2;
        if (height_2 > height) max_height = height_2;
      }
      ...
      x = x + MAX(0, allocation.width - ((max_width + overall_border.left +
                                          overall_border.right) *
                                             2 +
                                         view->spacing * 3)) /
                  2;
      ...
      x = x + MAX(0, allocation.width - (width + border->left + border->right +
                                         view->spacing * 2)) /
                  2;
      y = y + MAX(0, allocation.height - (height + border->top +
                                          border->bottom + view->spacing * 2)) /
                  2;
    }
...
static void get_doc_page_size(EvView *view, gint page, gdouble *width,
                              gdouble *height) {
  double w, h;
...
static void find_page_at_location(EvView *view, gdouble x, gdouble y,
  ...
  for (i = view->start_page; i >= 0 && i <= view->end_page; i++) {
    ...
    if (!ev_view_get_page_extents(view, i, &page_area, &border)) continue;
...
static gboolean location_in_text(EvView *view, gdouble x, gdouble y) {
  ...
  if (region)
    return cairo_region_contains_point(region, x_offset / view->scale,
                                       y_offset / view->scale);
  else
    return FALSE;
}
...
static gboolean location_in_selected_text(EvView *view, gdouble x, gdouble y) {
  ...
  region = ev_pixbuf_cache_get_selection_region(view->pixbuf_cache, page,
                                                view->scale);
...
static gboolean get_doc_point_from_offset(EvView *view, gint page,
...
static gboolean get_doc_point_from_location(EvView *view, gdouble x, gdouble y,
                                            gint *page, gint *x_new,
                                            gint *y_new) {
  gint x_offset = 0, y_offset = 0;
  ...
  return get_doc_point_from_offset(view, *page, x_offset, y_offset, x_new,
                                   y_new);
}

static void ev_view_get_area_from_mapping(EvView *view, guint page,
...
static void ev_view_put(EvView *view, GtkWidget *child_widget, gint x, gint y,
                        guint page, EvRectangle *doc_rect) {
  EvViewChild *child;
...
static void ev_view_put_to_doc_rect(EvView *view, GtkWidget *child_widget,
                                    guint page, EvRectangle *doc_rect) {
  GdkRectangle area;
...
/*** Hyperref ***/
static EvMapping *get_link_mapping_at_location(EvView *view, gdouble x,
                                               gdouble y, gint *page) {
  gint x_new = 0, y_new = 0;
  EvMappingList *link_mapping;

  if (!EV_IS_DOCUMENT_LINKS(view->document)) return NULL;

  if (!get_doc_point_from_location(view, x, y, page, &x_new, &y_new))
    return NULL;

  link_mapping = ev_page_cache_get_link_mapping(view->page_cache, *page);
  if (link_mapping) return ev_mapping_list_get(link_mapping, x_new, y_new);

  return NULL;
}

static EvLink *ev_view_get_link_at_location(EvView *view, gdouble x,
                                            gdouble y) {
  EvMapping *mapping;
  gint page;
...
static void goto_fith_dest(EvView *view, EvLinkDest *dest) {
  ...
  zoom = zoom_for_size_fit_width(doc_width, top, allocation.width,
                                 allocation.height);
...
static void goto_fit_dest(EvView *view, EvLinkDest *dest) {
  ...
  zoom = zoom_for_size_fit_page(doc_width, doc_height, allocation.width,
                                allocation.height);
...
static void goto_dest(EvView *view, EvLinkDest *dest) {
  ...
  switch (type) {
    ...
    case EV_LINK_DEST_TYPE_PAGE_LABEL:
      ev_document_model_set_page_by_label(view->model,
...
static char *tip_from_link(EvView *view, EvLink *link) {
  ...
  if (!action) return title ? g_strdup(title) : NULL;
  ...
  switch (type) {
    case EV_LINK_ACTION_TYPE_GOTO_DEST:
      page_label = ev_document_links_get_dest_page_label(
          EV_DOCUMENT_LINKS(view->document), ev_link_action_get_dest(action));
      if (page_label) {
    ...
    case EV_LINK_ACTION_TYPE_LAUNCH:
      msg =
          g_strdup_printf(_("Launch %s"), ev_link_action_get_filename(action));
      break;
    case EV_LINK_ACTION_TYPE_NAMED:
      msg = tip_from_action_named(action);
      break;
    default:
      if (title) msg = g_strdup(title);
      break;
  }
...
static void ev_view_handle_cursor_over_xy(EvView *view, gint x, gint y) {
  ...
  if (view->cursor == EV_VIEW_CURSOR_HIDDEN) return;
  ...
  if (link || annot) g_object_set(view, "has-tooltip", TRUE, NULL);
}

/*** Images ***/
static EvImage *ev_view_get_image_at_location(EvView *view, gdouble x,
                                              gdouble y) {
  gint page = -1;
  gint x_new = 0, y_new = 0;
  EvMappingList *image_mapping;

  if (!EV_IS_DOCUMENT_IMAGES(view->document)) return NULL;
...
static gboolean ev_view_get_focused_area(EvView *view, GdkRectangle *area) {
  if (!view->focused_element) return FALSE;

  _ev_view_transform_doc_rect_to_view_rect(view, view->focused_element_page,
                                           &view->focused_element->area, area);
...
void _ev_view_set_focused_element(EvView *view, EvMapping *element_mapping,
                                  gint page) {
  GdkRectangle view_rect;
  cairo_region_t *region = NULL;

  if (view->focused_element == element_mapping) return;
...
/*** Forms ***/
static EvMapping *get_form_field_mapping_at_location(EvView *view, gdouble x,
                                                     gdouble y, gint *page) {
  gint x_new = 0, y_new = 0;
  EvMappingList *forms_mapping;

  if (!EV_IS_DOCUMENT_FORMS(view->document)) return NULL;
  ...
  if (forms_mapping) return ev_mapping_list_get(forms_mapping, x_new, y_new);
...
static cairo_region_t *ev_view_form_field_get_region(EvView *view,
                                                     EvFormField *field) {
  ...
  forms_mapping = ev_page_cache_get_form_field_mapping(view->page_cache,
                                                       field->page->index);
  ev_view_get_area_from_mapping(view, field->page->index, forms_mapping, field,
                                &view_area);
...
static void ev_view_form_field_button_toggle(EvView *view, EvFormField *field) {
  ...
  if (field_button->type == EV_FORM_FIELD_BUTTON_PUSH) return;

  state = ev_document_forms_form_field_button_get_state(
      EV_DOCUMENT_FORMS(view->document), field);

  /* FIXME: it actually depends on NoToggleToOff flags */
  if (field_button->type == EV_FORM_FIELD_BUTTON_RADIO && state &&
      field_button->state)
    return;
  ...
  for (l = ev_mapping_list_get_list(forms_mapping); l; l = g_list_next(l)) {
    ...
    if (button->id == field->id) continue;
  ...
  ev_document_forms_form_field_button_set_state(
      EV_DOCUMENT_FORMS(view->document), field, !state);
...
static void ev_view_form_field_text_save(EvView *view, GtkWidget *widget) {
  ...
  if (field->changed) {
    ...
    ev_document_forms_form_field_text_set_text(
        EV_DOCUMENT_FORMS(view->document), field, field_text->text);
    field->changed = FALSE;
...
static void ev_view_form_field_text_changed(GtkWidget *widget,
                                            EvFormField *field) {
  EvFormFieldText *field_text = EV_FORM_FIELD_TEXT(field);
  gchar *text = NULL;
  ...
  if (GTK_IS_ENTRY(widget)) {
    ...
    text =
        gtk_text_buffer_get_text(GTK_TEXT_BUFFER(widget), &start, &end, FALSE);
  }
...
static GtkWidget *ev_view_form_field_text_create_widget(EvView *view,
                                                        EvFormField *field) {
  EvFormFieldText *field_text = EV_FORM_FIELD_TEXT(field);
  GtkWidget *text = NULL;
  gchar *txt;

  txt = ev_document_forms_form_field_text_get_text(
      EV_DOCUMENT_FORMS(view->document), field);
  ...
  switch (field_text->type) {
    ...
    case EV_FORM_FIELD_TEXT_NORMAL:
      ...
      g_signal_connect(text, "focus-out-event",
                       G_CALLBACK(ev_view_form_field_text_focus_out), view);
      g_signal_connect(text, "changed",
                       G_CALLBACK(ev_view_form_field_text_changed), field);
      g_signal_connect_after(text, "activate",
                             G_CALLBACK(ev_view_form_field_destroy), view);
      break;
  ...
  g_object_weak_ref(G_OBJECT(text), (GWeakNotify)ev_view_form_field_text_save,
                    view);
  ...
  if (field->changed) {
    ...
    if (field_choice->is_editable) {
      ev_document_forms_form_field_choice_set_text(
          EV_DOCUMENT_FORMS(view->document), field, field_choice->text);
    } else {
      ...
      for (l = field_choice->selected_items; l; l = g_list_next(l)) {
        ev_document_forms_form_field_choice_select_item(
            EV_DOCUMENT_FORMS(view->document), field, GPOINTER_TO_INT(l->data));
      }
    }
    field->changed = FALSE;
    ev_view_reload_page(view, field->page->index, field_region);
...
static void ev_view_form_field_choice_changed(GtkWidget *widget,
  ...
  if (GTK_IS_COMBO_BOX(widget)) {
    ...
    if (!field_choice->selected_items ||
      ...
      field_choice->selected_items =
          g_list_prepend(field_choice->selected_items, GINT_TO_POINTER(item));
      field->changed = TRUE;
    }
    ...
    if (gtk_combo_box_get_has_entry(GTK_COMBO_BOX(widget))) {
      ...
      if (!field_choice->text ||
          (field_choice->text &&
           g_ascii_strcasecmp(field_choice->text, text) != 0)) {
...
static GtkWidget *ev_view_form_field_choice_create_widget(EvView *view,
  ...
  n_items = ev_document_forms_form_field_choice_get_n_items(
      EV_DOCUMENT_FORMS(view->document), field);
  ...
  for (i = 0; i < n_items; i++) {
    ...
    item = ev_document_forms_form_field_choice_get_item(
    ...
    if (ev_document_forms_form_field_choice_is_item_selected(
      ...
      field_choice->selected_items =
          g_list_prepend(field_choice->selected_items, GINT_TO_POINTER(i));
    }

    if (item) {
      gtk_list_store_append(GTK_LIST_STORE(model), &iter);
      gtk_list_store_set(GTK_LIST_STORE(model), &iter, 0, item, 1, i, -1);
      g_free(item);
    }
  }
  ...
  if (field_choice->type == EV_FORM_FIELD_CHOICE_LIST) {
    ...
    gtk_tree_view_insert_column_with_attributes(
        GTK_TREE_VIEW(tree_view), 0, "choix", renderer, "text", 0, NULL);
    ...
    g_signal_connect(selection, "changed",
                     G_CALLBACK(ev_view_form_field_choice_changed), field);
    ...
    text = ev_document_forms_form_field_choice_get_text(
        EV_DOCUMENT_FORMS(view->document), field);
    if (text) {
    ...
    g_signal_connect(choice, "changed",
                     G_CALLBACK(ev_view_form_field_choice_changed), field);
    g_signal_connect_after(gtk_bin_get_child(GTK_BIN(choice)), "activate",
                           G_CALLBACK(ev_view_form_field_destroy), view);
  } else { /* ComboBoxText */
    GtkCellRenderer *renderer;
    ...
    renderer = gtk_cell_renderer_text_new();
    gtk_cell_layout_pack_start(GTK_CELL_LAYOUT(choice), renderer, TRUE);
    gtk_cell_layout_set_attributes(GTK_CELL_LAYOUT(choice), renderer, "text", 0,
                                   NULL);
  ...
  g_object_weak_ref(G_OBJECT(choice),
                    (GWeakNotify)ev_view_form_field_choice_save, view);
...
void _ev_view_focus_form_field(EvView *view, EvFormField *field) {
  ...
  g_object_set_data_full(G_OBJECT(field_widget), "form-field",
                         g_object_ref(field), (GDestroyNotify)g_object_unref);
  ...
  ev_view_put_to_doc_rect(view, field_widget, field->page->index,
                          &mapping->area);
...
static void ev_view_handle_form_field(EvView *view, EvFormField *field) {
  ...
  if (field->activation_link) ev_view_handle_link(view, field->activation_link);
...
static EvViewWindowChild *ev_view_get_window_child(EvView *view,
  ...
  while (children) {
    ...
    if (child->window == window) return child;
  }

  return NULL;
}

static void ev_view_window_child_move(EvView *view, EvViewWindowChild *child,
                                      gint x, gint y) {
  GtkAllocation allocation;
  gint width, height;
  ...
  child->y = y;
  gtk_window_move(
      GTK_WINDOW(child->window),
      CLAMP(x, child->parent_x, child->parent_x + allocation.width - width),
      CLAMP(y, child->parent_y, child->parent_y + allocation.height - height));
}

static void ev_view_window_child_move_with_parent(EvView *view,
                                                  GtkWidget *window) {
  EvViewWindowChild *child;
  gint root_x, root_y;

  child = ev_view_get_window_child(view, window);
  gdk_window_get_origin(gtk_widget_get_window(GTK_WIDGET(view)), &root_x,
                        &root_y);
...
static void ev_view_window_child_put(EvView *view, GtkWidget *window,
                                     guint page, gint x, gint y, gdouble orig_x,
                                     gdouble orig_y) {
  EvViewWindowChild *child;
  gint root_x, root_y;
...
static EvViewWindowChild *ev_view_find_window_child_for_annot(
    EvView *view, guint page, EvAnnotation *annot) {
  GList *children = view->window_children;
  ...
  while (children) {
    ...
    if (child->page != page) continue;

    wannot = ev_annotation_window_get_annotation(
        EV_ANNOTATION_WINDOW(child->window));
    if (ev_annotation_equal(wannot, annot)) return child;
  }
...
static void ev_view_window_children_free(EvView *view) {
  ...
  if (!view->window_children) return;
...
static void annotation_window_grab_focus(GtkWidget *widget, EvView *view) {
  if (view->window_child_focus)
    ev_annotation_window_ungrab_focus(
        EV_ANNOTATION_WINDOW(view->window_child_focus->window));
...
static void annotation_window_moved(EvAnnotationWindow *window, gint x, gint y,
                                    EvView *view) {
  EvViewWindowChild *child;
  GdkRectangle page_area;
  GtkBorder border;
  ...
  _ev_view_transform_view_rect_to_doc_rect(view, &view_rect, &page_area,
                                           &doc_rect);
  child->orig_x = doc_rect.x1;
  child->orig_y = doc_rect.y1;
}

static void ev_view_annotation_save_contents(EvView *view, GParamSpec *pspec,
                                             EvAnnotation *annot) {
  if (!view->document) return;

  ev_document_doc_mutex_lock();
  ev_document_annotations_save_annotation(
      EV_DOCUMENT_ANNOTATIONS(view->document), annot,
...
static GtkWidget *ev_view_create_annotation_window(EvView *view,
  ...
  g_signal_connect(window, "grab_focus",
                   G_CALLBACK(annotation_window_grab_focus), view);
  g_signal_connect(window, "closed", G_CALLBACK(annotation_window_closed),
                   view);
  g_signal_connect(window, "moved", G_CALLBACK(annotation_window_moved), view);
  g_signal_connect_swapped(annot, "notify::contents",
                           G_CALLBACK(ev_view_annotation_save_contents), view);
  ...
  ev_view_window_child_put(view, window, page, view_rect.x, view_rect.y,
...
static void show_annotation_windows(EvView *view, gint page) {
  ...
  for (l = ev_mapping_list_get_list(annots); l && l->data; l = g_list_next(l)) {
    ...
    if (!EV_IS_ANNOTATION_MARKUP(annot)) continue;

    if (!ev_annotation_markup_has_popup(EV_ANNOTATION_MARKUP(annot))) continue;
...
static EvMapping *get_annotation_mapping_at_location(EvView *view, gdouble x,
                                                     gdouble y, gint *page) {
  gint x_new = 0, y_new = 0;
  EvMappingList *annotations_mapping;

  if (!EV_IS_DOCUMENT_ANNOTATIONS(view->document)) return NULL;
...
static void ev_view_handle_annotation(EvView *view, EvAnnotation *annot,
                                      gdouble x, gdouble y, guint32 timestamp) {
  ...
  if (EV_IS_ANNOTATION_ATTACHMENT(annot)) {
    ...
    if (attachment) {
      ...
      ev_attachment_open(attachment, gtk_widget_get_screen(GTK_WIDGET(view)),
                         timestamp, &error);
...
static void ev_view_create_annotation(EvView *view, EvAnnotationType annot_type,
                                      gint x, gint y) {
  EvAnnotation *annot;
  GdkPoint point;
  GdkRectangle page_area;
  ...
  if (EV_IS_ANNOTATION_MARKUP(annot)) {
    ...
    g_object_set(annot, "rectangle", &popup_rect, "has_popup", TRUE,
                 "popup_is_open", FALSE, "label", g_get_real_name(), "opacity",
                 1.0, NULL);
  }
  ev_document_annotations_add_annotation(
      EV_DOCUMENT_ANNOTATIONS(view->document), annot, &doc_rect);
  ev_document_doc_mutex_unlock();
  ...
  _ev_view_transform_doc_rect_to_view_rect(view, view->current_page, &doc_rect,
                                           &view_rect);
  view_rect.x -= view->scroll_x;
  view_rect.y -= view->scroll_y;
  region = cairo_region_create_rectangle(&view_rect);
...
void ev_view_focus_annotation(EvView *view, EvMapping *annot_mapping) {
  if (!EV_IS_DOCUMENT_ANNOTATIONS(view->document)) return;
...
void ev_view_begin_add_annotation(EvView *view, EvAnnotationType annot_type) {
  if (annot_type == EV_ANNOTATION_TYPE_UNKNOWN) return;

  if (view->adding_annot) return;
...
static gboolean ev_view_synctex_backward_search(EvView *view, gdouble x,
                                                gdouble y) {
  gint page = -1;
  gint x_new = 0, y_new = 0;
  EvSourceLink *link;

  if (!ev_document_has_synctex(view->document)) return FALSE;

  if (!get_doc_point_from_location(view, x, y, &page, &x_new, &y_new))
    return FALSE;

  link =
      ev_document_synctex_backward_search(view->document, page, x_new, y_new);
  if (link) {
...
static inline gboolean cursor_is_in_visible_page(EvView *view) {
...
static gboolean cursor_should_blink(EvView *view) {
  if (view->caret_enabled && view->rotation == 0 &&
      ...
      gtk_widget_has_focus(GTK_WIDGET(view)) && view->pixbuf_cache &&
      !ev_pixbuf_cache_get_selection_region(view->pixbuf_cache,
                                            view->cursor_page, view->scale)) {
...
static gboolean get_caret_cursor_area(EvView *view, gint page, gint offset,
                                      GdkRectangle *area) {
  ...
  if (!view->caret_enabled || view->rotation != 0) return FALSE;

  if (!view->page_cache) return FALSE;

  ev_page_cache_get_text_layout(view->page_cache, page, &areas, &n_areas);
  if (!areas) return FALSE;

  if (offset > n_areas) return FALSE;

  doc_rect = areas + offset;
  if (offset == n_areas ||
      ((doc_rect->x1 == doc_rect->x2 || doc_rect->y1 == doc_rect->y2) &&
       offset > 0)) {
    EvRectangle *prev;
...
static void show_cursor(EvView *view) {
  ...
  if (gtk_widget_has_focus(widget) &&
      get_caret_cursor_area(view, view->cursor_page, view->cursor_offset,
                            &view_rect)) {
    gtk_widget_queue_draw_area(widget, view_rect.x, view_rect.y,
...
static gboolean blink_cb(EvView *view) {
  ...
  if (view->cursor_blink_time > 1000 * blink_timeout &&
  ...
  view->cursor_blink_timeout_id = gdk_threads_add_timeout(
      blink_time / CURSOR_DIVIDER, (GSourceFunc)blink_cb, view);

  return FALSE;
}

static void ev_view_check_cursor_blink(EvView *view) {
  if (cursor_should_blink(view)) {
    if (view->cursor_blink_timeout_id == 0) {
      show_cursor(view);
      view->cursor_blink_timeout_id = gdk_threads_add_timeout(
          get_cursor_blink_time(view) * CURSOR_ON_MULTIPLIER / CURSOR_DIVIDER,
...
static void ev_view_pend_cursor_blink(EvView *view) {
  if (!cursor_should_blink(view)) return;
...
gboolean ev_view_supports_caret_navigation(EvView *view) {
  ...
  if (!view->document || !EV_IS_DOCUMENT_TEXT(view->document)) return FALSE;

  iface = EV_DOCUMENT_TEXT_GET_IFACE(view->document);
  if (!iface->get_text_layout || !iface->get_text) return FALSE;
...
static void ev_view_size_request_continuous_dual_page(
  ...
  switch (view->sizing_mode) {
    ...
    case EV_SIZING_FREE: {
      ...
      compute_border(view, &border);
      requisition->width =
          (max_width + border.left + border.right) * 2 + (view->spacing * 3);
    } break;
    default:
      g_assert_not_reached();
  }
...
static void ev_view_size_request_continuous(EvView *view,
  ...
  switch (view->sizing_mode) {
    ...
    case EV_SIZING_FREE: {
      ...
      compute_border(view, &border);
      requisition->width =
          max_width + (view->spacing * 2) + border.left + border.right;
    } break;
    default:
      g_assert_not_reached();
  }
...
static void ev_view_size_request_dual_page(EvView *view,
  ...
  if (view->current_page + 1 < ev_document_get_n_pages(view->document)) {
    gint width_2, height_2;
    ev_view_get_page_size(view, view->current_page + 1, &width_2, &height_2);
    if (width_2 > width) {
      width = width_2;
      height = height_2;
    }
  ...
  requisition->height =
      (height + border.top + border.bottom) + (view->spacing * 2);
}
...
static void ev_view_size_request(GtkWidget *widget,
  ...
  if (!view->internal_size_request &&
    ...
    gtk_widget_get_allocation(widget, &allocation);
    ev_view_zoom_for_size(view, allocation.width, allocation.height);
  }
...
static void ev_view_get_preferred_width(GtkWidget *widget, gint *minimum,
                                        gint *natural) {
  GtkRequisition requisition;
...
static void ev_view_get_preferred_height(GtkWidget *widget, gint *minimum,
                                         gint *natural) {
  GtkRequisition requisition;
...
static void ev_view_size_allocate(GtkWidget *widget,
  ...
  if (view->document) view_update_range_and_current_page(view);
  ...
  for (l = view->children; l && l->data; l = g_list_next(l)) {
    ...
    _ev_view_transform_doc_rect_to_view_rect(view, child->page,
                                             &child->doc_rect, &view_area);
    view_area.x -= view->scroll_x;
    view_area.y -= view->scroll_y;

    gtk_widget_set_size_request(child->widget, view_area.width,
                                view_area.height);
  ...
  for (l = view->window_children; l && l->data; l = g_list_next(l)) {
    ...
    ev_annotation_window_get_rectangle(EV_ANNOTATION_WINDOW(child->window),
                                       &doc_rect);
    if (child->moved) {
      doc_rect.x1 = child->orig_x;
      doc_rect.y1 = child->orig_y;
    }
    _ev_view_transform_doc_rect_to_view_rect(view, child->page, &doc_rect,
                                             &view_rect);
    view_rect.x -= view->scroll_x;
    view_rect.y -= view->scroll_y;
    ...
    if (view_rect.x != child->orig_x || view_rect.y != child->orig_y) {
      child->parent_x = root_x;
      child->parent_y = root_y;
      ev_view_window_child_move(view, child, view_rect.x + root_x,
                                view_rect.y + root_y);
    }
  }
}
  ...
  if (state == GDK_CONTROL_MASK) {
    ...
    switch (event->direction) {
      ...
      case GDK_SCROLL_RIGHT:
        if (ev_view_can_zoom_out(view)) ev_view_zoom_out(view);
        break;
      case GDK_SCROLL_UP:
      case GDK_SCROLL_LEFT:
        if (ev_view_can_zoom_in(view)) ev_view_zoom_in(view);
        break;
      case GDK_SCROLL_SMOOTH: {
        gdouble delta = event->delta_x + event->delta_y;
        gdouble factor =
            pow(delta < 0 ? ZOOM_IN_FACTOR : ZOOM_OUT_FACTOR, fabs(delta));
    ...
    switch (event->direction) {
      ...
      case GDK_SCROLL_SMOOTH: {
        ...
        if ((fit_width && fit_height) ||
          ...
          for (; fabs(view->total_delta) >= 1.0;
               view->total_delta -= decrement) {
            if (decrement < 0)
              ev_view_previous_page(view);
            else
              ev_view_next_page(view);
...
static EvViewSelection *find_selection_for_page(EvView *view, gint page) {
  ...
  for (list = view->selection_info.selections; list != NULL;
       list = list->next) {
    EvViewSelection *selection;

    selection = (EvViewSelection *)list->data;

    if (selection->page == page) return selection;
  }
...
static void get_cursor_color(GtkStyleContext *context, GdkRGBA *color) {
  ...
  gtk_style_context_get_style(context, "cursor-color", &style_color, NULL);
...
static void draw_caret_cursor(EvView *view, cairo_t *cr) {
  ...
  if (!get_caret_cursor_area(view, view->cursor_page, view->cursor_offset,
                             &view_rect))
    return;

  get_cursor_color(gtk_widget_get_style_context(GTK_WIDGET(view)),
                   &cursor_color);

  cairo_save(cr);
  gdk_cairo_set_source_rgba(cr, &cursor_color);
  cairo_rectangle(cr, view_rect.x, view_rect.y, view_rect.width,
                  view_rect.height);
  cairo_fill(cr);
  cairo_restore(cr);
}
...
static gboolean should_draw_caret_cursor(EvView *view, gint page) {
  return (view->caret_enabled && view->cursor_page == page &&
          view->cursor_visible && gtk_widget_has_focus(GTK_WIDGET(view)) &&
          !ev_pixbuf_cache_get_selection_region(view->pixbuf_cache, page,
                                                view->scale));
}

static void draw_focus(EvView *view, cairo_t *cr, gint page,
                       GdkRectangle *clip) {
  GtkWidget *widget = GTK_WIDGET(view);
  GdkRectangle rect;
  GdkRectangle intersect;

  if (view->focused_element_page != page) return;

  if (!ev_view_get_focused_area(view, &rect)) return;

  if (gdk_rectangle_intersect(&rect, clip, &intersect)) {
    gtk_render_focus(gtk_widget_get_style_context(widget), cr, intersect.x,
...
static gboolean ev_view_draw(GtkWidget *widget, cairo_t *cr) {
  ...
  if (view->document == NULL) return FALSE;

  if (!gdk_cairo_get_clip_rectangle(cr, &clip_rect)) return FALSE;
...
static void ev_view_set_focused_element_at_location(EvView *view, gdouble x,
                                                    gdouble y) {
  EvMapping *mapping;
  EvFormField *field;
  gint page;
...
static gboolean ev_view_do_popup_menu(EvView *view, gdouble x, gdouble y) {
  ...
  if (image) items = g_list_prepend(items, image);

  link = ev_view_get_link_at_location(view, x, y);
  if (link) items = g_list_prepend(items, link);

  annot = ev_view_get_annotation_at_location(view, x, y);
  if (annot) items = g_list_prepend(items, annot);
...
static void get_link_area(EvView *view, gint x, gint y, EvLink *link,
                          GdkRectangle *area) {
  EvMappingList *link_mapping;
  gint page;
  gint x_offset = 0, y_offset = 0;
  ...
  link_mapping = ev_page_cache_get_link_mapping(view->page_cache, page);
  ev_view_get_area_from_mapping(view, page, link_mapping, link, area);
}

static void get_annot_area(EvView *view, gint x, gint y, EvAnnotation *annot,
                           GdkRectangle *area) {
  ...
  ev_view_get_area_from_mapping(view, page, annot_mapping, annot, area);
}

static gboolean ev_view_query_tooltip(GtkWidget *widget, gint x, gint y,
...
static void start_selection_for_event(EvView *view, GdkEventButton *event) {
  ...
  compute_selections(view, view->selection_info.style,
                     &(view->selection_info.start),
                     &(view->selection_info.start));
}

gint _ev_view_get_caret_cursor_offset_at_doc_point(EvView *view, gint page,
                                                   gdouble doc_x,
                                                   gdouble doc_y) {
  EvRectangle *areas = NULL;
  guint n_areas = 0;
  ...
  while (i < n_areas && offset == -1) {
    ...
    if (first_line_offset == -1) i++;
  }

  if (last_line_offset == -1) return -1;

  if (offset == -1) offset = last_line_offset;

  return offset;
}

static gboolean position_caret_cursor_at_doc_point(EvView *view, gint page,
                                                   gdouble doc_x,
                                                   gdouble doc_y) {
  gint offset;
...
static gboolean position_caret_cursor_at_location(EvView *view, gdouble x,
                                                  gdouble y) {
  gint page;
  gint doc_x, doc_y;
...
static gboolean ev_view_button_press_event(GtkWidget *widget,
  ...
  if (gtk_gesture_is_recognized(view->zoom_gesture)) return TRUE;
  ...
  if (view->adding_annot) return FALSE;

  if (view->scroll_info.autoscrolling) return TRUE;
  ...
  switch (event->button) {
    case 1: {
      ...
      if (EV_IS_SELECTION(view->document) && view->selection_info.selections) {
        ...
        } else if (location_in_selected_text(view, event->x + view->scroll_x,
      ...
      } else if ((annot = ev_view_get_annotation_at_location(view, event->x,
                                                             event->y))) {
        ev_view_handle_annotation(view, annot, event->x, event->y, event->time);
      } else if ((field = ev_view_get_form_field_at_location(view, event->x,
                                                             event->y))) {
        ev_view_remove_all(view);
        ev_view_handle_form_field(view, field);
      } else if ((link = get_link_mapping_at_location(view, event->x, event->y,
                                                      &page))) {
        _ev_view_set_focused_element(view, link, page);
      } else if (!location_in_text(view, event->x + view->scroll_x,
                                   event->y + view->scroll_y) &&
                 (image = ev_view_get_image_at_location(view, event->x,
                                                        event->y))) {
...
static void ev_view_remove_all(EvView *view) {
  gtk_container_foreach(GTK_CONTAINER(view), (GtkCallback)gtk_widget_destroy,
                        NULL);
  GList *children, *child;

  children = gtk_container_get_children(GTK_CONTAINER(view));
  for (child = children; child && child->data; child = g_list_next(child)) {
    gtk_container_remove(GTK_CONTAINER(view), GTK_WIDGET(child->data));
  }
  g_list_free(children);
}

/*** Drag and Drop ***/
static void ev_view_drag_data_get(GtkWidget *widget, GdkDragContext *context,
                                  GtkSelectionData *selection_data, guint info,
                                  guint time) {
  EvView *view = EV_VIEW(widget);
    ...
    case TARGET_DND_IMAGE:
      if (view->image_dnd_info.image) {
        ...
        pixbuf = ev_document_images_get_image(
            EV_DOCUMENT_IMAGES(view->document), view->image_dnd_info.image);
        ev_document_doc_mutex_unlock();
...
static gboolean ev_view_drag_motion(GtkWidget *widget, GdkDragContext *context,
...
static gboolean selection_update_idle_cb(EvView *view) {
  compute_selections(view, view->selection_info.style,
                     &view->selection_info.start, &view->motion);
...
static gboolean ev_view_drag_update_momentum(EvView *view) {
  int i;
  if (!view->drag_info.in_drag) return FALSE;
  ...
  view->drag_info.momentum.x = (view->drag_info.buffer[DRAG_HISTORY - 1].x -
                                view->drag_info.buffer[0].x);
  view->drag_info.momentum.y = (view->drag_info.buffer[DRAG_HISTORY - 1].y -
                                view->drag_info.buffer[0].y);
...
static gboolean ev_view_scroll_drag_release(EvView *view) {
  ...
  view->drag_info.momentum.y /=
      1.2; /* Alter these constants to change "friction" */
  ...
  dhadj_value =
      h_page_size * (gdouble)view->drag_info.momentum.x / allocation.width;
  dvadj_value =
      v_page_size * (gdouble)view->drag_info.momentum.y / allocation.height;
  ...
  /* When we reach the edges, we need either to absorb some momentum and bounce
   * by multiplying it on -0.5 or stop scrolling by setting momentum to 0. */
  ...
  gtk_adjustment_set_value(view->hadjustment, MIN(oldhadjustment + dhadj_value,
                                                  h_upper - h_page_size));
  gtk_adjustment_set_value(view->vadjustment, MIN(oldvadjustment + dvadj_value,
                                                  v_upper - v_page_size));
...
static gboolean ev_view_motion_notify_event(GtkWidget *widget,
  ...
  if (view->selection_info.in_drag) {
    if (gtk_drag_check_threshold(widget, view->selection_info.start.x,
                                 view->selection_info.start.y, x, y)) {
      GtkTargetList *target_list = gtk_target_list_new(NULL, 0);

      gtk_target_list_add_text_targets(target_list, TARGET_DND_TEXT);

      gtk_drag_begin_with_coordinates(widget, target_list, GDK_ACTION_COPY, 1,
                                      (GdkEvent *)event, event->x, event->y);
  ...
  switch (view->pressed_button) {
    case 1:
       ...
       * documents only.
       */
      if (view->rotation != 0) return FALSE;

      /* Schedule timeout to scroll during selection and additionally
       * scroll once to allow arbitrary speed. */
      if (!view->selection_scroll_id)
        view->selection_scroll_id = g_timeout_add(
            SCROLL_TIME, (GSourceFunc)selection_scroll_timeout_cb, view);
      else
      ...
      if (!view->selection_update_id)
        view->selection_update_id =
            g_idle_add((GSourceFunc)selection_update_idle_cb, view);
    ...
    case 2:
      if (!view->drag_info.in_drag) {
        ...
        start = gtk_drag_check_threshold(widget, view->drag_info.start.x,
                                         view->drag_info.start.y, event->x_root,
                                         event->y_root);
        view->drag_info.in_drag = start;
        view->drag_info.drag_timeout_id =
            g_timeout_add(10, (GSourceFunc)ev_view_drag_update_momentum, view);
...
static gboolean ev_view_button_release_event(GtkWidget *widget,
  ...
  if (view->adding_annot && view->pressed_button == 1) {
    ...
    ev_view_create_annotation(view, view->adding_annot_type,
  ...
  if (view->selection_info.selections) {
    ...
    if (view->selection_info.in_drag) clear_selection(view);
    view->selection_info.in_drag = FALSE;
  } else if (link) {
    if (event->button == 2) {
      EvLinkAction *action;
...
static gboolean ev_view_forward_key_event_to_focused_child(EvView *view,
  ...
  if (new_event->window) g_object_ref(new_event->window);
...
static gint go_to_next_page(EvView *view, gint page) {
  ...
  if (page < n_pages) return page;

  if (dual_page && page == n_pages) return page - 1;
...
static gboolean cursor_go_to_page_end(EvView *view) {
  ...
  ev_page_cache_get_text_log_attrs(view->page_cache, view->cursor_page,
                                   &log_attrs, &n_attrs);
  if (!log_attrs) return FALSE;
...
static gboolean cursor_backward_char(EvView *view) {
  ...
  if (view->cursor_offset == 0) return cursor_go_to_previous_page(view);

  do {
    view->cursor_offset--;
  } while (view->cursor_offset >= 0 &&
           !log_attrs[view->cursor_offset].is_cursor_position);
...
static gboolean cursor_forward_char(EvView *view) {
  ...
  if (view->cursor_offset >= n_attrs) return cursor_go_to_next_page(view);

  do {
    view->cursor_offset++;
  } while (view->cursor_offset <= n_attrs &&
           !log_attrs[view->cursor_offset].is_cursor_position);
...
static gboolean cursor_forward_word_end(EvView *view) {
  ...
  for (i = view->cursor_offset; i < n_attrs && log_attrs[i].is_word_end; i++)
    ;
  if (i >= n_attrs) {
    if (cursor_go_to_next_page(view)) return cursor_forward_word_end(view);
    return FALSE;
  }
...
static gboolean cursor_backward_line(EvView *view) {
  ...
  if (!cursor_go_to_line_start(view)) return FALSE;
  ...
  do {
    view->cursor_offset--;
           ...
           !log_attrs[view->cursor_offset].is_mandatory_break);
...
static gboolean cursor_go_to_line_end(EvView *view) {
  ...
  for (i = view->cursor_offset + 1;
       i <= n_attrs && !log_attrs[i].is_mandatory_break; i++)
    ;
  view->cursor_offset = MIN(i, n_attrs);

  if (view->cursor_offset == n_attrs) return TRUE;
...
static gboolean cursor_forward_line(EvView *view) {
  ...
  if (!cursor_go_to_line_end(view)) return FALSE;
...
static void extend_selection(EvView *view, GdkPoint *start_point,
                             GdkPoint *end_point) {
  if (!view->selection_info.selections) {
  ...
  compute_selections(view, EV_SELECTION_STYLE_GLYPH,
                     &(view->selection_info.start), end_point);
}
...
static gboolean cursor_clear_selection(EvView *view, gboolean forward) {
   ...
   */
  if (!view->selection_info.selections) return FALSE;

  l = forward ? g_list_last(view->selection_info.selections)
              : view->selection_info.selections;
  selection = (EvViewSelection *)l->data;
  if (!selection->covered_region ||
      cairo_region_is_empty(selection->covered_region))
    return FALSE;

  cairo_region_get_rectangle(
      selection->covered_region,
...
static gboolean ev_view_move_cursor(EvView *view, GtkMovementStep step,
                                    gint count, gboolean extend_selections) {
  GdkRectangle rect;
  ...
  clear_selections =
      !extend_selections && view->selection_info.selections != NULL;
  ...
  if (!clear_selections && prev_offset == view->cursor_offset &&
      prev_page == view->cursor_page) {
...
static gint ev_view_focus_in(GtkWidget *widget, GdkEventFocus *event) {
  ...
  if (view->pixbuf_cache) ev_pixbuf_cache_style_changed(view->pixbuf_cache);
...
static void draw_rubberband(EvView *view, cairo_t *cr, const GdkRectangle *rect,
                            gdouble alpha) {
  ...
  cairo_rectangle(cr, rect->x - view->scroll_x, rect->y - view->scroll_y,
...
static void highlight_find_results(EvView *view, cairo_t *cr, int page) {
  ...
  for (i = 0; i < n_results; i++) {
    ...
    _ev_view_transform_doc_rect_to_view_rect(view, page, rectangle,
                                             &view_rectangle);
...
static void highlight_forward_search_results(EvView *view, cairo_t *cr,
  ...
  if (GPOINTER_TO_INT(mapping->data) != page) return;
...
static void draw_surface(cairo_t *cr, cairo_surface_t *surface, gint x, gint y,
                         gint offset_x, gint offset_y, gint target_width,
                         gint target_height) {
  gdouble width, height;
  ...
  cairo_surface_set_device_offset(surface, offset_x * device_scale_x,
...
void _ev_view_get_selection_colors(EvView *view, GdkRGBA *bg_color,
  ...
  if (fg_color) gtk_style_context_get_color(context, state, fg_color);

  gtk_style_context_restore(context);
}

static void draw_selection_region(cairo_t *cr, cairo_region_t *region,
                                  GdkRGBA *color, gint x, gint y,
...
static void draw_one_page(EvView *view, gint page, cairo_t *cr,
  ...
  if (!gdk_rectangle_intersect(page_area, expose_area, &overlap)) return;
  ...
  *page_ready = TRUE;
  ...
  gtk_render_background(context, cr, page_area->x, page_area->y,
                        page_area->width, page_area->height);
  gtk_render_frame(context, cr, page_area->x, page_area->y, page_area->width,
                   page_area->height);
  gtk_style_context_restore(context);
  ...
  if (gdk_rectangle_intersect(&real_page_area, expose_area, &overlap)) {
    ...
    if (!page_surface) {
      if (page == current_page) ev_view_set_loading(view, TRUE);
    ...
    if (page == current_page) ev_view_set_loading(view, FALSE);

    ev_view_get_page_size(view, page, &width, &height);
    offset_x = overlap.x - real_page_area.x;
    offset_y = overlap.y - real_page_area.y;

    draw_surface(cr, page_surface, overlap.x, overlap.y, offset_x, offset_y,
                 width, height);

    /* Get the selection pixbuf iff we have something to draw */
    if (!find_selection_for_page(view, page)) return;

    selection_surface = ev_pixbuf_cache_get_selection_surface(
        view->pixbuf_cache, page, view->scale);
    if (selection_surface) {
      draw_surface(cr, selection_surface, overlap.x, overlap.y, offset_x,
    ...
    if (region) {
      ...
      cairo_surface_get_device_scale(page_surface, &device_scale_x,
                                     &device_scale_y);
      ...
      draw_selection_region(cr, region, &color, real_page_area.x,
                            real_page_area.y, scale_x, scale_y);
    }
  }
}
...
static void ev_view_finalize(GObject *object) {
  ...
  if (view->selection_info.selections) {
    g_list_free_full(view->selection_info.selections,
                     (GDestroyNotify)selection_free);
  ...
  if (view->image_dnd_info.image) g_object_unref(view->image_dnd_info.image);
...
static void ev_view_get_property(GObject *object, guint prop_id, GValue *value,
                                 GParamSpec *pspec) {
  EvView *view = EV_VIEW(object);
...
static void ev_view_set_property(GObject *object, guint prop_id,
...
static AtkObject *ev_view_get_accessible(GtkWidget *widget) {
  ...
  if (!view->accessible) view->accessible = ev_view_accessible_new(widget);
...
static void ev_view_forall(GtkContainer *container, gboolean include_internals,
...
static void pan_gesture_pan_cb(GtkGesturePan *gesture,
                               GtkPanDirection direction, gdouble offset,
                               EvView *view) {
  GtkAllocation allocation;

  gtk_widget_get_allocation(GTK_WIDGET(view), &allocation);

  if (view->continuous || allocation.width < view->requisition.width) {
...
static void pan_gesture_end_cb(GtkGesture *gesture, GdkEventSequence *sequence,
                               EvView *view) {
  if (!gtk_gesture_handles_sequence(gesture, sequence)) return;
...
static void ev_view_hierarchy_changed(GtkWidget *widget,
  ...
  if (parent && !view->pan_gesture) {
    view->pan_gesture = gtk_gesture_pan_new(parent, GTK_ORIENTATION_HORIZONTAL);
    g_signal_connect(view->pan_gesture, "pan", G_CALLBACK(pan_gesture_pan_cb),
                     widget);
    g_signal_connect(view->pan_gesture, "end", G_CALLBACK(pan_gesture_end_cb),
                     widget);

    gtk_gesture_single_set_touch_only(GTK_GESTURE_SINGLE(view->pan_gesture),
                                      TRUE);
    gtk_event_controller_set_propagation_phase(
        GTK_EVENT_CONTROLLER(view->pan_gesture), GTK_PHASE_CAPTURE);
...
static void add_move_binding_keypad(GtkBindingSet *binding_set, guint keyval,
                                    GdkModifierType modifiers,
                                    GtkMovementStep step, gint count) {
  guint keypad_keyval = keyval - GDK_KEY_Left + GDK_KEY_KP_Left;

  gtk_binding_entry_add_signal(binding_set, keyval, modifiers, "move-cursor", 3,
                               GTK_TYPE_MOVEMENT_STEP, step, G_TYPE_INT, count,
                               G_TYPE_BOOLEAN, FALSE);
  gtk_binding_entry_add_signal(binding_set, keypad_keyval, modifiers,
                               "move-cursor", 3, GTK_TYPE_MOVEMENT_STEP, step,
                               G_TYPE_INT, count, G_TYPE_BOOLEAN, FALSE);

  /* Selection-extending version */
  gtk_binding_entry_add_signal(binding_set, keyval, modifiers | GDK_SHIFT_MASK,
                               "move-cursor", 3, GTK_TYPE_MOVEMENT_STEP, step,
                               G_TYPE_INT, count, G_TYPE_BOOLEAN, TRUE);
  gtk_binding_entry_add_signal(
      binding_set, keypad_keyval, modifiers | GDK_SHIFT_MASK, "move-cursor", 3,
      GTK_TYPE_MOVEMENT_STEP, step, G_TYPE_INT, count, G_TYPE_BOOLEAN, TRUE);
}
...
static void ev_view_class_init(EvViewClass *class) {
  ...
  g_object_class_install_property(
      ...
      g_param_spec_boolean("is-loading", "Is Loading",
                           "Whether the view is loading", FALSE,
  ...
  signals[SIGNAL_SCROLL] = g_signal_new(
      "scroll", G_TYPE_FROM_CLASS(object_class),
      G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION, G_STRUCT_OFFSET(EvViewClass, scroll),
      NULL, NULL, ev_view_marshal_VOID__ENUM_ENUM, G_TYPE_NONE, 2,
      GTK_TYPE_SCROLL_TYPE, GTK_TYPE_ORIENTATION);
  signals[SIGNAL_HANDLE_LINK] = g_signal_new(
      "handle-link", G_TYPE_FROM_CLASS(object_class),
      G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
      G_STRUCT_OFFSET(EvViewClass, handle_link), NULL, NULL,
      g_cclosure_marshal_VOID__OBJECT, G_TYPE_NONE, 1, G_TYPE_OBJECT);
  signals[SIGNAL_EXTERNAL_LINK] = g_signal_new(
      "external-link", G_TYPE_FROM_CLASS(object_class),
      G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
      G_STRUCT_OFFSET(EvViewClass, external_link), NULL, NULL,
      g_cclosure_marshal_VOID__OBJECT, G_TYPE_NONE, 1, G_TYPE_OBJECT);
  signals[SIGNAL_POPUP_MENU] = g_signal_new(
      "popup", G_TYPE_FROM_CLASS(object_class),
      G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
      G_STRUCT_OFFSET(EvViewClass, popup_menu), NULL, NULL,
      g_cclosure_marshal_VOID__POINTER, G_TYPE_NONE, 1, G_TYPE_POINTER);
  signals[SIGNAL_SELECTION_CHANGED] =
      g_signal_new("selection-changed", G_TYPE_FROM_CLASS(object_class),
                   G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
                   G_STRUCT_OFFSET(EvViewClass, selection_changed), NULL, NULL,
                   g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0, G_TYPE_NONE);
  signals[SIGNAL_SYNC_SOURCE] = g_signal_new(
      "sync-source", G_TYPE_FROM_CLASS(object_class),
      G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
      G_STRUCT_OFFSET(EvViewClass, sync_source), NULL, NULL,
      g_cclosure_marshal_VOID__POINTER, G_TYPE_NONE, 1, G_TYPE_POINTER);
  signals[SIGNAL_ANNOT_ADDED] = g_signal_new(
      "annot-added", G_TYPE_FROM_CLASS(object_class),
      G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
      G_STRUCT_OFFSET(EvViewClass, annot_added), NULL, NULL,
      g_cclosure_marshal_VOID__OBJECT, G_TYPE_NONE, 1, EV_TYPE_ANNOTATION);
  signals[SIGNAL_ANNOT_REMOVED] = g_signal_new(
      "annot-removed", G_TYPE_FROM_CLASS(object_class),
      G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
      G_STRUCT_OFFSET(EvViewClass, annot_removed), NULL, NULL,
      g_cclosure_marshal_VOID__OBJECT, G_TYPE_NONE, 1, EV_TYPE_ANNOTATION);
  signals[SIGNAL_LAYERS_CHANGED] =
      g_signal_new("layers-changed", G_TYPE_FROM_CLASS(object_class),
                   G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
                   G_STRUCT_OFFSET(EvViewClass, layers_changed), NULL, NULL,
                   g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0, G_TYPE_NONE);
  signals[SIGNAL_MOVE_CURSOR] =
      g_signal_new("move-cursor", G_TYPE_FROM_CLASS(object_class),
                   G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
                   G_STRUCT_OFFSET(EvViewClass, move_cursor), NULL, NULL,
                   ev_view_marshal_BOOLEAN__ENUM_INT_BOOLEAN, G_TYPE_BOOLEAN, 3,
                   GTK_TYPE_MOVEMENT_STEP, G_TYPE_INT, G_TYPE_BOOLEAN);
  signals[SIGNAL_CURSOR_MOVED] = g_signal_new(
      "cursor-moved", G_TYPE_FROM_CLASS(object_class), G_SIGNAL_RUN_LAST, 0,
      NULL, NULL, ev_view_marshal_VOID__INT_INT, G_TYPE_NONE, 2, G_TYPE_INT,
      G_TYPE_INT);

  binding_set = gtk_binding_set_by_class(class);

  add_move_binding_keypad(binding_set, GDK_KEY_Left, 0,
                          GTK_MOVEMENT_VISUAL_POSITIONS, -1);
  add_move_binding_keypad(binding_set, GDK_KEY_Right, 0,
                          GTK_MOVEMENT_VISUAL_POSITIONS, 1);
  add_move_binding_keypad(binding_set, GDK_KEY_Left, GDK_CONTROL_MASK,
                          GTK_MOVEMENT_WORDS, -1);
  add_move_binding_keypad(binding_set, GDK_KEY_Right, GDK_CONTROL_MASK,
                          GTK_MOVEMENT_WORDS, 1);
  add_move_binding_keypad(binding_set, GDK_KEY_Up, 0,
                          GTK_MOVEMENT_DISPLAY_LINES, -1);
  add_move_binding_keypad(binding_set, GDK_KEY_Down, 0,
                          GTK_MOVEMENT_DISPLAY_LINES, 1);
  add_move_binding_keypad(binding_set, GDK_KEY_Home, 0,
                          GTK_MOVEMENT_DISPLAY_LINE_ENDS, -1);
  add_move_binding_keypad(binding_set, GDK_KEY_End, 0,
                          GTK_MOVEMENT_DISPLAY_LINE_ENDS, 1);

  add_scroll_binding_keypad(binding_set, GDK_KEY_Left, 0,
                            GTK_SCROLL_STEP_BACKWARD,
                            GTK_ORIENTATION_HORIZONTAL);
  add_scroll_binding_keypad(binding_set, GDK_KEY_Right, 0,
                            GTK_SCROLL_STEP_FORWARD,
                            GTK_ORIENTATION_HORIZONTAL);
  add_scroll_binding_keypad(binding_set, GDK_KEY_Left, GDK_MOD1_MASK,
                            GTK_SCROLL_STEP_DOWN, GTK_ORIENTATION_HORIZONTAL);
  add_scroll_binding_keypad(binding_set, GDK_KEY_Right, GDK_MOD1_MASK,
                            GTK_SCROLL_STEP_UP, GTK_ORIENTATION_HORIZONTAL);
  add_scroll_binding_keypad(binding_set, GDK_KEY_Up, 0,
                            GTK_SCROLL_STEP_BACKWARD, GTK_ORIENTATION_VERTICAL);
  add_scroll_binding_keypad(binding_set, GDK_KEY_Down, 0,
                            GTK_SCROLL_STEP_FORWARD, GTK_ORIENTATION_VERTICAL);
  add_scroll_binding_keypad(binding_set, GDK_KEY_Up, GDK_MOD1_MASK,
                            GTK_SCROLL_STEP_DOWN, GTK_ORIENTATION_VERTICAL);
  add_scroll_binding_keypad(binding_set, GDK_KEY_Down, GDK_MOD1_MASK,
                            GTK_SCROLL_STEP_UP, GTK_ORIENTATION_VERTICAL);
  add_scroll_binding_keypad(binding_set, GDK_KEY_Page_Up, 0,
                            GTK_SCROLL_PAGE_BACKWARD, GTK_ORIENTATION_VERTICAL);
  add_scroll_binding_keypad(binding_set, GDK_KEY_Page_Down, 0,
                            GTK_SCROLL_PAGE_FORWARD, GTK_ORIENTATION_VERTICAL);
  ...
  gtk_binding_entry_add_signal(binding_set, GDK_KEY_Return, GDK_SHIFT_MASK,
                               ...
                               GTK_SCROLL_PAGE_BACKWARD, GTK_TYPE_ORIENTATION,
  ...
  gtk_binding_entry_add_signal(binding_set, GDK_KEY_BackSpace, GDK_SHIFT_MASK,
                               "scroll", 2, GTK_TYPE_SCROLL_TYPE,
                               GTK_SCROLL_PAGE_FORWARD, GTK_TYPE_ORIENTATION,
                               GTK_ORIENTATION_VERTICAL);
}
...
static void zoom_gesture_begin_cb(GtkGesture *gesture,
                                  GdkEventSequence *sequence, EvView *view) {
  view->prev_zoom_gesture_scale = 1;
}

static void zoom_gesture_scale_changed_cb(GtkGestureZoom *gesture,
                                          gdouble scale, EvView *view) {
  gdouble factor;
  ...
  gtk_gesture_get_bounding_box_center(
      GTK_GESTURE(gesture), &view->zoom_center_x, &view->zoom_center_y);
...
static void ev_view_init(EvView *view) {
  ...
  gtk_widget_set_events(GTK_WIDGET(view),
                        GDK_TOUCH_MASK | GDK_EXPOSURE_MASK |
  ...
  gtk_event_controller_set_propagation_phase(
      GTK_EVENT_CONTROLLER(view->zoom_gesture), GTK_PHASE_CAPTURE);
...
static void setup_caches(EvView *view) {
  ...
  view->pixbuf_cache = ev_pixbuf_cache_new(GTK_WIDGET(view), view->model,
                                           view->pixbuf_cache_size);
  ...
  g_signal_connect(view->pixbuf_cache, "job-finished",
                   G_CALLBACK(job_finished_cb), view);
}
...
void ev_view_set_page_cache_size(EvView *view, gsize cache_size) {
  if (view->pixbuf_cache_size == cache_size) return;
...
gboolean ev_view_is_loading(EvView *view) { return view->loading; }
...
static gboolean ev_view_autoscroll_cb(EvView *view) {
   ...
   * Replace 3 with another speed of choice: The higher, the faster it
   * accelerated based on the distance of the starting point from the mouse (All
   * also effected by the timeout interval of this callback) */

  if (view->scroll_info.start_y > view->scroll_info.last_y)
    speed = -pow(
        (((gdouble)view->scroll_info.start_y - view->scroll_info.last_y) / 100),
        3);
  else
    speed = pow(
        (((gdouble)view->scroll_info.last_y - view->scroll_info.start_y) / 100),
        3);
...
void ev_view_autoscroll_start(EvView *view) {
  ...
  if (view->scroll_info.autoscrolling) return;

  view->scroll_info.autoscrolling = TRUE;
  view->scroll_info.timeout_id =
      g_timeout_add(20, (GSourceFunc)ev_view_autoscroll_cb, view);
...
static void ev_view_rotation_changed_cb(EvDocumentModel *model,
  ...
  if (rotation != 0) clear_selection(view);
}
...
static void ev_view_sizing_mode_changed_cb(EvDocumentModel *model,
  ...
  if (mode != EV_SIZING_FREE) gtk_widget_queue_resize(GTK_WIDGET(view));
}
...
static void ev_view_scale_changed_cb(EvDocumentModel *model, GParamSpec *pspec,
                                     EvView *view) {
  gdouble scale = ev_document_model_get_scale(model);

  if (ABS(view->scale - scale) < EPSILON) return;
...
void ev_view_set_model(EvView *view, EvDocumentModel *model) {
  ...
  if (view->model) {
    g_signal_handlers_disconnect_by_func(view->model,
                                         ev_view_document_changed_cb, view);
    g_signal_handlers_disconnect_by_func(view->model, ev_view_page_changed_cb,
                                         view);
  ...
  g_signal_connect(view->model, "notify::document",
                   G_CALLBACK(ev_view_document_changed_cb), view);
  g_signal_connect(view->model, "notify::rotation",
                   G_CALLBACK(ev_view_rotation_changed_cb), view);
  g_signal_connect(view->model, "notify::inverted-colors",
                   G_CALLBACK(ev_view_inverted_colors_changed_cb), view);
  g_signal_connect(view->model, "notify::sizing-mode",
                   G_CALLBACK(ev_view_sizing_mode_changed_cb), view);
  g_signal_connect(view->model, "notify::page-layout",
                   G_CALLBACK(ev_view_page_layout_changed_cb), view);
  g_signal_connect(view->model, "notify::scale",
                   G_CALLBACK(ev_view_scale_changed_cb), view);
  g_signal_connect(view->model, "notify::continuous",
                   G_CALLBACK(ev_view_continuous_changed_cb), view);
  g_signal_connect(view->model, "notify::dual-odd-left",
                   G_CALLBACK(ev_view_dual_odd_left_changed_cb), view);
  g_signal_connect(view->model, "notify::fullscreen",
                   G_CALLBACK(ev_view_fullscreen_changed_cb), view);
  g_signal_connect(view->model, "page-changed",
                   G_CALLBACK(ev_view_page_changed_cb), view);
...
static void ev_view_reload_page(EvView *view, gint page,
                                cairo_region_t *region) {
  ev_pixbuf_cache_reload_page(view->pixbuf_cache, region, page, view->rotation,
                              view->scale);
}
...
gboolean ev_view_can_zoom_in(EvView *view) {
  return view->scale * ZOOM_IN_FACTOR <=
         ev_document_model_get_max_scale(view->model);
}

gboolean ev_view_can_zoom_out(EvView *view) {
  return view->scale * ZOOM_OUT_FACTOR >=
         ev_document_model_get_min_scale(view->model);
}
...
void ev_view_zoom_in(EvView *view) { ev_view_zoom(view, ZOOM_IN_FACTOR); }

void ev_view_zoom_out(EvView *view) { ev_view_zoom(view, ZOOM_OUT_FACTOR); }
...
static double zoom_for_size_automatic(GdkScreen *screen, gdouble doc_width,
  ...
  fit_width_scale = zoom_for_size_fit_width(doc_width, doc_height, target_width,
                                            target_height);

  if (doc_height < doc_width) {
    double fit_height_scale;

    fit_height_scale = zoom_for_size_fit_height(doc_width, doc_height,
...
static void ev_view_zoom_for_size_continuous_and_dual_page(EvView *view,
  ...
  switch (view->sizing_mode) {
    case EV_SIZING_FIT_WIDTH:
      scale = zoom_for_size_fit_width(doc_width, doc_height, width - sb_size,
                                      height);
      break;
    case EV_SIZING_FIT_PAGE:
      scale = zoom_for_size_fit_page(doc_width, doc_height, width - sb_size,
                                     height);
      break;
    case EV_SIZING_AUTOMATIC:
      scale = zoom_for_size_automatic(gtk_widget_get_screen(GTK_WIDGET(view)),
                                      doc_width, doc_height, width - sb_size,
                                      height);
      break;
    default:
      g_assert_not_reached();
...
static void ev_view_zoom_for_size_dual_page(EvView *view, int width,
  ...
  if (other_page < ev_document_get_n_pages(view->document)) {
    ...
    if (width_2 > doc_width) doc_width = width_2;
    if (height_2 > doc_height) doc_height = height_2;
  }
  compute_border(view, &border);
...
static gboolean ev_view_page_fits(EvView *view, GtkOrientation orientation) {
  ...
  if (view->sizing_mode == EV_SIZING_FIT_PAGE) return TRUE;
...
static EvRectangle *ev_view_find_get_result(EvView *view, gint page,
                                            gint result) {
  return view->find_pages
             ? (EvRectangle *)g_list_nth_data(view->find_pages[page], result)
             : NULL;
}
...
void ev_view_find_changed(EvView *view, GList **results, gint page) {
  ...
  if (view->current_page == page) gtk_widget_queue_draw(GTK_WIDGET(view));
}
...
void ev_view_find_previous(EvView *view) {
  ...
  if (view->find_result < 0) {
    ...
    view->find_result =
        MAX(0, ev_view_find_get_n_results(view, view->current_page) - 1);
    jump_to_find_result(view);
  } else {
    jump_to_find_result(view);
    gtk_widget_queue_draw(GTK_WIDGET(view));
...
void ev_view_find_cancel(EvView *view) { view->find_pages = NULL; }
...
void ev_view_highlight_forward_search(EvView *view, EvSourceLink *link) {
  ...
  if (!ev_document_has_synctex(view->document)) return;

  mapping = ev_document_synctex_forward_search(view->document, link);
  if (!mapping) return;

  if (view->synctex_result) g_free(view->synctex_result);
...
static inline gboolean gdk_point_equal(GdkPoint *a, GdkPoint *b) {
  return a->x == b->x && a->y == b->y;
}

static gboolean get_selection_page_range(EvView *view, EvSelectionStyle style,
                                         GdkPoint *start, GdkPoint *stop,
                                         gint *first_page, gint *last_page) {
...
static GList *compute_new_selection(EvView *view, EvSelectionStyle style,
                                    GdkPoint *start, GdkPoint *stop) {
  int i, first, last;
  GList *list = NULL;
  ...
  for (i = first; i <= last; i++) {
    ...
    if (i == first) {
      _ev_view_transform_view_point_to_doc_point(
          view, point, &page_area, &selection->rect.x1, &selection->rect.y1);
     ...
     * in selection->rect. */
    if (first == last) point = stop;

    if (i == last) {
      _ev_view_transform_view_point_to_doc_point(
          view, point, &page_area, &selection->rect.x2, &selection->rect.y2);
...
static void merge_selection_region(EvView *view, GList *new_list) {
  ...
  while (new_list_ptr || old_list_ptr) {
    ...
    if (cur_page < view->start_page || cur_page > view->end_page) continue;
    ...
    if (old_sel && new_sel) {
      if (old_sel->covered_region && new_sel->covered_region) {
        if (!cairo_region_equal(old_sel->covered_region,
    ...
    if (region) {
       ...
       * the area actually drawn by cairo is larger than the selected region,
       * due to rounding errors or pixel alignment.
       */
...
void _ev_view_clear_selection(EvView *view) { clear_selection(view); }

void _ev_view_set_selection(EvView *view, GdkPoint *start_point,
                            GdkPoint *end_point) {
  compute_selections(view, EV_SELECTION_STYLE_GLYPH, start_point, end_point);
}

static char *get_selected_text(EvView *view) {
  GString *text;
  GList *l;
  ...
  for (l = view->selection_info.selections; l != NULL; l = l->next) {
    ...
    tmp = ev_selection_get_selected_text(EV_SELECTION(view->document), page,
                                         selection->style, &(selection->rect));
    g_object_unref(page);
...
void ev_view_copy(EvView *ev_view) {
  ...
  if (!EV_IS_SELECTION(ev_view->document)) return;
...
static void ev_view_primary_get_cb(GtkClipboard *clipboard,
  ...
  if (ev_view->link_selected) {
    gtk_selection_data_set_text(
        selection_data, ev_link_action_get_uri(ev_view->link_selected), -1);
...
static void ev_view_update_primary_selection(EvView *ev_view) {
  ...
  clipboard =
      gtk_widget_get_clipboard(GTK_WIDGET(ev_view), GDK_SELECTION_PRIMARY);
  ...
  if (ev_view->selection_info.selections || ev_view->link_selected) {
    ...
    if (!gtk_clipboard_set_with_owner(
            clipboard, targets, n_targets, ev_view_primary_get_cb,
            ev_view_primary_clear_cb, G_OBJECT(ev_view)))
 ...
 * @view: #EvView instance
 *
 * Disconnect all signal handlers from the model, to ensure error free operation
 * of the webview, we have an equivalent function for the webview.
 */
void ev_view_disconnect_handlers(EvView *view) {
  g_signal_handlers_disconnect_by_func(
      view->model, G_CALLBACK(ev_view_rotation_changed_cb), view);
  g_signal_handlers_disconnect_by_func(
      view->model, G_CALLBACK(ev_view_inverted_colors_changed_cb), view);
  g_signal_handlers_disconnect_by_func(
      view->model, G_CALLBACK(ev_view_sizing_mode_changed_cb), view);
  g_signal_handlers_disconnect_by_func(
      view->model, G_CALLBACK(ev_view_page_layout_changed_cb), view);
  g_signal_handlers_disconnect_by_func(
      view->model, G_CALLBACK(ev_view_scale_changed_cb), view);
  g_signal_handlers_disconnect_by_func(
      view->model, G_CALLBACK(ev_view_continuous_changed_cb), view);
  g_signal_handlers_disconnect_by_func(
      view->model, G_CALLBACK(ev_view_fullscreen_changed_cb), view);
  g_signal_handlers_disconnect_by_func(
      view->model, G_CALLBACK(ev_view_page_changed_cb), view);
  g_signal_handlers_disconnect_by_func(
