...
enum { ACTION_SCROLL_UP, ACTION_SCROLL_DOWN, LAST_ACTION };

static const gchar *const ev_view_accessible_action_names[] = {
    N_("Scroll Up"), N_("Scroll Down"), NULL};

static const gchar *const ev_view_accessible_action_descriptions[] = {
    N_("Scroll View Up"), N_("Scroll View Down"), NULL};
    ...
    G_ADD_PRIVATE(EvViewAccessible)
        G_IMPLEMENT_INTERFACE(ATK_TYPE_ACTION,
            ...
            G_IMPLEMENT_INTERFACE(ATK_TYPE_DOCUMENT,
                                  ev_view_accessible_document_iface_init))

static gint get_relevant_page(EvView *view) {
  return ev_view_is_caret_navigation_enabled(view) ? view->cursor_page
                                                   : view->current_page;
}

static void clear_children(EvViewAccessible *self) {
  gint i;
  AtkObject *child;

  if (self->priv->children == NULL) return;
...
static void ev_view_accessible_finalize(GObject *object) {
  ...
  if (priv->action_idle_handler) g_source_remove(priv->action_idle_handler);
  for (i = 0; i < LAST_ACTION; i++) g_free(priv->action_descriptions[i]);
...
static void ev_view_accessible_initialize(AtkObject *obj, gpointer data) {
...
gint ev_view_accessible_get_n_pages(EvViewAccessible *self) {
  return self->priv->children == NULL ? 0 : self->priv->children->len;
}

static AtkObject *ev_view_accessible_ref_child(AtkObject *obj, gint i) {
  EvViewAccessible *self;
  EvView *view;

  g_return_val_if_fail(EV_IS_VIEW_ACCESSIBLE(obj), NULL);
  self = EV_VIEW_ACCESSIBLE(obj);
  g_return_val_if_fail(i >= 0 || i < ev_view_accessible_get_n_pages(self),
                       NULL);
  ...
  if (view->page_cache) ev_page_cache_ensure_page(view->page_cache, i);

  return g_object_ref(g_ptr_array_index(self->priv->children, i));
}

static gint ev_view_accessible_get_n_children(AtkObject *obj) {
...
static gint ev_view_accessible_get_page_count(AtkDocument *atk_document) {
...
static gint ev_view_accessible_get_current_page_number(
    AtkDocument *atk_document) {
  GtkWidget *widget;
...
static gboolean ev_view_accessible_idle_do_action(gpointer data) {
  ...
  ev_view_scroll(EV_VIEW(gtk_accessible_get_widget(GTK_ACCESSIBLE(data))),
                 priv->idle_scroll, FALSE);
  priv->action_idle_handler = 0;
  return FALSE;
}

static gboolean ev_view_accessible_action_do_action(AtkAction *action, gint i) {
  EvViewAccessiblePrivate *priv = EV_VIEW_ACCESSIBLE(action)->priv;

  if (gtk_accessible_get_widget(GTK_ACCESSIBLE(action)) == NULL) return FALSE;

  if (priv->action_idle_handler) return FALSE;
  ...
  priv->action_idle_handler =
      g_idle_add(ev_view_accessible_idle_do_action, action);
  return TRUE;
}

static gint ev_view_accessible_action_get_n_actions(AtkAction *action) {
  return LAST_ACTION;
}

static const gchar *ev_view_accessible_action_get_description(AtkAction *action,
                                                              gint i) {
  EvViewAccessiblePrivate *priv = EV_VIEW_ACCESSIBLE(action)->priv;

  if (i < 0 || i >= LAST_ACTION) return NULL;
...
static const gchar *ev_view_accessible_action_get_name(AtkAction *action,
                                                       gint i) {
  if (i < 0 || i >= LAST_ACTION) return NULL;
...
static gboolean ev_view_accessible_action_set_description(
    AtkAction *action, gint i, const gchar *description) {
  EvViewAccessiblePrivate *priv = EV_VIEW_ACCESSIBLE(action)->priv;
  gchar *old_description;

  if (i < 0 || i >= LAST_ACTION) return FALSE;
...
static void ev_view_accessible_cursor_moved(EvView *view, gint page,
                                            gint offset,
  ...
  if (priv->previous_cursor_page != page) {
    ...
    previous_page =
        g_ptr_array_index(priv->children, priv->previous_cursor_page);
...
static void ev_view_accessible_selection_changed(
    EvView *view, EvViewAccessible *view_accessible) {
...
static void page_changed_cb(EvDocumentModel *model, gint old_page,
                            gint new_page, EvViewAccessible *accessible) {
  EvView *view;
...
static void initialize_children(EvViewAccessible *self) {
  gint i;
  ...
  self->priv->children =
      g_ptr_array_new_full(n_pages, (GDestroyNotify)g_object_unref);
  for (i = 0; i < n_pages; i++) {
   ...
   */
  if (self->priv->end_page >= n_pages) self->priv->end_page = n_pages - 1;
}

static void document_changed_cb(EvDocumentModel *model, GParamSpec *pspec,
                                EvViewAccessible *accessible) {
...
void ev_view_accessible_set_model(EvViewAccessible *accessible,
  ...
  g_signal_connect(priv->model, "page-changed", G_CALLBACK(page_changed_cb),
                   accessible);
  g_signal_connect(priv->model, "notify::document",
                   G_CALLBACK(document_changed_cb), accessible);
}

static gboolean ev_view_accessible_focus_changed(GtkWidget *widget,
                                                 GdkEventFocus *event,
                                                 EvViewAccessible *self) {
  AtkObject *page_accessible;
  ...
  atk_object_notify_state_change(page_accessible, ATK_STATE_FOCUSED, event->in);
...
AtkObject *ev_view_accessible_new(GtkWidget *widget) {
  ...
  g_signal_connect(widget, "cursor-moved",
                   G_CALLBACK(ev_view_accessible_cursor_moved), accessible);
  g_signal_connect(widget, "selection-changed",
                   G_CALLBACK(ev_view_accessible_selection_changed),
                   accessible);
  g_signal_connect(widget, "focus-in-event",
                   G_CALLBACK(ev_view_accessible_focus_changed), accessible);
  g_signal_connect(widget, "focus-out-event",
                   G_CALLBACK(ev_view_accessible_focus_changed), accessible);

  view = EV_VIEW(widget);
  if (view->model)
    ev_view_accessible_set_model(EV_VIEW_ACCESSIBLE(accessible), view->model);
...
void _transform_doc_rect_to_atk_rect(EvViewAccessible *accessible, gint page,
  ...
  gtk_widget_translate_coordinates(widget, toplevel, 0, 0, &x_widget,
                                   &y_widget);
...
gboolean ev_view_accessible_is_doc_rect_showing(EvViewAccessible *accessible,
  ...
  view = EV_VIEW(gtk_accessible_get_widget(GTK_ACCESSIBLE(accessible)));
  if (page < view->start_page || page > view->end_page) return FALSE;
  ...
  _ev_view_transform_doc_rect_to_view_rect(view, page, doc_rect, &view_rect);
  hidden =
      view_rect.x + view_rect.width < x || view_rect.x > x + allocation.width ||
