...
G_DEFINE_TYPE_WITH_PRIVATE(EvTransitionAnimation, ev_transition_animation,
                           EV_TYPE_TIMELINE)

static void ev_transition_animation_init(EvTransitionAnimation *animation) {}

static void ev_transition_animation_set_property(GObject *object, guint prop_id,
                                                 const GValue *value,
                                                 GParamSpec *pspec) {
  EvTransitionAnimationPrivate *priv;

  priv = ev_transition_animation_get_instance_private(
      EV_TRANSITION_ANIMATION(object));

  switch (prop_id) {
    case PROP_EFFECT:
      if (priv->effect) g_object_unref(priv->effect);

      priv->effect = g_value_dup_object(value);
      break;
    case PROP_ORIGIN_SURFACE:
      ev_transition_animation_set_origin_surface(
          EV_TRANSITION_ANIMATION(object), g_value_get_pointer(value));
      break;
...
static void ev_transition_animation_get_property(GObject *object, guint prop_id,
                                                 GValue *value,
                                                 GParamSpec *pspec) {
  EvTransitionAnimationPrivate *priv;
...
static void ev_transition_animation_finalize(GObject *object) {
  ...
  if (priv->origin_surface) cairo_surface_destroy(priv->origin_surface);

  if (priv->dest_surface) cairo_surface_destroy(priv->dest_surface);

  G_OBJECT_CLASS(ev_transition_animation_parent_class)->finalize(object);
}

static GObject *ev_transition_animation_constructor(
  ...
  object = G_OBJECT_CLASS(ev_transition_animation_parent_class)
...
static void ev_transition_animation_class_init(
    EvTransitionAnimationClass *klass) {
  ...
  g_object_class_install_property(
      object_class, PROP_EFFECT,
      g_param_spec_object("effect", "Effect", "Transition effect description",
                          EV_TYPE_TRANSITION_EFFECT,
                          G_PARAM_CONSTRUCT_ONLY | G_PARAM_READWRITE));
  g_object_class_install_property(
      object_class, PROP_ORIGIN_SURFACE,
      g_param_spec_pointer("origin-surface", "Origin surface",
                           "Cairo surface from which the animation will happen",
                           G_PARAM_READWRITE));
  g_object_class_install_property(
      object_class, PROP_DEST_SURFACE,
      g_param_spec_pointer("dest-surface", "Destination surface",
                           "Cairo surface to which the animation will happen",
                           G_PARAM_READWRITE));
}

static void paint_surface(cairo_t *cr, cairo_surface_t *surface,
                          gdouble x_offset, gdouble y_offset, gdouble alpha,
...
static void ev_transition_animation_split(cairo_t *cr,
  ...
  g_object_get(effect, "alignment", &alignment, "direction", &direction, NULL);
  ...
  if (direction == EV_TRANSITION_DIRECTION_INWARD) {
    ...
    if (alignment == EV_TRANSITION_ALIGNMENT_HORIZONTAL) {
      cairo_rectangle(cr, 0, height * progress / 2, width,
                      height * (1 - progress));
    } else {
      cairo_rectangle(cr, width * progress / 2, 0, width * (1 - progress),
                      height);
    }
    ...
    if (alignment == EV_TRANSITION_ALIGNMENT_HORIZONTAL) {
      cairo_rectangle(cr, 0, (height / 2) - (height * progress / 2), width,
                      height * progress);
    } else {
      cairo_rectangle(cr, (width / 2) - (width * progress / 2), 0,
...
static void ev_transition_animation_blinds(cairo_t *cr,
  ...
  for (i = 0; i < N_BLINDS; i++) {
    ...
    if (alignment == EV_TRANSITION_ALIGNMENT_HORIZONTAL) {
      cairo_rectangle(cr, 0, height / N_BLINDS * i, width,
                      height / N_BLINDS * progress);
    } else {
      cairo_rectangle(cr, width / N_BLINDS * i, 0, width / N_BLINDS * progress,
                      height);
    }
...
static void ev_transition_animation_box(cairo_t *cr,
  ...
  if (direction == EV_TRANSITION_DIRECTION_INWARD) {
    ...
    cairo_rectangle(cr, width * progress / 2, height * progress / 2,
                    width * (1 - progress), height * (1 - progress));
    cairo_clip(cr);
    ...
    cairo_rectangle(cr, (width / 2) - (width * progress / 2),
                    (height / 2) - (height * progress / 2), width * progress,
                    height * progress);
    cairo_clip(cr);
...
static void ev_transition_animation_wipe(cairo_t *cr,
  ...
  if (angle == 0) {
    ...
    /* right to left */
    cairo_rectangle(cr, width * (1 - progress), 0, width * progress, height);
  } else if (angle <= 270) {
    /* top to bottom */
...
static void ev_transition_animation_push(cairo_t *cr,
  ...
  if (angle == 0) {
    /* left to right */
    paint_surface(cr, priv->origin_surface, -(width * progress), 0, 1.,
                  page_area);
    paint_surface(cr, priv->dest_surface, width * (1 - progress), 0, 1.,
                  page_area);
  } else {
    /* top to bottom */
    paint_surface(cr, priv->origin_surface, 0, -(height * progress), 1.,
                  page_area);
    paint_surface(cr, priv->dest_surface, 0, height * (1 - progress), 1.,
                  page_area);
  }
}
...
void ev_transition_animation_paint(EvTransitionAnimation *animation,
  ...
  switch (type) {
    ...
    case EV_TRANSITION_EFFECT_SPLIT:
      ev_transition_animation_split(cr, animation, priv->effect, progress,
                                    page_area);
      break;
    case EV_TRANSITION_EFFECT_BLINDS:
      ev_transition_animation_blinds(cr, animation, priv->effect, progress,
                                     page_area);
      break;
    case EV_TRANSITION_EFFECT_BOX:
      ev_transition_animation_box(cr, animation, priv->effect, progress,
                                  page_area);
      break;
    case EV_TRANSITION_EFFECT_WIPE:
      ev_transition_animation_wipe(cr, animation, priv->effect, progress,
                                   page_area);
      break;
    case EV_TRANSITION_EFFECT_DISSOLVE:
      ev_transition_animation_dissolve(cr, animation, priv->effect, progress,
                                       page_area);
      break;
    case EV_TRANSITION_EFFECT_PUSH:
      ev_transition_animation_push(cr, animation, priv->effect, progress,
                                   page_area);
      break;
    case EV_TRANSITION_EFFECT_COVER:
      ev_transition_animation_cover(cr, animation, priv->effect, progress,
                                    page_area);
      break;
    case EV_TRANSITION_EFFECT_UNCOVER:
      ev_transition_animation_uncover(cr, animation, priv->effect, progress,
                                      page_area);
      break;
    case EV_TRANSITION_EFFECT_FADE:
      ev_transition_animation_fade(cr, animation, priv->effect, progress,
                                   page_area);
      break;
    default: {
      ...
      enum_value = g_enum_get_value(
          g_type_class_peek(EV_TYPE_TRANSITION_EFFECT_TYPE), type);
...
EvTransitionAnimation *ev_transition_animation_new(EvTransitionEffect *effect) {
  ...
  return g_object_new(EV_TYPE_TRANSITION_ANIMATION, "effect", effect, NULL);
}
...
void ev_transition_animation_set_origin_surface(
  ...
  if (priv->origin_surface == origin_surface) return;
...
void ev_transition_animation_set_dest_surface(EvTransitionAnimation *animation,
  ...
  if (priv->dest_surface == dest_surface) return;
