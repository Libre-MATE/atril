...
/*** Callbacks ***/
static void ev_web_view_change_page(EvWebView *webview, gint new_page);

static void ev_web_view_page_changed_cb(EvDocumentModel *model, gint old_page,
                                        gint new_page, EvWebView *webview);
/*** GObject ***/
...
static void web_view_update_range_and_current_page(EvWebView *webview) {
  g_return_if_fail(EV_IS_WEB_VIEW(webview));

  if (ev_document_get_n_pages(webview->document) <= 0) return;
...
GtkWidget *ev_web_view_new(void) {
  ...
  EV_WEB_VIEW(webview)->findcontroller =
      webkit_web_view_get_find_controller(WEBKIT_WEB_VIEW(webview));
  EV_WEB_VIEW(webview)->findoptions =
      webkit_find_controller_get_options(EV_WEB_VIEW(webview)->findcontroller);
...
static void ev_web_view_document_changed_cb(EvDocumentModel *model,
                                            GParamSpec *pspec,
                                            EvWebView *webview) {
  g_return_if_fail(EV_IS_WEB_VIEW(webview));
  ...
  if (document != webview->document) {
    if (webview->document) g_object_unref(webview->document);
...
static void ev_web_view_inverted_colors_changed_cb(EvDocumentModel *model,
                                                   GParamSpec *pspec,
                                                   EvWebView *webview) {
  EvDocument *document = ev_document_model_get_document(model);

  if (!document || !document->iswebdocument) return;
...
void ev_web_view_set_model(EvWebView *webview, EvDocumentModel *model) {
  g_return_if_fail(EV_IS_WEB_VIEW(webview));
  g_return_if_fail(EV_IS_DOCUMENT_MODEL(model));

  if (model == webview->model) return;

  if (webview->model) {
    g_signal_handlers_disconnect_by_func(
        webview->model, ev_web_view_document_changed_cb, webview);
    g_signal_handlers_disconnect_by_func(webview->model,
                                         ev_web_view_page_changed_cb, webview);
  ...
  g_signal_connect(webview->model, "notify::document",
                   G_CALLBACK(ev_web_view_document_changed_cb), webview);
  g_signal_connect(webview->model, "notify::inverted-colors",
                   G_CALLBACK(ev_web_view_inverted_colors_changed_cb), webview);
  g_signal_connect(webview->model, "page-changed",
                   G_CALLBACK(ev_web_view_page_changed_cb), webview);
}

void ev_web_view_reload_page(EvWebView *webview, gint page) {
  webkit_web_view_reload(WEBKIT_WEB_VIEW(webview));
}
...
gboolean ev_web_view_next_page(EvWebView *webview) {
  ...
  if (!webview->document) return FALSE;
...
static void results_counted_cb(WebKitFindController *findcontroller,
                               guint match_count, EvWebView *webview) {
  if (match_count > 0 && webview->search->on_result < match_count) {
    webkit_find_controller_search(findcontroller,
                                  ...
                                  webview->findoptions, match_count);
 ...
 */
static void jump_to_find_results(EvWebView *webview, WebKitLoadEvent load_event,
                                 gpointer data) {
  ...
  webkit_find_controller_count_matches(webview->findcontroller,
                                       webview->search->search_string,
                                       webview->findoptions, G_MAXUINT);
  webview->search->search_jump = FALSE;
}

static gint ev_web_view_find_get_n_results(EvWebView *webview, gint page) {
 ...
 **/
static void jump_to_find_page(EvWebView *webview,
                              EvWebViewFindDirection direction, gint shift) {
  int n_pages, i;
  ...
  for (i = 0; i < n_pages; i++) {
    ...
    if (page == webview->current_page &&
        ev_web_view_find_get_n_results(webview, page) > 0) {
...
void ev_web_view_find_changed(EvWebView *webview, guint *results, gchar *text,
                              gboolean case_sensitive) {
...
void ev_web_view_find_previous(EvWebView *webview) {
  ...
  if (webview->search->on_result < 0) {
    ...
    webview->search->on_result = MAX(
        0, ev_web_view_find_get_n_results(webview, webview->current_page) - 1);
  } else {
...
void ev_web_view_set_handler(EvWebView *webview, gboolean visible) {
  if (visible) {
    g_signal_connect(webview, "load-changed", G_CALLBACK(jump_to_find_results),
                     NULL);
    g_signal_connect(webview->findcontroller, "counted-matches",
                     G_CALLBACK(results_counted_cb), webview);
  } else {
    g_signal_handlers_disconnect_by_func(webview, jump_to_find_results, NULL);
    g_signal_handlers_disconnect_by_func(webview, results_counted_cb, NULL);
  }
}
...
static void copy_text_cb(WebKitWebView *webview, GAsyncResult *res,
                         gpointer data) {
  gboolean okay_to_copy = webkit_web_view_can_execute_editing_command_finish(
      WEBKIT_WEB_VIEW(webview), res, NULL);
...
void ev_web_view_copy(EvWebView *webview) {
  webkit_web_view_can_execute_editing_command(
      WEBKIT_WEB_VIEW(webview), WEBKIT_EDITING_COMMAND_COPY, NULL,
      (GAsyncReadyCallback)copy_text_cb, NULL);
}
...
gboolean ev_web_view_zoom_out(EvWebView *webview) {
  if (webview->zoom_level == 1) return FALSE;
 ...
 * @webview : #EvWebView instance
 *
 * This function call will disconnect all model signal handlers from the
 * webview, to ensure smooth operation of the Atril-view. Equivalent to function
 * ev_view_disconnect_handlers in ev-view.c
 */
void ev_web_view_disconnect_handlers(EvWebView *webview) {
  g_signal_handlers_disconnect_by_func(
      webview->model, ev_web_view_document_changed_cb, webview);
  g_signal_handlers_disconnect_by_func(
      webview->model, ev_web_view_inverted_colors_changed_cb, webview);
  g_signal_handlers_disconnect_by_func(webview->model,
                                       ev_web_view_page_changed_cb, webview);
}
#endif
