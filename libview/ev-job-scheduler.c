...
static GQueue *job_queue[EV_JOB_N_PRIORITIES] = {&queue_urgent, &queue_high,
                                                 &queue_low, &queue_none};

static void ev_job_queue_push(EvSchedulerJob *job, EvJobPriority priority) {
  ev_debug_message(DEBUG_JOBS, "%s priority %d", EV_GET_TYPE_NAME(job->job),
                   priority);
...
static EvSchedulerJob *ev_job_queue_get_next_unlocked(void) {
  ...
  ev_debug_message(DEBUG_JOBS, "%s",
                   job ? EV_GET_TYPE_NAME(job->job) : "No jobs in queue");
...
static void ev_scheduler_job_destroy(EvSchedulerJob *job) {
  ...
  if (job->job->run_mode == EV_JOB_RUN_MAIN_LOOP) {
    g_signal_handlers_disconnect_by_func(
        job->job, G_CALLBACK(ev_scheduler_job_destroy), job);
  } else {
    g_signal_handlers_disconnect_by_func(
        job->job->cancellable, G_CALLBACK(ev_scheduler_thread_job_cancelled),
        job);
  }
...
void ev_job_scheduler_push_job(EvJob *job, EvJobPriority priority) {
  ...
  ev_debug_message(DEBUG_JOBS, "%s pirority %d", EV_GET_TYPE_NAME(job),
                   priority);
  ...
  switch (ev_job_get_run_mode(job)) {
    ...
    case EV_JOB_RUN_MAIN_LOOP:
      g_signal_connect_swapped(job, "finished",
                               G_CALLBACK(ev_scheduler_job_destroy), s_job);
      g_signal_connect_swapped(job, "cancelled",
                               G_CALLBACK(ev_scheduler_job_destroy), s_job);
      g_idle_add_full(G_PRIORITY_DEFAULT_IDLE, (GSourceFunc)ev_job_idle,
...
void ev_job_scheduler_update_job(EvJob *job, EvJobPriority priority) {
  ...
  if (ev_job_get_run_mode(job) == EV_JOB_RUN_MAIN_LOOP) return;
