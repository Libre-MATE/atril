...
G_DEFINE_TYPE_WITH_CODE(
    EvPageAccessible, ev_page_accessible, ATK_TYPE_OBJECT,
    G_ADD_PRIVATE(EvPageAccessible)
        G_IMPLEMENT_INTERFACE(ATK_TYPE_COMPONENT,
                              ev_page_accessible_component_iface_init)
            G_IMPLEMENT_INTERFACE(ATK_TYPE_HYPERTEXT,
                                  ev_page_accessible_hypertext_iface_init)
                G_IMPLEMENT_INTERFACE(ATK_TYPE_TEXT,
                                      ev_page_accessible_text_iface_init))
...
static void ev_page_accessible_initialize_children(EvPageAccessible *self) {
  ...
  if (self->priv->children_initialized) return;

  view = ev_page_accessible_get_view(self);
  if (!ev_page_cache_is_page_cached(view->page_cache, self->priv->page)) return;
  ...
  fields =
      ev_page_cache_get_form_field_mapping(view->page_cache, self->priv->page);
  if (!links && !images && !fields) return;

  children = g_list_copy(ev_mapping_list_get_list(links));
  children =
      g_list_concat(children, g_list_copy(ev_mapping_list_get_list(images)));
  children =
      g_list_concat(children, g_list_copy(ev_mapping_list_get_list(fields)));
  ...
  for (list = children; list && list->data; list = list->next) {
    ...
    if (links && ev_mapping_list_find(links, mapping->data)) {
      EvLinkAccessible *link =
          ev_link_accessible_new(self, EV_LINK(mapping->data), &mapping->area);
      AtkHyperlink *atk_link =
          atk_hyperlink_impl_get_hyperlink(ATK_HYPERLINK_IMPL(link));
      ...
      child = ATK_OBJECT(ev_form_field_accessible_new(
          self, EV_FORM_FIELD(mapping->data), &mapping->area));

    if (child) g_ptr_array_add(self->priv->children, child);
  }
...
static void ev_page_accessible_set_property(GObject *object, guint prop_id,
                                            const GValue *value,
                                            GParamSpec *pspec) {
  ...
  switch (prop_id) {
    case PROP_VIEW_ACCESSIBLE:
      accessible->priv->view_accessible =
          EV_VIEW_ACCESSIBLE(g_value_get_object(value));
      break;
    case PROP_PAGE:
...
static void ev_page_accessible_get_property(GObject *object, guint prop_id,
  ...
  switch (prop_id) {
    case PROP_VIEW_ACCESSIBLE:
      g_value_set_object(value,
                         ev_page_accessible_get_view_accessible(accessible));
      break;
    case PROP_PAGE:
 ...
 */
static AtkRelationSet *ev_page_accessible_ref_relation_set(
  ...
  relation_set = ATK_OBJECT_CLASS(ev_page_accessible_parent_class)
                     ->ref_relation_set(accessible);
  if (relation_set == NULL) return NULL;

  n_pages = ev_view_accessible_get_n_pages(self->priv->view_accessible);
  if (n_pages == 0) return relation_set;

  if ((self->priv->page + 1) < n_pages &&
      !atk_relation_set_contains(relation_set, ATK_RELATION_FLOWS_TO)) {
    AtkObject *next_page;

    next_page = atk_object_ref_accessible_child(
        ATK_OBJECT(self->priv->view_accessible), self->priv->page + 1);
  ...
  if (self->priv->page > 0 &&
      !atk_relation_set_contains(relation_set, ATK_RELATION_FLOWS_FROM)) {
    AtkObject *prev_page;

    prev_page = atk_object_ref_accessible_child(
        ATK_OBJECT(self->priv->view_accessible), self->priv->page - 1);
...
static AtkStateSet *ev_page_accessible_ref_state_set(AtkObject *accessible) {
  ...
  state_set = ATK_OBJECT_CLASS(ev_page_accessible_parent_class)
                  ->ref_state_set(accessible);
  atk_state_set_clear_states(state_set);

  view_accessible_state_set =
      atk_object_ref_state_set(ATK_OBJECT(self->priv->view_accessible));
  copy_set = atk_state_set_or_sets(state_set, view_accessible_state_set);

  if (self->priv->page >= view->start_page &&
      self->priv->page <= view->end_page)
    atk_state_set_add_state(copy_set, ATK_STATE_SHOWING);
  else
    atk_state_set_remove_state(copy_set, ATK_STATE_SHOWING);

  relevant_page =
      ev_view_accessible_get_relevant_page(self->priv->view_accessible);
  if (atk_state_set_contains_state(view_accessible_state_set,
                                   ATK_STATE_FOCUSED) &&
...
static void ev_page_accessible_class_init(EvPageAccessibleClass *klass) {
  ...
  g_object_class_install_property(
      g_object_class, PROP_VIEW_ACCESSIBLE,
      g_param_spec_object(
          "view-accessible", "View Accessible",
  ...
  g_object_class_install_property(
      ...
      g_param_spec_int(
          "page", "Page", "Page index this page represents", -1, G_MAXINT, -1,
          G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY | G_PARAM_STATIC_STRINGS));
}
...
EvView *ev_page_accessible_get_view(EvPageAccessible *page_accessible) {
  ...
  return EV_VIEW(gtk_accessible_get_widget(
      GTK_ACCESSIBLE(page_accessible->priv->view_accessible)));
}

/* ATs expect to be able to identify sentence boundaries based on content.
 * Valid, content-based boundaries may be present at the end of a newline, for
 * instance at the end of a heading within a document. Thus being able to
 * distinguish hard returns from soft returns is necessary. However, the text we
 * get from Poppler for non-tagged PDFs has "\n" inserted at the end of each
 * line resulting in a broken accessibility implementation w.r.t. sentences.
 */
static gboolean treat_as_soft_return(EvView *view, gint page,
                                     PangoLogAttr *log_attrs, gint offset) {
  ...
  if (!log_attrs[offset].is_white) return FALSE;

  ev_page_cache_get_text_layout(view->page_cache, page, &areas, &n_areas);
  if (n_areas <= offset + 1) return FALSE;
  ...
  /* In wrapped text, the chars on either side of the newline have very similar
   * heights. Examples where this condition might fail include a newline at the
   * end of a heading, and a newline at the end of a paragraph that is followed
   * by a heading.
   */
  ...
  if (ABS(this_line_height - (next_line_start->y2 - next_line_start->y1)) >
      0.25)
    return FALSE;

  /* If there is significant white space between this line and the next, odds
   * are this is not a soft return in wrapped text. Lines within a typical
   * paragraph are at most double-spaced. If the spacing is more than that,
   * assume a hard return is present.
   */
  line_spacing = next_line_start->y1 - this_line_end->y2;
  if (line_spacing - this_line_height > 1) return FALSE;

  /* Lines within a typical paragraph have *reasonably* similar x1 coordinates.
   * But we cannot count on them being nearly identical. Examples where
   * indentation can be present in wrapped text include indenting the first line
   * of the paragraph, and hanging indents (e.g. in the works cited within an
   * academic paper). So we'll be somewhat tolerant here.
   */
  for (; prev_offset > 0 && !log_attrs[prev_offset].is_mandatory_break;
       prev_offset--)
    ;
  this_line_start = areas + prev_offset;
  if (ABS(this_line_start->x1 - next_line_start->x1) > 20) return FALSE;

  /* Ditto for x2, but this line might be short due to a wide word on the next
   * line. */
  for (; next_offset < n_areas && !log_attrs[next_offset].is_word_end;
       next_offset++)
    ;
  ...
  for (;
       next_offset < n_areas && !log_attrs[next_offset + 1].is_mandatory_break;
       next_offset++)
    ;
...
static gchar *ev_page_accessible_get_substring(AtkText *text, gint start_offset,
                                               gint end_offset) {
  ...
  if (!view->page_cache) return NULL;
...
static gchar *ev_page_accessible_get_text(AtkText *text, gint start_pos,
                                          gint end_pos) {
  return ev_page_accessible_get_substring(text, start_pos, end_pos);
}

static gunichar ev_page_accessible_get_character_at_offset(AtkText *text,
                                                           gint offset) {
  gchar *string;
  gunichar unichar;
...
static void ev_page_accessible_get_range_for_boundary(
    AtkText *text, AtkTextBoundary boundary_type, gint offset,
  ...
  if (!view->page_cache) return;

  ev_page_cache_get_text_log_attrs(view->page_cache, self->priv->page,
                                   &log_attrs, &n_attrs);
  if (!log_attrs) return;

  if (offset < 0 || offset >= n_attrs) return;
  ...
  switch (boundary_type) {
    ...
    case ATK_TEXT_BOUNDARY_WORD_START:
      for (start = offset; start > 0 && !log_attrs[start].is_word_start;
           start--)
        ;
      for (end = offset + 1; end < n_attrs && !log_attrs[end].is_word_start;
           end++)
        ;
      break;
    case ATK_TEXT_BOUNDARY_SENTENCE_START:
      for (start = offset; start > 0; start--) {
        if (log_attrs[start].is_mandatory_break &&
            treat_as_soft_return(view, self->priv->page, log_attrs, start - 1))
          continue;
        if (log_attrs[start].is_sentence_start) break;
      }
      for (end = offset + 1; end < n_attrs; end++) {
        if (log_attrs[end].is_mandatory_break &&
            treat_as_soft_return(view, self->priv->page, log_attrs, end - 1))
          continue;
        if (log_attrs[end].is_sentence_start) break;
      }
      break;
    case ATK_TEXT_BOUNDARY_LINE_START:
      for (start = offset; start > 0 && !log_attrs[start].is_mandatory_break;
           start--)
        ;
      for (end = offset + 1;
           end < n_attrs && !log_attrs[end].is_mandatory_break; end++)
        ;
      break;
    default:
...
static gchar *ev_page_accessible_get_text_at_offset(
    AtkText *text, gint offset, AtkTextBoundary boundary_type,
    gint *start_offset, gint *end_offset) {
  gchar *retval;

  ev_page_accessible_get_range_for_boundary(text, boundary_type, offset,
                                            start_offset, end_offset);
  retval = ev_page_accessible_get_substring(text, *start_offset, *end_offset);

  /* If newlines appear inside the text of a sentence (i.e. between the start
   * and end offsets returned by ev_page_accessible_get_substring), it
   * interferes with the prosody of text-to-speech based-solutions such as a
   * screen reader because speech synthesizers tend to pause after the newline
   * char as if it were the end of the sentence.
   */
...
static gboolean ev_page_accessible_set_caret_offset(AtkText *text,
                                                    gint offset) {
  EvPageAccessible *self = EV_PAGE_ACCESSIBLE(text);
  EvView *view = ev_page_accessible_get_view(self);

  ev_view_set_caret_cursor_position(view, self->priv->page, offset);
...
static gint ev_page_accessible_get_character_count(AtkText *text) {
  ...
  retval = g_utf8_strlen(
      ev_page_cache_get_text(view->page_cache, self->priv->page), -1);

  return retval;
}

static gboolean get_selection_bounds(EvView *view, EvViewSelection *selection,
                                     gint *start_offset, gint *end_offset) {
  ...
  start = _ev_view_get_caret_cursor_offset_at_doc_point(
      view, selection->page, rect.x / view->scale,
      (rect.y + (rect.height / 2)) / view->scale);
  if (start == -1) return FALSE;

  cairo_region_get_rectangle(
      selection->covered_region,
      cairo_region_num_rectangles(selection->covered_region) - 1, &rect);
  end = _ev_view_get_caret_cursor_offset_at_doc_point(
      view, selection->page, (rect.x + rect.width) / view->scale,
      (rect.y + (rect.height / 2)) / view->scale);
...
static gint ev_page_accessible_get_n_selections(AtkText *text) {
  ...
  for (l = view->selection_info.selections; l != NULL; l = l->next) {
    ...
    if (selection->page != self->priv->page) continue;
...
static gchar *ev_page_accessible_get_selection(AtkText *text,
                                               gint selection_num,
  ...
  *end_pos = -1;
  ...
  if (selection_num != 0) return NULL;
  ...
  for (l = view->selection_info.selections; l != NULL; l = l->next) {
    ...
    if (get_selection_bounds(view, selection, &start, &end) && start != end) {
      ...
      selected_text =
          ev_selection_get_selected_text(EV_SELECTION(view->document), page,
...
static AtkAttributeSet *add_attribute(AtkAttributeSet *attr_set,
...
static AtkAttributeSet *get_run_attributes(PangoAttrList *attrs,
                                           const gchar *text, gint offset,
                                           gint *start_offset,
                                           gint *end_offset) {
  ...
  if (offset < 0 || offset >= text_length) return NULL;
  ...
  pango_string =
      (PangoAttrString *)pango_attr_iterator_get(iter, PANGO_ATTR_FAMILY);
  if (pango_string) {
  ...
  if (pango_int) {
    atk_attr_set =
        add_attribute(atk_attr_set, ATK_TEXT_ATTR_UNDERLINE,
                      g_strdup(atk_text_attribute_get_value(
                          ATK_TEXT_ATTR_UNDERLINE, pango_int->value)));
  }

  pango_color =
      (PangoAttrColor *)pango_attr_iterator_get(iter, PANGO_ATTR_FOREGROUND);
  if (pango_color) {
    attr_value =
        g_strdup_printf("%u,%u,%u", pango_color->color.red,
                        pango_color->color.green, pango_color->color.blue);
    atk_attr_set =
...
static AtkAttributeSet *ev_page_accessible_get_run_attributes(
...
static AtkAttributeSet *ev_page_accessible_get_default_attributes(
    AtkText *text) {
  /* No default attributes */
  return NULL;
}

static void ev_page_accessible_get_character_extents(AtkText *text, gint offset,
                                                     gint *x, gint *y,
  ...
  ev_page_cache_get_text_layout(view->page_cache, self->priv->page, &areas,
                                &n_areas);
  if (!areas || offset >= n_areas) return;

  doc_rect = areas + offset;
  _ev_view_transform_doc_rect_to_view_rect(view, self->priv->page, doc_rect,
                                           &view_rect);
  view_rect.x -= view->scroll_x;
  view_rect.y -= view->scroll_y;

  toplevel = gtk_widget_get_toplevel(GTK_WIDGET(view));
  gtk_widget_translate_coordinates(GTK_WIDGET(view), toplevel, 0, 0, &x_widget,
                                   &y_widget);
  ...
  if (coords == ATK_XY_SCREEN) {
    ...
    gdk_window_get_origin(gtk_widget_get_window(toplevel), &x_window,
                          &y_window);
...
static gint ev_page_accessible_get_offset_at_point(AtkText *text, gint x,
                                                   gint y,
                                                   AtkCoordType coords) {
  EvPageAccessible *self = EV_PAGE_ACCESSIBLE(text);
  EvView *view = ev_page_accessible_get_view(self);
  ...
  gtk_widget_translate_coordinates(GTK_WIDGET(self), toplevel, 0, 0, &x_widget,
                                   &y_widget);
  ...
  _ev_view_transform_view_point_to_doc_point(view, &view_point, &page_area,
                                             &doc_x, &doc_y);

  for (i = 0; i < n_areas; i++) {
    rect = areas + i;
    if (doc_x >= rect->x1 && doc_x <= rect->x2 && doc_y >= rect->y1 &&
        doc_y <= rect->y2)
      offset = i;
  }
 ...
 */
static gboolean ev_page_accessible_remove_selection(AtkText *text,
                                                    gint selection_num) {
  EvPageAccessible *self = EV_PAGE_ACCESSIBLE(text);
  EvView *view = ev_page_accessible_get_view(self);
...
static gboolean ev_page_accessible_set_selection(AtkText *text,
                                                 gint selection_num,
                                                 gint start_pos, gint end_pos) {
  EvPageAccessible *self = EV_PAGE_ACCESSIBLE(text);
  EvView *view = ev_page_accessible_get_view(self);
  ...
  ev_page_cache_get_text_layout(view->page_cache, self->priv->page, &areas,
                                &n_areas);
  if (start_pos < 0 || end_pos >= n_areas) return FALSE;

  _ev_view_transform_doc_rect_to_view_rect(view, self->priv->page,
                                           areas + start_pos, &start_rect);
  _ev_view_transform_doc_rect_to_view_rect(view, self->priv->page,
                                           areas + end_pos - 1, &end_rect);
...
static gboolean ev_page_accessible_add_selection(AtkText *text, gint start_pos,
                                                 gint end_pos) {
...
static GHashTable *ev_page_accessible_get_links(EvPageAccessible *accessible) {
  ...
  if (priv->links) return priv->links;

  priv->links = g_hash_table_new_full(g_direct_hash, g_direct_equal, NULL,
                                      (GDestroyNotify)g_object_unref);
  return priv->links;
}

static AtkHyperlink *ev_page_accessible_get_link(AtkHypertext *hypertext,
                                                 gint link_index) {
  GHashTable *links;
  ...
  link_mapping =
      ev_page_cache_get_link_mapping(view->page_cache, self->priv->page);
  if (!link_mapping) return NULL;

  n_links = ev_mapping_list_length(link_mapping);
  if (link_index > n_links - 1) return NULL;
  ...
  atk_link = ev_link_accessible_new(EV_PAGE_ACCESSIBLE(hypertext),
                                    EV_LINK(mapping->data), &mapping->area);
...
static gint ev_page_accessible_get_n_links(AtkHypertext *hypertext) {
  ...
  if (!EV_IS_DOCUMENT_LINKS(view->document)) return 0;
...
static gint ev_page_accessible_get_link_index(AtkHypertext *hypertext,
                                              gint offset) {
  guint i;
  ...
  for (i = 0; i < n_links; i++) {
    ...
    if (start_index <= offset && end_index >= offset) return i;
  }
...
static void ev_page_accessible_get_extents(AtkComponent *atk_component, gint *x,
  ...
  _transform_doc_rect_to_atk_rect(self->priv->view_accessible, self->priv->page,
                                  &doc_rect, &atk_rect, coord_type);
...
EvPageAccessible *ev_page_accessible_new(EvViewAccessible *view_accessible,
  ...
  atk_page = g_object_new(EV_TYPE_PAGE_ACCESSIBLE, "view-accessible",
                          view_accessible, "page", page, NULL);
