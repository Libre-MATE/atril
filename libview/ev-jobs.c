...
enum { CANCELLED, FINISHED, LAST_SIGNAL };

enum { FONTS_UPDATED, FONTS_LAST_SIGNAL };

enum { FIND_UPDATED, FIND_LAST_SIGNAL };
...
/* EvJob */
static void ev_job_init(EvJob *job) { job->cancellable = g_cancellable_new(); }

static void ev_job_dispose(GObject *object) {
  EvJob *job;
...
static void ev_job_class_init(EvJobClass *class) {
  ...
  job_signals[CANCELLED] =
      g_signal_new("cancelled", EV_TYPE_JOB, G_SIGNAL_RUN_LAST,
                   G_STRUCT_OFFSET(EvJobClass, cancelled), NULL, NULL,
                   g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
  job_signals[FINISHED] =
      g_signal_new("finished", EV_TYPE_JOB, G_SIGNAL_RUN_FIRST,
                   G_STRUCT_OFFSET(EvJobClass, finished), NULL, NULL,
                   g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
}

static gboolean emit_finished(EvJob *job) {
  ev_debug_message(DEBUG_JOBS, "%s (%p)", EV_GET_TYPE_NAME(job), job);

  job->idle_finished_id = 0;

  if (job->cancelled) {
    ev_debug_message(DEBUG_JOBS,
                     "%s (%p) job was cancelled, do not emit finished",
                     EV_GET_TYPE_NAME(job), job);
  } else {
...
static void ev_job_emit_finished(EvJob *job) {
  ev_debug_message(DEBUG_JOBS, "%s (%p)", EV_GET_TYPE_NAME(job), job);

  if (g_cancellable_is_cancelled(job->cancellable)) {
    ev_debug_message(DEBUG_JOBS, "%s (%p) job was cancelled, returning",
                     EV_GET_TYPE_NAME(job), job);
    return;
  }
  ...
  if (job->run_mode == EV_JOB_RUN_THREAD) {
    job->idle_finished_id =
        g_idle_add_full(G_PRIORITY_DEFAULT_IDLE, (GSourceFunc)emit_finished,
                        g_object_ref(job), (GDestroyNotify)g_object_unref);
  } else {
    ev_profiler_stop(EV_PROFILE_JOBS, "%s (%p)", EV_GET_TYPE_NAME(job), job);
    g_signal_emit(job, job_signals[FINISHED], 0);
  }
...
void ev_job_cancel(EvJob *job) {
  ...
  ev_debug_message(DEBUG_JOBS, "job %s (%p) cancelled", EV_GET_TYPE_NAME(job),
                   job);
  ev_profiler_stop(EV_PROFILE_JOBS, "%s (%p)", EV_GET_TYPE_NAME(job), job);
  ...
  if (job->finished && job->idle_finished_id == 0) return;
...
void ev_job_failed(EvJob *job, GQuark domain, gint code, const gchar *format,
  ...
  ev_debug_message(DEBUG_JOBS, "job %s (%p) failed", EV_GET_TYPE_NAME(job),
                   job);
...
void ev_job_succeeded(EvJob *job) {
  ...
  ev_debug_message(DEBUG_JOBS, "job %s (%p) succeeded", EV_GET_TYPE_NAME(job),
                   job);
...
gboolean ev_job_is_finished(EvJob *job) { return job->finished; }

gboolean ev_job_is_failed(EvJob *job) { return job->failed; }

EvJobRunMode ev_job_get_run_mode(EvJob *job) { return job->run_mode; }
...
static void ev_job_links_dispose(GObject *object) {
  EvJobLinks *job;
...
static gboolean fill_page_labels(GtkTreeModel *tree_model, GtkTreePath *path,
                                 GtkTreeIter *iter, EvJob *job) {
  ...
  if (!page_label) return FALSE;

  gtk_tree_store_set(GTK_TREE_STORE(tree_model), iter,
                     EV_DOCUMENT_LINKS_COLUMN_PAGE_LABEL, page_label, -1);
...
static gboolean ev_job_links_run(EvJob *job) {
  ...
  job_links->model =
      ev_document_links_get_links_model(EV_DOCUMENT_LINKS(job->document));
  ev_document_doc_mutex_unlock();

  gtk_tree_model_foreach(job_links->model,
                         (GtkTreeModelForeachFunc)fill_page_labels, job);
...
static void ev_job_attachments_dispose(GObject *object) {
...
static gboolean ev_job_attachments_run(EvJob *job) {
  ...
  ev_document_doc_mutex_lock();
  job_attachments->attachments = ev_document_attachments_get_attachments(
      EV_DOCUMENT_ATTACHMENTS(job->document));
  ev_document_doc_mutex_unlock();
...
static void ev_job_annots_dispose(GObject *object) {
  EvJobAnnots *job;
...
static gboolean ev_job_annots_run(EvJob *job) {
  ...
  for (i = 0; i < ev_document_get_n_pages(job->document); i++) {
    ...
    mapping_list = ev_document_annotations_get_annotations(
        EV_DOCUMENT_ANNOTATIONS(job->document), page);
    g_object_unref(page);
...
static void ev_job_render_dispose(GObject *object) {
  EvJobRender *job;
...
static gboolean ev_job_render_run(EvJob *job) {
  ...
  if ((job_render->surface = ev_document_render(job->document, rc)) == NULL) {
    ...
    ev_document_doc_mutex_unlock();
    g_object_unref(rc);
    ev_job_failed(job, EV_DOCUMENT_ERROR, EV_DOCUMENT_ERROR_INVALID,
                  _("Failed to render page %d"), job_render->page);
    return FALSE;
  }
  ...
  if (job_render->include_selection && EV_IS_SELECTION(job->document)) {
    ev_selection_render_selection(
        EV_SELECTION(job->document), rc, &(job_render->selection),
        &(job_render->selection_points), NULL, job_render->selection_style,
        &(job_render->text), &(job_render->base));
    job_render->selection_region = ev_selection_get_selection_region(
        EV_SELECTION(job->document), rc, job_render->selection_style,
...
EvJob *ev_job_render_new(EvDocument *document, gint page, gint rotation,
                         gdouble scale, gint width, gint height) {
  EvJobRender *job;
...
static gboolean ev_job_page_data_run(EvJob *job) {
  ...
  if ((job_pd->flags & EV_PAGE_DATA_INCLUDE_TEXT_MAPPING) &&
      EV_IS_DOCUMENT_TEXT(job->document))
    job_pd->text_mapping = ev_document_text_get_text_mapping(
        EV_DOCUMENT_TEXT(job->document), ev_page);
  if ((job_pd->flags & EV_PAGE_DATA_INCLUDE_TEXT) &&
      EV_IS_DOCUMENT_TEXT(job->document))
    job_pd->text =
        ev_document_text_get_text(EV_DOCUMENT_TEXT(job->document), ev_page);
  if ((job_pd->flags & EV_PAGE_DATA_INCLUDE_TEXT_LAYOUT) &&
      EV_IS_DOCUMENT_TEXT(job->document))
    ev_document_text_get_text_layout(EV_DOCUMENT_TEXT(job->document), ev_page,
                                     &(job_pd->text_layout),
                                     &(job_pd->text_layout_length));
  if ((job_pd->flags & EV_PAGE_DATA_INCLUDE_TEXT_ATTRS) &&
      EV_IS_DOCUMENT_TEXT(job->document))
    job_pd->text_attrs = ev_document_text_get_text_attrs(
        EV_DOCUMENT_TEXT(job->document), ev_page);
  if ((job_pd->flags & EV_PAGE_DATA_INCLUDE_TEXT_LOG_ATTRS) && job_pd->text) {
    job_pd->text_log_attrs_length = g_utf8_strlen(job_pd->text, -1);
    job_pd->text_log_attrs =
        g_new0(PangoLogAttr, job_pd->text_log_attrs_length + 1);
  ...
  if ((job_pd->flags & EV_PAGE_DATA_INCLUDE_LINKS) &&
      EV_IS_DOCUMENT_LINKS(job->document))
    job_pd->link_mapping =
        ev_document_links_get_links(EV_DOCUMENT_LINKS(job->document), ev_page);
  if ((job_pd->flags & EV_PAGE_DATA_INCLUDE_FORMS) &&
      EV_IS_DOCUMENT_FORMS(job->document))
    job_pd->form_field_mapping = ev_document_forms_get_form_fields(
        EV_DOCUMENT_FORMS(job->document), ev_page);
  if ((job_pd->flags & EV_PAGE_DATA_INCLUDE_IMAGES) &&
      EV_IS_DOCUMENT_IMAGES(job->document))
    job_pd->image_mapping = ev_document_images_get_image_mapping(
        EV_DOCUMENT_IMAGES(job->document), ev_page);
  if ((job_pd->flags & EV_PAGE_DATA_INCLUDE_ANNOTS) &&
      EV_IS_DOCUMENT_ANNOTATIONS(job->document))
    job_pd->annot_mapping = ev_document_annotations_get_annotations(
        EV_DOCUMENT_ANNOTATIONS(job->document), ev_page);
  g_object_unref(ev_page);
  ev_document_doc_mutex_unlock();
...
EvJob *ev_job_page_data_new(EvDocument *document, gint page,
...
static void ev_job_thumbnail_dispose(GObject *object) {
  EvJobThumbnail *job;
...
#if ENABLE_EPUB
static void snapshot_callback(WebKitWebView *webview, GAsyncResult *results,
  ...
  EvPage *page =
      ev_document_get_page(EV_JOB(job_thumb)->document, job_thumb->page);
  job_thumb->surface =
      webkit_web_view_get_snapshot_finish(webview, results, &error);
  ...
  EvRenderContext *rc =
      ev_render_context_new(page, job_thumb->rotation, job_thumb->scale);
  ...
  screenshotpage->backend_destroy_func =
      (EvBackendPageDestroyFunc)cairo_surface_destroy;
  ev_render_context_set_page(rc, screenshotpage);

  job_thumb->thumbnail = ev_document_thumbnails_get_thumbnail(
      EV_DOCUMENT_THUMBNAILS(EV_JOB(job_thumb)->document), rc, TRUE);
...
static void web_thumbnail_get_screenshot_cb(WebKitWebView *webview,
  ...
  webkit_web_view_get_snapshot(
      webview, WEBKIT_SNAPSHOT_REGION_VISIBLE, WEBKIT_SNAPSHOT_OPTIONS_NONE,
      NULL, (GAsyncReadyCallback)snapshot_callback, g_object_ref(job_thumb));
}

static gboolean webview_load_failed_cb(WebKitWebView *webview,
                                       WebKitLoadEvent event,
                                       gchar *failing_uri, gpointer error,
                                       EvJobThumbnail *job_thumb) {
...
static gboolean ev_job_thumbnail_run(EvJob *job) {
  ...
  if (job->document->iswebdocument) {
    ...
    if (!ev_document_doc_mutex_trylock()) return TRUE;
  } else {
    ev_document_doc_mutex_lock();
  }
  ...
  if (job->document->iswebdocument == TRUE) {
    ...
    webkit_web_view_load_uri(WEBKIT_WEB_VIEW(webview),
                             (gchar *)rc->page->backend_page);
  } else
#endif /* ENABLE_EPUB */
  {
    ev_document_doc_mutex_lock();
    job_thumb->thumbnail = ev_document_thumbnails_get_thumbnail(
        EV_DOCUMENT_THUMBNAILS(job->document), rc, TRUE);
    ev_document_doc_mutex_unlock();
    ev_job_succeeded(job);
  }
  g_object_unref(rc);
...
EvJob *ev_job_thumbnail_new(EvDocument *document, gint page, gint rotation,
                            gdouble scale) {
  EvJobThumbnail *job;
...
/* EvJobFonts */
static void ev_job_fonts_init(EvJobFonts *job) {
  EV_JOB(job)->run_mode = EV_JOB_RUN_MAIN_LOOP;
}

static gboolean ev_job_fonts_run(EvJob *job) {
  ...
  if (job_fonts->scan_completed) ev_job_succeeded(job);

  return !job_fonts->scan_completed;
}

static void ev_job_fonts_class_init(EvJobFontsClass *class) {
  EvJobClass *job_class = EV_JOB_CLASS(class);

  job_class->run = ev_job_fonts_run;

  job_fonts_signals[FONTS_UPDATED] = g_signal_new(
      "updated", EV_TYPE_JOB_FONTS, G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(EvJobFontsClass, updated), NULL, NULL,
...
static void ev_job_load_dispose(GObject *object) {
...
static gboolean ev_job_load_run(EvJob *job) {
  ...
  if (job->document) {
    ...
    uncompressed_uri =
        g_object_get_data(G_OBJECT(job->document), "uri-uncompressed");
    ev_document_load(job->document,
                     uncompressed_uri ? uncompressed_uri : job_load->uri,
                     &error);
  } else {
    job->document = ev_document_factory_get_document(job_load->uri, &error);
  }
...
void ev_job_load_set_uri(EvJobLoad *job, const gchar *uri) {
  ev_debug_message(DEBUG_JOBS, "%s", uri);

  if (job->uri) g_free(job->uri);
  job->uri = g_strdup(uri);
}
...
void ev_job_load_set_password(EvJobLoad *job, const gchar *password) {
  ...
  if (job->password) g_free(job->password);
...
static void ev_job_save_dispose(GObject *object) {
...
static gboolean ev_job_save_run(EvJob *job) {
  ...
  ev_debug_message(DEBUG_JOBS, "uri: %s, document_uri: %s", job_save->uri,
                   job_save->document_uri);
  ev_profiler_start(EV_PROFILE_JOBS, "%s (%p)", EV_GET_TYPE_NAME(job), job);
  ...
  if (g_object_get_data(G_OBJECT(job->document), "uri-uncompressed")) {
    ...
    if (ext && g_ascii_strcasecmp(ext, ".gz") == 0) ctype = EV_COMPRESSION_GZIP;
...
EvJob *ev_job_save_new(EvDocument *document, const gchar *uri,
...
static void ev_job_find_dispose(GObject *object) {
...
static gboolean ev_job_find_run(EvJob *job) {
  ...
  if (job->document->iswebdocument) {
    job_find->results[job_find->current_page] = ev_document_find_check_for_hits(
        find, ev_page, job_find->text, job_find->case_sensitive);
  } else {
...
static void ev_job_find_class_init(EvJobFindClass *class) {
  ...
  job_find_signals[FIND_UPDATED] =
      g_signal_new("updated", EV_TYPE_JOB_FIND, G_SIGNAL_RUN_LAST,
                   G_STRUCT_OFFSET(EvJobFindClass, updated), NULL, NULL,
                   g_cclosure_marshal_VOID__INT, G_TYPE_NONE, 1, G_TYPE_INT);
}

EvJob *ev_job_find_new(EvDocument *document, gint start_page, gint n_pages,
...
gint ev_job_find_get_n_results(EvJobFind *job, gint page) {
...
gdouble ev_job_find_get_progress(EvJobFind *job) {
  ...
  if (ev_job_is_finished(EV_JOB(job))) return 1.0;
...
gboolean ev_job_find_has_results(EvJobFind *job) { return job->has_results; }
 ...
 */
GList **ev_job_find_get_results(EvJobFind *job) { return job->pages; }
...
static void ev_job_layers_dispose(GObject *object) {
  EvJobLayers *job;
...
static gboolean ev_job_layers_run(EvJob *job) {
  ...
  ev_document_doc_mutex_lock();
  job_layers->model =
      ev_document_layers_get_layers(EV_DOCUMENT_LAYERS(job->document));
  ev_document_doc_mutex_unlock();
...
static void ev_job_export_dispose(GObject *object) {
  EvJobExport *job;
...
static gboolean ev_job_export_run(EvJob *job) {
...
void ev_job_export_set_page(EvJobExport *job, gint page) { job->page = page; }
...
static void ev_job_print_dispose(GObject *object) {
  EvJobPrint *job;
...
static gboolean ev_job_print_run(EvJob *job) {
  ...
  ev_document_print_print_page(EV_DOCUMENT_PRINT(job->document), ev_page,
                               job_print->cr);
  g_object_unref(ev_page);

  ev_document_doc_mutex_unlock();

  if (g_cancellable_is_cancelled(job->cancellable)) return FALSE;

  cr_status = cairo_status(job_print->cr);
  if (cr_status == CAIRO_STATUS_SUCCESS) {
    ev_job_succeeded(job);
  } else {
    ev_job_failed(job, GTK_PRINT_ERROR, GTK_PRINT_ERROR_GENERAL,
                  _("Failed to print page %d: %s"), job_print->page,
...
void ev_job_print_set_page(EvJobPrint *job, gint page) { job->page = page; }

void ev_job_print_set_cairo(EvJobPrint *job, cairo_t *cr) {
  if (job->cr == cr) return;

  if (job->cr) cairo_destroy(job->cr);
