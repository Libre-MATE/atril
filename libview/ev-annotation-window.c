...
enum { PROP_0, PROP_ANNOTATION, PROP_PARENT };

enum { CLOSED, MOVED, N_SIGNALS };
...
struct _EvAnnotationWindowClass {
  ...
  void (*moved)(EvAnnotationWindow *window, gint x, gint y);
};
...
static gdouble get_monitor_dpi(EvAnnotationWindow *window) {
...
static void ev_annotation_window_set_color(EvAnnotationWindow *window,
  ...
  gtk_style_properties_set(properties, 0, "background-color", color, NULL);

  provider = GTK_STYLE_PROVIDER(properties);
  gtk_style_context_add_provider(
      gtk_widget_get_style_context(GTK_WIDGET(window)), provider,
      GTK_STYLE_PROVIDER_PRIORITY_APPLICATION);
  gtk_style_context_add_provider(
      gtk_widget_get_style_context(window->close_button), provider,
      GTK_STYLE_PROVIDER_PRIORITY_APPLICATION);
  gtk_style_context_add_provider(
      gtk_widget_get_style_context(window->resize_se), provider,
      GTK_STYLE_PROVIDER_PRIORITY_APPLICATION);
  gtk_style_context_add_provider(
      gtk_widget_get_style_context(window->resize_sw), provider,
      GTK_STYLE_PROVIDER_PRIORITY_APPLICATION);
...
static void ev_annotation_window_label_changed(EvAnnotationMarkup *annot,
                                               GParamSpec *pspec,
...
static void ev_annotation_window_color_changed(EvAnnotation *annot,
                                               GParamSpec *pspec,
                                               EvAnnotationWindow *window) {
  GdkRGBA rgba;
...
static void ev_annotation_window_dispose(GObject *object) {
  EvAnnotationWindow *window = EV_ANNOTATION_WINDOW(object);

  if (window->annotation) ev_annotation_window_sync_contents(window);
...
static void ev_annotation_window_set_property(GObject *object, guint prop_id,
                                              const GValue *value,
                                              GParamSpec *pspec) {
  EvAnnotationWindow *window = EV_ANNOTATION_WINDOW(object);
...
static gboolean ev_annotation_window_resize(EvAnnotationWindow *window,
  ...
  if (event->type == GDK_BUTTON_PRESS && event->button == 1) {
    gtk_window_begin_resize_drag(
        GTK_WINDOW(window),
        window->resize_sw == ebox ? GDK_WINDOW_EDGE_SOUTH_WEST
...
static void ev_annotation_window_set_resize_cursor(GtkWidget *widget,
  ...
  if (gtk_widget_is_sensitive(widget)) {
    ...
    cursor = gdk_cursor_new_for_display(display, widget == window->resize_sw
...
static void text_view_state_flags_changed(GtkWidget *widget,
...
static void ev_annotation_window_close(EvAnnotationWindow *window) {
...
static gboolean ev_annotation_window_button_press_event(GtkWidget *widget,
  ...
  if (event->type == GDK_BUTTON_PRESS && event->button == 1) {
    ...
    gtk_window_begin_move_drag(GTK_WINDOW(widget), event->button, event->x_root,
...
static void ev_annotation_window_init(EvAnnotationWindow *window) {
  ...
  g_signal_connect_swapped(window->close_button, "clicked",
                           G_CALLBACK(ev_annotation_window_close), window);
  ...
  g_signal_connect(window->text_view, "state-flags-changed",
                   G_CALLBACK(text_view_state_flags_changed), window);
  ...
  g_signal_connect_swapped(window->resize_sw, "button-press-event",
                           G_CALLBACK(ev_annotation_window_resize), window);
  g_signal_connect(window->resize_sw, "realize",
                   G_CALLBACK(ev_annotation_window_set_resize_cursor), window);
...
static GObject *ev_annotation_window_constructor(
  ...
  object = G_OBJECT_CLASS(ev_annotation_window_parent_class)
  ...
  gtk_window_resize(GTK_WINDOW(window), (gint)((rect->x2 - rect->x1) * scale),
  ...
  g_signal_connect(annot, "notify::label",
                   G_CALLBACK(ev_annotation_window_label_changed), window);
  g_signal_connect(annot, "notify::rgba",
                   G_CALLBACK(ev_annotation_window_color_changed), window);
  g_signal_connect(annot, "notify::opacity",
                   G_CALLBACK(ev_annotation_window_opacity_changed), window);

  return object;
}

static gboolean ev_annotation_window_configure_event(GtkWidget *widget,
                                                     GdkEventConfigure *event) {
  EvAnnotationWindow *window = EV_ANNOTATION_WINDOW(widget);

  if (window->in_move && (window->x != event->x || window->y != event->y)) {
    window->x = event->x;
    window->y = event->y;
  }

  return GTK_WIDGET_CLASS(ev_annotation_window_parent_class)
      ->configure_event(widget, event);
}

static gboolean ev_annotation_window_focus_in_event(GtkWidget *widget,
                                                    GdkEventFocus *event) {
  EvAnnotationWindow *window = EV_ANNOTATION_WINDOW(widget);
...
static gboolean ev_annotation_window_focus_out_event(GtkWidget *widget,
                                                     GdkEventFocus *event) {
  EvAnnotationWindow *window = EV_ANNOTATION_WINDOW(widget);
...
static void ev_annotation_window_class_init(EvAnnotationWindowClass *klass) {
  ...
  g_object_class_install_property(
      g_object_class, PROP_ANNOTATION,
      g_param_spec_object("annotation", "Annotation",
                          "The annotation associated to the window",
                          EV_TYPE_ANNOTATION_MARKUP,
                          G_PARAM_WRITABLE | G_PARAM_CONSTRUCT_ONLY));
  g_object_class_install_property(
      g_object_class, PROP_PARENT,
      g_param_spec_object("parent", "Parent", "The parent window",
                          GTK_TYPE_WINDOW,
                          G_PARAM_WRITABLE | G_PARAM_CONSTRUCT_ONLY));
  signals[CLOSED] =
      g_signal_new("closed", G_TYPE_FROM_CLASS(g_object_class),
                   G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
                   G_STRUCT_OFFSET(EvAnnotationWindowClass, closed), NULL, NULL,
                   g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0, G_TYPE_NONE);
  signals[MOVED] = g_signal_new("moved", G_TYPE_FROM_CLASS(g_object_class),
                                G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
                                G_STRUCT_OFFSET(EvAnnotationWindowClass, moved),
                                NULL, NULL, ev_view_marshal_VOID__INT_INT,
...
GtkWidget *ev_annotation_window_new(EvAnnotation *annot, GtkWindow *parent) {
  ...
  window = g_object_new(EV_TYPE_ANNOTATION_WINDOW, "annotation", annot,
                        "parent", parent, NULL);
  return window;
}
...
void ev_annotation_window_set_annotation(EvAnnotationWindow *window,
  ...
  if (annot == window->annotation) return;
