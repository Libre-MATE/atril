...
enum { CHANGE_PAGE, FINISHED, SIGNAL_EXTERNAL_LINK, N_SIGNALS };
...
static void ev_view_presentation_set_cursor_for_location(
    EvViewPresentation *pview, gdouble x, gdouble y);

#define HIDE_CURSOR_TIMEOUT 5

G_DEFINE_TYPE(EvViewPresentation, ev_view_presentation, GTK_TYPE_WIDGET)

static void ev_view_presentation_set_normal(EvViewPresentation *pview) {
  GtkWidget *widget = GTK_WIDGET(pview);

  if (pview->state == EV_PRESENTATION_NORMAL) return;
...
static void ev_view_presentation_set_black(EvViewPresentation *pview) {
  GtkWidget *widget = GTK_WIDGET(pview);

  if (pview->state == EV_PRESENTATION_BLACK) return;
...
static void ev_view_presentation_set_white(EvViewPresentation *pview) {
  GtkWidget *widget = GTK_WIDGET(pview);

  if (pview->state == EV_PRESENTATION_WHITE) return;
...
static void ev_view_presentation_set_end(EvViewPresentation *pview) {
  GtkWidget *widget = GTK_WIDGET(pview);

  if (pview->state == EV_PRESENTATION_END) return;

  pview->state = EV_PRESENTATION_END;
  gtk_widget_queue_draw(widget);
}

static gdouble ev_view_presentation_get_scale_for_page(
    EvViewPresentation *pview, guint page) {
  if (!ev_document_is_page_size_uniform(pview->document) || pview->scale == 0) {
    ...
    pview->scale =
        MIN(pview->monitor_width / width, pview->monitor_height / height);
  }

  return pview->scale;
}

static void ev_view_presentation_get_page_area(EvViewPresentation *pview,
                                               GdkRectangle *area) {
  GtkWidget *widget = GTK_WIDGET(pview);
  ...
  ev_document_get_page_size(pview->document, pview->current_page, &doc_width,
                            &doc_height);
...
/* Page Transition */
static gboolean transition_next_page(EvViewPresentation *pview) {
  ev_view_presentation_next_page(pview);

  return FALSE;
}

static void ev_view_presentation_transition_stop(EvViewPresentation *pview) {
  if (pview->trans_timeout_id > 0) g_source_remove(pview->trans_timeout_id);
  pview->trans_timeout_id = 0;
}

static void ev_view_presentation_transition_start(EvViewPresentation *pview) {
  gdouble duration;

  if (!EV_IS_DOCUMENT_TRANSITION(pview->document)) return;

  ev_view_presentation_transition_stop(pview);

  duration = ev_document_transition_get_page_duration(
      EV_DOCUMENT_TRANSITION(pview->document), pview->current_page);
  if (duration >= 0) {
    pview->trans_timeout_id = g_timeout_add_seconds(
        duration, (GSourceFunc)transition_next_page, pview);
  }
}

/* Animations */
static void ev_view_presentation_animation_cancel(EvViewPresentation *pview) {
  if (pview->animation) {
...
static void ev_view_presentation_transition_animation_finish(
...
static void ev_view_presentation_transition_animation_frame(
    EvViewPresentation *pview, gdouble progress) {
  gtk_widget_queue_draw(GTK_WIDGET(pview));
}

static cairo_surface_t *get_surface_from_job(EvViewPresentation *pview,
                                             EvJob *job) {
...
static void ev_view_presentation_animation_start(EvViewPresentation *pview,
                                                 gint new_page) {
  ...
  if (!pview->enable_animations) return;

  if (pview->current_page == new_page) return;

  effect = ev_document_transition_get_effect(
      EV_DOCUMENT_TRANSITION(pview->document), new_page);
  if (!effect) return;
  ...
  ev_transition_animation_set_origin_surface(
      pview->animation, surface != NULL ? surface : pview->current_surface);
  ...
  g_signal_connect_swapped(
      pview->animation, "frame",
      G_CALLBACK(ev_view_presentation_transition_animation_frame), pview);
  g_signal_connect_swapped(
      pview->animation, "finished",
      G_CALLBACK(ev_view_presentation_transition_animation_finish), pview);
}

/* Page Navigation */
static void job_finished_cb(EvJob *job, EvViewPresentation *pview) {
  EvJobRender *job_render = EV_JOB_RENDER(job);

  if (pview->inverted_colors)
    ev_document_misc_invert_surface(job_render->surface);

  if (job != pview->curr_job) return;
...
static EvJob *ev_view_presentation_schedule_new_job(EvViewPresentation *pview,
                                                    gint page,
                                                    EvJobPriority priority) {
  EvJob *job;
  gdouble scale;

  if (page < 0 || page >= ev_document_get_n_pages(pview->document)) return NULL;
  ...
  g_signal_connect(job, "finished", G_CALLBACK(job_finished_cb), pview);
  ev_job_scheduler_push_job(job, priority);

  return job;
}

static void ev_view_presentation_delete_job(EvViewPresentation *pview,
                                            EvJob *job) {
  if (!job) return;
...
static void ev_view_presentation_reset_jobs(EvViewPresentation *pview) {
  if (pview->curr_job) {
...
static void ev_view_presentation_update_current_page(EvViewPresentation *pview,
                                                     guint page) {
  gint jump;

  if (page >= ev_document_get_n_pages(pview->document)) return;
  ...
  switch (jump) {
    case 0:
      if (!pview->curr_job)
        pview->curr_job = ev_view_presentation_schedule_new_job(
            pview, page, EV_JOB_PRIORITY_URGENT);
      if (!pview->next_job)
        pview->next_job = ev_view_presentation_schedule_new_job(
            pview, page + 1, EV_JOB_PRIORITY_HIGH);
      if (!pview->prev_job)
        pview->prev_job = ev_view_presentation_schedule_new_job(
            pview, page - 1, EV_JOB_PRIORITY_LOW);
      break;
    case -1:
      ev_view_presentation_delete_job(pview, pview->next_job);
...
static void ev_view_presentation_set_current_page(EvViewPresentation *pview,
                                                  guint page) {
  if (pview->current_page == page) return;
...
static void send_focus_change(GtkWidget *widget, gboolean in) {
  ...
  if (fevent->focus_change.window) g_object_ref(fevent->focus_change.window);
...
static void ev_view_presentation_goto_window_hide(EvViewPresentation *pview) {
  /* send focus-in event */
...
static gboolean ev_view_presentation_goto_window_delete_event(
    GtkWidget *widget, GdkEventAny *event, EvViewPresentation *pview) {
  ev_view_presentation_goto_window_hide(pview);

  return TRUE;
}

static gboolean ev_view_presentation_goto_window_key_press_event(
    GtkWidget *widget, GdkEventKey *event, EvViewPresentation *pview) {
  switch (event->keyval) {
    case GDK_KEY_Escape:
    case GDK_KEY_Tab:
    case GDK_KEY_KP_Tab:
    ...
    case GDK_KEY_Delete:
      return FALSE;
    default:
      if (!KEY_IS_NUMERIC(event->keyval)) return TRUE;
  }

  return FALSE;
}

static gboolean ev_view_presentation_goto_window_button_press_event(
    GtkWidget *widget, GdkEventButton *event, EvViewPresentation *pview) {
  ev_view_presentation_goto_window_hide(pview);

  return TRUE;
}

static void ev_view_presentation_goto_entry_activate(
    GtkEntry *entry, EvViewPresentation *pview) {
  const gchar *text;
  gint page;
...
static void ev_view_presentation_goto_window_create(EvViewPresentation *pview) {
  ...
  if (pview->goto_window) {
    ...
    if (gtk_window_has_group(toplevel))
      ...
      gtk_window_group_remove_window(gtk_window_get_group(goto_window),
                                     goto_window);
  ...
  g_signal_connect(pview->goto_window, "key_press_event",
                   ...
                   pview);
  g_signal_connect(
      pview->goto_window, "button_press_event",
      G_CALLBACK(ev_view_presentation_goto_window_button_press_event), pview);
  ...
  g_signal_connect(pview->goto_entry, "activate",
                   G_CALLBACK(ev_view_presentation_goto_entry_activate), pview);
...
static void ev_view_presentation_goto_entry_grab_focus(
    EvViewPresentation *pview) {
  GtkWidgetClass *entry_parent_class;

  entry_parent_class =
      g_type_class_peek_parent(GTK_ENTRY_GET_CLASS(pview->goto_entry));
  (entry_parent_class->grab_focus)(pview->goto_entry);

  send_focus_change(pview->goto_entry, TRUE);
}

static void monitor_get_dimesions(EvViewPresentation *pview, gint *width,
                                  gint *height) {
  GdkDisplay *display;
...
static void ev_view_presentation_goto_window_send_key_event(
    EvViewPresentation *pview, GdkEvent *event) {
  ...
  gtk_window_move(GTK_WINDOW(pview->goto_window), monitor_width + 1,
                  monitor_height + 1);
...
/* Links */
static gboolean ev_view_presentation_link_is_supported(
    EvViewPresentation *pview, EvLink *link) {
  EvLinkAction *action;
...
static EvLink *ev_view_presentation_get_link_at_location(
    EvViewPresentation *pview, gdouble x, gdouble y) {
  ...
  if (!pview->page_cache) return NULL;

  ev_document_get_page_size(pview->document, pview->current_page, &width,
                            &height);
  ...
  link_mapping =
      ev_page_cache_get_link_mapping(pview->page_cache, pview->current_page);

  link = link_mapping ? ev_mapping_list_get_data(link_mapping, new_x, new_y)
                      : NULL;

  return link && ev_view_presentation_link_is_supported(pview, link) ? link
                                                                     : NULL;
}

static void ev_vew_presentation_handle_link(EvViewPresentation *pview,
                                            EvLink *link) {
  EvLinkAction *action;
  ...
  switch (ev_link_action_get_action_type(action)) {
    case EV_LINK_ACTION_TYPE_NAMED: {
      ...
      if (g_ascii_strcasecmp(name, "FirstPage") == 0) {
        ...
        ev_view_presentation_update_current_page(pview,
                                                 pview->current_page - 1);
    ...
    case EV_LINK_ACTION_TYPE_GOTO_DEST: {
      ...
      page = ev_document_links_get_dest_page(EV_DOCUMENT_LINKS(pview->document),
                                             dest);
      ev_view_presentation_update_current_page(pview, page);
    } break;
    case EV_LINK_ACTION_TYPE_GOTO_REMOTE:
...
/* Cursors */
static void ev_view_presentation_set_cursor(EvViewPresentation *pview,
                                            EvViewCursor view_cursor) {
  GtkWidget *widget;
  GdkDisplay *display;
  GdkCursor *cursor;

  if (pview->cursor == view_cursor) return;
...
static gboolean hide_cursor_timeout_cb(EvViewPresentation *pview) {
...
static void ev_view_presentation_hide_cursor_timeout_stop(
    EvViewPresentation *pview) {
...
static void ev_view_presentation_hide_cursor_timeout_start(
    EvViewPresentation *pview) {
  ev_view_presentation_hide_cursor_timeout_stop(pview);
  pview->hide_cursor_timeout_id = g_timeout_add_seconds(
      HIDE_CURSOR_TIMEOUT, (GSourceFunc)hide_cursor_timeout_cb, pview);
}

static void ev_view_presentation_update_current_surface(
    EvViewPresentation *pview, cairo_surface_t *surface) {
  if (!surface || pview->current_surface == surface) return;

  cairo_surface_reference(surface);
  if (pview->current_surface) cairo_surface_destroy(pview->current_surface);
  pview->current_surface = surface;
}

static void ev_view_presentation_dispose(GObject *object) {
...
static void ev_view_presentation_get_preferred_width(GtkWidget *widget,
                                                     gint *minimum,
                                                     gint *natural) {
  *minimum = *natural = 0;
}

static void ev_view_presentation_get_preferred_height(GtkWidget *widget,
                                                      gint *minimum,
                                                      gint *natural) {
  *minimum = *natural = 0;
}

static void ev_view_presentation_draw_end_page(EvViewPresentation *pview,
                                               cairo_t *cr) {
  GtkWidget *widget = GTK_WIDGET(pview);
  PangoLayout *layout;
...
static gboolean ev_view_presentation_draw(GtkWidget *widget, cairo_t *cr) {
  ...
  if (gdk_rectangle_intersect(&page_area, area, &overlap)) {
    ...
    if (overlap.width == page_area.width) overlap.width--;
...
static gboolean ev_view_presentation_key_press_event(GtkWidget *widget,
                                                     GdkEventKey *event) {
  EvViewPresentation *pview = EV_VIEW_PRESENTATION(widget);
  ...
  if (ev_document_get_n_pages(pview->document) > 1 &&
...
static gboolean ev_view_presentation_button_release_event(
  ...
  switch (event->button) {
    case 1: {
      ...
      link =
          ev_view_presentation_get_link_at_location(pview, event->x, event->y);
      if (link)
...
static gint ev_view_presentation_focus_out(GtkWidget *widget,
                                           GdkEventFocus *event) {
  EvViewPresentation *pview = EV_VIEW_PRESENTATION(widget);

  if (pview->goto_window) ev_view_presentation_goto_window_hide(pview);

  return FALSE;
}

static gboolean ev_view_presentation_motion_notify_event(
...
static void ev_view_presentation_update_monitor_geometry(
    EvViewPresentation *pview) {
  GdkDisplay *display = gtk_widget_get_display(GTK_WIDGET(pview));
  GdkRectangle monitor;
  GdkMonitor *monitor_num;

  monitor_num = gdk_display_get_monitor_at_window(
      display, gtk_widget_get_window(GTK_WIDGET(pview)));
  gdk_monitor_get_geometry(monitor_num, &monitor);
  pview->monitor_width = monitor.width;
  pview->monitor_height = monitor.height;
}

static gboolean init_presentation(GtkWidget *widget) {
  EvViewPresentation *pview = EV_VIEW_PRESENTATION(widget);
...
static void ev_view_presentation_realize(GtkWidget *widget) {
  GdkWindow *window;
...
static void ev_view_presentation_change_page(EvViewPresentation *pview,
...
static gboolean ev_view_presentation_scroll_event(GtkWidget *widget,
...
static void add_change_page_binding_keypad(GtkBindingSet *binding_set,
                                           guint keyval,
                                           GdkModifierType modifiers,
                                           GtkScrollType scroll) {
  guint keypad_keyval = keyval - GDK_KEY_Left + GDK_KEY_KP_Left;

  gtk_binding_entry_add_signal(binding_set, keyval, modifiers, "change_page", 1,
                               GTK_TYPE_SCROLL_TYPE, scroll);
  gtk_binding_entry_add_signal(binding_set, keypad_keyval, modifiers,
                               "change_page", 1, GTK_TYPE_SCROLL_TYPE, scroll);
}

static void ev_view_presentation_set_property(GObject *object, guint prop_id,
                                              const GValue *value,
                                              GParamSpec *pspec) {
  EvViewPresentation *pview = EV_VIEW_PRESENTATION(object);
...
static void ev_view_presentation_get_property(GObject *object, guint prop_id,
                                              GValue *value,
                                              GParamSpec *pspec) {
  EvViewPresentation *pview = EV_VIEW_PRESENTATION(object);
...
static void ev_view_presentation_notify_scale_factor(
    EvViewPresentation *pview) {
  ev_view_presentation_update_monitor_geometry(pview);
  ev_view_presentation_reset_jobs(pview);
  ev_view_presentation_update_current_page(pview, pview->current_page);
...
static GObject *ev_view_presentation_constructor(
    GType type, guint n_construct_properties,
    GObjectConstructParam *construct_params) {
  GObject *object;
  EvViewPresentation *pview;

  object = G_OBJECT_CLASS(ev_view_presentation_parent_class)
...
static void ev_view_presentation_class_init(EvViewPresentationClass *klass) {
  ...
  widget_class->get_preferred_height =
      ev_view_presentation_get_preferred_height;
  ...
  widget_class->button_release_event =
      ev_view_presentation_button_release_event;
  ...
  g_object_class_install_property(
      ...
      g_param_spec_object("document", "Document", "Document", EV_TYPE_DOCUMENT,
                          G_PARAM_WRITABLE | G_PARAM_CONSTRUCT_ONLY));
  g_object_class_install_property(
      gobject_class, PROP_CURRENT_PAGE,
      g_param_spec_uint("current_page", "Current Page", "The current page", 0,
                        G_MAXUINT, 0, G_PARAM_WRITABLE | G_PARAM_CONSTRUCT));
  g_object_class_install_property(
      gobject_class, PROP_ROTATION,
      g_param_spec_uint("rotation", "Rotation", "Current rotation angle", 0,
                        360, 0, G_PARAM_READWRITE | G_PARAM_CONSTRUCT));
  g_object_class_install_property(
      gobject_class, PROP_INVERTED_COLORS,
      g_param_spec_boolean(
          "inverted_colors", "Inverted Colors",
          "Whether presentation is displayed with inverted colors", FALSE,
          G_PARAM_WRITABLE | G_PARAM_CONSTRUCT_ONLY));

  signals[CHANGE_PAGE] = g_signal_new(
      "change_page", G_OBJECT_CLASS_TYPE(gobject_class),
      G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
      G_STRUCT_OFFSET(EvViewPresentationClass, change_page), NULL, NULL,
      g_cclosure_marshal_VOID__ENUM, G_TYPE_NONE, 1, GTK_TYPE_SCROLL_TYPE);
  signals[FINISHED] = g_signal_new(
      "finished", G_OBJECT_CLASS_TYPE(gobject_class),
      G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
      G_STRUCT_OFFSET(EvViewPresentationClass, finished), NULL, NULL,
      g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0, G_TYPE_NONE);
  signals[SIGNAL_EXTERNAL_LINK] = g_signal_new(
      "external-link", G_TYPE_FROM_CLASS(gobject_class),
      G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
      G_STRUCT_OFFSET(EvViewPresentationClass, external_link), NULL, NULL,
      g_cclosure_marshal_VOID__OBJECT, G_TYPE_NONE, 1, G_TYPE_OBJECT);

  binding_set = gtk_binding_set_by_class(klass);
  add_change_page_binding_keypad(binding_set, GDK_KEY_Left, 0,
                                 GTK_SCROLL_PAGE_BACKWARD);
  add_change_page_binding_keypad(binding_set, GDK_KEY_Right, 0,
                                 GTK_SCROLL_PAGE_FORWARD);
  add_change_page_binding_keypad(binding_set, GDK_KEY_Up, 0,
                                 GTK_SCROLL_PAGE_BACKWARD);
  add_change_page_binding_keypad(binding_set, GDK_KEY_Down, 0,
                                 GTK_SCROLL_PAGE_FORWARD);
  gtk_binding_entry_add_signal(binding_set, GDK_KEY_space, 0, "change_page", 1,
                               GTK_TYPE_SCROLL_TYPE, GTK_SCROLL_PAGE_FORWARD);
  gtk_binding_entry_add_signal(binding_set, GDK_KEY_BackSpace, 0, "change_page",
                               1, GTK_TYPE_SCROLL_TYPE,
                               GTK_SCROLL_PAGE_BACKWARD);
  gtk_binding_entry_add_signal(binding_set, GDK_KEY_Page_Down, 0, "change_page",
                               1, GTK_TYPE_SCROLL_TYPE,
                               GTK_SCROLL_PAGE_FORWARD);
  gtk_binding_entry_add_signal(binding_set, GDK_KEY_Page_Up, 0, "change_page",
                               1, GTK_TYPE_SCROLL_TYPE,
                               GTK_SCROLL_PAGE_BACKWARD);
  gtk_binding_entry_add_signal(binding_set, GDK_KEY_J, 0, "change_page", 1,
                               GTK_TYPE_SCROLL_TYPE, GTK_SCROLL_PAGE_FORWARD);
  gtk_binding_entry_add_signal(binding_set, GDK_KEY_H, 0, "change_page", 1,
                               GTK_TYPE_SCROLL_TYPE, GTK_SCROLL_PAGE_BACKWARD);
  gtk_binding_entry_add_signal(binding_set, GDK_KEY_L, 0, "change_page", 1,
                               GTK_TYPE_SCROLL_TYPE, GTK_SCROLL_PAGE_FORWARD);
  gtk_binding_entry_add_signal(binding_set, GDK_KEY_K, 0, "change_page", 1,
                               GTK_TYPE_SCROLL_TYPE, GTK_SCROLL_PAGE_BACKWARD);
}

static void ev_view_presentation_init(EvViewPresentation *pview) {
...
GtkWidget *ev_view_presentation_new(EvDocument *document, guint current_page,
  ...
  return GTK_WIDGET(g_object_new(EV_TYPE_VIEW_PRESENTATION, "document",
                                 document, "current_page", current_page,
                                 "rotation", rotation, "inverted_colors",
                                 inverted_colors, NULL));
}
...
void ev_view_presentation_set_rotation(EvViewPresentation *pview,
  ...
  if (pview->rotation == rotation) return;

  pview->rotation = rotation;
  g_object_notify(G_OBJECT(pview), "rotation");
  if (pview->is_constructing) return;
