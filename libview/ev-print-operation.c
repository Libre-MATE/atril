...
struct _EvPrintOperationClass {
  ...
  void (*set_current_page)(EvPrintOperation *op, gint current_page);
  ...
  void (*set_job_name)(EvPrintOperation *op, const gchar *job_name);
  const gchar *(*get_job_name)(EvPrintOperation *op);
  void (*run)(EvPrintOperation *op, GtkWindow *parent);
  void (*cancel)(EvPrintOperation *op);
  void (*get_error)(EvPrintOperation *op, GError **error);
  void (*set_embed_page_setup)(EvPrintOperation *op, gboolean embed);
  gboolean (*get_embed_page_setup)(EvPrintOperation *op);

  /* signals */
  void (*done)(EvPrintOperation *op, GtkPrintOperationResult result);
...
static void ev_print_operation_set_property(GObject *object, guint prop_id,
                                            const GValue *value,
                                            GParamSpec *pspec) {
  EvPrintOperation *op = EV_PRINT_OPERATION(object);
...
static void ev_print_operation_class_init(EvPrintOperationClass *klass) {
  ...
  g_object_class_install_property(
      ...
      g_param_spec_object("document", "Document", "The document to print",
                          EV_TYPE_DOCUMENT,
                          G_PARAM_WRITABLE | G_PARAM_CONSTRUCT_ONLY));
  signals[DONE] =
      g_signal_new("done", G_TYPE_FROM_CLASS(g_object_class), G_SIGNAL_RUN_LAST,
                   G_STRUCT_OFFSET(EvPrintOperationClass, done), NULL, NULL,
                   g_cclosure_marshal_VOID__ENUM, G_TYPE_NONE, 1,
                   GTK_TYPE_PRINT_OPERATION_RESULT);
  signals[BEGIN_PRINT] = g_signal_new(
      "begin_print", G_TYPE_FROM_CLASS(g_object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(EvPrintOperationClass, begin_print), NULL, NULL,
      g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
  signals[STATUS_CHANGED] = g_signal_new(
      "status_changed", G_TYPE_FROM_CLASS(g_object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(EvPrintOperationClass, status_changed), NULL, NULL,
      g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
}
...
static void ev_print_operation_update_status(EvPrintOperation *op, gint page,
                                             gint n_pages, gdouble progress) {
  if (op->status && op->progress == progress) return;
  ...
  if (page == -1) {
    ...
    op->status = g_strdup_printf(_("Printing page %d of %d…"), page, n_pages);
  }
...
#define EV_PRINT_OPERATION_EXPORT(object)                               \
  (G_TYPE_CHECK_INSTANCE_CAST((object), EV_TYPE_PRINT_OPERATION_EXPORT, \
...
G_DEFINE_TYPE(EvPrintOperationExport, ev_print_operation_export,
...
static void ev_print_queue_init(void) {
  if (G_UNLIKELY(print_queue == NULL)) {
    print_queue = g_hash_table_new_full(g_direct_hash, g_direct_equal, NULL,
                                        (GDestroyNotify)queue_free);
  }
}

static void remove_document_queue(gpointer data, GObject *document) {
  if (print_queue) g_hash_table_remove(print_queue, document);
}

static gboolean ev_print_queue_is_empty(EvDocument *document) {
  GQueue *queue;
...
static void ev_print_queue_push(EvPrintOperation *op) {
  GQueue *queue;

  queue = g_hash_table_lookup(print_queue, op->document);
  if (!queue) {
    queue = g_queue_new();
    g_hash_table_insert(print_queue, op->document, queue);
    g_object_weak_ref(G_OBJECT(op->document),
                      (GWeakNotify)remove_document_queue, NULL);
  }

  g_queue_push_head(queue, g_object_ref(op));
}

static EvPrintOperation *ev_print_queue_pop(EvDocument *document) {
  EvPrintOperation *op;
  GQueue *queue;

  queue = g_hash_table_lookup(print_queue, document);
  if (!queue || g_queue_is_empty(queue)) return NULL;
...
static EvPrintOperation *ev_print_queue_peek(EvDocument *document) {
  GQueue *queue;
...
static void ev_print_operation_export_set_current_page(EvPrintOperation *op,
                                                       gint current_page) {
...
static void ev_print_operation_export_set_print_settings(
    EvPrintOperation *op, GtkPrintSettings *print_settings) {
  EvPrintOperationExport *export = EV_PRINT_OPERATION_EXPORT(op);

  if (print_settings == export->print_settings) return;

  g_object_ref(print_settings);
  if (export->print_settings) g_object_unref(export->print_settings);
  export->print_settings = print_settings;
}

static GtkPrintSettings *ev_print_operation_export_get_print_settings(
    EvPrintOperation *op) {
  EvPrintOperationExport *export = EV_PRINT_OPERATION_EXPORT(op);
...
static void ev_print_operation_export_set_default_page_setup(
    EvPrintOperation *op, GtkPageSetup *page_setup) {
  EvPrintOperationExport *export = EV_PRINT_OPERATION_EXPORT(op);

  if (page_setup == export->page_setup) return;

  g_object_ref(page_setup);
  if (export->page_setup) g_object_unref(export->page_setup);
  export->page_setup = page_setup;
}

static GtkPageSetup *ev_print_operation_export_get_default_page_setup(
    EvPrintOperation *op) {
  EvPrintOperationExport *export = EV_PRINT_OPERATION_EXPORT(op);
...
static void ev_print_operation_export_set_job_name(EvPrintOperation *op,
                                                   const gchar *job_name) {
  EvPrintOperationExport *export = EV_PRINT_OPERATION_EXPORT(op);
...
static const gchar *ev_print_operation_export_get_job_name(
    EvPrintOperation *op) {
  EvPrintOperationExport *export = EV_PRINT_OPERATION_EXPORT(op);
...
static void ev_print_operation_export_set_printer(
    EvPrintOperationExport *export, GtkPrinter *printer) {
  if (printer == export->printer) return;

  g_object_ref(printer);
  if (export->printer) g_object_unref(export->printer);
  export->printer = printer;
}

static void find_range(EvPrintOperationExport *export) {
  GtkPageRange *range;
...
static gboolean clamp_ranges(EvPrintOperationExport *export) {
  ...
  for (i = 0; i < export->n_ranges; i++) {
    ...
    if (export->page_set == GTK_PAGE_SET_ALL) {
      ...
      if (n_pages == 1 && export->ranges[i].start % 2 == 0)
        ...
        n_pages_to_print +=
            export->ranges[i].start % 2 == 0 ? n_pages / 2 : (n_pages / 2) + 1;
    } else if (export->page_set == GTK_PAGE_SET_ODD) {
      if (n_pages == 1 && export->ranges[i].start % 2 != 0)
        null_flag = TRUE;
      else
        n_pages_to_print +=
            export->ranges[i].start % 2 == 0 ? (n_pages / 2) + 1 : n_pages / 2;
    }
  }
...
static void get_first_and_last_page(EvPrintOperationExport *export, gint *first,
                                    gint *last) {
  gint i;
  ...
  for (i = 0; i < export->n_ranges; i++) {
    ...
    if (export->ranges[i].end > last_page) last_page = export->ranges[i].end;
  }
...
static gboolean export_print_inc_page(EvPrintOperationExport *export) {
  do {
    ...
    if (export->page == export->end) {
      ...
      if (export->range == -1 || export->range == export->n_ranges) {
        ...
        /* when printing multiple collated copies & multiple pages per sheet we
         * want to prevent the next copy bleeding into the last sheet of the
         * previous one we've reached the last range to be printed now, so this
        ...
        if (export->pages_per_sheet > 1 && export->collate == 1 &&
          ...
          export->page_count +=
              export->pages_per_sheet -
              (export->page_count - 1) % export->pages_per_sheet;
          if (export->page_set == GTK_PAGE_SET_ALL ||
              (export->page_set == GTK_PAGE_SET_EVEN &&
               export->sheet % 2 == 0) ||
              (export->page_set == GTK_PAGE_SET_ODD &&
               export->sheet % 2 == 1)) {
            ev_file_exporter_end_page(EV_FILE_EXPORTER(op->document));
          }
          ev_document_doc_mutex_unlock();
          export->sheet =
              1 + (export->page_count - 1) / export->pages_per_sheet;
        }

        if (export->uncollated == export->uncollated_copies) return FALSE;
    ...
    /* in/decrement the page number until we reach the first page on the next
     * EVEN or ODD sheet if we're not collating, we have to make sure that this
     * is done only once! */
...
static void ev_print_operation_export_clear_temp_file(
    EvPrintOperationExport *export) {
  if (!export->temp_file) return;
...
static void ev_print_operation_export_run_next(EvPrintOperationExport *export) {
  EvPrintOperation *op = EV_PRINT_OPERATION(export);
  ...
  if (next) ev_print_operation_export_begin(EV_PRINT_OPERATION_EXPORT(next));
}

static void gtk_print_job_finished(GtkPrintJob *print_job,
                                   EvPrintOperationExport *export,
                                   GError *error) {
  EvPrintOperation *op = EV_PRINT_OPERATION(export);

  if (error) {
    g_set_error_literal(&export->error, GTK_PRINT_ERROR,
                        GTK_PRINT_ERROR_GENERAL, error->message);
...
static void export_print_done(EvPrintOperationExport *export) {
  EvPrintOperation *op = EV_PRINT_OPERATION(export);
  ...
  if (capabilities & EV_FILE_EXPORTER_CAN_NUMBER_UP) {
    ...
    gtk_print_settings_set_int(settings, "cups-" GTK_PRINT_SETTINGS_NUMBER_UP,
                               1);
  }
  ...
  if (export->print_preview) {
    ...
    if (data) {
      ...
      fd =
          g_file_open_tmp("print-settingsXXXXXX", &print_settings_file, &error);
      if (!error)
    ...
    if (!error) {
      ...
      cmd = g_strdup_printf(
          "atril-previewer --unlink-tempfile --print-settings %s %s",
      ...
      if (app != NULL) {
        ctx = gdk_display_get_app_launch_context(
            gtk_widget_get_display(GTK_WIDGET(export->parent_window)));
        gdk_app_launch_context_set_screen(
            ctx, gtk_window_get_screen(export->parent_window));
    ...
    if (error) {
      if (print_settings_file) g_unlink(print_settings_file);
    ...
    job = gtk_print_job_new(export->job_name, export->printer, settings,
                            export->page_setup);
    gtk_print_job_set_source_file(job, export->temp_file, &error);
    if (!error) {
      gtk_print_job_send(job, (GtkPrintJobCompleteFunc)gtk_print_job_finished,
                         g_object_ref(export), (GDestroyNotify)g_object_unref);
    }
  }
...
static void export_print_page_idle_finished(EvPrintOperationExport *export) {
  export->idle_id = 0;
}

static void export_job_finished(EvJobExport *job,
                                EvPrintOperationExport *export) {
  EvPrintOperation *op = EV_PRINT_OPERATION(export);
  ...
  /* Reschedule */
  export->idle_id =
      g_idle_add_full(G_PRIORITY_DEFAULT_IDLE, (GSourceFunc)export_print_page,
                      export, (GDestroyNotify)export_print_page_idle_finished);
}

static void export_job_cancelled(EvJobExport *job,
                                 EvPrintOperationExport *export) {
...
static void export_cancel(EvPrintOperationExport *export) {
  ...
  if (export->idle_id > 0) g_source_remove(export->idle_id);
  export->idle_id = 0;

  if (export->job_export) {
    g_signal_handlers_disconnect_by_func(export->job_export,
                                         export_job_finished, export);
    g_signal_handlers_disconnect_by_func(export->job_export,
                                         export_job_cancelled, export);
...
static void update_progress(EvPrintOperationExport *export) {
  EvPrintOperation *op = EV_PRINT_OPERATION(export);

  ev_print_operation_update_status(
      op, export->total, export->n_pages_to_print,
...
static gboolean export_print_page(EvPrintOperationExport *export) {
  ...
  if (!export->temp_file) return FALSE; /* cancelled */
  ...
  if (!export->job_export) {
    ...
    g_signal_connect(export->job_export, "finished",
                     G_CALLBACK(export_job_finished), (gpointer) export);
    g_signal_connect(export->job_export, "cancelled",
                     G_CALLBACK(export_job_cancelled), (gpointer) export);
  }
...
static void ev_print_operation_export_begin(EvPrintOperationExport *export) {
  ...
  if (!export->temp_file) return; /* cancelled */
...
static void ev_print_operation_export_print_dialog_response_cb(
    GtkDialog *dialog, gint response, EvPrintOperationExport *export) {
  ...
  filename = g_strdup_printf("atril_print.%s.XXXXXX",
                             file_format != NULL ? file_format : "");
  ...
  switch (print_pages) {
    ...
    case GTK_PRINT_PAGES_RANGES: {
      ...
      export->ranges =
          gtk_print_settings_get_page_ranges(print_settings, &export->n_ranges);
      for (i = 0; i < export->n_ranges; i++)
        if (export->ranges[i].end == -1 ||
            export->ranges[i].end >= export->n_pages)
          export->ranges[i].end = export->n_pages - 1;
    } break;
    default:
  ...
  if (export->n_ranges < 1 || !clamp_ranges(export)) {
    ...
    message_dialog = gtk_message_dialog_new(
        ...
        GTK_BUTTONS_CLOSE, "%s", _("Invalid page selection"));
    gtk_window_set_title(GTK_WINDOW(message_dialog), _("Warning"));
    gtk_message_dialog_format_secondary_text(
        GTK_MESSAGE_DIALOG(message_dialog), "%s",
        _("Your print range selection does not include any pages"));
    g_signal_connect(message_dialog, "response", G_CALLBACK(gtk_widget_destroy),
                     NULL);
  ...
  export->pages_per_sheet =
      MAX(1, gtk_print_settings_get_number_up(print_settings));
...
static void ev_print_operation_export_run(EvPrintOperation *op,
                                          GtkWindow *parent) {
  EvPrintOperationExport *export = EV_PRINT_OPERATION_EXPORT(op);
  GtkWidget *dialog;
  ...
  g_signal_connect(
      dialog, "response",
      G_CALLBACK(ev_print_operation_export_print_dialog_response_cb), export);

  gtk_window_present(GTK_WINDOW(dialog));
}

static void ev_print_operation_export_cancel(EvPrintOperation *op) {
  EvPrintOperationExport *export = EV_PRINT_OPERATION_EXPORT(op);

  if (export->job_export && !ev_job_is_finished(export->job_export)) {
...
static void ev_print_operation_export_get_error(EvPrintOperation *op,
                                                GError **error) {
  EvPrintOperationExport *export = EV_PRINT_OPERATION_EXPORT(op);
...
static void ev_print_operation_export_set_embed_page_setup(EvPrintOperation *op,
                                                           gboolean embed) {
  EvPrintOperationExport *export = EV_PRINT_OPERATION_EXPORT(op);
...
static gboolean ev_print_operation_export_get_embed_page_setup(
    EvPrintOperation *op) {
  EvPrintOperationExport *export = EV_PRINT_OPERATION_EXPORT(op);
...
static void ev_print_operation_export_finalize(GObject *object) {
  ...
  if (export->ranges) {
    if (export->ranges != &export->one_range) g_free(export->ranges);
    export->ranges = NULL;
    export->n_ranges = 0;
  }
...
static void ev_print_operation_export_init(EvPrintOperationExport *export) {
  /* sheets are counted from 1 to be physical */
  export->sheet = 1;
}

static GObject *ev_print_operation_export_constructor(
  ...
  object = G_OBJECT_CLASS(ev_print_operation_export_parent_class)
...
static void ev_print_operation_export_class_init(
    EvPrintOperationExportClass *klass) {
  GObjectClass *g_object_class = G_OBJECT_CLASS(klass);
  EvPrintOperationClass *ev_print_op_class = EV_PRINT_OPERATION_CLASS(klass);

  ev_print_op_class->set_current_page =
      ev_print_operation_export_set_current_page;
  ev_print_op_class->set_print_settings =
      ev_print_operation_export_set_print_settings;
  ev_print_op_class->get_print_settings =
      ev_print_operation_export_get_print_settings;
  ev_print_op_class->set_default_page_setup =
      ev_print_operation_export_set_default_page_setup;
  ev_print_op_class->get_default_page_setup =
  ...
  ev_print_op_class->set_embed_page_setup =
      ev_print_operation_export_set_embed_page_setup;
  ev_print_op_class->get_embed_page_setup =
...
#define EV_PRINT_OPERATION_PRINT(object)                               \
  (G_TYPE_CHECK_INSTANCE_CAST((object), EV_TYPE_PRINT_OPERATION_PRINT, \
...
G_DEFINE_TYPE(EvPrintOperationPrint, ev_print_operation_print,
              EV_TYPE_PRINT_OPERATION)
...
static void ev_print_operation_print_set_print_settings(
    EvPrintOperation *op, GtkPrintSettings *print_settings) {
  EvPrintOperationPrint *print = EV_PRINT_OPERATION_PRINT(op);
...
static GtkPrintSettings *ev_print_operation_print_get_print_settings(
    EvPrintOperation *op) {
  EvPrintOperationPrint *print = EV_PRINT_OPERATION_PRINT(op);
...
static void ev_print_operation_print_set_default_page_setup(
    EvPrintOperation *op, GtkPageSetup *page_setup) {
  EvPrintOperationPrint *print = EV_PRINT_OPERATION_PRINT(op);
...
static GtkPageSetup *ev_print_operation_print_get_default_page_setup(
    EvPrintOperation *op) {
  EvPrintOperationPrint *print = EV_PRINT_OPERATION_PRINT(op);
...
static const gchar *ev_print_operation_print_get_job_name(
    EvPrintOperation *op) {
  EvPrintOperationPrint *print = EV_PRINT_OPERATION_PRINT(op);
...
static void ev_print_operation_print_run(EvPrintOperation *op,
  ...
  gtk_print_operation_run(print->op, GTK_PRINT_OPERATION_ACTION_PRINT_DIALOG,
                          parent, NULL);
}
...
static gboolean ev_print_operation_print_get_embed_page_setup(
    EvPrintOperation *op) {
  EvPrintOperationPrint *print = EV_PRINT_OPERATION_PRINT(op);
...
static void ev_print_operation_print_status_changed(
  ...
  if (status == GTK_PRINT_STATUS_GENERATING_DATA)
    print->n_pages_to_print =
        gtk_print_operation_get_n_pages_to_print(print->op);
}
...
static void print_job_finished(EvJobPrint *job, EvPrintOperationPrint *print) {
  ...
  print->total++;
  ev_print_operation_update_status(
      op, print->total, print->n_pages_to_print,
...
static void ev_print_operation_print_request_page_setup(
    EvPrintOperationPrint *print, GtkPrintContext *context, gint page_nr,
    GtkPageSetup *setup) {
  EvPrintOperation *op = EV_PRINT_OPERATION(print);
  gdouble width, height;
  GtkPaperSize *paper_size;

  ev_document_get_page_size(op->document, page_nr, &width, &height);

  if (print->use_source_size) {
    paper_size = gtk_paper_size_new_custom("custom", "custom", width, height,
                                           GTK_UNIT_POINTS);
...
static void ev_print_operation_print_draw_page(EvPrintOperationPrint *print,
  ...
  if (!print->job_print) {
    ...
    g_signal_connect(G_OBJECT(print->job_print), "finished",
                     G_CALLBACK(print_job_finished), (gpointer)print);
    g_signal_connect(G_OBJECT(print->job_print), "cancelled",
                     G_CALLBACK(print_job_cancelled), (gpointer)print);
...
static GObject *ev_print_operation_print_create_custom_widget(
    EvPrintOperationPrint *print, GtkPrintContext *context) {
  GtkPrintSettings *settings;
  GtkWidget *label;
  GtkWidget *grid;
  EvPrintScale page_scale;
  ...
  page_scale = gtk_print_settings_get_int_with_default(
      settings, EV_PRINT_SETTING_PAGE_SCALE, 1);
  autorotate =
      gtk_print_settings_has_key(settings, EV_PRINT_SETTING_AUTOROTATE)
   ...
   * printing */
  gtk_combo_box_text_append_text(GTK_COMBO_BOX_TEXT(print->scale_combo),
                                 _("None"));
  gtk_combo_box_text_append_text(GTK_COMBO_BOX_TEXT(print->scale_combo),
  ...
  gtk_widget_set_tooltip_text(
      print->scale_combo,
      _("Scale document pages to fit the selected printer page. Select from "
        "one of the following:\n"
        "\n"
        "• \"None\": No page scaling is performed.\n"
        "\n"
        "• \"Shrink to Printable Area\": Document pages larger than the "
        "printable area"
  ...
  print->autorotate_button =
      gtk_check_button_new_with_label(_("Auto Rotate and Center"));
  gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(print->autorotate_button),
                               autorotate);
  gtk_widget_set_tooltip_text(
      print->autorotate_button,
      _("Rotate printer page orientation of each page to match orientation of "
        "each document page. "
  ...
  print->source_button = gtk_check_button_new_with_label(
      _("Select page size using document page size"));
  gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(print->source_button),
                               use_source_size);
  gtk_widget_set_tooltip_text(print->source_button,
                              _("When enabled, each page will be printed on "
...
static void ev_print_operation_print_custom_widget_apply(
    EvPrintOperationPrint *print, GtkPrintContext *context) {
  GtkPrintSettings *settings;

  print->page_scale =
      gtk_combo_box_get_active(GTK_COMBO_BOX(print->scale_combo));
  print->autorotate =
      gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(print->autorotate_button));
  print->use_source_size =
      gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(print->source_button));
  settings = gtk_print_operation_get_print_settings(print->op);
  gtk_print_settings_set_int(settings, EV_PRINT_SETTING_PAGE_SCALE,
                             print->page_scale);
  gtk_print_settings_set_bool(settings, EV_PRINT_SETTING_AUTOROTATE,
                              print->autorotate);
...
static void ev_print_operation_print_finalize(GObject *object) {
  ...
  if (print->job_print) {
    if (!ev_job_is_finished(print->job_print)) ev_job_cancel(print->job_print);
    g_signal_handlers_disconnect_by_func(print->job_print, print_job_finished,
                                         print);
    g_signal_handlers_disconnect_by_func(print->job_print, print_job_cancelled,
                                         print);
...
static void ev_print_operation_print_init(EvPrintOperationPrint *print) {
  ...
  g_signal_connect_swapped(print->op, "done",
                           G_CALLBACK(ev_print_operation_print_done), print);
  ...
  g_signal_connect_swapped(
      print->op, "request_page_setup",
      G_CALLBACK(ev_print_operation_print_request_page_setup), print);
  g_signal_connect_swapped(
      print->op, "create_custom_widget",
      G_CALLBACK(ev_print_operation_print_create_custom_widget), print);
  g_signal_connect_swapped(
      print->op, "custom_widget_apply",
      G_CALLBACK(ev_print_operation_print_custom_widget_apply), print);
...
static void ev_print_operation_print_class_init(
    EvPrintOperationPrintClass *klass) {
  GObjectClass *g_object_class = G_OBJECT_CLASS(klass);
  EvPrintOperationClass *ev_print_op_class = EV_PRINT_OPERATION_CLASS(klass);

  ev_print_op_class->set_current_page =
      ev_print_operation_print_set_current_page;
  ev_print_op_class->set_print_settings =
      ev_print_operation_print_set_print_settings;
  ev_print_op_class->get_print_settings =
      ev_print_operation_print_get_print_settings;
  ev_print_op_class->set_default_page_setup =
      ev_print_operation_print_set_default_page_setup;
  ev_print_op_class->get_default_page_setup =
      ev_print_operation_print_get_default_page_setup;
  ...
  ev_print_op_class->set_embed_page_setup =
      ev_print_operation_print_set_embed_page_setup;
  ev_print_op_class->get_embed_page_setup =
      ev_print_operation_print_get_embed_page_setup;
