...
typedef enum { SCROLL_DIRECTION_DOWN, SCROLL_DIRECTION_UP } ScrollDirection;
...
static gboolean new_selection_surface_needed(EvPixbufCache *pixbuf_cache,
                                             CacheJobInfo *job_info, gint page,
                                             gfloat scale);
...
#define VISIBLE_NEXT_LEN(pixbuf_cache)                   \
  (MIN(pixbuf_cache->preload_cache_size,                 \
       ev_document_get_n_pages(pixbuf_cache->document) - \
           (1 + pixbuf_cache->end_page)))
...
static void ev_pixbuf_cache_class_init(EvPixbufCacheClass *class) {
  ...
  signals[JOB_FINISHED] = g_signal_new(
      "job-finished", G_OBJECT_CLASS_TYPE(object_class),
      G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
      G_STRUCT_OFFSET(EvPixbufCacheClass, job_finished), NULL, NULL,
...
static void end_job(CacheJobInfo *job_info, gpointer data) {
  g_signal_handlers_disconnect_by_func(job_info->job,
                                       G_CALLBACK(job_finished_cb), data);
...
static void dispose_cache_job_info(CacheJobInfo *job_info, gpointer data) {
  ...
  if (job_info->job) end_job(job_info, data);
...
void ev_pixbuf_cache_set_max_size(EvPixbufCache *pixbuf_cache, gsize max_size) {
  if (pixbuf_cache->max_size == max_size) return;

  if (pixbuf_cache->max_size > max_size) ev_pixbuf_cache_clear(pixbuf_cache);
  pixbuf_cache->max_size = max_size;
}
...
static void copy_job_to_job_info(EvJobRender *job_render,
  ...
  if (job_render->include_selection) {
    ...
    job_info->selection_region =
        cairo_region_reference(job_render->selection_region);
    job_info->selection_region_scale = job_render->scale;

    job_info->points_set = TRUE;
  }

  if (job_info->job) end_job(job_info, pixbuf_cache);
...
static void job_finished_cb(EvJob *job, EvPixbufCache *pixbuf_cache) {
  ...
  if ((job_render->page <
       (pixbuf_cache->start_page - pixbuf_cache->preload_cache_size)) ||
      (job_render->page >
       (pixbuf_cache->end_page + pixbuf_cache->preload_cache_size))) {
    g_object_unref(job);
    return;
  }
...
static void check_job_size_and_unref(EvPixbufCache *pixbuf_cache,
                                     CacheJobInfo *job_info, gfloat scale) {
  gint width, height;
  gint device_scale;

  g_assert(job_info);

  if (job_info->job == NULL) return;

  device_scale = get_device_scale(pixbuf_cache);
  if (job_info->device_scale == device_scale) {
    _get_page_size_for_scale_and_rotation(
        job_info->job->document, EV_JOB_RENDER(job_info->job)->page, scale,
        EV_JOB_RENDER(job_info->job)->rotation, &width, &height);
 ...
 */
static void move_one_job(CacheJobInfo *job_info, EvPixbufCache *pixbuf_cache,
                         int page, CacheJobInfo *new_job_list,
                         CacheJobInfo *new_prev_job, CacheJobInfo *new_next_job,
                         int new_preload_cache_size, int start_page,
                         int end_page, gint priority) {
  ...
  if (page < start_page) {
    ...
    g_assert(page_offset >= 0 && page_offset < new_preload_cache_size);
    ...
    g_assert(page_offset >= 0 && page_offset <= ((end_page - start_page) + 1));
...
static gsize ev_pixbuf_cache_get_page_size(EvPixbufCache *pixbuf_cache,
                                           gint page_index, gdouble scale,
                                           gint rotation) {
  gint width, height;

  _get_page_size_for_scale_and_rotation(pixbuf_cache->document, page_index,
...
static gint ev_pixbuf_cache_get_preload_size(EvPixbufCache *pixbuf_cache,
  ...
  for (i = start_page; i <= end_page; i++) {
    range_size +=
        ev_pixbuf_cache_get_page_size(pixbuf_cache, i, scale, rotation);
  }

  if (range_size >= pixbuf_cache->max_size) return new_preload_cache_size;
  ...
  while (((start_page - i > 0) || (end_page + i < n_pages)) &&
    ...
    if (start_page - i > 0) {
      ...
      if (page_size + range_size <= pixbuf_cache->max_size) {
        range_size += page_size;
        if (!updated) new_preload_cache_size++;
      } else {
        break;
      }
    }
...
static void ev_pixbuf_cache_update_range(EvPixbufCache *pixbuf_cache,
  ...
  new_preload_cache_size = ev_pixbuf_cache_get_preload_size(
      pixbuf_cache, start_page, end_page, scale, rotation);
  ...
  if (new_preload_cache_size > 0) {
    new_prev_job =
        g_slice_alloc0(sizeof(CacheJobInfo) * new_preload_cache_size);
    new_next_job =
        g_slice_alloc0(sizeof(CacheJobInfo) * new_preload_cache_size);
  }
  ...
  for (i = 0; i < pixbuf_cache->preload_cache_size; i++) {
    if (page < 0) {
      ...
      move_one_job(pixbuf_cache->prev_job + i, pixbuf_cache, page, new_job_list,
                   new_prev_job, new_next_job, new_preload_cache_size,
  ...
  for (i = 0; i < PAGE_CACHE_LEN(pixbuf_cache) && page >= 0; i++) {
    move_one_job(pixbuf_cache->job_list + i, pixbuf_cache, page, new_job_list,
                 new_prev_job, new_next_job, new_preload_cache_size, start_page,
                 end_page, EV_JOB_PRIORITY_URGENT);
    page++;
  }
  ...
  for (i = 0; i < pixbuf_cache->preload_cache_size; i++) {
    if (page >= ev_document_get_n_pages(pixbuf_cache->document)) {
      ...
      move_one_job(pixbuf_cache->next_job + i, pixbuf_cache, page, new_job_list,
                   new_prev_job, new_next_job, new_preload_cache_size,
                   start_page, end_page, EV_JOB_PRIORITY_LOW);
    }
    page++;
...
static CacheJobInfo *find_job_cache(EvPixbufCache *pixbuf_cache, int page) {
  ...
  g_assert(page_offset >= 0 && page_offset <= PAGE_CACHE_LEN(pixbuf_cache));
...
static void add_job(EvPixbufCache *pixbuf_cache, CacheJobInfo *job_info,
                    cairo_region_t *region, gint width, gint height, gint page,
                    gint rotation, gfloat scale, EvJobPriority priority) {
  job_info->device_scale = get_device_scale(pixbuf_cache);
  job_info->page_ready = FALSE;

  if (job_info->region) cairo_region_destroy(job_info->region);
  job_info->region = region ? cairo_region_reference(region) : NULL;

  if (job_info->job) end_job(job_info, pixbuf_cache);

  job_info->job = ev_job_render_new(
      pixbuf_cache->document, page, rotation, scale * job_info->device_scale,
      width * job_info->device_scale, height * job_info->device_scale);
  ...
  if (new_selection_surface_needed(pixbuf_cache, job_info, page, scale)) {
    ...
    ev_job_render_set_selection_info(EV_JOB_RENDER(job_info->job),
                                     ...
                                     job_info->selection_style, &text, &base);
  }

  g_signal_connect(job_info->job, "finished", G_CALLBACK(job_finished_cb),
                   pixbuf_cache);
  ev_job_scheduler_push_job(job_info->job, priority);
}

static void add_job_if_needed(EvPixbufCache *pixbuf_cache,
                              CacheJobInfo *job_info, gint page, gint rotation,
                              gfloat scale, EvJobPriority priority) {
  gint device_scale = get_device_scale(pixbuf_cache);
  gint width, height;

  if (job_info->job) return;

  _get_page_size_for_scale_and_rotation(pixbuf_cache->document, page, scale,
                                        rotation, &width, &height);

  if (job_info->surface && job_info->device_scale == device_scale &&
      cairo_image_surface_get_width(job_info->surface) ==
          width * device_scale &&
      cairo_image_surface_get_height(job_info->surface) ==
          height * device_scale)
    return;
  ...
  add_job(pixbuf_cache, job_info, NULL, width, height, page, rotation, scale,
          priority);
}

static void add_prev_jobs_if_needed(EvPixbufCache *pixbuf_cache, gint rotation,
                                    gfloat scale) {
  CacheJobInfo *job_info;
  int page;
  int i;

  for (i = pixbuf_cache->preload_cache_size - 1;
       i >= FIRST_VISIBLE_PREV(pixbuf_cache); i--) {
    job_info = (pixbuf_cache->prev_job + i);
    page = pixbuf_cache->start_page - pixbuf_cache->preload_cache_size + i;

    add_job_if_needed(pixbuf_cache, job_info, page, rotation, scale,
                      EV_JOB_PRIORITY_LOW);
  }
}

static void add_next_jobs_if_needed(EvPixbufCache *pixbuf_cache, gint rotation,
                                    gfloat scale) {
  CacheJobInfo *job_info;
  int page;
  int i;
...
static ScrollDirection ev_pixbuf_cache_get_scroll_direction(
    EvPixbufCache *pixbuf_cache, gint start_page, gint end_page) {
  ...
  if (start_page > pixbuf_cache->start_page) return SCROLL_DIRECTION_DOWN;

  if (end_page < pixbuf_cache->end_page) return SCROLL_DIRECTION_UP;
...
void ev_pixbuf_cache_set_page_range(EvPixbufCache *pixbuf_cache,
  ...
  g_return_if_fail(start_page >= 0 && start_page < ev_document_get_n_pages(
                                                       pixbuf_cache->document));
  g_return_if_fail(end_page >= 0 &&
                   end_page < ev_document_get_n_pages(pixbuf_cache->document));
  g_return_if_fail(end_page >= start_page);

  pixbuf_cache->scroll_direction =
      ev_pixbuf_cache_get_scroll_direction(pixbuf_cache, start_page, end_page);

  /* First, resize the page_range as needed.  We cull old pages
   * mercilessly. */
  ev_pixbuf_cache_update_range(pixbuf_cache, start_page, end_page, rotation,
                               scale);
...
void ev_pixbuf_cache_set_inverted_colors(EvPixbufCache *pixbuf_cache,
  ...
  if (pixbuf_cache->inverted_colors == inverted_colors) return;
...
cairo_surface_t *ev_pixbuf_cache_get_surface(EvPixbufCache *pixbuf_cache,
  ...
  if (job_info == NULL) return NULL;

  if (job_info->page_ready) return job_info->surface;

  /* We don't need to wait for the idle to handle the callback */
  if (job_info->job && EV_JOB_RENDER(job_info->job)->page_ready) {
...
static gboolean new_selection_surface_needed(EvPixbufCache *pixbuf_cache,
                                             CacheJobInfo *job_info, gint page,
                                             gfloat scale) {
  if (job_info->selection) return job_info->selection_scale != scale;
...
static void clear_selection_surface_if_needed(EvPixbufCache *pixbuf_cache,
                                              CacheJobInfo *job_info, gint page,
                                              gfloat scale) {
  if (new_selection_surface_needed(pixbuf_cache, job_info, page, scale)) {
    if (job_info->selection) cairo_surface_destroy(job_info->selection);
    job_info->selection = NULL;
    job_info->selection_points.x1 = -1;
  }
}

static void clear_selection_region_if_needed(EvPixbufCache *pixbuf_cache,
                                             CacheJobInfo *job_info, gint page,
                                             gfloat scale) {
...
void ev_pixbuf_cache_clear(EvPixbufCache *pixbuf_cache) {
  ...
  if (!pixbuf_cache->job_list) return;
...
cairo_surface_t *ev_pixbuf_cache_get_selection_surface(
  ...
  if (!EV_IS_SELECTION(pixbuf_cache->document)) return NULL;

  job_info = find_job_cache(pixbuf_cache, page);
  if (job_info == NULL) return NULL;

  /* No selection on this page */
  if (!job_info->points_set) return NULL;
  ...
  if (ev_rect_cmp(&(job_info->target_points), &(job_info->selection_points))) {
    ...
    get_selection_colors(EV_VIEW(pixbuf_cache->view), &text, &base);
    ev_selection_render_selection(EV_SELECTION(pixbuf_cache->document), rc,
                                  &(job_info->selection),
                                  &(job_info->target_points), old_points,
                                  job_info->selection_style, &text, &base);
    if (job_info->selection)
      set_device_scale_on_surface(job_info->selection, job_info->device_scale);
...
cairo_region_t *ev_pixbuf_cache_get_selection_region(
  ...
  if (job_info->job && EV_JOB_RENDER(job_info->job)->include_selection)
    return job_info->selection_region &&
                   !cairo_region_is_empty(job_info->selection_region)
               ? job_info->selection_region
               : NULL;
   ...
   * if needed.
   */
  if (ev_rect_cmp(&(job_info->target_points),
                  &(job_info->selection_region_points))) {
    EvRenderContext *rc;
    EvPage *ev_page;
    ...
    if (job_info->selection_region)
      cairo_region_destroy(job_info->selection_region);
    job_info->selection_region = ev_selection_get_selection_region(
        EV_SELECTION(pixbuf_cache->document), rc, job_info->selection_style,
        &(job_info->target_points));
...
void ev_pixbuf_cache_set_selection_list(EvPixbufCache *pixbuf_cache,
  ...
  if (!EV_IS_SELECTION(pixbuf_cache->document)) return;

  if (pixbuf_cache->start_page == -1 || pixbuf_cache->end_page == -1) return;
  ...
  for (i = 0; i < pixbuf_cache->preload_cache_size; i++) {
    if (page >= ev_document_get_n_pages(pixbuf_cache->document)) break;
    ...
    if (pixbuf_cache->prev_job[i].selection_points.x1 != -1) {
      ...
      if (pixbuf_cache->prev_job[i].selection_region)
        selection->covered_region =
            cairo_region_reference(pixbuf_cache->prev_job[i].selection_region);
  ...
  for (i = 0; i < PAGE_CACHE_LEN(pixbuf_cache); i++) {
    if (pixbuf_cache->job_list[i].selection_points.x1 != -1) {
      ...
      if (pixbuf_cache->job_list[i].selection_region)
        selection->covered_region =
            cairo_region_reference(pixbuf_cache->job_list[i].selection_region);
      retval = g_list_prepend(retval, selection);
    }
  ...
  for (i = 0; i < pixbuf_cache->preload_cache_size; i++) {
    ...
    if (pixbuf_cache->next_job[i].selection_points.x1 != -1) {
      ...
      if (pixbuf_cache->next_job[i].selection_region)
        selection->covered_region =
            cairo_region_reference(pixbuf_cache->next_job[i].selection_region);
      retval = g_list_prepend(retval, selection);
    }
...
void ev_pixbuf_cache_reload_page(EvPixbufCache *pixbuf_cache,
  ...
  _get_page_size_for_scale_and_rotation(pixbuf_cache->document, page, scale,
                                        rotation, &width, &height);
  add_job(pixbuf_cache, job_info, region, width, height, page, rotation, scale,
          EV_JOB_PRIORITY_URGENT);
}
