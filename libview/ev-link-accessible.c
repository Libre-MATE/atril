...
#define EV_HYPERLINK(obj) \
  (G_TYPE_CHECK_INSTANCE_CAST((obj), EV_TYPE_HYPERLINK, EvHyperlink))
...
static gchar *ev_hyperlink_get_uri(AtkHyperlink *atk_hyperlink, gint i) {
  ...
  if (!hyperlink->link_impl) return NULL;
...
static gint ev_hyperlink_get_start_index(AtkHyperlink *atk_hyperlink) {
  ...
  if (!hyperlink->link_impl) return -1;

  impl_priv = hyperlink->link_impl->priv;
  if (impl_priv->start_index != -1) return impl_priv->start_index;
...
static gint ev_hyperlink_get_end_index(AtkHyperlink *atk_hyperlink) {
  ...
  impl_priv = hyperlink->link_impl->priv;
  if (impl_priv->end_index != -1) return impl_priv->end_index;
...
static void ev_link_accessible_hyperlink_impl_iface_init(
...
G_DEFINE_TYPE_WITH_CODE(
    EvLinkAccessible, ev_link_accessible, ATK_TYPE_OBJECT,
    G_ADD_PRIVATE(EvLinkAccessible)
        G_IMPLEMENT_INTERFACE(ATK_TYPE_HYPERLINK_IMPL,
            ...
            G_IMPLEMENT_INTERFACE(ATK_TYPE_ACTION,
                                  ev_link_accessible_action_interface_init)
                G_IMPLEMENT_INTERFACE(ATK_TYPE_COMPONENT,
                                      ev_link_accessible_component_iface_init))

static const gchar *ev_link_accessible_get_name(AtkObject *atk_object) {
  EvLinkAccessiblePrivate *priv;
  gint start_index;
  gint end_index;

  priv = EV_LINK_ACCESSIBLE(atk_object)->priv;
  if (priv->name) return priv->name;
  ...
  priv->name = atk_text_get_text(ATK_TEXT(atk_object_get_parent(atk_object)),
...
static AtkStateSet *ev_link_accessible_ref_state_set(AtkObject *atk_object) {
  ...
  state_set = ATK_OBJECT_CLASS(ev_link_accessible_parent_class)
                  ->ref_state_set(atk_object);
  atk_state_set_clear_states(state_set);

  page_accessible_state_set =
      atk_object_ref_state_set(ATK_OBJECT(self->priv->page));
  ...
  if (!ev_view_accessible_is_doc_rect_showing(view_accessible, page,
                                              &self->priv->area))
...
static AtkHyperlink *ev_link_accessible_get_hyperlink(
    AtkHyperlinkImpl *hyperlink_impl) {
  EvLinkAccessible *link = EV_LINK_ACCESSIBLE(hyperlink_impl);

  if (link->priv->hyperlink) return ATK_HYPERLINK(link->priv->hyperlink);

  link->priv->hyperlink = g_object_new(EV_TYPE_HYPERLINK, NULL);

  link->priv->hyperlink->link_impl = link;
  g_object_add_weak_pointer(G_OBJECT(link),
                            (gpointer *)&link->priv->hyperlink->link_impl);
...
static gboolean ev_link_accessible_action_do_action(AtkAction *atk_action,
                                                    gint i) {
  EvLinkAccessiblePrivate *priv = EV_LINK_ACCESSIBLE(atk_action)->priv;
  EvView *view;

  view = ev_page_accessible_get_view(priv->page);
  if (!ev_link_get_action(priv->link)) return FALSE;
...
static const gchar *ev_link_accessible_action_get_description(
    AtkAction *atk_action, gint i) {
  /* TODO */
  return NULL;
}

static const gchar *ev_link_accessible_action_get_name(AtkAction *atk_action,
                                                       gint i) {
...
static void ev_link_accessible_get_extents(AtkComponent *atk_component, gint *x,
  ...
  view_accessible = ev_page_accessible_get_view_accessible(self->priv->page);
  page = ev_page_accessible_get_page(self->priv->page);
  _transform_doc_rect_to_atk_rect(view_accessible, page, &self->priv->area,
                                  &atk_rect, coord_type);
  *x = atk_rect.x1;
  *y = atk_rect.y1;
