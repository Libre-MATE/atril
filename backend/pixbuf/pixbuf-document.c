...
EV_BACKEND_REGISTER_WITH_CODE(
    PixbufDocument, pixbuf_document,
    {EV_BACKEND_IMPLEMENT_INTERFACE(
        EV_TYPE_DOCUMENT_THUMBNAILS,
        pixbuf_document_document_thumbnails_iface_init)});

static gboolean pixbuf_document_load(EvDocument *document, const char *uri,
                                     GError **error) {
...
static gboolean pixbuf_document_save(EvDocument *document, const char *uri,
                                     GError **error) {
  PixbufDocument *pixbuf_document = PIXBUF_DOCUMENT(document);
...
static int pixbuf_document_get_n_pages(EvDocument *document) { return 1; }

static void pixbuf_document_get_page_size(EvDocument *document, EvPage *page,
...
static cairo_surface_t *pixbuf_document_render(EvDocument *document,
                                               EvRenderContext *rc) {
  PixbufDocument *pixbuf_document = PIXBUF_DOCUMENT(document);
...
static void pixbuf_document_finalize(GObject *object) {
...
static void pixbuf_document_class_init(PixbufDocumentClass *klass) {
...
static GdkPixbuf *pixbuf_document_thumbnails_get_thumbnail(
...
static void pixbuf_document_thumbnails_get_dimensions(
