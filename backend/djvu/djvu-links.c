...
static guint64 get_djvu_link_page(const DjvuDocument *djvu_document,
                                  const gchar *link_name, int base_page) {
  guint64 page_num = 0;
...
static EvLinkDest *get_djvu_link_dest(const DjvuDocument *djvu_document,
                                      const gchar *link_name, int base_page) {
  return ev_link_dest_new_page(
      get_djvu_link_page(djvu_document, link_name, base_page));
}
...
static gchar *str_to_utf8(const gchar *text) {
  ...
  for (i = 0; i < n_encodings_to_try; i++) {
    utf8_text =
        g_convert(text, -1, "UTF-8", encodings_to_try[i], NULL, NULL, NULL);
    if (utf8_text) break;
  }
 ...
 *   ... )
 */
static void build_tree(const DjvuDocument *djvu_document, GtkTreeModel *model,
                       GtkTreeIter *parent, miniexp_t iter) {
...
static gboolean get_djvu_hyperlink_area(ddjvu_pageinfo_t *page_info,
  ...
  if ((miniexp_car(iter) == miniexp_symbol("rect") ||
       miniexp_car(iter) == miniexp_symbol("oval")) &&
      miniexp_length(iter) == 5) {
    /* FIXME: get bounding box for (oval) since Atril doesn't support shaped
     * links */
...
unknown_link:
  g_warning("DjvuLibre error: Unknown hyperlink area %s",
            miniexp_to_name(miniexp_car(sexp)));
  return FALSE;
}
...
static EvMapping *get_djvu_hyperlink_mapping(DjvuDocument *djvu_document,
  ...
  iter = miniexp_cdr(iter);
  if (!get_djvu_hyperlink_area(page_info, miniexp_car(iter), ev_link_mapping))
    goto unknown_mapping;
...
unknown_mapping:
  ...
  g_warning("DjvuLibre error: Unknown hyperlink %s",
            miniexp_to_name(miniexp_car(sexp)));
  return NULL;
}
...
gboolean djvu_links_has_document_links(EvDocumentLinks *document_links) {
  ...
  while ((outline = ddjvu_document_get_outline(djvu_document->d_document)) ==
         miniexp_dummy)
...
EvMappingList *djvu_links_get_links(EvDocumentLinks *document_links, gint page,
  ...
  while ((page_annotations = ddjvu_document_get_pageanno(
              djvu_document->d_document, page)) == miniexp_dummy)
    djvu_handle_events(djvu_document, TRUE, NULL);

  while (ddjvu_document_get_pageinfo(djvu_document->d_document, page,
  ...
  if (page_annotations) {
    ...
    if (hyperlinks) {
      for (iter = hyperlinks; *iter; ++iter) {
        ev_link_mapping =
            get_djvu_hyperlink_mapping(djvu_document, page, &page_info, *iter);
        if (ev_link_mapping) {
