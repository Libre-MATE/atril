...
static void djvu_document_document_thumbnails_iface_init(
...
static void djvu_document_document_links_iface_init(
...
EV_BACKEND_REGISTER_WITH_CODE(DjvuDocument, djvu_document, {
  EV_BACKEND_IMPLEMENT_INTERFACE(EV_TYPE_DOCUMENT_THUMBNAILS,
                                 djvu_document_document_thumbnails_iface_init);
...
static GQuark ev_djvu_error_quark(void) {
  static GQuark q = 0;
  if (q == 0) q = g_quark_from_string("ev-djvu-quark");

  return q;
}

static void handle_message(const ddjvu_message_t *msg, GError **error) {
  switch (msg->m_any.tag) {
    case DDJVU_ERROR: {
      ...
      if (msg->m_error.filename) {
        ...
        error_str =
            g_strdup_printf("DjvuLibre error: %s", msg->m_error.message);
      }
...
static void djvu_wait_for_message(DjvuDocument *djvu_document,
                                  ddjvu_message_tag_t message, GError **error) {
  ddjvu_context_t *ctx = djvu_document->d_context;
  const ddjvu_message_t *msg;
  ...
  if (msg && msg->m_any.tag == message) ddjvu_message_pop(ctx);
}

static gboolean djvu_document_load(EvDocument *document, const char *uri,
                                   GError **error) {
  ...
  doc = ddjvu_document_create_by_filename(djvu_document->d_context, filename,
                                          TRUE);
  ...
  if (ddjvu_document_get_type(djvu_document->d_document) ==
    ...
    for (i = 0; i < n_files; i++) {
      ...
      ddjvu_document_get_fileinfo(djvu_document->d_document, i, &fileinfo);

      if (fileinfo.type != 'P') continue;
...
static gboolean djvu_document_save(EvDocument *document, const char *uri,
                                   GError **error) {
  DjvuDocument *djvu_document = DJVU_DOCUMENT(document);
...
static void document_get_page_size(DjvuDocument *djvu_document, gint page,
                                   double *width, double *height) {
  ddjvu_pageinfo_t info;
  ddjvu_status_t r;

  while ((r = ddjvu_document_get_pageinfo(djvu_document->d_document, page,
                                          &info)) < DDJVU_JOB_OK)
    djvu_handle_events(djvu_document, TRUE, NULL);

  if (r >= DDJVU_JOB_FAILED) djvu_handle_events(djvu_document, TRUE, NULL);

  *width = info.width * SCALE_FACTOR;
  *height = info.height * SCALE_FACTOR;
}

static void djvu_document_get_page_size(EvDocument *document, EvPage *page,
                                        double *width, double *height) {
  DjvuDocument *djvu_document = DJVU_DOCUMENT(document);
...
static cairo_surface_t *djvu_document_render(EvDocument *document,
  ...
  d_page =
      ddjvu_page_create_by_pageno(djvu_document->d_document, rc->page->index);
  ...
  surface =
      cairo_image_surface_create(CAIRO_FORMAT_RGB24, page_width, page_height);
  ...
  ddjvu_page_render(d_page, DDJVU_RENDER_COLOR, &prect, &rrect,
                    djvu_document->d_format, rowstride, pixels);
...
static void djvu_document_finalize(GObject *object) {
  DjvuDocument *djvu_document = DJVU_DOCUMENT(object);

  if (djvu_document->d_document)
    ddjvu_document_release(djvu_document->d_document);

  if (djvu_document->opts) g_string_free(djvu_document->opts, TRUE);

  if (djvu_document->ps_filename) g_free(djvu_document->ps_filename);
...
static void djvu_document_class_init(DjvuDocumentClass *klass) {
...
static gchar *djvu_text_copy(DjvuDocument *djvu_document, gint page,
                             EvRectangle *rectangle) {
  miniexp_t page_text;
  gchar *text = NULL;

  while ((page_text = ddjvu_document_get_pagetext(
              djvu_document->d_document, page, "char")) == miniexp_dummy)
    djvu_handle_events(djvu_document, TRUE, NULL);
...
static gchar *djvu_selection_get_selected_text(EvSelection *selection,
                                               EvPage *page,
  ...
  djvu_document_get_page_size(EV_DOCUMENT(djvu_document), page, &width,
                              &height);
...
static void djvu_document_thumbnails_get_dimensions(
    EvDocumentThumbnails *document, EvRenderContext *rc, gint *width,
    gint *height) {
  DjvuDocument *djvu_document = DJVU_DOCUMENT(document);
  gdouble page_width, page_height;

  djvu_document_get_page_size(EV_DOCUMENT(djvu_document), rc->page, &page_width,
                              &page_height);
...
static GdkPixbuf *djvu_document_thumbnails_get_thumbnail(
  ...
  pixbuf =
      gdk_pixbuf_new(GDK_COLORSPACE_RGB, FALSE, 8, thumb_width, thumb_height);
  gdk_pixbuf_fill(pixbuf, 0xffffffff);
  pixels = gdk_pixbuf_get_pixels(pixbuf);

  while (ddjvu_thumbnail_status(djvu_document->d_document, rc->page->index, 1) <
         DDJVU_JOB_OK)
    djvu_handle_events(djvu_document, TRUE, NULL);
  ...
  ddjvu_thumbnail_render(djvu_document->d_document, rc->page->index,
                         ...
                         gdk_pixbuf_get_rowstride(pixbuf), (gchar *)pixels);
...
/* EvFileExporterIface */
static void djvu_document_file_exporter_begin(EvFileExporter *exporter,
...
static void djvu_document_file_exporter_do_page(EvFileExporter *exporter,
                                                EvRenderContext *rc) {
  DjvuDocument *djvu_document = DJVU_DOCUMENT(exporter);
...
static void djvu_document_file_exporter_end(EvFileExporter *exporter) {
  int d_optc = 1;
  ...
  ddjvu_job_t *job =
      ddjvu_document_print(djvu_document->d_document, fn, d_optc, d_optv);
...
static EvFileExporterCapabilities djvu_document_file_exporter_get_capabilities(
    EvFileExporter *exporter) {
  return EV_FILE_EXPORTER_CAN_PAGE_SET | EV_FILE_EXPORTER_CAN_COPIES |
         EV_FILE_EXPORTER_CAN_COLLATE | EV_FILE_EXPORTER_CAN_REVERSE |
...
static GList *djvu_document_find_find_text(EvDocumentFind *document,
                                           EvPage *page, const char *text,
  ...
  while ((page_text = ddjvu_document_get_pagetext(
              djvu_document->d_document, page->index, "char")) == miniexp_dummy)
    djvu_handle_events(djvu_document, TRUE, NULL);
...
static EvMappingList *djvu_document_links_get_links(
