...
/* EvDocumentIface */
static gboolean ps_document_load(EvDocument *document, const char *uri,
                                 GError **error) {
  ...
  if (spectre_document_status(ps->doc)) {
    ...
    g_set_error(error, G_FILE_ERROR, G_FILE_ERROR_FAILED,
                _("Failed to load document “%s”"), filename_dsp);
    g_free(filename_dsp);
    g_free(filename);
...
static gboolean ps_document_save(EvDocument *document, const char *uri,
                                 GError **error) {
  PSDocument *ps = PS_DOCUMENT(document);
  gchar *filename;
  ...
  if (spectre_document_status(ps->doc)) {
    ...
    filename_dsp = g_filename_display_name(filename);
    g_set_error(error, G_FILE_ERROR, G_FILE_ERROR_FAILED,
                _("Failed to save document “%s”"), filename_dsp);
    g_free(filename_dsp);
    g_free(filename);
...
static EvPage *ps_document_get_page(EvDocument *document, gint index) {
  PSDocument *ps = PS_DOCUMENT(document);
  SpectrePage *ps_page;
  EvPage *page;
...
static void ps_document_get_page_size(EvDocument *document, EvPage *page,
...
static char *ps_document_get_page_label(EvDocument *document, EvPage *page) {
  const gchar *label =
      spectre_page_get_label((SpectrePage *)page->backend_page);
  gchar *utf8;

  if (!label) return NULL;

  if (g_utf8_validate(label, -1, NULL)) return g_strdup(label);

  /* Try with latin1 and ASCII encondings */
  utf8 = g_convert(label, -1, "utf-8", "latin1", NULL, NULL, NULL);
  if (!utf8) utf8 = g_convert(label, -1, "utf-8", "ASCII", NULL, NULL, NULL);
...
static EvDocumentInfo *ps_document_get_info(EvDocument *document) {
  ...
  info->fields_mask = EV_DOCUMENT_INFO_TITLE | EV_DOCUMENT_INFO_FORMAT |
                      EV_DOCUMENT_INFO_CREATOR | EV_DOCUMENT_INFO_N_PAGES |
  ...
  info->creator =
      g_strdup(creator ? creator : spectre_document_get_for(ps->doc));
...
static cairo_surface_t *ps_document_render(EvDocument *document,
  ...
  spectre_render_context_set_scale(src, (gdouble)width / width_points,
...
static GdkPixbuf *ps_document_thumbnails_get_thumbnail(
...
static void ps_document_thumbnails_get_dimensions(
    EvDocumentThumbnails *document_thumbnails, EvRenderContext *rc, gint *width,
    gint *height) {
  PSDocument *ps = PS_DOCUMENT(document_thumbnails);
  gdouble page_width, page_height;

  ps_document_get_page_size(EV_DOCUMENT(ps), rc->page, &page_width,
                            &page_height);
...
static void ps_document_file_exporter_begin(EvFileExporter *exporter,
  ...
  if (ps->exporter) spectre_exporter_free(ps->exporter);

  switch (fc->format) {
    case EV_FILE_FORMAT_PS:
      ps->exporter = spectre_exporter_new(ps->doc, SPECTRE_EXPORTER_FORMAT_PS);
      break;
    case EV_FILE_FORMAT_PDF:
      ps->exporter = spectre_exporter_new(ps->doc, SPECTRE_EXPORTER_FORMAT_PDF);
      break;
    default:
...
static EvFileExporterCapabilities ps_document_file_exporter_get_capabilities(
  ...
  return EV_FILE_EXPORTER_CAN_PAGE_SET | EV_FILE_EXPORTER_CAN_COPIES |
         ...
         EV_FILE_EXPORTER_CAN_GENERATE_PS | EV_FILE_EXPORTER_CAN_GENERATE_PDF;
}
