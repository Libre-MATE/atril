...
static void PSHead(TIFF2PSContext*, TIFF*, uint32, uint32, double, double,
                   double, double);
static void PSTail(TIFF2PSContext*);

#if defined(EXP_ASCII85ENCODER)
static int Ascii85EncodeBlock(TIFF2PSContext*, uint8* ascii85_p, unsigned f_eod,
                              const uint8* raw_p, int raw_l);
#endif

#define IMAGEOP(ctx) \
  ((ctx)->useImagemask && ((ctx)->bitspersample == 1)) ? "imagemask" : "image"
...
static int checkImage(TIFF2PSContext* ctx, TIFF* tif) {
  switch (ctx->photometric) {
    case PHOTOMETRIC_YCBCR:
      if ((ctx->compression == COMPRESSION_JPEG ||
        ...
        if (ctx->level2 || ctx->level3) break;
    ...
    case PHOTOMETRIC_LOGLUV:
      if (ctx->compression != COMPRESSION_SGILOG &&
        ...
        TIFFError(
            ...
            (ctx->photometric == PHOTOMETRIC_LOGL) ? "LogL" : "LogLuv");
        return (0);
      }
 ...
 */
G_GNUC_PRINTF(6, 7)
static void PhotoshopBanner(TIFF2PSContext* ctx, uint32 w, uint32 h, int bs,
                            int nc, const char* startline, ...) {
  va_list args;
  fprintf(ctx->fd, "%%ImageData: %ld %ld %d %d 0 %d 2 \"", (long)w, (long)h,
          ctx->bitspersample, nc, bs);
 ...
 */
static void setupPageState(TIFF2PSContext* ctx, TIFF* tif, uint32* pw,
                           uint32* ph, double* pprw, double* pprh) {
   ...
   */
  if (!TIFFGetField(tif, TIFFTAG_XRESOLUTION, &xres) || fabs(xres) < 0.0000001)
    xres = PS_UNIT_SIZE;
  if (!TIFFGetField(tif, TIFFTAG_YRESOLUTION, &yres) || fabs(yres) < 0.0000001)
    yres = PS_UNIT_SIZE;
  switch (ctx->res_unit) {
 ...
 */
static int PlaceImage(TIFF2PSContext* ctx, double pagewidth, double pageheight,
  ...
  if (ctx->rotate) fputs("1 1 translate 180 ctx->rotate\n", ctx->fd);

  return splitpage;
}

void tiff2ps_process_page(TIFF2PSContext* ctx, TIFF* tif, double pw, double ph,
  ...
  if (!TIFFGetField(tif, TIFFTAG_XPOSITION, &ox)) ox = 0;
  if (!TIFFGetField(tif, TIFFTAG_YPOSITION, &oy)) oy = 0;
  setupPageState(ctx, tif, &w, &h, &prw, &prh);

  ctx->tf_numberstrips = TIFFNumberOfStrips(tif);
  TIFFGetFieldDefaulted(tif, TIFFTAG_ROWSPERSTRIP, &ctx->tf_rowsperstrip);
  setupPageState(ctx, tif, &w, &h, &prw, &prh);
  if (!ctx->npages) PSHead(ctx, tif, w, h, prw, prh, ox, oy);
  TIFFGetFieldDefaulted(tif, TIFFTAG_BITSPERSAMPLE, &ctx->bitspersample);
  TIFFGetFieldDefaulted(tif, TIFFTAG_SAMPLESPERPIXEL, &ctx->samplesperpixel);
  TIFFGetFieldDefaulted(tif, TIFFTAG_PLANARCONFIG, &ctx->planarconfiguration);
  TIFFGetField(tif, TIFFTAG_COMPRESSION, &ctx->compression);
  TIFFGetFieldDefaulted(tif, TIFFTAG_EXTRASAMPLES, &ctx->extrasamples,
                        &sampleinfo);
  ctx->alpha =
      (ctx->extrasamples == 1 && sampleinfo[0] == EXTRASAMPLE_ASSOCALPHA);
  ...
  if (checkImage(ctx, tif)) {
    ...
    if (!ctx->generateEPSF && (ctx->level2 || ctx->level3)) {
      ...
      if (psw != 0.0) {
        psw = pw * PS_UNIT_SIZE;
        if (ctx->res_unit == RESUNIT_CENTIMETER) psw *= 2.54F;
      } else
        psw = ctx->rotate ? prh : prw;
      if (psh != 0.0) {
        psh = ph * PS_UNIT_SIZE;
        if (ctx->res_unit == RESUNIT_CENTIMETER) psh *= 2.54F;
      } else
        psh = ctx->rotate ? prw : prh;
      fprintf(ctx->fd,
              "1 dict begin /PageSize [ %s %s ] def currentdict end "
              "setpagedevice\n",
    ...
    if (pw != 0 || ph != 0) {
      ...
      if (ctx->maxPageHeight) { /* used -H option */
        split = PlaceImage(ctx, pw, ph, prw, prh, 0, lm, bm, cnt);
        while (split) {
          PSpage(ctx, tif, w, h);
          fprintf(ctx->fd, "end\n");
          ...
          fprintf(ctx->fd, "gsave\n");
          fprintf(ctx->fd, "100 dict begin\n");
          split = PlaceImage(ctx, pw, ph, prw, prh, split, lm, bm, cnt);
        }
      } else {
        pw *= PS_UNIT_SIZE;
        ph *= PS_UNIT_SIZE;
        ...
        scale = pw / prw < ph / prh ? pw / prw : ph / prh;
        if (scale > 1.0) scale = 1.0;
        if (cnt) {
          bottom_offset += (ph - prh * scale) / 2;
          left_offset += (pw - prw * scale) / 2;
        }
        fprintf(ctx->fd, "%s %s translate\n",
                g_ascii_dtostr(buf[0], sizeof(buf[0]), left_offset),
                g_ascii_dtostr(buf[1], sizeof(buf[1]), bottom_offset));
...
void PSHead(TIFF2PSContext* ctx, TIFF* tif, uint32 w, uint32 h, double pw,
            double ph, double ox, double oy) {
  time_t t;
  ...
  if (ctx->PSduplex) fprintf(ctx->fd, "%s", DuplexPreamble);
  if (ctx->PStumble) fprintf(ctx->fd, "%s", TumblePreamble);
  if (ctx->PSavoiddeadzone && (ctx->level2 || ctx->level3))
    fprintf(ctx->fd, "%s", AvoidDeadZonePreamble);
  fprintf(ctx->fd, "%%%%EndSetup\n");
}

static void PSTail(TIFF2PSContext* ctx) {
  if (!ctx->npages) return;
  fprintf(ctx->fd, "%%%%Trailer\n");
  fprintf(ctx->fd, "%%%%Pages: %d\n", ctx->npages);
  fprintf(ctx->fd, "%%%%EOF\n");
}

static int checkcmap(TIFF2PSContext* ctx, TIFF* tif, int n, uint16* r,
                     uint16* g, uint16* b) {
  (void)tif;
  while (n-- > 0)
...
static void PS_Lvl2colorspace(TIFF2PSContext* ctx, TIFF* tif) {
...
#undef CVT
  ...
  for (i = 0; i < num_colors; i++) {
    if (ctx->ascii85) {
      ...
      fputs((i % 8) ? " " : "\n  ", ctx->fd);
      fprintf(ctx->fd, "%02x%02x%02x", rmap[i], gmap[i], bmap[i]);
    }
  }
  if (ctx->ascii85)
    Ascii85Flush(ctx);
...
static int PS_Lvl2ImageDict(TIFF2PSContext* ctx, TIFF* tif, uint32 w,
                            uint32 h) {
  int use_rawdata;
  ...
  if (TIFFIsTiled(tif)) {
    ...
    if (tile_height > h) tile_height = h;
    if (repeat_count > 1) {
      fputs("/im_y 0 def\n", ctx->fd);
  ...
  if (repeat_count > 1) fprintf(ctx->fd, "%d { %% repeat\n", repeat_count);
  ...
  if (ctx->planarconfiguration == PLANARCONFIG_SEPARATE &&
     ...
     */
    for (i = 1; i < ctx->samplesperpixel; i++) fputs(" dup", ctx->fd);
    fputs(" ]", ctx->fd);
  }
    ...
    if (tile_height < h) {
      if (tile_width >= w) {
        fprintf(ctx->fd, " /im_y im_y %lu add def\n",
                (unsigned long)tile_height);
        if (!TIFFIsTiled(tif)) {
          fprintf(ctx->fd, " /im_h %lu im_y sub", (unsigned long)h);
          fprintf(ctx->fd, " dup %lu gt { pop", (unsigned long)tile_height);
...
static int PS_Lvl2page(TIFF2PSContext* ctx, TIFF* tif, uint32 w, uint32 h) {
  uint16 fillorder;
...
/* See http://bugzilla.remotesensing.org/show_bug.cgi?id=80 */
  ...
  if (!buf_data) {
    TIFFError(ctx->filename, "Can't alloc %u bytes for %s.", (uint)chunk_size,
              tiled_image ? "tiles" : "strips");
    return (FALSE);
  }
  ...
  if (ctx->ascii85) {
    ...
    if (!ascii85_p) {
      ...
      TIFFError(ctx->filename, "Cannot allocate ASCII85 encoding buffer.");
      return (FALSE);
    }
  }
#endif
  ...
  for (chunk_no = 0; chunk_no < num_chunks; chunk_no++) {
    ...
    if (use_rawdata) {
      if (tiled_image)
        byte_count = TIFFReadRawTile(tif, chunk_no, buf_data, chunk_size);
      else
        byte_count = TIFFReadRawStrip(tif, chunk_no, buf_data, chunk_size);
      if (fillorder == FILLORDER_LSB2MSB) TIFFReverseBits(buf_data, byte_count);
    } else {
      if (tiled_image)
        byte_count = TIFFReadEncodedTile(tif, chunk_no, buf_data, chunk_size);
      else
        byte_count = TIFFReadEncodedStrip(tif, chunk_no, buf_data, chunk_size);
    }
    if (byte_count < 0) {
      TIFFError(ctx->filename, "Can't read %s %d.",
                tiled_image ? "tile" : "strip", chunk_no);
      if (ctx->ascii85) Ascii85Put(ctx, '\0');
    }
    /*
    ...
    if (ctx->ascii85) {
#if defined(EXP_ASCII85ENCODER)
      ascii85_l = Ascii85EncodeBlock(ctx, ascii85_p, 1, buf_data, byte_count);

      if (ascii85_l > 0) fwrite(ascii85_p, ascii85_l, 1, ctx->fd);
#else
      for (cp = buf_data; byte_count > 0; byte_count--) Ascii85Put(ctx, *cp++);
#endif
    } else {
    ...
    if (!ctx->ascii85) {
      if (ctx->level2 || ctx->level3) putc('>', ctx->fd);
      putc('\n', ctx->fd);
    }
#if !defined(EXP_ASCII85ENCODER)
    else
...
#if defined(EXP_ASCII85ENCODER)
  if (ascii85_p) _TIFFfree(ascii85_p);
#endif
  _TIFFfree(buf_data);
#ifdef ENABLE_BROKEN_BEGINENDDATA
  fputs("%%EndData\n", ctx->fd);
...
void PSpage(TIFF2PSContext* ctx, TIFF* tif, uint32 w, uint32 h) {
  if ((ctx->level2 || ctx->level3) && PS_Lvl2page(ctx, tif, w, h)) return;
  ctx->ps_bytesperrow =
      ctx->tf_bytesperrow - (ctx->extrasamples * ctx->bitspersample / 8) * w;
  switch (ctx->photometric) {
    case PHOTOMETRIC_RGB:
    ...
    case PHOTOMETRIC_PALETTE:
      ...
      fprintf(ctx->fd, "{currentfile scanLine readhexstring pop} bind\n");
...
void PSColorContigPreamble(TIFF2PSContext* ctx, uint32 w, uint32 h, int nc) {
...
void PSColorSeparatePreamble(TIFF2PSContext* ctx, uint32 w, uint32 h, int nc) {
  int i;
...
void PSDataColorContig(TIFF2PSContext* ctx, TIFF* tif, uint32 w, uint32 h,
                       int nc) {
  uint32 row;
  ...
  for (row = 0; row < h; row++) {
    if (TIFFReadScanline(tif, tf_buf, row, 0) < 0) break;
    cp = tf_buf;
    if (ctx->alpha) {
      int adjust;
      cc = 0;
...
void PSDataColorSeparate(TIFF2PSContext* ctx, TIFF* tif, uint32 w, uint32 h,
                         int nc) {
  uint32 row;
  ...
  for (row = 0; row < h; row++) {
    for (s = 0; s < maxs; s++) {
      if (TIFFReadScanline(tif, tf_buf, row, s) < 0) break;
...
void PSDataPalette(TIFF2PSContext* ctx, TIFF* tif, uint32 w, uint32 h) {
  uint16 *rmap, *gmap, *bmap;
  uint32 row;
  ...
  for (row = 0; row < h; row++) {
    ...
    for (cp = tf_buf, cc = 0; cc < ctx->tf_bytesperrow; cc++) {
      ...
      switch (ctx->bitspersample) {
        ...
        case 4:
          c = *cp++;
          PUTRGBHEX(c & 0xf, ctx->fd);
          c >>= 4;
          PUTRGBHEX(c, ctx->fd);
          break;
        case 2:
          c = *cp++;
          PUTRGBHEX(c & 0x3, ctx->fd);
          c >>= 2;
          PUTRGBHEX(c & 0x3, ctx->fd);
          c >>= 2;
          PUTRGBHEX(c & 0x3, ctx->fd);
...
void PSDataBW(TIFF2PSContext* ctx, TIFF* tif, uint32 w, uint32 h) {
  ...
  if (ctx->ascii85) Ascii85Init(ctx);
  ...
  for (s = 0; s < TIFFNumberOfStrips(tif); s++) {
    ...
    if (ctx->ascii85) {
      ...
      ascii85_l = Ascii85EncodeBlock(ctx, ascii85_p, 1, cp, cc);

      if (ascii85_l > 0) fwrite(ascii85_p, ascii85_l, 1, ctx->fd);
#else
      while (cc-- > 0) Ascii85Put(ctx, *cp++);
  ...
  if (!ctx->ascii85) {
    if (ctx->level2 || ctx->level3) fputs(">\n", ctx->fd);
  }
#if !defined(EXP_ASCII85ENCODER)
  else
    Ascii85Flush(ctx);
...
static void Ascii85Init(TIFF2PSContext* ctx) {
  ctx->ascii85breaklen = 2 * MAXLINE;
  ctx->ascii85count = 0;
}

static void Ascii85Encode(unsigned char* raw, char* buf) {
  uint32 word;
...
void Ascii85Put(TIFF2PSContext* ctx, unsigned char code) {
  ...
  if (ctx->ascii85count >= 4) {
    ...
    for (n = ctx->ascii85count, p = ctx->ascii85buf; n >= 4; n -= 4, p += 4) {
      char* cp;
      Ascii85Encode(p, buf);
      for (cp = buf; *cp; cp++) {
        putc(*cp, ctx->fd);
...
void Ascii85Flush(TIFF2PSContext* ctx) {
...
int Ascii85EncodeBlock(TIFF2PSContext* ctx, uint8* ascii85_p, unsigned f_eod,
                       const uint8* raw_p, int raw_l)
