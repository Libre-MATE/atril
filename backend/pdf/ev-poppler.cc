...
/* fields from the XMP Rights Management Schema, XMP Specification Sept 2005,
 * pag. 45 */
#define LICENSE_MARKED "/x:xmpmeta/rdf:RDF/rdf:Description/xmpRights:Marked"
#define LICENSE_TEXT \
  "/x:xmpmeta/rdf:RDF/rdf:Description/dc:rights/rdf:Alt/rdf:li[lang('%s')]"
#define LICENSE_WEB_STATEMENT \
  "/x:xmpmeta/rdf:RDF/rdf:Description/xmpRights:WebStatement"
/* license field from Creative Commons schema, http://creativecommons.org/ns */
#define LICENSE_URI \
  "/x:xmpmeta/rdf:RDF/rdf:Description/cc:license/@rdf:resource"
...
static void pdf_document_security_iface_init(
    EvDocumentSecurityInterface *iface);
static void pdf_document_document_thumbnails_iface_init(
    EvDocumentThumbnailsInterface *iface);
static void pdf_document_document_links_iface_init(
    EvDocumentLinksInterface *iface);
static void pdf_document_document_images_iface_init(
    EvDocumentImagesInterface *iface);
static void pdf_document_document_forms_iface_init(
    EvDocumentFormsInterface *iface);
static void pdf_document_document_fonts_iface_init(
    EvDocumentFontsInterface *iface);
static void pdf_document_document_layers_iface_init(
    EvDocumentLayersInterface *iface);
static void pdf_document_document_print_iface_init(
    EvDocumentPrintInterface *iface);
static void pdf_document_document_annotations_iface_init(
    EvDocumentAnnotationsInterface *iface);
static void pdf_document_document_attachments_iface_init(
    EvDocumentAttachmentsInterface *iface);
static void pdf_document_find_iface_init(EvDocumentFindInterface *iface);
static void pdf_document_file_exporter_iface_init(
    EvFileExporterInterface *iface);
...
static void pdf_document_thumbnails_get_dimensions(
    EvDocumentThumbnails *document_thumbnails, EvRenderContext *rc, gint *width,
    gint *height);
...
EV_BACKEND_REGISTER_WITH_CODE(PdfDocument, pdf_document, {
  ...
  EV_BACKEND_IMPLEMENT_INTERFACE(EV_TYPE_SELECTION, pdf_selection_iface_init);
...
static void convert_error(GError *poppler_error, GError **error) {
  ...
  if (poppler_error->domain == POPPLER_ERROR) {
    ...
    g_set_error_literal(error, EV_DOCUMENT_ERROR, code, poppler_error->message);
...
/* EvDocument */
static gboolean pdf_document_save(EvDocument *document, const char *uri,
                                  GError **error) {
  ...
  if (pdf_document->forms_modified || pdf_document->annots_modified) {
    retval = poppler_document_save(pdf_document->document, uri, &poppler_error);
    if (retval) {
      pdf_document->forms_modified = FALSE;
      pdf_document->annots_modified = FALSE;
    }
  } else {
    retval = poppler_document_save_a_copy(pdf_document->document, uri,
                                          &poppler_error);
  }

  if (!retval) convert_error(poppler_error, error);

  return retval;
}

static gboolean pdf_document_load(EvDocument *document, const char *uri,
                                  GError **error) {
  GError *poppler_error = NULL;
  PdfDocument *pdf_document = PDF_DOCUMENT(document);

  pdf_document->document = poppler_document_new_from_file(
      uri, pdf_document->password, &poppler_error);
...
static void pdf_document_get_page_size(EvDocument *document, EvPage *page,
...
static char *pdf_document_get_page_label(EvDocument *document, EvPage *page) {
  ...
  g_object_get(G_OBJECT(page->backend_page), "label", &label, NULL);
  return label;
}

static cairo_surface_t *pdf_page_render(PopplerPage *page, gint width,
                                        gint height, EvRenderContext *rc) {
...
static cairo_surface_t *pdf_document_render(EvDocument *document,
                                            EvRenderContext *rc) {
  PopplerPage *poppler_page;
  ...
  return pdf_page_render(poppler_page, width, height, rc);
}
...
*/
static char *pdf_document_get_format_from_metadata(
  ...
  xmlXPathRegisterNs(xpathCtx, BAD_CAST "rdf",
                     BAD_CAST "http://www.w3.org/1999/02/22-rdf-syntax-ns#");
  xmlXPathRegisterNs(xpathCtx, BAD_CAST "pdfaid",
                     BAD_CAST "http://www.aiim.org/pdfa/ns/id/");

  /* reads pdf/a part */
  /* first syntax: child node */
  xpathObj = xmlXPathEvalExpression(
      BAD_CAST "/x:xmpmeta/rdf:RDF/rdf:Description/pdfaid:part", xpathCtx);
  if (xpathObj != NULL) {
  ...
  /* first syntax: child node */
  xpathObj = xmlXPathEvalExpression(
      BAD_CAST "/x:xmpmeta/rdf:RDF/rdf:Description/pdfaid:conformance",
      xpathCtx);
  if (xpathObj != NULL) {
    if (xpathObj->nodesetval != NULL && xpathObj->nodesetval->nodeNr != 0)
  ...
  if (part != NULL && conf != NULL) {
    ...
    for (i = 0; conf[i]; i++) conf[i] = g_ascii_tolower(conf[i]);
...
static EvDocumentLicense *pdf_document_get_license_from_metadata(
    xmlDocPtr doc, xmlXPathContextPtr xpathCtx) {
  xmlXPathObjectPtr xpathObj;
  xmlChar *marked = NULL;
  ...
  xmlXPathRegisterNs(xpathCtx, BAD_CAST "rdf",
                     BAD_CAST "http://www.w3.org/1999/02/22-rdf-syntax-ns#");
  xmlXPathRegisterNs(xpathCtx, BAD_CAST "dc",
                     BAD_CAST "http://purl.org/dc/elements/1.1/");
  /* XMP Rights Management Schema */
  xmlXPathRegisterNs(xpathCtx, BAD_CAST "xmpRights",
                     BAD_CAST "http://ns.adobe.com/xap/1.0/rights/");
  /* Creative Commons Schema */
  xmlXPathRegisterNs(xpathCtx, BAD_CAST "cc",
                     BAD_CAST "http://creativecommons.org/ns#");
  ...
  if (g_strrstr((char *)marked, "False") != NULL) {
    ...
    while (i-- && !license->text) {
      ...
      if (xpathObj != NULL) {
        if (xpathObj->nodesetval != NULL && xpathObj->nodesetval->nodeNr != 0)
          license->text =
              (gchar *)xmlNodeGetContent(xpathObj->nodesetval->nodeTab[0]);
        xmlXPathFreeObject(xpathObj);
      }
      g_free(tag);
      g_free(aux);
...
static void pdf_document_parse_metadata(const gchar *metadata,
                                        EvDocumentInfo *info) {
  xmlDocPtr doc;
  ...
  if (doc == NULL) return; /* invalid xml metadata */
...
static EvDocumentInfo *pdf_document_get_info(EvDocument *document) {
  ...
  info->fields_mask = EV_DOCUMENT_INFO_TITLE | EV_DOCUMENT_INFO_FORMAT |
                      EV_DOCUMENT_INFO_AUTHOR | EV_DOCUMENT_INFO_SUBJECT |
                      EV_DOCUMENT_INFO_KEYWORDS | EV_DOCUMENT_INFO_LAYOUT |
                      EV_DOCUMENT_INFO_START_MODE |
                      EV_DOCUMENT_INFO_PERMISSIONS | EV_DOCUMENT_INFO_UI_HINTS |
                      EV_DOCUMENT_INFO_CREATOR | EV_DOCUMENT_INFO_PRODUCER |
                      EV_DOCUMENT_INFO_CREATION_DATE |
                      EV_DOCUMENT_INFO_MOD_DATE | EV_DOCUMENT_INFO_LINEARIZED |
                      EV_DOCUMENT_INFO_N_PAGES | EV_DOCUMENT_INFO_SECURITY |
                      EV_DOCUMENT_INFO_PAPER_SIZE | EV_DOCUMENT_INFO_LICENSE;

  g_object_get(PDF_DOCUMENT(document)->document, "title", &(info->title),
               "format", &(info->format), "author", &(info->author), "subject",
               &(info->subject), "keywords", &(info->keywords), "page-mode",
               &mode, "page-layout", &layout, "viewer-preferences", &view_prefs,
               "permissions", &permissions, "creator", &(info->creator),
               "producer", &(info->producer), "linearized", &linearized,
               "metadata", &metadata, NULL);

  info->creation_date = (gint64)poppler_document_get_creation_date(
      PDF_DOCUMENT(document)->document);
  info->modified_date = (gint64)poppler_document_get_modification_date(
      PDF_DOCUMENT(document)->document);
  ...
  if (ev_document_security_has_document_security(
          EV_DOCUMENT_SECURITY(document))) {
...
static gboolean pdf_document_get_backend_info(EvDocument *document,
...
/* EvDocumentSecurity */
static gboolean pdf_document_has_document_security(
    EvDocumentSecurity *document_security) {
  /* FIXME: do we really need to have this? */
  return FALSE;
}

static void pdf_document_set_password(EvDocumentSecurity *document_security,
                                      const char *password) {
  PdfDocument *document = PDF_DOCUMENT(document_security);

  if (document->password) g_free(document->password);
...
static gdouble pdf_document_fonts_get_progress(
...
static gboolean pdf_document_fonts_scan(EvDocumentFonts *document_fonts,
                                        int n_pages) {
  PdfDocument *pdf_document = PDF_DOCUMENT(document_fonts);
  gboolean result;
...
static void pdf_document_fonts_fill_model(EvDocumentFonts *document_fonts,
                                          GtkTreeModel *model) {
  PdfDocument *pdf_document = PDF_DOCUMENT(document_fonts);
  ...
  do {
    ...
    type = font_type_to_string(poppler_fonts_iter_get_font_type(iter));
    ...
    gtk_list_store_set(GTK_LIST_STORE(model), &list_iter,
                       ...
                       EV_DOCUMENT_FONTS_COLUMN_DETAILS, details, -1);
...
static gboolean pdf_document_links_has_document_links(
...
static EvLinkDest *ev_link_dest_from_dest(PdfDocument *pdf_document,
  ...
  switch (dest->type) {
    case POPPLER_DEST_XYZ: {
      ...
      ev_dest = ev_link_dest_new_xyz(
          dest->page_num - 1, dest->left, height - MIN(height, dest->top),
          dest->zoom, dest->change_left, dest->change_top, dest->change_zoom);
      g_object_unref(poppler_page);
    } break;
    ...
    case POPPLER_DEST_FITV:
      ev_dest = ev_link_dest_new_fitv(dest->page_num - 1, dest->left,
                                      dest->change_left);
      break;
    case POPPLER_DEST_FITR: {
      ...
      poppler_page = poppler_document_get_page(pdf_document->document,
                                               MAX(0, dest->page_num - 1));
      poppler_page_get_size(poppler_page, NULL, &height);
      ev_dest = ev_link_dest_new_fitr(
          dest->page_num - 1, dest->left, height - MIN(height, dest->bottom),
  ...
  if (!ev_dest) ev_dest = ev_link_dest_new_page(dest->page_num - 1);
...
static EvLink *ev_link_from_action(PdfDocument *pdf_document,
  ...
  switch (action->type) {
    ...
    case POPPLER_ACTION_GOTO_REMOTE: {
      ...
      ev_action =
          ev_link_action_new_remote(dest, action->goto_remote.file_name);
    ...
    case POPPLER_ACTION_OCG_STATE: {
      ...
      for (l = action->ocg_state.state_list; l; l = g_list_next(l)) {
        ...
        for (m = action_layer->layers; m; m = g_list_next(m)) {
          ...
          g_object_set_data_full(G_OBJECT(ev_layer), "poppler-layer",
                                 g_object_ref(layer),
...
static void build_tree(PdfDocument *pdf_document, GtkTreeModel *model,
                       GtkTreeIter *parent, PopplerIndexIter *iter) {
  do {
    ...
    gtk_tree_store_set(GTK_TREE_STORE(model), &tree_iter,
                       ...
                       EV_DOCUMENT_LINKS_COLUMN_EXPAND, expand, -1);

    g_free(title_markup);
    g_object_unref(link);

    child = poppler_index_iter_get_child(iter);
    if (child) build_tree(pdf_document, model, &tree_iter, child);
...
static GtkTreeModel *pdf_document_links_get_links_model(
    EvDocumentLinks *document_links) {
  PdfDocument *pdf_document = PDF_DOCUMENT(document_links);
...
static EvMappingList *pdf_document_links_get_links(
  ...
  for (list = mapping_list; list; list = list->next) {
    ...
    ev_link_mapping->data =
        ev_link_from_action(pdf_document, link_mapping->action);
...
static EvLinkDest *pdf_document_links_find_link_dest(
  ...
  pdf_document = PDF_DOCUMENT(document_links);
  dest = poppler_document_find_dest(pdf_document->document, link_name);
  if (dest) {
...
static gint pdf_document_links_find_link_page(EvDocumentLinks *document_links,
...
static EvMappingList *pdf_document_images_get_image_mapping(
...
static GdkPixbuf *pdf_document_images_get_image(
    EvDocumentImages *document_images, EvImage *image) {
...
static GdkPixbuf *make_thumbnail_for_page(PopplerPage *poppler_page,
...
static GdkPixbuf *pdf_document_thumbnails_get_thumbnail(
    EvDocumentThumbnails *document_thumbnails, EvRenderContext *rc,
    gboolean border) {
  ...
  if (pixbuf != NULL) {
    ...
    if (thumb_width == width) {
      ...
      rotated_pixbuf = gdk_pixbuf_rotate_simple(
          pixbuf, (GdkPixbufRotation)(360 - rc->rotation));
...
static void pdf_document_thumbnails_get_dimensions(
  ...
  poppler_page_get_size(POPPLER_PAGE(rc->page->backend_page), &page_width,
                        &page_height);
...
static GList *pdf_document_find_find_text(EvDocumentFind *document_find,
  ...
  if (case_sensitive) options = POPPLER_FIND_CASE_SENSITIVE;
...
static void pdf_document_file_exporter_begin(EvFileExporter *exporter,
  ...
  if (pdf_document->print_ctx) pdf_print_context_free(pdf_document->print_ctx);
...
#ifdef HAVE_CAIRO_PS
      surface = cairo_ps_surface_create(fc->filename, fc->paper_width,
                                        fc->paper_height);
#endif
      break;
    case EV_FILE_FORMAT_PDF:
#ifdef HAVE_CAIRO_PDF
      surface = cairo_pdf_surface_create(fc->filename, fc->paper_width,
                                         fc->paper_height);
#endif
      break;
    default:
  ...
  if (ctx->format == EV_FILE_FORMAT_PS) {
    ctx->ps_file =
        poppler_ps_file_new(pdf_document->document, fc->filename,
                            fc->first_page, fc->last_page - fc->first_page + 1);
    poppler_ps_file_set_paper_size(ctx->ps_file, fc->paper_width,
                                   fc->paper_height);
...
static void pdf_document_file_exporter_begin_page(EvFileExporter *exporter) {
  ...
  if (ctx->paper_width > ctx->paper_height) {
    if (ctx->format == EV_FILE_FORMAT_PS) {
      cairo_ps_surface_set_size(cairo_get_target(ctx->cr), ctx->paper_height,
                                ctx->paper_width);
    } else if (ctx->format == EV_FILE_FORMAT_PDF) {
      cairo_pdf_surface_set_size(cairo_get_target(ctx->cr), ctx->paper_height,
                                 ctx->paper_width);
    }
  }
#endif /* HAVE_CAIRO_PRINT */
...
static void pdf_document_file_exporter_do_page(EvFileExporter *exporter,
                                               EvRenderContext *rc) {
  PdfDocument *pdf_document = PDF_DOCUMENT(exporter);
  PdfPrintContext *ctx = pdf_document->print_ctx;
  PopplerPage *poppler_page;
  ...
  cairo_translate(ctx->cr, x * (rotate ? pheight : pwidth),
...
static EvFileExporterCapabilities pdf_document_file_exporter_get_capabilities(
    EvFileExporter *exporter) {
  return (EvFileExporterCapabilities)(EV_FILE_EXPORTER_CAN_PAGE_SET |
...
/* EvDocumentPrint */
static void pdf_document_print_print_page(EvDocumentPrint *document,
...
static void pdf_selection_render_selection(
    EvSelection *selection, EvRenderContext *rc, cairo_surface_t **surface,
    EvRectangle *points, EvRectangle *old_points, EvSelectionStyle style,
  ...
  poppler_page_render_selection(poppler_page, cr, (PopplerRectangle *)points,
                                (PopplerRectangle *)old_points,
                                (PopplerSelectionStyle)style, &text_color,
                                &base_color);
  cairo_destroy(cr);
}

static gchar *pdf_selection_get_selected_text(EvSelection *selection,
                                              EvPage *page,
...
static cairo_region_t *create_region_from_poppler_region(GList *region,
                                                         gdouble scale) {
  GList *l;
...
static cairo_region_t *pdf_selection_get_selection_region(
    EvSelection *selection, EvRenderContext *rc, EvSelectionStyle style,
    EvRectangle *points) {
  PopplerPage *poppler_page;
  cairo_region_t *retval;
  GList *region;
...
static cairo_region_t *pdf_document_text_get_text_mapping(
    EvDocumentText *document_text, EvPage *page) {
  PopplerPage *poppler_page;
  PopplerRectangle points;
  GList *region;
  cairo_region_t *retval;
  ...
  region = poppler_page_get_selection_region(poppler_page, 1.0,
                                             POPPLER_SELECTION_GLYPH, &points);
...
static gchar *pdf_document_text_get_text(EvDocumentText *selection,
                                         EvPage *page) {
  g_return_val_if_fail(POPPLER_IS_PAGE(page->backend_page), NULL);
...
static gboolean pdf_document_text_get_text_layout(EvDocumentText *selection,
                                                  EvPage *page,
                                                  EvRectangle **areas,
                                                  guint *n_areas) {
...
static PangoAttrList *pdf_document_text_get_text_attrs(
    EvDocumentText *document_text, EvPage *page) {
  ...
  backend_attrs_list =
      poppler_page_get_text_attributes(POPPLER_PAGE(page->backend_page));
  if (!backend_attrs_list) return NULL;
...
/* Page Transitions */
static gdouble pdf_document_get_page_duration(EvDocumentTransition *trans,
                                              gint page) {
  PdfDocument *pdf_document;
  PopplerPage *poppler_page;
  gdouble duration = -1;
...
static EvTransitionEffect *pdf_document_get_effect(EvDocumentTransition *trans,
                                                   gint page) {
  PdfDocument *pdf_document;
  PopplerPage *poppler_page;
  ...
  effect = ev_transition_effect_new(
      (EvTransitionEffectType)page_transition->type, "alignment",
      page_transition->alignment, "direction", page_transition->direction,
      "duration", page_transition->duration, "angle", page_transition->angle,
      "scale", page_transition->scale, "rectangular",
      page_transition->rectangular, NULL);
...
static EvFormField *ev_form_field_from_poppler_field(
    PdfDocument *pdf_document, PopplerFormField *poppler_field) {
  ...
  switch (poppler_form_field_get_field_type(poppler_field)) {
    case POPPLER_FORM_FIELD_TEXT: {
      ...
      field_text->do_spell_check =
          poppler_form_field_text_do_spell_check(poppler_field);
      field_text->do_scroll = poppler_form_field_text_do_scroll(poppler_field);
      field_text->is_rich_text =
          poppler_form_field_text_is_rich_text(poppler_field);
      field_text->is_password =
          poppler_form_field_text_is_password(poppler_field);
    ...
    case POPPLER_FORM_FIELD_CHOICE: {
      ...
      field_choice->is_editable =
          poppler_form_field_choice_is_editable(poppler_field);
      field_choice->multi_select =
          poppler_form_field_choice_can_select_multiple(poppler_field);
      field_choice->do_spell_check =
          poppler_form_field_choice_do_spell_check(poppler_field);
      field_choice->commit_on_sel_change =
          poppler_form_field_choice_commit_on_change(poppler_field);

      /* TODO: we need poppler_form_field_choice_get_selected_items in poppler
      field_choice->selected_items =
      poppler_form_field_choice_get_selected_items (poppler_field);*/
...
static EvMappingList *pdf_document_forms_get_form_fields(
    EvDocumentForms *document, EvPage *page) {
  PopplerPage *poppler_page;
  GList *retval = NULL;
  GList *fields;
  GList *list;
  ...
  for (list = fields; list; list = list->next) {
    ...
    ev_field = ev_form_field_from_poppler_field(PDF_DOCUMENT(document),
                                                mapping->field);
    if (!ev_field) continue;
    ...
    g_object_set_data_full(G_OBJECT(ev_field), "poppler-field",
...
static gboolean pdf_document_forms_document_is_modified(
    EvDocumentForms *document) {
  return PDF_DOCUMENT(document)->forms_modified;
}

static gchar *pdf_document_forms_form_field_text_get_text(
    EvDocumentForms *document, EvFormField *field)

{
  ...
  poppler_field =
      POPPLER_FORM_FIELD(g_object_get_data(G_OBJECT(field), "poppler-field"));
  if (!poppler_field) return NULL;
...
static void pdf_document_forms_form_field_text_set_text(
    EvDocumentForms *document, EvFormField *field, const gchar *text) {
  PopplerFormField *poppler_field;

  poppler_field =
      POPPLER_FORM_FIELD(g_object_get_data(G_OBJECT(field), "poppler-field"));
  if (!poppler_field) return;

  poppler_form_field_text_set_text(poppler_field, text);
  PDF_DOCUMENT(document)->forms_modified = TRUE;
}

static void pdf_document_forms_form_field_button_set_state(
    EvDocumentForms *document, EvFormField *field, gboolean state) {
  PopplerFormField *poppler_field;
...
static gboolean pdf_document_forms_form_field_button_get_state(
    EvDocumentForms *document, EvFormField *field) {
  PopplerFormField *poppler_field;
  gboolean state;
...
static gchar *pdf_document_forms_form_field_choice_get_item(
    EvDocumentForms *document, EvFormField *field, gint index) {
  PopplerFormField *poppler_field;
  gchar *text;
...
static int pdf_document_forms_form_field_choice_get_n_items(
    EvDocumentForms *document, EvFormField *field) {
  PopplerFormField *poppler_field;
  gint n_items;
...
static gboolean pdf_document_forms_form_field_choice_is_item_selected(
    EvDocumentForms *document, EvFormField *field, gint index) {
  PopplerFormField *poppler_field;
  gboolean selected;
...
static void pdf_document_forms_form_field_choice_select_item(
    EvDocumentForms *document, EvFormField *field, gint index) {
  PopplerFormField *poppler_field;
...
static void pdf_document_forms_form_field_choice_toggle_item(
    EvDocumentForms *document, EvFormField *field, gint index) {
  PopplerFormField *poppler_field;
...
static void pdf_document_forms_form_field_choice_unselect_all(
    EvDocumentForms *document, EvFormField *field) {
  PopplerFormField *poppler_field;
...
static void pdf_document_forms_form_field_choice_set_text(
    EvDocumentForms *document, EvFormField *field, const gchar *text) {
  PopplerFormField *poppler_field;
...
static gchar *pdf_document_forms_form_field_choice_get_text(
    EvDocumentForms *document, EvFormField *field) {
  PopplerFormField *poppler_field;
  gchar *text;
...
static EvAnnotation *ev_annot_from_poppler_annot(PopplerAnnot *poppler_annot,
                                                 EvPage *page) {
  ...
  switch (poppler_annot_get_annot_type(poppler_annot)) {
    case POPPLER_ANNOT_TEXT: {
      ...
      ev_annotation_text_set_is_open(
          ev_annot_text, poppler_annot_text_get_is_open(poppler_text));
      ev_annotation_text_set_icon(ev_annot_text,
                                  get_annot_text_icon(poppler_text));
    } break;
    case POPPLER_ANNOT_FILE_ATTACHMENT: {
      ...
      poppler_attachment = poppler_annot_file_attachment_get_attachment(
          poppler_annot_attachment);
      ...
      if (poppler_attachment &&
        ...
        ev_attachment = ev_attachment_new(
            poppler_attachment->name, poppler_attachment->description,
            poppler_attachment->mtime, poppler_attachment->ctime, size, data);
      ...
      if (poppler_attachment) g_object_unref(poppler_attachment);
    } break;
    ...
    default: {
      ...
      enum_value = g_enum_get_value(
          (GEnumClass *)g_type_class_ref(POPPLER_TYPE_ANNOT_TYPE),
          poppler_annot_get_annot_type(poppler_annot));
      unimplemented_annot =
          enum_value ? enum_value->value_name : "Unknown annotation";
    }
  }
  ...
  if (ev_annot) {
    ...
    if (POPPLER_IS_ANNOT_MARKUP(poppler_annot)) {
      ...
      if (poppler_annot_markup_get_popup_rectangle(markup, &poppler_rect)) {
        ...
        g_object_set(ev_annot, "rectangle", &ev_rect, "popup_is_open", is_open,
                     "has_popup", TRUE, NULL);
      } else {
        g_object_set(ev_annot, "has_popup", FALSE, NULL);
      }

      label = poppler_annot_markup_get_label(markup);
      opacity = poppler_annot_markup_get_opacity(markup);

      g_object_set(ev_annot, "label", label, "opacity", opacity, NULL);
...
static EvMappingList *pdf_document_annotations_get_annotations(
    EvDocumentAnnotations *document_annotations, EvPage *page) {
  GList *retval = NULL;
  PdfDocument *pdf_document;
  PopplerPage *poppler_page;
  ...
  if (pdf_document->annots) {
    mapping_list = (EvMappingList *)g_hash_table_lookup(
        pdf_document->annots, GINT_TO_POINTER(page->index));
    if (mapping_list) return ev_mapping_list_ref(mapping_list);
  }
  ...
  for (list = annots; list; list = list->next) {
    ...
    if (!ev_annot) continue;

    i++;

    /* Make sure annot has a unique name */
    if (!ev_annotation_get_name(ev_annot)) annot_set_unique_name(ev_annot);
    ...
    g_object_set_data_full(G_OBJECT(ev_annot), "poppler-annot",
  ...
  if (!pdf_document->annots) {
    pdf_document->annots = g_hash_table_new_full(
        g_direct_hash, g_direct_equal, (GDestroyNotify)NULL,
        (GDestroyNotify)ev_mapping_list_unref);
  }

  mapping_list = ev_mapping_list_new(page->index, g_list_reverse(retval),
                                     (GDestroyNotify)g_object_unref);
  g_hash_table_insert(pdf_document->annots, GINT_TO_POINTER(page->index),
                      ev_mapping_list_ref(mapping_list));

  return mapping_list;
}

static gboolean pdf_document_annotations_document_is_modified(
    EvDocumentAnnotations *document_annotations) {
  return PDF_DOCUMENT(document_annotations)->annots_modified;
}

static void pdf_document_annotations_remove_annotation(
    EvDocumentAnnotations *document_annotations, EvAnnotation *annot) {
  PopplerPage *poppler_page;
  PdfDocument *pdf_document;
  EvPage *page;
  ...
  poppler_annot =
      POPPLER_ANNOT(g_object_get_data(G_OBJECT(annot), "poppler-annot"));
  pdf_document = PDF_DOCUMENT(document_annotations);
  ...
  /* We don't check for pdf_document->annots, if it were NULL then something is
   * really wrong */
  mapping_list = (EvMappingList *)g_hash_table_lookup(
      pdf_document->annots, GINT_TO_POINTER(page->index));
  if (mapping_list) {
...
static void pdf_document_annotations_add_annotation(
  ...
  switch (ev_annotation_get_annotation_type(annot)) {
    case EV_ANNOTATION_TYPE_TEXT: {
      ...
      poppler_annot =
          poppler_annot_text_new(pdf_document->document, &poppler_rect);
  ...
  if (EV_IS_ANNOTATION_MARKUP(annot)) {
    ...
    if (ev_annotation_markup_has_popup(markup)) {
      ...
      poppler_annot_markup_set_popup(POPPLER_ANNOT_MARKUP(poppler_annot),
                                     &poppler_rect);
      poppler_annot_markup_set_popup_is_open(
          POPPLER_ANNOT_MARKUP(poppler_annot),
  ...
  g_object_set_data_full(G_OBJECT(annot), "poppler-annot", poppler_annot,
                         (GDestroyNotify)g_object_unref);
  ...
  if (mapping_list) {
    ...
    list = g_list_append(list, annot_mapping);
    mapping_list =
        ev_mapping_list_new(page->index, list, (GDestroyNotify)g_object_unref);
    g_hash_table_insert(pdf_document->annots, GINT_TO_POINTER(page->index),
                        ev_mapping_list_ref(mapping_list));
  }
...
static void pdf_document_annotations_save_annotation(
    EvDocumentAnnotations *document_annotations, EvAnnotation *annot,
  ...
  if (EV_IS_ANNOTATION_MARKUP(annot)) {
    ...
    if (mask & EV_ANNOTATIONS_SAVE_LABEL)
      poppler_annot_markup_set_label(markup,
                                     ev_annotation_markup_get_label(ev_markup));
    if (mask & EV_ANNOTATIONS_SAVE_OPACITY)
      poppler_annot_markup_set_opacity(
          markup, ev_annotation_markup_get_opacity(ev_markup));
    if (mask & EV_ANNOTATIONS_SAVE_POPUP_IS_OPEN)
      poppler_annot_markup_set_popup_is_open(
          markup, ev_annotation_markup_get_popup_is_open(ev_markup));
  }
...
static gboolean attachment_save_to_buffer(PopplerAttachment *attachment,
  ...
  *buffer_size = 0;
  ...
  if (!poppler_attachment_save_to_callback(
          attachment, attachment_save_to_buffer_callback, &sdata, error)) {
    g_free(sdata.buffer);
    return FALSE;
  }
...
static GList *pdf_document_attachments_get_attachments(
    EvDocumentAttachments *document) {
  PdfDocument *pdf_document = PDF_DOCUMENT(document);
  GList *attachments;
  GList *list;
  GList *retval = NULL;
  ...
  for (list = attachments; list; list = list->next) {
    ...
    if (attachment_save_to_buffer(attachment, &data, &size, &error)) {
      ev_attachment =
          ev_attachment_new(attachment->name, attachment->description,
                            attachment->mtime, attachment->ctime, size, data);
...
static gboolean pdf_document_attachments_has_attachments(
    EvDocumentAttachments *document) {
  PdfDocument *pdf_document = PDF_DOCUMENT(document);
...
static void build_layers_tree(PdfDocument *pdf_document, GtkTreeModel *model,
                              GtkTreeIter *parent, PopplerLayersIter *iter) {
  do {
    GtkTreeIter tree_iter;
    ...
    if (layer) {
      ...
      ev_layer = ev_layer_new(poppler_layer_is_parent(layer), rb_group);
      g_object_set_data_full(G_OBJECT(ev_layer), "poppler-layer",
                             g_object_ref(layer),
                             (GDestroyNotify)g_object_unref);
    } else {
    ...
    gtk_tree_store_set(GTK_TREE_STORE(model), &tree_iter,
                       ...
                       EV_DOCUMENT_LAYERS_COLUMN_LAYER, ev_layer, -1);
    if (ev_layer) g_object_unref(ev_layer);
    g_free(markup);

    child = poppler_layers_iter_get_child(iter);
    if (child) build_layers_tree(pdf_document, model, &tree_iter, child);
    poppler_layers_iter_free(child);
  } while (poppler_layers_iter_next(iter));
}

static GtkTreeModel *pdf_document_layers_get_layers(
...
static void pdf_document_layers_show_layer(EvDocumentLayers *document,
                                           EvLayer *layer) {
  PopplerLayer *poppler_layer;

  poppler_layer =
      POPPLER_LAYER(g_object_get_data(G_OBJECT(layer), "poppler-layer"));
  poppler_layer_show(poppler_layer);
}

static void pdf_document_layers_hide_layer(EvDocumentLayers *document,
                                           EvLayer *layer) {
  PopplerLayer *poppler_layer;
...
static gboolean pdf_document_layers_layer_is_visible(EvDocumentLayers *document,
                                                     EvLayer *layer) {
  PopplerLayer *poppler_layer;
