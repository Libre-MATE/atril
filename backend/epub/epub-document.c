...
static void epub_document_document_thumbnails_iface_init(
    EvDocumentThumbnailsInterface *iface);
static void epub_document_document_find_iface_init(
    EvDocumentFindInterface *iface);
static void epub_document_document_links_iface_init(
...
static void epub_document_thumbnails_get_dimensions(
...
static GdkPixbuf *epub_document_thumbnails_get_thumbnail(
...
static gboolean in_tag(const char *found) {
...
static int get_substr_count(const char *haystack, const char *needle,
...
static guint epub_document_check_hits(EvDocumentFind *document_find,
                                      EvPage *page, const gchar *text,
                                      gboolean case_sensitive) {
  gchar *filepath =
      g_filename_from_uri((gchar *)page->backend_page, NULL, NULL);
...
static gboolean epub_document_links_has_document_links(
  ...
  if (!epub_document->index) return FALSE;
...
static void epub_document_make_tree_entry(linknode *ListData,
                                          LinksCBStruct *UserData) {
  ...
  if (g_strrstr(ListData->pagelink, "#") == NULL) {
    ...
    ev_dest =
        ev_link_dest_new_hlink((gchar *)ListData->pagelink, ListData->page);
  }
  ...
  gtk_tree_store_append(GTK_TREE_STORE(UserData->model), &tree_iter,
                        (UserData->parent));
  ...
  if (ListData->children) {
    ...
    g_list_foreach(ListData->children, (GFunc)epub_document_make_tree_entry,
                   &cbstruct);
  }
...
static GtkTreeModel *epub_document_links_get_links_model(
    EvDocumentLinks *document_links) {
  ...
  gtk_tree_store_set(GTK_TREE_STORE(model), &parent,
                     ...
                     EV_DOCUMENT_LINKS_COLUMN_LINK, link,
                     EV_DOCUMENT_LINKS_COLUMN_EXPAND, TRUE, -1);

  g_object_unref(link);

  if (epub_document->index) {
    g_list_foreach(epub_document->index, (GFunc)epub_document_make_tree_entry,
                   &linkStruct);
  }
...
static EvMappingList *epub_document_links_get_links(
...
static gboolean epub_document_save(EvDocument *document, const char *uri,
                                   GError **error) {
  EpubDocument *epub_document = EPUB_DOCUMENT(document);

  gchar *source_uri =
      g_filename_to_uri(epub_document->archivename, NULL, error);
  if (source_uri == NULL) return FALSE;
...
static int epub_document_get_n_pages(EvDocument *document) {
  ...
  if (epub_document->contentList == NULL) return 0;
 ...
 */
static int epub_remove_temporary_dir(gchar *path_name) {
  GDir *content_dir;
  ...
  if (g_file_test(path_name, G_FILE_TEST_IS_DIR)) {
    ...
    while (filename) {
      filename_with_path = g_build_filename(path_name, filename, NULL);
...
static gboolean check_mime_type(const gchar *uri, GError **error);

static gboolean open_xml_document(const gchar *filename);

static gboolean set_xml_root_node(xmlChar *rootname);

static xmlNodePtr xml_get_pointer_to_node(xmlChar *parserfor,
                                          xmlChar *attributename,
                                          xmlChar *attributevalue);
static void xml_parse_children_of_node(xmlNodePtr parent, xmlChar *parserfor,
                                       xmlChar *attributename,
                                       xmlChar *attributevalue);

static gboolean xml_check_attribute_value(xmlNode *node, xmlChar *attributename,
                                          xmlChar *attributevalue);

static xmlChar *xml_get_data_from_node(xmlNodePtr node,
                                       xmlParseReturnType rettype,
                                       xmlChar *attributename);

static void xml_free_doc(void);

static void free_tree_nodes(gpointer data);
...
static xmlNodePtr xml_get_pointer_to_node(xmlChar *parserfor,
  ...
  while (topchild != NULL) {
    if (!xmlStrcmp(topchild->name, parserfor)) {
      if (xml_check_attribute_value(topchild, attributename, attributevalue) ==
          TRUE) {
        xmlretval = topchild;
        return xmlretval;
      } else {
...
static void xml_parse_children_of_node(xmlNodePtr parent, xmlChar *parserfor,
  ...
  while (child != NULL) {
    if (!xmlStrcmp(child->name, parserfor)) {
      if (xml_check_attribute_value(child, attributename, attributevalue) ==
          TRUE) {
        xmlretval = child;
        return;
      } else {
...
static gboolean check_mime_type(const gchar *uri, GError **error) {
  ...
  if (mimeFromFile) {
    ...
    for (i = 0; i < g_strv_length(mimetypes); i++) {
      if (strcmp(mimeFromFile, mimetypes[i]) == 0) return TRUE;
    }
    ...
    if (strcmp(mimeFromFile, "application/zip") == 0) {
      ...
      if (mimeFromFile) {
        for (i = 0; i < g_strv_length(mimetypes); i++) {
          if (g_strcmp0(mimeFromFile, mimetypes[i]) == 0) return TRUE;
        }
...
static gboolean extract_one_file(EpubDocument *epub_document, GError **error) {
  GFile *outfile;
  gsize writesize = 0;
  GString *gfilepath;
  ...
  unzGetCurrentFileInfo64(epub_document->epubDocument, &info, currentfilename,
                          512, NULL, 0, NULL, 0);
  ...
  if (directory != NULL && *directory == '\0') {
    ...
    /*Since a substring can't be longer than the parent string, allocating space
     * equal to the parent's size should suffice*/
    gchar *createdirname = g_malloc0(strlen(currentfilename));
    /* Add the name of the directory and subdirectories,if any to a buffer and
     * then create it */
  ...
  while ((writesize = unzReadCurrentFile(epub_document->epubDocument, buffer,
                                         512)) != 0) {
    if (g_output_stream_write((GOutputStream *)outstream, buffer, writesize,
                              NULL, error) == -1) {
      result = FALSE;
      break;
    }
...
static gboolean extract_epub_from_container(const gchar *uri,
  ...
  epub_document->tmp_archive_dir =
      ev_mkdtemp(temporary_sub_directory->str, error);
  ...
  while (TRUE) {
    ...
    if (unzGoToNextFile(epub_document->epubDocument) ==
...
static gchar *get_uri_to_content(const gchar *uri, GError **error,
  ...
  xmlNodePtr rootfileNode =
      xml_get_pointer_to_node((xmlChar *)"rootfile", (xmlChar *)"media-type",
                              (xmlChar *)"application/oebps-package+xml");
  ...
  xmlChar *relativepath = xml_get_data_from_node(rootfileNode, XML_ATTRIBUTE,
                                                 (xmlChar *)"full-path");
...
static gboolean link_present_on_page(const gchar *link, const gchar *page_uri) {
  gchar *res;
...
static GList *setup_document_content_list(const gchar *content_uri,
                                          GError **error, gchar *documentdir) {
  GError *err = NULL;
  gint indexcounter = 1;
  ...
  if ((spine = xml_get_pointer_to_node((xmlChar *)"spine", NULL, NULL)) ==
      NULL) {
    g_set_error_literal(error, EV_DOCUMENT_ERROR, EV_DOCUMENT_ERROR_INVALID,
  ...
  if ((manifest = xml_get_pointer_to_node((xmlChar *)"manifest", NULL, NULL)) ==
      NULL) {
    g_set_error_literal(error, EV_DOCUMENT_ERROR, EV_DOCUMENT_ERROR_INVALID,
  ...
  do {
    ...
    if (xmlStrcmp(itemrefptr->name, (xmlChar *)"itemref") == 0) {
      ...
      newnode->key = (gchar *)xml_get_data_from_node(itemrefptr, XML_ATTRIBUTE,
                                                     (xmlChar *)"idref");
      if (newnode->key == NULL) {
        g_free(newnode);
        errorflag = TRUE;
      ...
      xmlretval = NULL;
      xml_parse_children_of_node(manifest, (xmlChar *)"item", (xmlChar *)"id",
                                 (xmlChar *)newnode->key);
      ...
      gchar *relativepath = (gchar *)xml_get_data_from_node(
          itemptr, XML_ATTRIBUTE, (xmlChar *)"href");
      g_string_append_printf(absolutepath, "/%s", relativepath);
      g_free(relativepath);
  ...
  if (errorflag) {
    if (err) {
      ...
      g_set_error_literal(
          error, EV_DOCUMENT_ERROR, EV_DOCUMENT_ERROR_INVALID,
...
static void free_link_nodes(gpointer data) {
...
static gchar *get_toc_file_name(gchar *containeruri) {
  ...
  xmlNodePtr manifest =
      xml_get_pointer_to_node((xmlChar *)"manifest", NULL, NULL);
  ...
  /*In an epub3, there is sometimes no toc, and we need to then use the nav file
   * for this.*/
  if (ncx == NULL) {
    return NULL;
  }
  ...
  gchar *tocfilename = (gchar *)xml_get_data_from_node(xmlretval, XML_ATTRIBUTE,
                                                       (xmlChar *)"href");
  xml_free_doc();
...
static gchar *epub_document_get_nav_file(gchar *containeruri) {
  ...
  xml_parse_children_of_node(manifest, (xmlChar *)"item",
                             (xmlChar *)"properties", (xmlChar *)"nav");

  gchar *uri = (gchar *)xml_get_data_from_node(xmlretval, XML_ATTRIBUTE,
                                               (xmlChar *)"href");
...
static GList *get_child_list(xmlNodePtr ol, gchar *documentdir) {
  ...
  while (li != NULL) {
    ...
    while (children != NULL) {
      if (!xmlStrcmp(children->name, (xmlChar *)"a")) {
        newlinknode->linktext =
            (gchar *)xml_get_data_from_node(children, XML_KEYWORD, NULL);
        gchar *filename = (gchar *)xml_get_data_from_node(
            children, XML_ATTRIBUTE, (xmlChar *)"href");
...
static GList *setup_index_from_navfile(gchar *tocpath) {
  GList *index = NULL;
  open_xml_document(tocpath);
  set_xml_root_node(NULL);
  xmlNodePtr nav = xml_get_pointer_to_node((xmlChar *)"nav", (xmlChar *)"type",
                                           (xmlChar *)"toc");
...
static GList *setup_document_index(EpubDocument *epub_document,
                                   gchar *containeruri) {
  ...
  if (tocfilename == NULL) {
    ...
    // Apparently, sometimes authors don't even care to add a TOC!! Guess
  ...
  xmlNodePtr docTitle =
      xml_get_pointer_to_node((xmlChar *)"docTitle", NULL, NULL);
  xmlretval = NULL;
  ...
  while (navPoint != NULL) {
    if (!xmlStrcmp(navPoint->name, (xmlChar *)"navPoint")) {
      ...
      newnode->pagelink = (gchar *)xml_get_data_from_node(
          xmlretval, XML_ATTRIBUTE, (xmlChar *)"src");
...
static EvDocumentInfo *epub_document_get_info(EvDocument *document) {
  EpubDocument *epub_document = EPUB_DOCUMENT(document);
  GError *error = NULL;
  gchar *infofile;
  xmlNodePtr metanode;
  ...
  epubinfo->fields_mask =
      EV_DOCUMENT_INFO_TITLE | EV_DOCUMENT_INFO_FORMAT |
      EV_DOCUMENT_INFO_AUTHOR | EV_DOCUMENT_INFO_SUBJECT |
      EV_DOCUMENT_INFO_KEYWORDS | EV_DOCUMENT_INFO_LAYOUT |
      EV_DOCUMENT_INFO_CREATOR | EV_DOCUMENT_INFO_LINEARIZED |
      EV_DOCUMENT_INFO_PERMISSIONS | EV_DOCUMENT_INFO_N_PAGES;
  ...
  if (metanode == NULL)
    ...
    epubinfo->title =
        (char *)xml_get_data_from_node(metanode, XML_KEYWORD, NULL);
  ...
  buffer = g_string_new((gchar *)xml_get_data_from_node(xmlroot, XML_ATTRIBUTE,
                                                        (xmlChar *)"version"));
  ...
  if (xmldocument) xml_free_doc();
  return epubinfo;
}

static EvPage *epub_document_get_page(EvDocument *document, gint index) {
  EpubDocument *epub_document = EPUB_DOCUMENT(document);
...
static void change_to_night_sheet(contentListNode *nodedata,
                                  gpointer user_data) {
  ...
  gchar *class = NULL;
  xmlretval = NULL;
  xml_parse_children_of_node(head, (xmlChar *)"link", (xmlChar *)"rel",
                             (xmlChar *)"stylesheet");

  xmlNodePtr day = xmlretval;
  if ((class = (gchar *)xml_get_data_from_node(day, XML_ATTRIBUTE,
                                               (xmlChar *)"class")) == NULL) {
  ...
  xmlretval = NULL;
  xml_parse_children_of_node(head, (xmlChar *)"link", (xmlChar *)"class",
                             (xmlChar *)"night");
...
static void change_to_day_sheet(contentListNode *nodedata, gpointer user_data) {
  gchar *filename = g_filename_from_uri(nodedata->value, NULL, NULL);
  open_xml_document(filename);
  set_xml_root_node(NULL);
  xmlNodePtr head = xml_get_pointer_to_node((xmlChar *)"head", NULL, NULL);
...
static gchar *epub_document_get_alternate_stylesheet(gchar *docuri) {
  ...
  if (xmlretval != NULL) {
    return (gchar *)xml_get_data_from_node(xmlretval, XML_ATTRIBUTE,
                                           (xmlChar *)"href");
  }
  xml_free_doc();
  return NULL;
...
static void add_night_sheet(contentListNode *listdata, gchar *sheet) {
...
static void epub_document_check_add_night_sheet(EvDocument *document) {
  EpubDocument *epub_document = EPUB_DOCUMENT(document);
  ...
  gchar *stylesheetfilename =
      epub_document_get_alternate_stylesheet((gchar *)node->value);
  ...
  if (stylesheetfilename == NULL) {
    ...
    gchar *csspath =
        g_strdup_printf("%s/atrilnightstyle.css", epub_document->documentdir);

    GFile *styles = g_file_new_for_path(csspath);
    GOutputStream *outstream = (GOutputStream *)g_file_create(
        styles, G_FILE_CREATE_PRIVATE, NULL, NULL);
    if (g_output_stream_write((GOutputStream *)outstream, style, strlen(style),
                              NULL, NULL) == -1) {
      return;
    }
...
static void epub_document_toggle_night_mode(EvDocument *document,
                                            gboolean night) {
  EpubDocument *epub_document = EPUB_DOCUMENT(document);

  g_return_if_fail(EPUB_IS_DOCUMENT(epub_document));
  if (night)
    g_list_foreach(epub_document->contentList, (GFunc)change_to_night_sheet,
                   NULL);
  else
    g_list_foreach(epub_document->contentList, (GFunc)change_to_day_sheet,
                   NULL);
}

static gchar *epub_document_set_document_title(gchar *containeruri) {
  open_xml_document(containeruri);
  gchar *doctitle;
  set_xml_root_node(NULL);
...
static void page_set_function(linknode *Link, GList *contentList) {
...
static void epub_document_set_index_pages(GList *index, GList *contentList) {
...
static void add_mathjax_script_node_to_file(gchar *filename, gchar *data) {
  ...
  xmlNodePtr script =
      xmlNewTextChild(head, NULL, (xmlChar *)"script", (xmlChar *)"");
...
static void epub_document_add_mathJax(gchar *containeruri, gchar *documentdir) {
  gchar *containerfilename = g_filename_from_uri(containeruri, NULL, NULL);
  GString *mathjaxdir = g_string_new("/usr/share/javascript/mathjax");

  gchar *mathjaxref = g_filename_to_uri(mathjaxdir->str, NULL, NULL);
  gchar *nodedata =
      g_strdup_printf("%s/MathJax.js?config=TeX-AMS-MML_SVG", mathjaxref);
  ...
  while (item != NULL) {
    ...
    xmlChar *mathml =
        xml_get_data_from_node(item, XML_ATTRIBUTE, (xmlChar *)"properties");

    if (mathml != NULL && !xmlStrcmp(mathml, (xmlChar *)"mathml")) {
      gchar *href = (gchar *)xml_get_data_from_node(item, XML_ATTRIBUTE,
                                                    (xmlChar *)"href");
...
static gboolean epub_document_load(EvDocument *document, const char *uri,
                                   GError **error) {
  EpubDocument *epub_document = EPUB_DOCUMENT(document);
  GError *err = NULL;
  ...
  epub_document->contentList = setup_document_content_list(
      contentOpfUri, &err, epub_document->documentdir);

  if (epub_document->index != NULL && epub_document->contentList != NULL)
    epub_document_set_index_pages(epub_document->index,
                                  epub_document->contentList);
...
static void epub_document_init(EpubDocument *epub_document) {
...
static void epub_document_finalize(GObject *object) {
  ...
  if (epub_document->contentList) {
    g_list_free_full(epub_document->contentList,
                     (GDestroyNotify)free_tree_nodes);
    epub_document->contentList = NULL;
  }
...
static void epub_document_class_init(EpubDocumentClass *klass) {
  ...
  ev_document_class->check_add_night_sheet =
      epub_document_check_add_night_sheet;
}
