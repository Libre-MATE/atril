...
static void comics_document_document_thumbnails_iface_init(
    EvDocumentThumbnailsInterface *iface);

static GSList *get_supported_image_extensions(void);
static void get_page_size_area_prepared_cb(GdkPixbufLoader *loader,
                                           gpointer data);
static void render_pixbuf_size_prepared_cb(GdkPixbufLoader *loader, gint width,
                                           gint height, gpointer data);
static char **extract_argv(EvDocument *document, gint page);
 ...
 **/
static gchar *comics_regex_quote(const gchar *unquoted_string) {
  const gchar *p;
  GString *dest;
...
static gboolean comics_decompress_temp_dir(const gchar *command_decompress_tmp,
                                           const gchar *command,
                                           GError **error) {
  gboolean success;
  ...
  success = g_spawn_command_line_sync(command_decompress_tmp, &std_out, NULL,
                                      &retval, &err);
  ...
  if (!success) {
    ...
    g_set_error(error, EV_DOCUMENT_ERROR, EV_DOCUMENT_ERROR_INVALID,
                _("The command “%s” did not end normally."), basename);
    g_free(std_out);
  }
  g_free(basename);
  return FALSE;
 ...
 * this information */
static gboolean comics_generate_command_lines(ComicsDocument *comics_document,
                                              GError **error) {
  ...
  if (comics_document->regex_arg) {
    ...
    comics_document->list_command =
        g_strdup_printf(command_usage_def[type].list,
                        comics_document->alternative_command, quoted_file_aux);
    g_free(quoted_file_aux);
  } else {
    quoted_file = g_shell_quote(comics_document->archive);
    comics_document->list_command = g_strdup_printf(
        command_usage_def[type].list, quoted_command, quoted_file);
  }
  comics_document->extract_command =
      g_strdup_printf(command_usage_def[type].extract, quoted_command);
  comics_document->offset = command_usage_def[type].offset;
  if (command_usage_def[type].decompress_tmp) {
    comics_document->dir = ev_mkdtemp("atril-comics-XXXXXX", error);
    if (comics_document->dir == NULL) return FALSE;

    /* unrar-free can't create directories, but ev_mkdtemp already created the
     * dir */

    comics_document->decompress_tmp =
        g_strdup_printf(command_usage_def[type].decompress_tmp, quoted_command,
                        quoted_file, comics_document->dir);
    g_free(quoted_file);
...
static gboolean comics_check_decompress_command(gchar *mime_type,
  ...
  if (g_content_type_is_a(mime_type, "application/x-cbr") ||
    ...
    comics_document->selected_command = g_find_program_in_path("unrar-free");
    if (comics_document->selected_command) {
      comics_document->command_usage = GNAUNRAR;
      return TRUE;
    }
    comics_document->selected_command = g_find_program_in_path("lsar");
    if (comics_document->selected_command) {
      comics_document->command_usage = UNARCHIVER;
      return TRUE;
    }
    comics_document->selected_command = g_find_program_in_path("bsdtar");
    if (comics_document->selected_command) {
    ...
    comics_document->selected_command = g_find_program_in_path("unzip");
    comics_document->alternative_command = g_find_program_in_path("zipnote");
    if (comics_document->selected_command &&
    ...
    comics_document->selected_command = g_find_program_in_path("7zr");
    if (comics_document->selected_command) {
      comics_document->command_usage = P7ZIP;
      return TRUE;
    }
    comics_document->selected_command = g_find_program_in_path("7za");
    if (comics_document->selected_command) {
      comics_document->command_usage = P7ZIP;
      return TRUE;
    }
    comics_document->selected_command = g_find_program_in_path("7z");
    if (comics_document->selected_command) {
      comics_document->command_usage = P7ZIP;
      return TRUE;
    }
    ...
    comics_document->selected_command = g_find_program_in_path("tar");
    if (comics_document->selected_command) {
      comics_document->command_usage = TAR;
      return TRUE;
    }
    ...
    g_set_error(error, EV_DOCUMENT_ERROR, EV_DOCUMENT_ERROR_INVALID,
                _("Not a comic book MIME type: %s"), mime_type);
    return FALSE;
  }
...
static gboolean comics_document_load(EvDocument *document, const char *uri,
                                     GError **error) {
  ...
  if (!comics_document->archive) return FALSE;
  ...
  success = g_spawn_command_line_sync(comics_document->list_command, &std_out,
                                      NULL, &retval, error);
  ...
  for (i = 0; cb_files[i] != NULL; i++) {
    if (comics_document->offset != NO_OFFSET) {
      if (g_utf8_strlen(cb_files[i], -1) > comics_document->offset) {
        cb_file =
            g_utf8_offset_to_pointer(cb_files[i], comics_document->offset);
      } else {
        continue;
      }
    } else {
  ...
  if (comics_document->page_names->len == 0) {
    g_set_error(error, EV_DOCUMENT_ERROR, EV_DOCUMENT_ERROR_INVALID,
                _("No images found in archive %s"), uri);
    return FALSE;
  }
...
static gboolean comics_document_save(EvDocument *document, const char *uri,
                                     GError **error) {
  ComicsDocument *comics_document = COMICS_DOCUMENT(document);
...
static int comics_document_get_n_pages(EvDocument *document) {
  ...
  if (comics_document->page_names == NULL) return 0;

  return comics_document->page_names->len;
}

static void comics_document_get_page_size(EvDocument *document, EvPage *page,
  ...
  if (!comics_document->decompress_tmp) {
    ...
    success = g_spawn_async_with_pipes(
        ...
        NULL, NULL, &child_pid, NULL, &outpipe, NULL, NULL);
    g_strfreev(argv);
    g_return_if_fail(success == TRUE);

    loader = gdk_pixbuf_loader_new();
    g_signal_connect(loader, "area-prepared",
                     G_CALLBACK(get_page_size_area_prepared_cb), &got_size);

    while (outpipe >= 0) {
      bytes = read(outpipe, buf, 1024);

      if (bytes > 0) gdk_pixbuf_loader_write(loader, buf, bytes, NULL);
      if (bytes <= 0 || got_size) {
        close(outpipe);
        outpipe = -1;
    ...
    filename = g_build_filename(
        comics_document->dir,
        (char *)comics_document->page_names->pdata[page->index], NULL);
...
static GdkPixbuf *comics_document_render_pixbuf(EvDocument *document,
  ...
  if (!comics_document->decompress_tmp) {
    ...
    rotated_pixbuf = gdk_pixbuf_rotate_simple(tmp_pixbuf, 360 - rc->rotation);
    g_spawn_close_pid(child_pid);
    g_object_unref(loader);
  } else {
    filename = g_build_filename(
        comics_document->dir,
        (char *)comics_document->page_names->pdata[rc->page->index], NULL);

    gdk_pixbuf_get_file_info(filename, &width, &height);

    tmp_pixbuf = gdk_pixbuf_new_from_file_at_size(
        filename, width * (rc->scale) + 0.5, height * (rc->scale) + 0.5, NULL);
    rotated_pixbuf = gdk_pixbuf_rotate_simple(tmp_pixbuf, 360 - rc->rotation);
    g_free(filename);
 ...
 */
static int comics_remove_dir(gchar *path_name) {
  GDir *content_dir;
...
static void comics_document_finalize(GObject *object) {
  ...
  if (comics_document->decompress_tmp) {
    if (comics_remove_dir(comics_document->dir) == -1)
      g_warning(_("There was an error deleting “%s”."), comics_document->dir);
...
static GSList *get_supported_image_extensions(void) {
  ...
  for (l = formats; l != NULL; l = l->next) {
    ...
    for (i = 0; ext[i] != NULL; i++) {
      extensions = g_slist_append(extensions, g_strdup(ext[i]));
    }
...
static GdkPixbuf *comics_document_thumbnails_get_thumbnail(
...
static void comics_document_thumbnails_get_dimensions(
    EvDocumentThumbnails *document, EvRenderContext *rc, gint *width,
    gint *height) {
  gdouble page_width, page_height;

  comics_document_get_page_size(EV_DOCUMENT(document), rc->page, &page_width,
                                &page_height);
...
static char **extract_argv(EvDocument *document, gint page) {
  ...
  if (page >= comics_document->page_names->len) return NULL;
  ...
  command_line = g_strdup_printf("%s %s %s", comics_document->extract_command,
