...
static void dvi_document_document_thumbnails_iface_init(
    EvDocumentThumbnailsInterface *iface);
static void dvi_document_file_exporter_iface_init(
    EvFileExporterInterface *iface);
static void dvi_document_do_color_special(DviContext *dvi, const char *prefix,
                                          const char *arg);
...
static gboolean dvi_document_load(EvDocument *document, const char *uri,
                                  GError **error) {
  gchar *filename;
  ...
  if (dvi_document->context) mdvi_destroy_context(dvi_document->context);

  dvi_document->context =
      mdvi_init_context(dvi_document->params, dvi_document->spec, filename);
  ...
  dvi_document->base_width =
      dvi_document->context->dvi_page_w * dvi_document->context->params.conv +
      2 * unit2pix(dvi_document->params->dpi, MDVI_HMARGIN) /
          dvi_document->params->hshrink;

  dvi_document->base_height =
      dvi_document->context->dvi_page_h * dvi_document->context->params.vconv +
      2 * unit2pix(dvi_document->params->vdpi, MDVI_VMARGIN) /
          dvi_document->params->vshrink;
...
static gboolean dvi_document_save(EvDocument *document, const char *uri,
                                  GError **error) {
  DviDocument *dvi_document = DVI_DOCUMENT(document);
...
static void dvi_document_get_page_size(EvDocument *document, EvPage *page,
...
static cairo_surface_t *dvi_document_render(EvDocument *document,
  ...
  mdvi_cairo_device_set_margins(&dvi_document->context->device, xmargin,
                                ymargin);
  ...
  rotated_surface = ev_document_misc_surface_rotate_and_scale(
      surface, required_width, required_height, rc->rotation);
...
static void dvi_document_finalize(GObject *object) {
  ...
  if (dvi_document->params) g_free(dvi_document->params);

  if (dvi_document->exporter_filename) g_free(dvi_document->exporter_filename);
...
static void dvi_document_class_init(DviDocumentClass *klass) {
  ...
  mdvi_init_kpathsea("atril", MDVI_MFMODE, MDVI_FALLBACK_FONT, MDVI_DPI,
                     getenv("TEXMFCNF"));

  mdvi_register_special("Color", "color", NULL, dvi_document_do_color_special,
                        1);
  mdvi_register_fonts();
...
static GdkPixbuf *dvi_document_thumbnails_get_thumbnail(
  ...
  mdvi_set_shrink(dvi_document->context,
                  (int)dvi_document->base_width *
                      dvi_document->params->hshrink / thumb_width,
                  (int)dvi_document->base_height *
                      dvi_document->params->vshrink / thumb_height);
  ...
  if (border) {
    ...
    mdvi_cairo_device_set_margins(
        &dvi_document->context->device,
...
static void dvi_document_file_exporter_end(EvFileExporter *exporter) {
  ...
  command_line = g_strdup_printf(
      "dvipdfm %s -o %s %s", /* dvipdfm -s 1,2,.., -o exporter_filename
                                dvi_filename */
      dvi_document->exporter_opts->str, dvi_document->exporter_filename,
      quoted_filename);
  g_free(quoted_filename);

  success =
      g_spawn_command_line_sync(command_line, NULL, NULL, &exit_stat, &err);
...
static EvFileExporterCapabilities dvi_document_file_exporter_get_capabilities(
...
static gboolean hsb2rgb(float h, float s, float v, guchar *red, guchar *green,
                        guchar *blue) {
  float f, p, q, t, r, g, b;
  int i;
...
static void dvi_document_init_params(DviDocument *dvi_document) {
  ...
  dvi_document->params->hshrink =
      MDVI_SHRINK_FROM_DPI(dvi_document->params->dpi);
  dvi_document->params->vshrink =
      MDVI_SHRINK_FROM_DPI(dvi_document->params->vdpi);
