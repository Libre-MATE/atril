...
static void dvi_cairo_draw_glyph(DviContext *dvi, DviFontChar *ch, int x0,
                                 int y0) {
  ...
  if (x < 0 || y < 0 || x + w > cairo_image_surface_get_width(surface) ||
      y + h > cairo_image_surface_get_height(surface))
    return;

  cairo_save(cairo_device->cr);
  if (isbox) {
    cairo_rectangle(cairo_device->cr, x - cairo_device->xmargin,
                    y - cairo_device->ymargin, w, h);
    cairo_stroke(cairo_device->cr);
  } else {
    cairo_translate(cairo_device->cr, x, y);
    cairo_set_source_surface(cairo_device->cr, (cairo_surface_t *)glyph->data,
                             0, 0);
...
static void dvi_cairo_draw_rule(DviContext *dvi, int x, int y, Uint width,
                                Uint height, int fill) {
  DviCairoDevice *cairo_device;
  Ulong color;
  ...
  cairo_set_source_rgb(cairo_device->cr, ((color >> 16) & 0xff) / 255.,
                       ((color >> 8) & 0xff) / 255.,
                       ((color >> 0) & 0xff) / 255.);

  cairo_rectangle(cairo_device->cr, x + cairo_device->xmargin,
                  y + cairo_device->ymargin, width, height);
  if (fill == 0) {
    cairo_stroke(cairo_device->cr);
  } else {
...
#ifdef HAVE_SPECTRE
static void dvi_cairo_draw_ps(DviContext *dvi, const char *filename, int x,
  ...
  spectre_render_context_set_scale(rc, (double)width / w, (double)height / h);
  ...
  image = cairo_image_surface_create_for_data(
      (unsigned char *)data, CAIRO_FORMAT_RGB24, width, height, row_length);

  cairo_save(cairo_device->cr);

  cairo_translate(cairo_device->cr, x + cairo_device->xmargin,
...
static int dvi_cairo_alloc_colors(void *device_data, Ulong *pixels, int npixels,
                                  Ulong fg, Ulong bg, double gamma,
                                  int density) {
  double frac;
  ...
  n = npixels - 1;
  for (i = 0; i < npixels; i++) {
    frac = (gamma > 0) ? pow((double)i / n, 1 / gamma)
...
static void *dvi_cairo_create_image(void *device_data, Uint width, Uint height,
                                    Uint bpp) {
...
static void dvi_cairo_put_pixel(void *image, int x, int y, Ulong color) {
  ...
  p = (guint32 *)(cairo_image_surface_get_data(surface) + y * rowstride +
                  x * 4);
...
void mdvi_cairo_device_free(DviDevice *device) {
  ...
  if (cairo_device->cr) cairo_destroy(cairo_device->cr);
...
void mdvi_cairo_device_render(DviContext *dvi) {
  ...
  surface =
      cairo_image_surface_create(CAIRO_FORMAT_ARGB32, page_width, page_height);
