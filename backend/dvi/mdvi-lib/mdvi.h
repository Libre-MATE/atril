...
typedef enum { FALSE = 0, TRUE = 1 } DviBool;
...
typedef void(*DviGlyphDraw)
    __PROTO((DviContext * context, DviFontChar *glyph, int x, int y));

typedef void(*DviRuleDraw) __PROTO((DviContext * context, int x, int y,
                                    Uint width, Uint height, int fill));

typedef int(*DviColorScale)
    __PROTO((void *device_data, Ulong *pixels, int npixels, Ulong foreground,
             Ulong background, double gamma, int density));
typedef void *(*DviCreateImage)__PROTO((void *device_data, Uint width,
...
typedef void(*DviPSDraw) __PROTO((DviContext * context, const char *filename,
...
typedef int(*DviFontEncodeFunc)
    __PROTO((DviParams *, DviFont *, DviEncoding *));
...
extern void mdvi_shrink_glyph __PROTO((DviContext *, DviFont *, DviFontChar *,
                                       DviGlyph *));
extern void mdvi_shrink_box __PROTO((DviContext *, DviFont *, DviFontChar *,
                                     DviGlyph *));
extern void mdvi_shrink_glyph_grey __PROTO((DviContext *, DviFont *,
                                            DviFontChar *, DviGlyph *));
...
extern void mdvi_init_kpathsea __PROTO((const char *, const char *,
                                        const char *, int, const char *));

extern DviContext *mdvi_init_context __PROTO((DviParams *, DviPageSpec *,
                                              const char *));
...
#define mdvi_set_shrink(d, h, v)                                    \
  mdvi_configure((d), MDVI_SET_XSHRINK, (h), MDVI_SET_YSHRINK, (v), \
                 MDVI_PARAM_LAST)

extern DviRange *mdvi_parse_range __PROTO((const char *, DviRange *, int *,
                                           char **));
...
/* Specials */
extern int mdvi_register_special __PROTO((const char *label, const char *prefix,
                                          const char *regex,
...
#define FONTCHAR(font, code)                                    \
  (((code) < font->loc || (code) > font->hic || !(font)->chars) \
       ? NULL                                                   \
       : &font->chars[(code) - (font)->loc])
#define FONT_GLYPH_COUNT(font) ((font)->hic - (font)->loc + 1)

#define glyph_present(x) ((x) && (x)->offset)

/* create a reference to a font */
extern DviFontRef *font_reference __PROTO((DviParams * params, Int32 dvi_id,
                                           const char *font_name,
                                           Int32 checksum, int xdpi, int ydpi,
                                           Int32 scale_factor));
...
#define font_free_glyph(dev, font, code) \
  font_reset_one_glyph((dev), FONTCHAR((font), (code)), MDVI_FONTSEL_GLYPH)
