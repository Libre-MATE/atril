...
BITMAP *bitmap_convert_lsb8(Uchar *bits, int w, int h, int stride) {
  ...
  for (i = 0; i < h; i++) {
    ...
    for (j = 0; j < bytes; curr++, j++) unit[j] = bit_swap[*curr];
    cur += stride - bytes;
#endif
    memzero(unit + bytes, bm->stride - bytes);
    unit += bm->stride;
  }
  if (SHOW_OP_DATA) bitmap_print(stderr, bm);
  return bm;
}
...
void bitmap_print(FILE *out, BITMAP *bm) {
  int i, j;
  BmUnit *a, mask;
  static const char labels[] = {'1', '2', '3', '4', '5',
                                '6', '7', '8', '9', '0'};
  int sub;
  ...
  for (j = 0; j < bm->width; j++) putc(labels[j % 10], out);
  putchar('\n');
...
void bitmap_paint_bits(BmUnit *ptr, int n, int count) {
  ...
  /* paint the tail */
  if (count > 0) *ptr |= SEGMENT(count, 0);
}
...
void bitmap_clear_bits(BmUnit *ptr, int n, int count) {
  ...
  for (; count >= BITMAP_BITS; count -= BITMAP_BITS) *ptr++ = 0;
...
void mdvi_shrink_box(DviContext *dvi, DviFont *font, DviFontChar *pk,
                     DviGlyph *dest) {
  int x, y, z;
  DviGlyph *glyph;
  int hs, vs;
  ...
  DEBUG((DBG_BITMAPS, "shrink_box: (%dw,%dh,%dx,%dy) -> (%dw,%dh,%dx,%dy)\n",
         glyph->w, glyph->h, glyph->x, glyph->y, dest->w, dest->h, dest->x,
         dest->y));
}

void mdvi_shrink_glyph(DviContext *dvi, DviFont *font, DviFontChar *pk,
                       DviGlyph *dest) {
  ...
  while (rows_left) {
    ...
    while (cols_left > 0) {
      ...
      sample = do_sample(old_ptr, old_stride, glyph->w - cols_left, cols, rows);
      if (sample >= min_sample) *cp |= m;
      if (m == LASTMASK) {
        m = FIRSTMASK;
        cp++;
      } else
  ...
  DEBUG((DBG_BITMAPS, "shrink_glyph: (%dw,%dh,%dx,%dy) -> (%dw,%dh,%dx,%dy)\n",
         glyph->w, glyph->h, glyph->x, glyph->y, dest->w, dest->h, dest->x,
         dest->y));
  if (DEBUGGING(BITMAP_DATA)) bitmap_print(stderr, newmap);
}

void mdvi_shrink_glyph_grey(DviContext *dvi, DviFont *font, DviFontChar *pk,
                            DviGlyph *dest) {
  int rows_left, rows;
  ...
  while (rows_left && y < h) {
    ...
    while (cols_left && x < w) {
      if (cols > cols_left) cols = cols_left;
      sampleval =
          do_sample(old_ptr, map->stride, glyph->w - cols_left, cols, rows);
    ...
    for (; x < w; x++) dev->put_pixel(image, x, y, pixels[0]);
  ...
  for (; y < h; y++) {
    for (x = 0; x < w; x++) dev->put_pixel(image, x, y, pixels[0]);
  }
