...
static char *const messages[] = {N_("Ooops!"),
                                 N_("Aieeeee!!"),
                                 N_("Ouch!"),
...
static void vputlog(int level, const char *head, const char *format,
                    va_list ap) {
  if (logfile != NULL && _mdvi_log_level >= level) {
    if (head != NULL) fprintf(logfile, "%s: ", head);
...
int mdvi_set_logfile(const char *filename) {
  ...
  if (filename && (f = fopen(filename, "w")) == NULL) return -1;
  if (logfile != NULL && !isatty(fileno(logfile))) {
    fclose(logfile);
    logfile = NULL;
  }
  if (filename) logfile = f;
  return 0;
}
...
void *mdvi_realloc(void *data, size_t newsize) {
  ...
  if (newsize == 0) mdvi_crash(_("attempted to reallocate with zero size\n"));
...
void *mdvi_calloc(size_t nmemb, size_t size) {
  ...
  if (nmemb == 0) mdvi_crash(_("attempted to callocate 0 members\n"));
  if (size == 0)
...
void mdvi_free(void *ptr) {
  if (ptr == NULL) mdvi_crash(_("attempted to free NULL pointer\n"));
  free(ptr);
}
...
char *mdvi_strrstr(const char *haystack, const char *needle) {
  ...
  if (haystack_len < needle_len) return (char *)haystack;
...
Ulong get_mtime(int fd) {
  ...
  if (fstat(fd, &st) == 0) return (Ulong)st.st_mtime;
  return 0;
}
...
int dstring_insert(Dstring *dstr, int pos, const char *string, int len) {
  ASSERT(pos >= 0);
  if (pos == dstr->length) return dstring_append(dstr, string, len);
  if (len < 0) len = strlen(string);
  if (len) {
    if (dstr->length + len >= dstr->size) {
      dstr->size = pow2(dstr->length + len + 1);
...
void dstring_reset(Dstring *dstr) {
  if (dstr->data) mdvi_free(dstr->data);
  dstring_init(dstr);
}
