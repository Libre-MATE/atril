...
void mdvi_push_color(DviContext *dvi, Ulong fg, Ulong bg) {
  if (dvi->color_top == dvi->color_size) {
    dvi->color_size += 32;
    dvi->color_stack =
        mdvi_realloc(dvi->color_stack, dvi->color_size * sizeof(DviColorPair));
  }
...
Ulong *get_color_table(DviDevice *dev, int nlevels, Ulong fg, Ulong bg,
  ...
  status = dev->alloc_colors(dev->device_data, pixels, nlevels, fg, bg, gamma,
                             density);
  if (status < 0) {
    mdvi_free(pixels);
    return NULL;
