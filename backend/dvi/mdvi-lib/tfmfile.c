...
static inline void swap_array(Uint32 *ptr, int n) {
  ...
  while (n-- > 0) {
    i = *ptr;
    *ptr++ = ((i & 0xff000000) >> 24) | ((i & 0x00ff0000) >> 8) |
...
int afm_load_file(const char *filename, TFMInfo *info) {
  ...
  for (cm = fi->cmi; cm < fi->cmi + fi->numOfChars; cm++) {
    ...
    code = cm->code;
    if (code < 0 || code > 255) continue; /* ignore it */
    ch = &info->chars[code];
    ch->present = 1;
    if (code < info->loc) info->loc = code;
    if (code > info->hic) info->hic = code;
  ...
  if (info->loc > 0 || info->hic < 256) {
    memmove(&info->chars[0], &info->chars[info->loc],
...
int tfm_load_file(const char *filename, TFMInfo *info) {
  ...
  if (fstat(fileno(in), &st) < 0) return -1;
  /* according to the spec, TFM files are smaller than 16K */
  if (st.st_size == 0 || st.st_size >= 16384) goto bad_tfm;
  ...
  if (fread(tfm, st.st_size, 1, in) != 1) goto error;
  /* we don't need this anymore */
  fclose(in);
  in = NULL;
  ...
  if (widths[0] || heights[0] || depths[0] || checksum != lf || bc - 1 > ec ||
      ec > 255 || ne > 256)
    goto bad_tfm;
...
static int ofm1_load_file(FILE *in, TFMInfo *info) {
  ...
  for (tch = info->chars, i = 0; i < ncw; i++) {
    ...
    while (nr-- >= 0 && tch < end) memcpy(tch++, &ch, sizeof(TFMChar));
    if (tch == end) goto bad_tfm;
  }
  ...
  if (widths[0] || heights[0] || depths[0]) goto bad_tfm;
...
static int ofm_load_file(const char *filename, TFMInfo *info) {
  ...
  if (checksum != lf || bc > ec + 1 || ec > 65535) goto bad_tfm;
  ...
  switch (*type) {
    ...
    case DviFontOFM: {
      ...
      if (file != NULL) {
        ...
        if (ext && STREQ(ext, "tfm")) *type = DviFontTFM;
      }
      break;
    }
#ifdef WITH_AFM_FILES
 ...
 *    return an error.
 */
TFMInfo *get_font_metrics(const char *short_name, int type,
                          const char *filename) {
  TFMPool *tfm = NULL;
  int status;
  char *file;
  ...
  if (tfmpool.count) {
    tfm = (TFMPool *)mdvi_hash_lookup(&tfmhash, MDVI_KEY(short_name));
    if (tfm != NULL) {
  ...
  if (file != filename) mdvi_free(file);
  if (status < 0) {
    mdvi_free(tfm);
    return NULL;
  }
  ...
  /* add it to the pool */
  if (tfmpool.count == 0) mdvi_hash_create(&tfmhash, TFM_HASH_SIZE);
  mdvi_hash_add(&tfmhash, MDVI_KEY(tfm->short_name), tfm, MDVI_HASH_UNCHECKED);
...
void free_font_metrics(TFMInfo *info) {
  ...
  DEBUG(
      (DBG_FONTS, "(mt) removing unused TFM data for `%s'\n", tfm->short_name));
