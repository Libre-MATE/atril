...
DviRange *mdvi_parse_range(const char *format, DviRange *limit, int *nitems,
                           char **endptr) {
  int quoted;
  int size;
  int curr;
  ...
  if (done) cp--;
  if (quoted && *cp == '}') cp++;
  if (endptr) *endptr = (char *)format + (cp - copy);
  if (curr && curr < size) range = mdvi_realloc(range, curr * sizeof(DviRange));
  *nitems = curr;
  mdvi_free(copy);
  return range;
}
...
DviPageSpec *mdvi_parse_page_spec(const char *format) {
  ...
  if (*ptr != '.') {
    if (*ptr) mdvi_warning(_("garbage after DVI page specification ignored\n"));
    return spec;
  }
...
int mdvi_page_selected(DviPageSpec *spec, PageNum page, int dvipage) {
  ...
  if (spec[0]) {
    not_found = mdvi_in_range(spec[0]->ranges, spec[0]->nranges, dvipage);
    if (not_found < 0) return 0;
  }
  for (i = 1; i <= 10; i++) {
    if (spec[i] == NULL) continue;
    not_found = mdvi_in_range(spec[i]->ranges, spec[i]->nranges, (int)page[i]);
    if (not_found < 0) return 0;
  }
  return 1;
}
...
int mdvi_in_range(DviRange *range, int nitems, int value) {
  ...
  for (r = range; r < range + nitems; r++) {
    ...
    switch (r->type) {
      case MDVI_RANGE_BOUNDED:
        if (value == r->from) return (r - range);
        if (r->step < 0)
          cond = (value <= r->from) && (value >= r->to);
        else
          cond = (value <= r->to) && (value >= r->from);
        if (cond && ((value - r->from) % r->step) == 0) return (r - range);
        break;
      case MDVI_RANGE_LOWER:
        if (value == r->from) return (r - range);
        if (r->step < 0)
        ...
        break;
      case MDVI_RANGE_UPPER:
        if (value == r->to) return (r - range);
        if (r->step < 0)
          cond = (value > r->to);
        else
          cond = (value < r->to);
        if (cond && ((value - r->to) % r->step) == 0) return (r - range);
        break;
      case MDVI_RANGE_UNBOUNDED:
        if ((value % r->step) == 0) return (r - range);
        break;
    }
  }
  return -1;
...
int mdvi_range_length(DviRange *range, int nitems) {
  ...
  for (r = range; r < range + nitems; r++) {
    ...
    if (r->type != MDVI_RANGE_BOUNDED) return -2;
