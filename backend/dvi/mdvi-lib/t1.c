...
static void t1_font_shrink_glyph __PROTO((DviContext *, DviFont *,
...
DviFontInfo t1_font_info = {"Type1",
...
char *t1_lookup_font(const char *name, Ushort *hdpi, Ushort *vdpi) {
  ...
  if (ext && !STREQ(ext, "pfa") && !STREQ(ext, "pfb")) {
    DEBUG((DBG_TYPE1, "(t1) %s: associated name `%s' is not Type1\n", name,
           info.fontfile));
  ...
  /* look it up */
  DEBUG(
      (DBG_TYPE1, "(t1) looking for `%s' on behalf of `%s'\n", newname, name));
  filename = kpse_find_file(newname, kpse_type1_format, 1);

  /* we don't need this anymore */
  if (newname != name) mdvi_free(newname);
  if (filename == NULL) {
...
static void t1_reset_resolution(int xdpi, int ydpi) {
  ...
  for (i = 0; i < nfonts; i++) T1_DeleteAllSizes(i);
  ...
  if (T1_SetDeviceResolutions((float)xdpi, (float)ydpi) < 0)
    ...
    DEBUG((DBG_TYPE1, "(t1) reset successful, new resolution is (%d, %d)\n",
           xdpi, ydpi));
  t1lib_xdpi = xdpi;
  t1lib_ydpi = ydpi;
}
...
static void t1_transform_font(T1Info *info) {
  ...
  if (info->mapinfo.slant) {
    ...
    T1_SlantFont(info->t1id, MDVI_FMAP_SLANT(&info->mapinfo));
  }
  if (info->mapinfo.extend) {
    DEBUG((DBG_TYPE1, "(t1) %s: extending by %.3f\n", info->fontname,
           MDVI_FMAP_EXTEND(&info->mapinfo)));
    T1_ExtendFont(info->t1id, MDVI_FMAP_EXTEND(&info->mapinfo));
  }
}
...
static int t1_really_load_font(DviParams *params, DviFont *font, T1Info *info) {
  ...
  if (info->tfminfo == NULL) {
    DEBUG(
        (DBG_FONTS, "(t1) %s: no metric data, font ignored\n", info->fontname));
    goto t1_error;
  }
  /* fix this */
  ...
  if (old && old->t1id == -1) {
    ...
    mdvi_hash_add(&t1hash, (unsigned char *)info->fontname, info,
                  MDVI_HASH_UNCHECKED);
  }
...
static int init_t1lib(DviParams *params) {
  ...
  if (DEBUGGING(TYPE1)) t1flags |= LOGFILE;
  if (T1_InitLib(t1flags) == NULL) return (t1lib_initialized = -1);
  if (DEBUGGING(TYPE1)) {
...
static int t1_load_font(DviParams *params, DviFont *font) {
  ...
  if (t1hash.nbucks == 0) mdvi_hash_create(&t1hash, T1_HASH_SIZE);
  mdvi_hash_add(&t1hash, (unsigned char *)info->fontname, info,
                MDVI_HASH_UNIQUE);
...
#define GLYPH_HEIGHT(g) ((g)->metrics.ascent - (g)->metrics.descent)
...
static inline BITMAP *t1_glyph_bitmap(GLYPH *glyph) {
  ...
  if (!w || !h) return MDVI_GLYPH_EMPTY;

  pad = T1_GetBitmapPad();
  return bitmap_convert_lsb8((unsigned char *)glyph->bits, w, h,
                             ROUND(w, pad) * (pad >> 3));
}

static void t1_font_shrink_glyph(DviContext *dvi, DviFont *font,
                                 DviFontChar *ch, DviGlyph *dest) {
  double size;
  GLYPH *glyph;
  T1Info *info;
  T1_TMATRIX matrix;
  ...
  if (DEBUGGING(BITMAP_DATA)) {
    DEBUG((DBG_BITMAP_DATA,
           "(t1) %s: t1_shrink_glyph(%d): (%dw,%dh,%dx,%dy) -> "
           "(%dw,%dh,%dx,%dy)\n",
           ch->glyph.w, ch->glyph.h, ch->glyph.x, ch->glyph.y, dest->w, dest->h,
           dest->x, dest->y));
...
static void t1_font_remove(T1Info *info) {
  ...
  if (old == info) {
    ...
    for (old = (T1Info *)t1fonts.head; old; old = old->next)
      if (STREQ(old->fontname, info->fontname)) break;
    if (old != NULL)
