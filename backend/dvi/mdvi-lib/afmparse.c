...
static char *keyStrings[] = {"Ascender",
                             "B",
                             "C",
                             "CC",
                             "CapHeight",
...
static char *token(FILE *stream) {
  ...
  while ((ch = fgetc(stream)) == ' ' || ch == lineterm || ch == ',' ||
         ch == '\t' || ch == ';')
    ;

  idx = 0;
  while (ch != EOF && ch != ' ' && ch != lineterm && ch != '\t' && ch != ':' &&
         ch != ';' && idx < (MAX_NAME - 1)) {
    ident[idx++] = ch;
    ch = fgetc(stream);
  } /* while */
...
static BOOL parseGlobals(FILE *fp, GlobalFontInfo *gfi) {
  ...
  while (cont) {
    ...
    if (!save) /* get tokens until the end of the Global Font info section */
      ...
      switch (recognize(keyword)) {
        ...
        case ENCODINGSCHEME:
          keyword = token(fp);
          gfi->encodingScheme = (char *)malloc(strlen(keyword) + 1);
...
static int parseCharMetrics(FILE *fp, FontInfo *fi) {
  ...
  if ((error == ok) && (count != fi->numOfChars)) error = parseError;
...
static int parseTrackKernData(FILE *fp, FontInfo *fi) {
  ...
  while (cont) {
    ...
    if (!save) /* get tokens until the end of the Track Kerning Data */
  ...
  if (error == ok && tcount != fi->numOfTracks) error = parseError;
...
static int parsePairKernData(FILE *fp, FontInfo *fi) {
  ...
  while (cont) {
    ...
    if (!save) /* get tokens until the end of the Pair Kerning Data */
               /* section without saving any of the data */
      switch (recognize(keyword)) {
        case ENDKERNPAIRS:
        case ENDKERNDATA:
      ...
      switch (recognize(keyword)) {
        ...
        case KERNPAIR:
          if (pcount < fi->numOfPairs) {
            keyword = token(fp);
            fi->pkd[pos].name1 = (char *)malloc(strlen(keyword) + 1);
            strcpy(fi->pkd[pos].name1, keyword);
            keyword = token(fp);
            fi->pkd[pos].name2 = (char *)malloc(strlen(keyword) + 1);
  ...
  if (error == ok && pcount != fi->numOfPairs) error = parseError;
...
static int parseCompCharData(FILE *fp, FontInfo *fi) {
  ...
  while (cont) {
    ...
    if (!save) /* get tokens until the end of the Composite Character info */
               /* section without saving any of the data */
      switch (recognize(keyword)) {
        case ENDCOMPOSITES:
          cont = FALSE;
      ...
      switch (recognize(keyword)) {
        ...
        case COMPCHAR:
          if (ccount < fi->numOfComps) {
            keyword = token(fp);
            if (pcount != fi->ccd[pos].numOfPieces) error = parseError;
            pcount = 0;
            if (firstTime)
              firstTime = FALSE;
            else
              pos++;
            fi->ccd[pos].ccName = (char *)malloc(strlen(keyword) + 1);
            strcpy(fi->ccd[pos].ccName, keyword);
            keyword = token(fp);
            fi->ccd[pos].numOfPieces = atoi(keyword);
            fi->ccd[pos].pieces =
                (Pcc *)calloc(fi->ccd[pos].numOfPieces, sizeof(Pcc));
            j = 0;
            ccount++;
          } else {
            error = parseError;
  ...
  if (error == ok && ccount != fi->numOfComps) error = parseError;
...
extern int afm_parse_file(FILE *fp, FontInfo **fi, FLAGS flags) {
  ...
  if ((code != normalEOF) && (code != earlyEOF)) {
    ...
    if (flags & (P_M ^ P_W)) {
      (*fi)->cmi =
          (CharMetricInfo *)calloc((*fi)->numOfChars, sizeof(CharMetricInfo));
      if ((*fi)->cmi == NULL) {
        error = storageProblem;
        return (error);
      }
    ...
    switch (recognize(keyword)) {
      ...
      case STARTTRACKKERN:
        ...
        if (flags & P_T) {
          ...
          (*fi)->tkd = (TrackKernData *)calloc((*fi)->numOfTracks,
      ...
      case STARTKERNPAIRS:
        ...
        if (flags & P_P) {
          ...
          (*fi)->pkd =
              (PairKernData *)calloc((*fi)->numOfPairs, sizeof(PairKernData));
          if ((*fi)->pkd == NULL) {
            error = storageProblem;
            return (error);
          }
...
void afm_free_fontinfo(FontInfo *fi) {
  if (fi != NULL) {
    if (fi->gfi != NULL) {
      ...
      fi->gfi->afmVersion = NULL;
      free(fi->gfi->fontName);
      fi->gfi->fontName = NULL;
      free(fi->gfi->fullName);
      fi->gfi->fullName = NULL;
      free(fi->gfi->familyName);
      fi->gfi->familyName = NULL;
      free(fi->gfi->weight);
      fi->gfi->weight = NULL;
      free(fi->gfi->version);
      fi->gfi->version = NULL;
