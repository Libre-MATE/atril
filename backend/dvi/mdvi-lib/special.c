...
static struct {
  ...
  char *regex;
  DviSpecialHandler handler;
} builtins[] = {{"Layers", "layer", NULL, sp_layer},
...
static void register_builtin_specials(void) {
  ...
  for (i = 0; i < NSPECIALS; i++)
    mdvi_register_special(builtins[i].label, builtins[i].prefix,
                          builtins[i].regex, builtins[i].handler,
                          1 /* replace if exists */);
}
...
static DviSpecial *find_special_prefix(const char *prefix) {
  ...
  for (sp = (DviSpecial *)specials.head; sp; sp = sp->next) {
    if (STRCEQ(sp->prefix, prefix)) break;
  }
  return sp;
}

int mdvi_register_special(const char *label, const char *prefix,
                          const char *regex, DviSpecialHandler handler,
                          int replace) {
  DviSpecial *sp;
  int newsp = 0;

  if (!registered_builtins) register_builtin_specials();
  ...
  if (newsp) listh_prepend(&specials, LIST(sp));
  DEBUG((DBG_SPECIAL, "New \\special handler `%s' with prefix `%s'\n", label,
         prefix));
  return 0;
}
...
int mdvi_do_special(DviContext *dvi, char *string) {
  ...
  for (sp = (DviSpecial *)specials.head; sp; sp = sp->next) {
#ifdef WITH_REGEX_SPECIALS
    if (sp->has_reg && !regexec(&sp->reg, ptr, 0, 0, 0)) break;
#endif
    /* check the prefix */
  ...
  if (ptr == string) {
    prefix = NULL;
    DEBUG((DBG_SPECIAL, "REGEX match with `%s' (arg `%s')\n", sp->label, ptr));
  } else {
    if (*ptr) *ptr++ = 0;
    prefix = string;
    DEBUG((DBG_SPECIAL, "PREFIX match with `%s' (prefix `%s', arg `%s')\n",
           sp->label, prefix, ptr));
  }
