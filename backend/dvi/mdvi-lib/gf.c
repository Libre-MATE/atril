...
#define COLOR(x) ((x) ? "BLACK" : "WHITE")
  ...
  while ((op = fuget1(p)) != GF_EOC) {
    ...
    if (op == GF_PAINT0) {
      ...
      if (y >= ch->height || x + par >= ch->width) goto toobig;
      ...
      if (paint_switch == BLACK)
        bitmap_paint_bits(line + (x / BITMAP_BITS), x % BITMAP_BITS, par);
      paint_switch = !paint_switch;
      x += par;
      ...
      switch (op) {
        ...
        case GF_YYY:
          n = fuget4(p);
          DEBUG(
              (DBG_SPECIAL, "(gf) Character %d: MF special %u\n", ch->code, n));
          break;
        case GF_NOOP:
    ...
    if (x > ch->width || y > ch->height) goto toobig;
    DEBUG((DBG_BITMAPS, "(gf) curr_loc @ (%d,%d)\n", x, y));
  }

  if (op != GF_EOC) goto error;
...
static int gf_load_font(DviParams *unused, DviFont *font) {
  ...
  loc = fuget1(p);
  hic = fuget1(p);
  if (loc != GF_PRE || hic != GF_ID) goto badgf;
  loc = fuget1(p);
#ifndef NODEBUG
  for (i = 0; i < loc; i++) s[i] = fuget1(p);
  s[i] = 0;
  ...
  if (fseek(p, (long)-1, SEEK_END) == -1) return -1;
  ...
  for (loc = 0; loc < 256; loc++) font->chars[loc].offset = 0;
...
static int gf_font_get_glyph(DviParams *params, DviFont *font, int code) {
  ...
  if (code < font->loc || code > font->hic || !font->chars) return -1;
  ...
  if (!ch->loaded) {
    ...
    if (gf_read_bitmap(font->in, ch) < 0) return -1;
    ch->loaded = 1;
  }
  return 0;
}
