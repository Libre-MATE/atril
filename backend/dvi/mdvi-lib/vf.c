...
DviFontInfo vf_font_info = {
    "VF",           1,    /* virtual fonts scale just fine */
    vf_load_font,   NULL, /* get_glyph */
    NULL,                 /* shrink0 */
    NULL,                 /* shrink1 */
    vf_free_macros, NULL, /* reset */
    NULL,                 /* lookup */
    kpse_vf_format, NULL};

DviFontInfo ovf_font_info = {"OVF",
...
static int vf_load_font(DviParams *params, DviFont *font) {
  ...
  if (fuget1(p) != 247 || fuget1(p) != 202) goto badvf;
  mlen = fuget1(p);
  ...
  while (op >= DVI_FNT_DEF1 && op <= DVI_FNT_DEF4) {
    ...
    DEBUG((DBG_FONTS, "(vf) %s: defined font `%s' at %.1fpt (%dx%d dpi)\n",
           font->fontname, name, (double)scale / (params->tfm_conv * 0x100000),
           hdpi, vdpi));
  ...
  if (op >= DVI_FNT_DEF1 && op <= DVI_FNT_DEF4) goto error;
  ...
  while (op <= 242) {
    ...
    if (loc < 0 || cc < loc) loc = cc;
    if (hic < 0 || cc > hic) hic = cc;
    if (cc >= nchars) {
      font->chars = xresize(font->chars, DviFontChar, cc + 16);
      for (i = nchars; i < cc + 16; i++) font->chars[i].offset = 0;
      nchars = cc + 16;
    }
    ...
    if (pl && fread(macros + mlen, 1, pl, p) != pl) break;
  ...
  DEBUG((DBG_FONTS | DBG_GLYPHS, "(vf) %s: macros use %d bytes\n",
         font->fontname, msize));
...
error:
  ...
  if (macros) mdvi_free(macros);
  return -1;
}

static void vf_free_macros(DviFont *font) { mdvi_free(font->private); }
