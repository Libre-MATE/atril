...
static char *pk_lookup(const char *name, Ushort *hdpi, Ushort *vdpi) {
  ...
  filename = kpse_find_glyph(name, Max(*hdpi, *vdpi), kpse_pk_format, &type);
...
static int pk_packed_num(FILE *p, pkread *pkr, int *repeat) {
  ...
  if (i == 0) {
    ...
    while (i-- > 0) j = (j << 4) + pk_get_nyb(p, pkr);
    return (j - 15 + ((13 - dyn_f) << 4) + dyn_f);
  } else if (i <= dyn_f)
    return i;
  else if (i < 14)
    return ((i - dyn_f - 1) << 4) + pk_get_nyb(p, pkr) + dyn_f + 1;
  else {
    *repeat = 1;
    if (i == 14) *repeat = pk_packed_num(p, pkr, repeat);
...
static BITMAP *get_bitmap(FILE *p, int w, int h, int flags) {
  ...
  bitpos = -1;
  if ((bm = bitmap_alloc(w, h)) == NULL) return NULL;
  DEBUG(
      (DBG_BITMAPS, "get_bitmap(%d,%d,%d): reading raw bitmap\n", w, h, flags));
  ptr = bm->data;
  currch = 0;
  for (i = 0; i < h; i++) {
    BmUnit mask;
...
static BITMAP *get_packed(FILE *p, int w, int h, int flags) {
  ...
  while (row < h) {
    ...
    if (count >= inrow) {
      ...
      if (paint) bitmap_set_row(bm, row, w - inrow, inrow, paint);
    ...
    if (count > 0) bitmap_set_row(bm, row, w - inrow, count, paint);
    inrow -= count;
    paint = !paint;
  }
  if (row != h || inrow != w) {
...
static int pk_load_font(DviParams *unused, DviFont *font) {
  ...
  for (i = 0; i < 256; i++) font->chars[i].offset = 0;

  /* check the preamble */
  loc = fuget1(p);
  hic = fuget1(p);
  if (loc != PK_PRE || hic != PK_ID) goto badpk;
  i = fuget1(p);
#ifndef NODEBUG
  for (loc = 0; loc < i; loc++) s[loc] = fuget1(p);
  s[loc] = 0;
  ...
  while ((flag_byte = fuget1(p)) != PK_POST) {
    ...
    if (flag_byte >= PK_CMD_START) {
      switch (flag_byte) {
        ...
        case PK_X4: {
          ...
          if (i < 256)
            ...
            t = mdvi_malloc(i + 1);
          for (n = 0; n < i; n++) t[n] = fuget1(p);
          t[n] = 0;
          DEBUG((DBG_SPECIAL, "(pk) %s: Special \"%s\"\n", font->fontname, t));
          if (t != &s[0]) mdvi_free(t);
#else
          i = fugetn(p, flag_byte - PK_X1 + 1);
          while (i-- > 0) fuget1(p);
#endif
      ...
      if (cc > hic) hic = cc;
      if (cc > maxch) {
        font->chars = xresize(font->chars, DviFontChar, cc + 16);
        for (i = maxch; i < cc + 16; i++) font->chars[i].offset = 0;
        maxch = cc + 16;
      }
  ...
  if (loc > 0 || hic < maxch - 1) {
    ...
    font->chars = xresize(font->chars, DviFontChar, hic - loc + 1);
  }
  font->loc = loc;
  font->hic = hic;
  return 0;
...
static int pk_font_get_glyph(DviParams *params, DviFont *font, int code) {
  ...
  if ((ch = FONTCHAR(font, code)) == NULL) return -1;

  if (ch->offset == 0) return -1;
  DEBUG((DBG_GLYPHS,
         "(pk) loading glyph for character %d (%dx%d) in font `%s'\n", code,
         ch->width, ch->height, font->fontname));
  ...
  if (fseek(font->in, ch->offset, SEEK_SET) == -1) return -1;
  ch->glyph.data = get_char(font->in, ch->width, ch->height, ch->flags);
  if (ch->glyph.data) {
