...
char *read_string(FILE *in, int s, char *buffer, size_t len) {
  ...
  n = fugetn(in, s ? s : 1);
  if ((str = buffer) == NULL || n + 1 > len) str = mdvi_malloc(n + 1);
...
size_t read_bcpl(FILE *in, char *buffer, size_t maxlen, size_t wanted) {
  ...
  if (fread(buffer, i, 1, in) != 1) return -1;
  buffer[i] = '\0';
  while (wanted-- > i) (void)fgetc(in);
  return i;
}
...
void buff_free(Buffer *buf) {
  if (buf->data) mdvi_free(buf->data);
  buff_init(buf);
}
