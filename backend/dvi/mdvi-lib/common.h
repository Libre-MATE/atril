...
#define FULLWORD(a, b, c, d)                                              \
  ((((Int8)(a) << 24) & 0xff000000) | (((Uint8)(b) << 16) & 0x00ff0000) | \
   (((Uint8)(c) << 8) & 0x0000ff00) | ((Uint8)(d)&0xff))
...
#define ASSERT(x)                                                         \
  do {                                                                    \
    if (!(x))                                                             \
      mdvi_crash("%s:%d: Assertion %s failed\n", __FILE__, __LINE__, #x); \
  } while (0)
...
#define TFMSCALE(z, t, a, b) \
  ((B3(t) == 255) ? __tfm_mul((z), (t)) / (b) - (a) : __tfm_mul((z), (t)) / (b))
