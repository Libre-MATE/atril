...
static void tt_shrink_glyph __PROTO((DviContext *, DviFont *, DviFontChar *,
                                     DviGlyph *));
static void tt_font_remove __PROTO((FTInfo *));

DviFontInfo tt_font_info = {"TT",
                            0,
                            tt_load_font,
                            tt_font_get_glyph,
                            tt_shrink_glyph,
...
static void tt_encode_font(DviFont *font, FTInfo *info) {
  ...
  if (TT_Get_Face_Properties(info->face, &prop)) return;

  for (i = 0; i < prop.num_Glyphs; i++) {
    char *string;
    int ndx;

    if (TT_Get_PS_Name(info->face, i, &string)) continue;
    ndx = mdvi_encode_glyph(info->encoding, string);
    if (ndx < font->loc || ndx > font->hic) continue;
...
static int tt_really_load_font(DviParams *params, DviFont *font, FTInfo *info) {
  ...
  if (info->loaded) {
    ...
    TT_Set_Instance_Resolutions(info->instance, params->dpi, params->vdpi);
  ...
  for (i = 0; map_found < 0 && i < props.num_CharMaps; i++) {
    ...
    switch (pid) {
      ...
      case TT_PLATFORM_ISO:
        if (eid == TT_ISO_ID_7BIT_ASCII || eid == TT_ISO_ID_8859_1)
          map_found = 1;
        break;
      case TT_PLATFORM_MICROSOFT:
        if (eid == TT_MS_ID_UNICODE_CS) map_found = 1;
        break;
    }
  }
  if (map_found < 0) {
  ...
  DEBUG((DBG_TT, "(tt) %s: Set_Char_Size(%.2f, %d, %d)\n", font->fontname,
         point_size, font->hdpi, font->vdpi));
  status =
      TT_Set_Instance_Resolutions(info->instance, params->dpi, params->vdpi);
  if (status) {
  ...
  status = TT_Set_Instance_CharSize(info->instance, FROUND(point_size * 64));
  if (status) {
  ...
  info->tfminfo =
      get_font_metrics(info->fontname, info->fmftype, info->fmfname);
...
static int tt_load_font(DviParams *params, DviFont *font) {
  ...
  if (!initialized && init_freetype() < 0) return -1;
...
static int tt_get_bitmap(DviParams *params, DviFont *font, int code,
                         double xscale, double yscale, DviGlyph *glyph) {
  TT_Outline outline;
  ...
  error = TT_Load_Glyph(info->instance, info->glyph, code, TTLOAD_DEFAULT);
  ...
  return 0;
tt_error:
  if (have_outline) TT_Done_Outline(&outline);
  return -1;
}
...
static int tt_font_get_glyph(DviParams *params, DviFont *font, int code) {
  ...
  if (!ch || !glyph_present(ch)) return -1;
  ch->loaded = 1;
  if (!ch->width || !ch->height) goto blank;
  if (ch->code == 0) {
    ch->glyph.data = NULL;
    goto missing;
  }
  ...
  error = tt_get_bitmap(params, font, ch->code, (double)font->hdpi / dpi,
                        (double)font->vdpi / dpi, &ch->glyph);
  if (error) goto missing;
  ch->x = ch->glyph.x;
  ch->y = ch->glyph.y;
...
static void tt_shrink_glyph(DviContext *dvi, DviFont *font, DviFontChar *ch,
                            DviGlyph *dest) {
  tt_get_bitmap(&dvi->params, font, ch->code,
