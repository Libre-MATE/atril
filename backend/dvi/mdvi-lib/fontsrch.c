 ...
 * was supposed to exist may have to be discarded. Until now, MDVI had no
 * ability to "resume" a search, so in this case it would have produced an
 * error, regardless of whether the offending font existed in other formats.
 *   Also, given the large number of font types supported by MDVI, some
 * mechanism was necessary to bring some order into the chaos.
 *
...
static void init_font_classes(void) {
  ...
  for (i = 0; i < MAX_CLASS; i++) listh_init(&font_classes[i]);
  initialized = 1;
}

int mdvi_get_font_classes(void) { return (MAX_CLASS - 2); }
...
char **mdvi_list_font_class(int klass) {
  ...
  if (klass == -1) klass = MAX_CLASS - 1;
  if (klass < 0 || klass >= MAX_CLASS) return NULL;
...
int mdvi_register_font_type(DviFontInfo *info, int klass) {
  ...
  if (klass == -1) klass = MAX_CLASS - 1;
  if (klass < 0 || klass >= MAX_CLASS) return -1;
  if (!initialized) init_font_classes();
...
int mdvi_unregister_font_type(const char *name, int klass) {
  ...
  if (klass >= 0 && klass < MAX_CLASS) {
    ...
    LIST_FOREACH(fc, DviFontClass, &font_classes[k]) {
      if (STREQ(fc->info.name, name)) break;
    }
  } else if (klass < 0) {
...
static char *lookup_font(DviFontClass *ptr, const char *name, Ushort *h,
                         Ushort *v) {
  char *filename;
  ...
  if (ptr->info.lookup)
    ...
    filename = kpse_find_glyph(name, Max(*h, *v), ptr->info.kpse_type, &type);
...
DviFont *mdvi_add_font(const char *name, Int32 sum, int hdpi, int vdpi,
                       Int32 scale) {
  DviFont *font;
