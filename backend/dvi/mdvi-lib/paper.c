...
static const DviPaperSpec papers[] = {{"ISO", 0, 0},
                                      {"4A0", "1682mm", "2378mm"},
                                      {"2A0", "1189mm", "1682mm"},
                                      {"A0", "841mm", "1189mm"},
                                      {"A1", "594mm", "841mm"},
...
DviPaperSpec *mdvi_get_paper_specs(DviPaperClass pclass) {
  ...
  first = -1;
  count = 0;
  if (pclass == MDVI_PAPER_CLASS_ANY || pclass == MDVI_PAPER_CLASS_CUSTOM) {
    first = 0;
...
void mdvi_free_paper_specs(DviPaperSpec *spec) { mdvi_free(spec); }
