...
static int load_font_file(DviParams *params, DviFont *font) {
  ...
  if (SEARCH_DONE(font->search)) return -1;
  if (font->in == NULL && font_reopen(font) < 0) return -1;
...
int font_free_unused(DviDevice *dev) {
  ...
  for (font = (DviFont *)fontlist.head; font; font = next) {
    ...
    next = font->next;
    if (font->links) continue;
    count++;
    DEBUG((DBG_FONTS, "removing unused %s font `%s'\n", TYPENAME(font),
           font->fontname));
    listh_remove(&fontlist, LIST(font));
    if (font->in) fclose(font->in);
    ...
    if (font->finfo->freedata) font->finfo->freedata(font);
    /* destroy characters */
    if (font->chars) mdvi_free(font->chars);
...
/* used from context: params and device */
DviFontRef *font_reference(DviParams *params, /* rendering parameters */
                           ...
                           Int32 scale) /* scaling factor (from DVI or VF) */
{
  ...
  for (font = (DviFont *)fontlist.head; font; font = font->next) {
    if (strcmp(name, font->fontname) == 0 &&
        ...
        font->hdpi == hdpi && font->vdpi == vdpi && font->scale == scale)
      break;
  }
  ...
  font->links++;
  for (subfont_ref = font->subfonts; subfont_ref;
       subfont_ref = subfont_ref->next) {
    /* just adjust the reference counts */
...
void font_transform_glyph(DviOrientation orient, DviGlyph *g) {
  ...
  map = (BITMAP *)g->data;
  if (MDVI_GLYPH_ISEMPTY(map)) map = NULL;
...
static int load_one_glyph(DviContext *dvi, DviFont *font, int code) {
  ...
  if (DEBUGGING(BITMAP_DATA)) {
    DEBUG((DBG_BITMAP_DATA, "%s: new %s bitmap for character %d:\n",
  ...
  if (!font->finfo->scalable && font->hdpi != font->vdpi) {
    ...
    if (ch->width && ch->height && (hs > 1 || vs > 1)) {
      ...
      DEBUG((DBG_FONTS, "%s: scaling glyph %d to resolution %dx%d\n",
...
again:
  ...
  if (!font->chars && load_font_file(&dvi->params, font) < 0) return NULL;

  /* get the unscaled glyph, maybe loading it from disk */
  ch = FONTCHAR(font, code);
  if (!ch || !glyph_present(ch)) return NULL;
  ...
  if (!ch->width || !ch->height || font->finfo->getglyph == NULL ||
  ...
  if (ch->missing || MDVI_GLYPH_ISEMPTY(ch->glyph.data)) {
    ...
    if (ch->grey.data && !MDVI_GLYPH_ISEMPTY(ch->grey.data) &&
        ch->fg == dvi->curr_fg && ch->bg == dvi->curr_bg)
      return ch;
    if (ch->grey.data && !MDVI_GLYPH_ISEMPTY(ch->grey.data)) {
      if (dvi->device.free_image) dvi->device.free_image(ch->grey.data);
      ch->grey.data = NULL;
    }
...
void font_reset_one_glyph(DviDevice *dev, DviFontChar *ch, int what) {
  ...
  if (what & MDVI_FONTSEL_GREY) {
    if (MDVI_GLYPH_NONEMPTY(ch->grey.data)) {
      if (dev->free_image) dev->free_image(ch->grey.data);
    }
    ch->grey.data = NULL;
  }
...
void font_reset_font_glyphs(DviDevice *dev, DviFont *font, int what) {
  ...
  if (font->finfo->getglyph == NULL) return;
  DEBUG((DBG_FONTS, "resetting glyphs in font `%s'\n", font->fontname));
  for (ch = font->chars, i = font->loc; i <= font->hic; ch++, i++) {
    if (glyph_present(ch)) font_reset_one_glyph(dev, ch, what);
  }
...
void font_finish_definitions(DviContext *dvi) {
  ...
  for (count = 0, ref = dvi->fonts; ref; ref = ref->next) map[count++] = ref;
