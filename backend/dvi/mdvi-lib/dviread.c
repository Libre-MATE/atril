...
static const DviCommand dvi_commands[] = {
    set_char,   set_char,   set_char,   set_char,   set_char,
    set_char,   set_char,   set_char,   set_char,   set_char,
    set_char,   set_char,   set_char,   set_char,   set_char,
    set_char,   set_char,   set_char,   set_char,   set_char,
    set_char,   set_char,   set_char,   set_char,   set_char,
    ...
    set_char,   set_char,   set_char,   set_char,   set_char,
    set_char,   set_char,   set_char,   set_char,   set_char,
    set_char,   set_char,   set_char,                       /* 0 - 127 */
    ...
    sel_font,   sel_font,   sel_font,   sel_font,   sel_font,
    sel_font,   sel_font,   sel_font,   sel_font,   sel_font,
    sel_font,   sel_font,   sel_font,   sel_font,   sel_font,
    sel_font,   sel_font,   sel_font,   sel_font,   sel_font,
    sel_font,   sel_font,   sel_font,   sel_font,   sel_font,
...
static void dummy_draw_rule(DviContext *dvi, int x, int y, Uint w, Uint h,
                            int f) {}

static int dummy_alloc_colors(void *a, Ulong *b, int c, Ulong d, Ulong e,
                              double f, int g) {
  return -1;
}
...
static void dvierr(DviContext *dvi, const char *format, ...) {
  ...
  va_start(ap, format);
  fprintf(stderr, "%s[%d]: Error: ", dvi->filename, dvi->currpage);
...
static void dviwarn(DviContext *dvi, const char *format, ...) {
  ...
  fprintf(stderr, "%s[%d]: Warning: ", dvi->filename, dvi->currpage);
  va_start(ap, format);
  vfprintf(stderr, format, ap);
  va_end(ap);
}
...
static int get_bytes(DviContext *dvi, size_t n) {
  ...
  if (NEEDBYTES(dvi, n)) {
    ...
    if (dvi->buffer.data == NULL) {
      ...
      memmove(dvi->buffer.data, dvi->buffer.data + dvi->buffer.pos,
              dvi->buffer.length);
    } else {
    ...
    if (required > dvi->buffer.size - dvi->buffer.length) {
      ...
      dvi->buffer.data =
          (Uchar *)xresize(dvi->buffer.data, char, dvi->buffer.size);
    }
    /* now read into the buffer */
    newlen = fread(dvi->buffer.data + dvi->buffer.length, 1,
                   dvi->buffer.size - dvi->buffer.length, dvi->in);
    if (newlen == -1) {
...
static int dskip(DviContext *dvi, long offset) {
  ...
  if (NEEDBYTES(dvi, offset) && get_bytes(dvi, offset) == -1) return -1;
...
static long dsgetn(DviContext *dvi, size_t n) {
  ...
  if (NEEDBYTES(dvi, n) && get_bytes(dvi, n) == -1) return -1;
...
static int dread(DviContext *dvi, char *buffer, size_t len) {
  if (NEEDBYTES(dvi, len) && get_bytes(dvi, len) == -1) return -1;
...
static long dtell(DviContext *dvi) {
  return dvi->depth ? dvi->buffer.pos
                    : ftell(dvi->in) - dvi->buffer.length + dvi->buffer.pos;
}

static void dreset(DviContext *dvi) {
  if (!dvi->buffer.frozen && dvi->buffer.data) mdvi_free(dvi->buffer.data);
...
#ifndef NODEBUG
static void dviprint(DviContext *dvi, const char *command, int sub,
                     const char *fmt, ...) {
  int i;
  va_list ap;

  printf("%s: ", dvi->filename);
  for (i = 0; i < dvi->depth; i++) printf("  ");
...
#define SHOWCMD(x)                        \
  if (_mdvi_debug_mask & DBG_OPCODE) do { \
      dviprint x;                         \
  } while (0)
#else
#define SHOWCMD(x) \
...
int mdvi_find_tex_page(DviContext *dvi, int tex_page) {
  ...
  for (i = 0; i < dvi->npages; i++)
    if (dvi->pagemap[i][1] == tex_page) return i;
  return -1;
}
...
void mdvi_sort_pages(DviContext *dvi, DviPageSort type) {
  ...
  if (sortfunc) qsort(dvi->pagemap, dvi->npages, sizeof(PageNum), sortfunc);
}
...
static DviFontRef *define_font(DviContext *dvi, int op) {
  ...
  DEBUG((DBG_FONTS, "requesting font %d = `%s' at %.1fpt (%dx%d dpi)\n", arg,
         name, (double)scale / (dvi->params.tfm_conv * 0x100000), hdpi, vdpi));
...
static char *opendvi(const char *name) {
  ...
  if (len >= 4 && STREQ(name + len - 4, ".dvi")) {
    ...
    if (access(name, R_OK) == 0) return mdvi_strdup(name);
  }
  ...
  strcpy(file + len, ".dvi");
  DEBUG((DBG_DVI | DBG_FILES, "opendvi: Trying `%s'\n", file));
  if (access(file, R_OK) == 0) return file;
  /* try the given name */
  file[len] = 0;
  DEBUG((DBG_DVI | DBG_FILES, "opendvi: Trying `%s'\n", file));
  if (access(file, R_OK) == 0) return file;
...
int mdvi_reload(DviContext *dvi, DviParams *np) {
  ...
  if (dvi->fontmap) mdvi_free(dvi->fontmap);
  dvi->currfont = NULL;
  ...
  if (dvi->currpage > dvi->npages - 1) dvi->currpage = 0;
  ...
  if (dvi->device.refresh) dvi->device.refresh(dvi, dvi->device.device_data);
...
int mdvi_configure(DviContext *dvi, DviParamCode option, ...) {
  ...
  if (np.dpi <= 0 || np.vdpi <= 0) return -1;
  if (np.mag <= 0.0) return -1;
  if (np.hshrink < 1 || np.vshrink < 1) return -1;
  if (np.hdrift < 0 || np.vdrift < 0) return -1;
  if (np.fg == np.bg) return -1;
  ...
  if (reset_all) return (mdvi_reload(dvi, &np) == 0);

  if (np.hshrink != dvi->params.hshrink) {
    np.conv = dvi->dviconv;
    if (np.hshrink) np.conv /= np.hshrink;
  }
  if (np.vshrink != dvi->params.vshrink) {
    np.vconv = dvi->dvivconv;
    if (np.vshrink) np.vconv /= np.vshrink;
  }
 ...
 */
DviContext *mdvi_init_context(DviParams *par, DviPageSpec *spec,
                              const char *file) {
  FILE *p;
  Int32 arg;
  int op;
  ...
  if (fuget1(p) != DVI_PRE) goto bad_dvi;
  ...
  if (!dvi->num || !dvi->den || !dvi->dvimag) goto bad_dvi;

  dvi->params.mag = (par->mag > 0 ? par->mag : (double)dvi->dvimag / 1000.0);
  ...
  dvi->params.tfm_conv =
      (25400000.0 / dvi->num) * ((double)dvi->den / 473628672) / 16.0;
  ...
  if (dvi->params.hshrink) dvi->params.conv /= dvi->params.hshrink;
  if (dvi->params.vshrink) dvi->params.vconv /= dvi->params.vshrink;
  ...
  if (fseek(p, (long)-1, SEEK_END) == -1) goto error;
  for (n = 0; (op = fuget1(p)) == DVI_TRAILER; n++)
    if (fseek(p, (long)-2, SEEK_CUR) < 0) break;
  if (op != arg || n < 4) goto bad_dvi;
  /* get the pointer to postamble */
  fseek(p, (long)-5, SEEK_CUR);
  arg = fuget4(p);
  /* jump to it */
  fseek(p, (long)arg, SEEK_SET);
  if (fuget1(p) != DVI_POST) goto bad_dvi;
  offset = fuget4(p);
  ...
  DEBUG((DBG_DVI, "%s: from postamble: stack depth %d, %d page%s\n", filename,
         dvi->stacksize, dvi->npages, dvi->npages > 1 ? "s" : ""));
  ...
  if (op != DVI_POST_POST) goto bad_dvi;
  ...
  while (offset != -1) {
    ...
    op = fuget1(p);
    if (op != DVI_BOP || n < 0) goto bad_dvi;
    for (i = 1; i <= 10; i++) page[i] = fsget4(p);
    page[0] = offset;
    offset = fsget4(p);
    /* check if the page is selected */
    if (spec && mdvi_page_selected(spec, page, n) == 0) {
      DEBUG((DBG_DVI,
             "Page %d (%ld.%ld.%ld.%ld.%ld.%ld.%ld.%ld.%ld.%ld) ignored by "
             "request\n",
             n, page[1], page[2], page[3], page[4], page[5], page[6], page[7],
...
void mdvi_destroy_context(DviContext *dvi) {
  if (dvi->device.dev_destroy) dvi->device.dev_destroy(dvi->device.device_data);
  /* release all fonts */
  if (dvi->fonts) {
    font_drop_chain(dvi->fonts);
    font_free_unused(&dvi->device);
  ...
  if (dvi->fontmap) mdvi_free(dvi->fontmap);
  if (dvi->filename) mdvi_free(dvi->filename);
  if (dvi->stack) mdvi_free(dvi->stack);
  if (dvi->pagemap) mdvi_free(dvi->pagemap);
  if (dvi->fileid) mdvi_free(dvi->fileid);
  if (dvi->in) fclose(dvi->in);
  if (dvi->buffer.data && !dvi->buffer.frozen) mdvi_free(dvi->buffer.data);
  if (dvi->color_stack) mdvi_free(dvi->color_stack);
...
void mdvi_setpage(DviContext *dvi, int pageno) {
  ...
  if (pageno > dvi->npages - 1) pageno = dvi->npages - 1;
  dvi->currpage = pageno;
}
...
static int mdvi_run_macro(DviContext *dvi, Uchar *macro, size_t len) {
  ...
  while ((opcode = duget1(dvi)) != DVI_EOP) {
    if (dvi_commands[opcode](dvi, opcode) < 0) break;
  }
  if (opcode != DVI_EOP)
...
again:
  ...
  while ((op = duget1(dvi)) != DVI_EOP) {
    if (dvi_commands[op](dvi, op) < 0) break;
  }

  fflush(stdout);
  fflush(stderr);
  if (op != DVI_EOP) return -1;
  if (dvi->stacktop) dviwarn(dvi, _("stack not empty at end of page\n"));
  return 0;
}
...
inline static int move_vertical(DviContext *dvi, int amount) {
  ...
  if (!dvi->params.vdrift) return rvv;
...
#define DBGSUM(a, b, c) (a), (b) > 0 ? '+' : '-', (b) > 0 ? (b) : -(b), (c)

static void draw_shrink_rule(DviContext *dvi, int x, int y, Uint w, Uint h,
                             int f) {
  Ulong fg, bg;
...
int set_char(DviContext *dvi, int opcode) {
  ...
  if (ch == NULL || ch->missing) {
    ...
    if (ISVIRTUAL(font))
      mdvi_run_macro(dvi, (Uchar *)font->private + ch->offset, ch->width);
    else if (ch->width && ch->height)
      dvi->device.draw_glyph(dvi, ch, dvi->pos.hh, dvi->pos.vv);
  }
  if (opcode >= DVI_PUT1 && opcode <= DVI_PUT4) {
    SHOWCMD((dvi, "putchar", opcode - DVI_PUT1 + 1, "char %d (%s)\n", num,
             dvi->currfont->ref->fontname));
  } else {
    h = dvi->pos.h + ch->tfmwidth;
    hh = dvi->pos.hh + pixel_round(dvi, ch->tfmwidth);
    SHOWCMD((dvi, "setchar", num, "(%d,%d) h:=%d%c%d=%d, hh:=%d (%s)\n",
             dvi->pos.hh, dvi->pos.vv, DBGSUM(dvi->pos.h, ch->tfmwidth, h), hh,
             font->fontname));
    dvi->pos.h = h;
    dvi->pos.hh = hh;
...
int set_rule(DviContext *dvi, int opcode) {
  ...
  if (a > 0 && b > 0) {
    ...
    if (dvi->curr_layer <= dvi->params.layer) {
      draw_shrink_rule(dvi, dvi->pos.hh, dvi->pos.vv - h + 1, w, h, 1);
    }
  } else {
    SHOWCMD((dvi, opcode == DVI_SET_RULE ? "setrule" : "putrule", -1,
...
int push(DviContext *dvi, int opcode) {
  if (dvi->stacktop == dvi->stacksize) {
    if (!dvi->depth) dviwarn(dvi, _("enlarging stack\n"));
    dvi->stacksize += 8;
    dvi->stack = xresize(dvi->stack, DviState, dvi->stacksize);
  }
  memcpy(&dvi->stack[dvi->stacktop], &dvi->pos, sizeof(DviState));
  SHOWCMD((dvi, "push", -1,
           "level %d: (h=%d,v=%d,w=%d,x=%d,y=%d,z=%d,hh=%d,vv=%d)\n",
           dvi->stacktop, dvi->pos.h, dvi->pos.v, dvi->pos.w, dvi->pos.x,
...
int move_right(DviContext *dvi, int opcode) {
  ...
  SHOWCMD((dvi, "right", opcode - DVI_RIGHT1 + 1, "%d h:=%d%c%d=%d, hh:=%d\n",
...
int move_down(DviContext *dvi, int opcode) {
  ...
  SHOWCMD((dvi, "down", opcode - DVI_DOWN1 + 1, "%d v:=%d%c%d=%d, vv:=%d\n",
...
int move_w(DviContext *dvi, int opcode) {
  ...
  if (opcode != DVI_W0) dvi->pos.w = dsgetn(dvi, opcode - DVI_W0);
  h = dvi->pos.h;
  hh = move_horizontal(dvi, dvi->pos.w);
  SHOWCMD((dvi, "w", opcode - DVI_W0, "%d h:=%d%c%d=%d, hh:=%d\n", dvi->pos.w,
           DBGSUM(h, dvi->pos.w, dvi->pos.h), hh));
  dvi->pos.hh = hh;
  return 0;
}
...
int move_x(DviContext *dvi, int opcode) {
  ...
  if (opcode != DVI_X0) dvi->pos.x = dsgetn(dvi, opcode - DVI_X0);
  h = dvi->pos.h;
  hh = move_horizontal(dvi, dvi->pos.x);
  SHOWCMD((dvi, "x", opcode - DVI_X0, "%d h:=%d%c%d=%d, hh:=%d\n", dvi->pos.x,
           DBGSUM(h, dvi->pos.x, dvi->pos.h), hh));
  dvi->pos.hh = hh;
  return 0;
}
...
int move_y(DviContext *dvi, int opcode) {
  ...
  if (opcode != DVI_Y0) dvi->pos.y = dsgetn(dvi, opcode - DVI_Y0);
  v = dvi->pos.v;
  vv = move_vertical(dvi, dvi->pos.y);
  SHOWCMD((dvi, "y", opcode - DVI_Y0, "%d h:=%d%c%d=%d, hh:=%d\n", dvi->pos.y,
           DBGSUM(v, dvi->pos.y, dvi->pos.v), vv));
  dvi->pos.vv = vv;
  return 0;
}
...
int move_z(DviContext *dvi, int opcode) {
  ...
  if (opcode != DVI_Z0) dvi->pos.z = dsgetn(dvi, opcode - DVI_Z0);
  v = dvi->pos.v;
  vv = move_vertical(dvi, dvi->pos.z);
  SHOWCMD((dvi, "z", opcode - DVI_Z0, "%d h:=%d%c%d=%d, hh:=%d\n", dvi->pos.z,
           DBGSUM(v, dvi->pos.z, dvi->pos.v), vv));
  dvi->pos.vv = vv;
  return 0;
}
...
int sel_font(DviContext *dvi, int opcode) {
  ...
  SHOWCMD((dvi, "fntnum", opcode - DVI_FNT_NUM0, "current font is %s\n",
           ref->ref->fontname));
  dvi->currfont = ref;
  return 0;
}
...
int sel_fontn(DviContext *dvi, int opcode) {
  ...
  SHOWCMD((dvi, "fnt", opcode - DVI_FNT1 + 1, "current font is %s (id %d)\n",
...
int special(DviContext *dvi, int opcode) {
  ...
  SHOWCMD((dvi, "XXXX", opcode - DVI_XXX1 + 1, "[%s]", s));
  mdvi_free(s);
  return 0;
}
...
int def_font(DviContext *dvi, int opcode) {
  ...
  SHOWCMD((dvi, "fntdef", opcode - DVI_FNT_DEF1 + 1, "%d -> %s (%d links)\n",
           ref->fontid, ref->ref->fontname, ref->ref->links));
  return 0;
}
