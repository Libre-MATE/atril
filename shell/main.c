...
static gboolean option_version_cb(const gchar *option_name, const gchar *value,
...
static const GOptionEntry goption_options[] = {
    {"page-label", 'p', 0, G_OPTION_ARG_STRING, &ev_page_label,
     N_("The page label of the document to display."), N_("PAGE")},
    {"page-index", 'i', 0, G_OPTION_ARG_INT, &ev_page_index,
     N_("The page number of the document to display."), N_("NUMBER")},
    {"fullscreen", 'f', 0, G_OPTION_ARG_NONE, &fullscreen_mode,
     N_("Run atril in fullscreen mode"), NULL},
    {"named-dest", 'n', 0, G_OPTION_ARG_STRING, &ev_named_dest,
     N_("Named destination to display."), N_("DEST")},
    {"presentation", 's', 0, G_OPTION_ARG_NONE, &presentation_mode,
     N_("Run atril in presentation mode"), NULL},
    {"preview", 'w', 0, G_OPTION_ARG_NONE, &preview_mode,
     N_("Run atril as a previewer"), NULL},
    {"find", 'l', 0, G_OPTION_ARG_STRING, &ev_find_string,
     N_("The word or phrase to find in the document"), N_("STRING")},
    {"unlink-tempfile", 'u', G_OPTION_FLAG_HIDDEN, G_OPTION_ARG_NONE,
     &unlink_temp_file, NULL, NULL},
    {"print-settings", 't', G_OPTION_FLAG_HIDDEN, G_OPTION_ARG_FILENAME,
...
static gboolean launch_previewer(void) {
  ...
  if (unlink_temp_file) g_string_append(cmd_str, " --unlink-tempfile");
...
static void load_files(const char **files) {
  ...
  for (i = 0; files[i]; i++) {
    ...
    ev_application_open_uri_at_dest(EV_APP, uri, screen, dest, mode,
                                    ev_find_string, GDK_CURRENT_TIME);
