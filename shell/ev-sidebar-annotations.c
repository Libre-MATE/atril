...
enum { COLUMN_MARKUP, COLUMN_ICON, COLUMN_ANNOT_MAPPING, N_COLUMNS };

enum { ANNOT_ACTIVATED, BEGIN_ANNOT_ADD, ANNOT_ADD_CANCELLED, N_SIGNALS };
...
static void ev_sidebar_annotations_page_iface_init(
...
G_DEFINE_TYPE_EXTENDED(
    EvSidebarAnnotations, ev_sidebar_annotations, GTK_TYPE_BOX, 0,
...
static GtkTreeModel *ev_sidebar_annotations_create_simple_model(
    const gchar *message) {
  GtkTreeModel *retval;
  GtkTreeIter iter;
  gchar *markup;
  ...
  retval = (GtkTreeModel *)gtk_list_store_new(N_COLUMNS, G_TYPE_STRING,
                                              GDK_TYPE_PIXBUF, G_TYPE_POINTER);

  gtk_list_store_append(GTK_LIST_STORE(retval), &iter);
  markup = g_strdup_printf("<span size=\"larger\" style=\"italic\">%s</span>",
                           message);
  gtk_list_store_set(GTK_LIST_STORE(retval), &iter, COLUMN_MARKUP, markup, -1);
  g_free(markup);

  return retval;
}

static void ev_sidebar_annotations_text_annot_button_toggled(
    GtkWidget *button, EvSidebarAnnotations *sidebar_annots) {
...
static void ev_sidebar_annotations_init(EvSidebarAnnotations *ev_annots) {
  ...
  gtk_orientable_set_orientation(GTK_ORIENTABLE(ev_annots),
  ...
  gtk_tree_view_set_headers_visible(GTK_TREE_VIEW(ev_annots->priv->tree_view),
                                    FALSE);
  selection =
      gtk_tree_view_get_selection(GTK_TREE_VIEW(ev_annots->priv->tree_view));
  ...
  gtk_tree_view_column_set_attributes(column, renderer, "markup", COLUMN_MARKUP,
                                      NULL);
  ...
  gtk_container_add(GTK_CONTAINER(ev_annots->priv->annot_text_item),
                    buttonarea);
  ...
  gtk_box_pack_start(GTK_BOX(hbox), ev_annots->priv->annot_text_item, TRUE,
                     TRUE, 6);
...
static void ev_sidebar_annotations_get_property(GObject *object, guint prop_id,
                                                GValue *value,
                                                GParamSpec *pspec) {
...
static void ev_sidebar_annotations_class_init(
    EvSidebarAnnotationsClass *klass) {
  ...
  signals[ANNOT_ACTIVATED] = g_signal_new(
      "annot-activated", G_TYPE_FROM_CLASS(g_object_class),
      G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
      G_STRUCT_OFFSET(EvSidebarAnnotationsClass, annot_activated), NULL, NULL,
      g_cclosure_marshal_VOID__POINTER, G_TYPE_NONE, 1, G_TYPE_POINTER);
  signals[BEGIN_ANNOT_ADD] = g_signal_new(
      "begin-annot-add", G_TYPE_FROM_CLASS(g_object_class),
      G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
      G_STRUCT_OFFSET(EvSidebarAnnotationsClass, begin_annot_add), NULL, NULL,
      g_cclosure_marshal_VOID__ENUM, G_TYPE_NONE, 1, EV_TYPE_ANNOTATION_TYPE);
  signals[ANNOT_ADD_CANCELLED] = g_signal_new(
      "annot-add-cancelled", G_TYPE_FROM_CLASS(g_object_class),
      G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
      G_STRUCT_OFFSET(EvSidebarAnnotationsClass, annot_add_cancelled), NULL,
...
void ev_sidebar_annotations_annot_added(EvSidebarAnnotations *sidebar_annots,
  ...
  if (EV_IS_ANNOTATION_TEXT(annot)) {
    ...
    g_signal_handlers_block_by_func(
        toggle_button, ev_sidebar_annotations_text_annot_button_toggled,
        sidebar_annots);
...
void ev_sidebar_annotations_annot_removed(
...
static void selection_changed_cb(GtkTreeSelection *selection,
  ...
  if (gtk_tree_selection_get_selected(selection, &model, &iter)) {
    ...
    gtk_tree_model_get(model, &iter, COLUMN_ANNOT_MAPPING, &mapping, -1);
    if (mapping)
...
static void job_finished_callback(EvJobAnnots *job,
  ...
  if (!job->annots) {
    ...
    list = ev_sidebar_annotations_create_simple_model(
  ...
  if (priv->selection_changed_id == 0) {
    priv->selection_changed_id = g_signal_connect(
        selection, "changed", G_CALLBACK(selection_changed_cb), sidebar_annots);
  }

  model = gtk_tree_store_new(N_COLUMNS, G_TYPE_STRING, GDK_TYPE_PIXBUF,
                             G_TYPE_POINTER);
  ...
  for (l = job->annots; l; l = g_list_next(l)) {
    ...
    gtk_tree_store_set(model, &iter, COLUMN_MARKUP, page_label, -1);
    g_free(page_label);

    for (ll = ev_mapping_list_get_list(mapping_list); ll;
         ll = g_list_next(ll)) {
      EvAnnotation *annot;
      const gchar *label;
      ...
      if (EV_IS_ANNOTATION_TEXT(annot)) {
        if (!text_icon) {
          text_icon = gtk_icon_theme_load_icon(
              icon_theme, "starred", GTK_ICON_SIZE_BUTTON,
              GTK_ICON_LOOKUP_FORCE_REGULAR, NULL);
        }
        pixbuf = text_icon;
      } else if (EV_IS_ANNOTATION_ATTACHMENT(annot)) {
        if (!attachment_icon) {
          attachment_icon = gtk_icon_theme_load_icon(
              icon_theme, "mail-attachment", GTK_ICON_SIZE_BUTTON,
              GTK_ICON_LOOKUP_FORCE_REGULAR, NULL);
        }
      ...
      gtk_tree_store_set(model, &child_iter, COLUMN_MARKUP, markup, COLUMN_ICON,
                         pixbuf, COLUMN_ANNOT_MAPPING, ll->data, -1);
      g_free(markup);
      found = TRUE;
    }

    if (!found) gtk_tree_store_remove(model, &iter);
  }
  ...
  if (text_icon) g_object_unref(text_icon);
  if (attachment_icon) g_object_unref(attachment_icon);
...
static void ev_sidebar_annotations_document_changed_cb(
  ...
  if (!EV_IS_DOCUMENT_ANNOTATIONS(document)) return;

  if (priv->document) g_object_unref(priv->document);
  priv->document = g_object_ref(document);

  enable_add = ev_document_annotations_can_add_annotation(
...
static gboolean ev_sidebar_annotations_support_document(
    EvSidebarPage *sidebar_page, EvDocument *document) {
  return (EV_IS_DOCUMENT_ANNOTATIONS(document));
}

static const gchar *ev_sidebar_annotations_get_label(
