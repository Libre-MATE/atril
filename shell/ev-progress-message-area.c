...
static void ev_progress_message_area_class_init(
    EvProgressMessageAreaClass *class) {
  ...
  g_object_class_install_property(
      ...
      g_param_spec_string("status", "Status",
                          "The status text of the progress area", NULL,
                          G_PARAM_READWRITE));
  g_object_class_install_property(
      gobject_class, PROP_FRACTION,
      g_param_spec_double("fraction", "Fraction",
  ...
  switch (prop_id) {
    ...
    case PROP_FRACTION: {
      ...
      fraction = gtk_progress_bar_get_fraction(
          GTK_PROGRESS_BAR(area->priv->progress_bar));
...
GtkWidget *ev_progress_message_area_new(const gchar *stock_id,
  ...
  widget = g_object_new(EV_TYPE_PROGRESS_MESSAGE_AREA, "message-type",
                        GTK_MESSAGE_OTHER, "text", text, NULL);
