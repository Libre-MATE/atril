...
static void ev_properties_fonts_dispose(GObject *object) {
  ...
  if (properties->fonts_job) {
    g_signal_handlers_disconnect_by_func(properties->fonts_job,
                                         job_fonts_finished_cb, properties);
...
static void ev_properties_fonts_class_init(
...
static void font_cell_data_func(GtkTreeViewColumn *col,
  ...
  gtk_tree_model_get(model, iter, EV_DOCUMENT_FONTS_COLUMN_NAME, &name,
...
static void ev_properties_fonts_init(EvPropertiesFonts *properties) {
  ...
  renderer = GTK_CELL_RENDERER(
      g_object_new(GTK_TYPE_CELL_RENDERER_TEXT, "ypad", 6, NULL));
  ...
  gtk_tree_view_column_set_cell_data_func(column, renderer, font_cell_data_func,
                                          NULL, NULL);

  gtk_container_add(GTK_CONTAINER(swindow), properties->fonts_treeview);
  gtk_widget_show(properties->fonts_treeview);

  gtk_box_pack_start(GTK_BOX(properties), swindow, TRUE, TRUE, 0);
  gtk_widget_show(swindow);

  properties->fonts_progress_label = gtk_label_new(NULL);
  g_object_set(G_OBJECT(properties->fonts_progress_label), "xalign", 0.0, NULL);
  gtk_box_pack_start(GTK_BOX(properties), properties->fonts_progress_label,
                     FALSE, FALSE, 0);
...
static void job_fonts_updated_cb(EvJobFonts *job, gdouble progress,
...
void ev_properties_fonts_set_document(EvPropertiesFonts *properties,
  ...
  g_signal_connect(properties->fonts_job, "updated",
                   G_CALLBACK(job_fonts_updated_cb), properties);
  g_signal_connect(properties->fonts_job, "finished",
                   G_CALLBACK(job_fonts_finished_cb), properties);
  ev_job_scheduler_push_job(properties->fonts_job, EV_JOB_PRIORITY_NONE);
}

GtkWidget *ev_properties_fonts_new() {
  EvPropertiesFonts *properties;
