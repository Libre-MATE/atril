...
G_DEFINE_TYPE_WITH_PRIVATE(EvNavigationAction, ev_navigation_action,
                           GTK_TYPE_ACTION)
G_GNUC_END_IGNORE_DEPRECATIONS;

#define MAX_LABEL_LENGTH 48

static void ev_navigation_action_history_changed(EvHistory *history,
                                                 gpointer data) {
...
static void activate_menu_item_cb(GtkWidget *widget,
...
static GtkWidget *new_history_menu_item(EvNavigationAction *action,
                                        EvLink *link, int index) {
  GtkLabel *label;
  GtkWidget *item;
  const char *title;
  ...
  g_signal_connect(item, "activate", G_CALLBACK(activate_menu_item_cb), action);
...
static GtkWidget *build_menu(EvNavigationAction *action) {
  GtkMenuShell *menu;
  GtkWidget *item;
  EvLink *link;
...
static void menu_activated_cb(EvNavigationActionWidget *button,
                              EvNavigationAction *action) {
  GtkWidget *menu;
...
static void connect_proxy(GtkAction *action, GtkWidget *proxy) {
  ...
  if (GTK_IS_TOOL_ITEM(proxy)) {
    ...
    menu = gtk_menu_new();
    ev_navigation_action_widget_set_menu(EV_NAVIGATION_ACTION_WIDGET(proxy),
                                         menu);

    g_signal_connect(proxy, "show-menu", G_CALLBACK(menu_activated_cb), action);
  }

  G_GNUC_BEGIN_IGNORE_DEPRECATIONS;
  GTK_ACTION_CLASS(ev_navigation_action_parent_class)
...
static GtkWidget *create_menu_item(GtkAction *action) {
  ...
  G_GNUC_BEGIN_IGNORE_DEPRECATIONS;
  menu_item = GTK_ACTION_CLASS(ev_navigation_action_parent_class)
                  ->create_menu_item(action);
  G_GNUC_END_IGNORE_DEPRECATIONS;
...
static void ev_navigation_action_finalize(GObject *object) {
...
static void ev_navigation_action_class_init(EvNavigationActionClass *class) {
  ...
  widget_signals[WIDGET_ACTIVATE_LINK] = g_signal_new(
      ...
      G_STRUCT_OFFSET(EvNavigationActionClass, activate_link), NULL, NULL,
