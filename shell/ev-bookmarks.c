...
static gint ev_bookmark_compare(EvBookmark *a, EvBookmark *b) {
  if (a->page < b->page) return -1;
  if (a->page > b->page) return 1;
  return 0;
}

static void ev_bookmark_free(EvBookmark *bm) {
  if (G_UNLIKELY(!bm)) return;

  g_free(bm->title);
  g_slice_free(EvBookmark, bm);
}

static void ev_bookmarks_finalize(GObject *object) {
...
static void ev_bookmarks_init(EvBookmarks *bookmarks) {}

static void ev_bookmarks_set_property(GObject *object, guint prop_id,
...
static void ev_bookmarks_constructed(GObject *object) {
  EvBookmarks *bookmarks = EV_BOOKMARKS(object);
  gchar *bm_list_str;
  GVariant *bm_list;
  GVariantIter iter;
  ...
  if (!bm_list_str || bm_list_str[0] == '\0') return;

  bm_list = g_variant_parse((const GVariantType *)"a(us)", bm_list_str, NULL,
                            NULL, &error);
  if (!bm_list) {
...
static void ev_bookmarks_class_init(EvBookmarksClass *klass) {
  ...
  g_object_class_install_property(
      ...
      g_param_spec_object("metadata", "Metadata", "The document metadata",
                          EV_TYPE_METADATA,
                          G_PARAM_CONSTRUCT_ONLY | G_PARAM_WRITABLE));
  /* Signals */
  signals[CHANGED] =
      g_signal_new("changed", EV_TYPE_BOOKMARKS, G_SIGNAL_RUN_LAST,
                   G_STRUCT_OFFSET(EvBookmarksClass, changed), NULL, NULL,
...
EvBookmarks *ev_bookmarks_new(EvMetadata *metadata) {
  ...
  return EV_BOOKMARKS(
      g_object_new(EV_TYPE_BOOKMARKS, "metadata", metadata, NULL));
}

static void ev_bookmarks_save(EvBookmarks *bookmarks) {
  GList *l;
...
void ev_bookmarks_add(EvBookmarks *bookmarks, EvBookmark *bookmark) {
  ...
  if (g_list_find_custom(bookmarks->items, bookmark,
                         (GCompareFunc)ev_bookmark_compare))
    return;
...
void ev_bookmarks_delete(EvBookmarks *bookmarks, EvBookmark *bookmark) {
  ...
  bm_link = g_list_find_custom(bookmarks->items, bookmark,
                               (GCompareFunc)ev_bookmark_compare);
  if (!bm_link) return;
...
void ev_bookmarks_update(EvBookmarks *bookmarks, EvBookmark *bookmark) {
  ...
  if (strcmp(bookmark->title, bm->title) == 0) return;
