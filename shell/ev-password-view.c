...
static void ev_password_view_finalize(GObject *object) {
...
static void ev_password_view_class_init(EvPasswordViewClass *class) {
  GObjectClass *g_object_class;

  g_object_class = G_OBJECT_CLASS(class);

  password_view_signals[UNLOCK] =
      g_signal_new("unlock", G_TYPE_FROM_CLASS(g_object_class),
                   G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
                   G_STRUCT_OFFSET(EvPasswordViewClass, unlock), NULL, NULL,
...
static void ev_password_view_init(EvPasswordView *password_view) {
  GtkWidget *vbox;
  GtkWidget *hbox;
  GtkWidget *image;
  GtkWidget *button;
  ...
  password_view->priv->label = (GtkWidget *)g_object_new(
      GTK_TYPE_LABEL, "wrap", TRUE, "selectable", TRUE, NULL);
  gtk_box_pack_start(GTK_BOX(vbox), password_view->priv->label, FALSE, FALSE,
                     0);
  ...
  markup = g_strdup_printf("<span size=\"x-large\">%s</span>",
                           _("This document is locked and can only be read by "
  ...
  g_signal_connect(button, "clicked", G_CALLBACK(ev_password_view_clicked_cb),
                   password_view);
...
void ev_password_view_set_uri(EvPasswordView *password_view, const char *uri) {
  ...
  markup = g_markup_printf_escaped(
      "<span size=\"x-large\" weight=\"bold\">%s</span>", file_name);
  g_free(file_name);
...
static void ev_password_dialog_got_response(GtkDialog *dialog, gint response_id,
  ...
  if (response_id == GTK_RESPONSE_OK) {
    g_free(password_view->priv->password);
    password_view->priv->password = g_strdup(
        gtk_entry_get_text(GTK_ENTRY(password_view->priv->password_entry)));
...
static void ev_password_dialog_remember_button_toggled(
    GtkToggleButton *button, EvPasswordView *password_view) {
...
void ev_password_view_ask_password(EvPasswordView *password_view) {
  ...
  gtk_window_set_transient_for(GTK_WINDOW(dialog),
                               password_view->priv->parent_window);
  gtk_window_set_modal(GTK_WINDOW(dialog), TRUE);

  gtk_dialog_add_buttons(dialog, "gtk-cancel", GTK_RESPONSE_CANCEL,
                         _("_Unlock Document"), GTK_RESPONSE_OK, NULL);
  ...
  gtk_label_set_line_wrap(GTK_LABEL(label), TRUE);
  file_name = g_file_get_basename(password_view->priv->uri_file);
  text = g_markup_printf_escaped(_("The document “%s” is locked and requires a "
                                   "password before it can be opened."),
                                 file_name);
  markup =
      g_strdup_printf("<span size=\"larger\" weight=\"bold\">%s</span>\n\n%s",
                      _("Password required"), text);
  gtk_label_set_markup(GTK_LABEL(label), markup);
  g_free(text);
  g_free(markup);
  g_free(file_name);
  ...
  gtk_entry_set_visibility(GTK_ENTRY(password_view->priv->password_entry),
                           FALSE);
  g_signal_connect(password_view->priv->password_entry, "changed",
                   G_CALLBACK(ev_password_dialog_entry_changed_cb), dialog);
  g_signal_connect(password_view->priv->password_entry, "activate",
                   G_CALLBACK(ev_password_dialog_entry_activated_cb), dialog);

  gtk_grid_attach(GTK_GRID(grid), label, 0, 0, 1, 1);
  gtk_widget_show(label);

  gtk_grid_attach(GTK_GRID(grid), password_view->priv->password_entry, 1, 0, 1,
                  1);
  ...
  if (ev_keyring_is_available()) {
    ...
    choice = gtk_radio_button_new_with_mnemonic(
        NULL, _("Forget password _immediately"));
    gtk_toggle_button_set_active(
        GTK_TOGGLE_BUTTON(choice),
        password_view->priv->password_save == G_PASSWORD_SAVE_NEVER);
    ...
    choice = gtk_radio_button_new_with_mnemonic(
        group, _("Remember password until you _log out"));
    gtk_toggle_button_set_active(
        GTK_TOGGLE_BUTTON(choice),
  ...
  g_signal_connect(dialog, "response",
                   G_CALLBACK(ev_password_dialog_got_response), password_view);
...
GPasswordSave ev_password_view_get_password_save_flags(
    EvPasswordView *password_view) {
