...
#define EV_DAEMON_APPLICATION(object)                               \
  (G_TYPE_CHECK_INSTANCE_CAST((object), EV_TYPE_DAEMON_APPLICATION, \
                              EvDaemonApplication))
...
static gboolean spawn_atril(const gchar *uri) {
  gchar *argv[3];
  gboolean retval;
  GError *error = NULL;
  ...
  retval = g_spawn_async(NULL /* wd */, argv, NULL /* env */, 0, NULL, NULL,
                         NULL, &error);
  if (!retval) {
...
static void name_vanished_cb(GDBusConnection *connection, const gchar *name,
  ...
  for (l = application->docs; l != NULL; l = l->next) {
    ...
    if (strcmp(doc->dbus_name, name) != 0) continue;
...
static gboolean handle_register_document_cb(EvDaemon *object,
  ...
  doc->loaded_id = g_dbus_connection_signal_subscribe(
      connection, doc->dbus_name, EV_DBUS_WINDOW_INTERFACE_NAME,
      "DocumentLoaded", NULL, NULL, 0, document_loaded_cb, application, NULL);
  doc->watch_id = g_bus_watch_name_on_connection(
      connection, sender, G_BUS_NAME_WATCHER_FLAGS_NONE, name_appeared_cb,
...
static gboolean handle_unregister_document_cb(
    EvDaemon *object, GDBusMethodInvocation *invocation, const gchar *uri,
    EvDaemonApplication *application) {
  EvDoc *doc;
  const char *sender;
  ...
  if (strcmp(doc->dbus_name, sender) != 0) {
    ...
    g_dbus_method_invocation_return_error_literal(
        invocation, G_DBUS_ERROR, G_DBUS_ERROR_BAD_ADDRESS,
  ...
  if (doc->loaded_id != 0) {
    g_dbus_connection_signal_unsubscribe(
        g_dbus_method_invocation_get_connection(invocation), doc->loaded_id);
    doc->loaded_id = 0;
  }
...
static gboolean handle_find_document_cb(EvDaemon *object,
                                        GDBusMethodInvocation *invocation,
                                        const gchar *uri, gboolean spawn,
                                        EvDaemonApplication *application) {
  EvDoc *doc;
  ...
  if (spawn) {
    ...
    uri_invocations =
        g_hash_table_lookup(application->pending_invocations, uri);
    ...
    if (ret_val) {
      ...
      g_hash_table_insert(application->pending_invocations, g_strdup(uri),
                          uri_invocations);
      return TRUE;
    }
  }
...
static void ev_daemon_application_dbus_unregister(GApplication *gapplication,
  ...
  if (application->daemon) {
    g_dbus_interface_skeleton_unexport(
        G_DBUS_INTERFACE_SKELETON(application->daemon));
    g_object_unref(application->daemon);
    application->daemon = NULL;
  }

  G_APPLICATION_CLASS(ev_daemon_application_parent_class)
...
static void ev_daemon_application_init(EvDaemonApplication *application) {
  application->pending_invocations = g_hash_table_new_full(
...
gint main(gint argc, gchar **argv) {
  ...
  application = g_object_new(EV_TYPE_DAEMON_APPLICATION, "application-id",
                             EV_DBUS_DAEMON_NAME, "flags", flags, NULL);
