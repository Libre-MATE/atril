...
G_DEFINE_TYPE(EvAnnotationPropertiesDialog, ev_annotation_properties_dialog,
              GTK_TYPE_DIALOG)

static void ev_annotation_properties_dialog_dispose(GObject *object) {
  EvAnnotationPropertiesDialog *dialog =
      EV_ANNOTATION_PROPERTIES_DIALOG(object);
...
static void ev_annotation_properties_dialog_constructed(GObject *object) {
  ...
  switch (dialog->annot_type) {
    case EV_ANNOTATION_TYPE_TEXT:
      ...
      gtk_combo_box_text_append_text(GTK_COMBO_BOX_TEXT(dialog->icon),
                                     _("Note"));
      gtk_combo_box_text_append_text(GTK_COMBO_BOX_TEXT(dialog->icon),
                                     _("Comment"));
      gtk_combo_box_text_append_text(GTK_COMBO_BOX_TEXT(dialog->icon),
...
static void ev_annotation_properties_dialog_init(
    EvAnnotationPropertiesDialog *annot_dialog) {
  ...
  gtk_dialog_add_buttons(dialog, "gtk-close", GTK_RESPONSE_CLOSE, "gtk-apply",
  ...
  gtk_combo_box_text_append_text(GTK_COMBO_BOX_TEXT(annot_dialog->popup_state),
                                 _("Open"));
  gtk_combo_box_text_append_text(GTK_COMBO_BOX_TEXT(annot_dialog->popup_state),
                                 _("Close"));
...
static void ev_annotation_properties_dialog_class_init(
    EvAnnotationPropertiesDialogClass *klass) {
  ...
  g_object_class_install_property(
      object_class, PROP_ANNOT_TYPE,
      g_param_spec_enum("annot-type", "AnnotType", "The type of annotation",
                        EV_TYPE_ANNOTATION_TYPE, EV_ANNOTATION_TYPE_TEXT,
...
GtkWidget *ev_annotation_properties_dialog_new_with_annotation(
  ...
  dialog = (EvAnnotationPropertiesDialog *)ev_annotation_properties_dialog_new(
  ...
  if (label) gtk_entry_set_text(GTK_ENTRY(dialog->author), label);
  ...
  gtk_combo_box_set_active(GTK_COMBO_BOX(dialog->popup_state), is_open ? 0 : 1);
