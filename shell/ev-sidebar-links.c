...
static void update_page_callback(EvSidebarLinks *sidebar_links, gint old_page,
                                 gint current_page);
static void row_activated_callback(GtkTreeView *treeview, GtkTreePath *arg1,
...
G_DEFINE_TYPE_EXTENDED(EvSidebarLinks, ev_sidebar_links, GTK_TYPE_BOX, 0,
                       G_ADD_PRIVATE(EvSidebarLinks) G_IMPLEMENT_INTERFACE(
                           EV_TYPE_SIDEBAR_PAGE,
                           ev_sidebar_links_page_iface_init))

static void ev_sidebar_links_set_property(GObject *object, guint prop_id,
                                          const GValue *value,
                                          GParamSpec *pspec) {
  ...
  switch (prop_id) {
    case PROP_MODEL:
      ev_sidebar_links_set_links_model(ev_sidebar_links,
...
static void ev_sidebar_links_get_property(GObject *object, guint prop_id,
...
static void ev_sidebar_links_map(GtkWidget *widget) {
  ...
  if (links->priv->model) {
    ev_sidebar_links_set_current_page(
        links, ev_document_model_get_page(links->priv->doc_model));
  }
}

static void ev_sidebar_links_class_init(
    EvSidebarLinksClass *ev_sidebar_links_class) {
  ...
  signals[LINK_ACTIVATED] = g_signal_new(
      "link-activated", G_TYPE_FROM_CLASS(g_object_class),
      G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
      G_STRUCT_OFFSET(EvSidebarLinksClass, link_activated), NULL, NULL,
      g_cclosure_marshal_VOID__OBJECT, G_TYPE_NONE, 1, G_TYPE_OBJECT);

  g_object_class_install_property(
      g_object_class, PROP_MODEL,
      g_param_spec_object("model", "Model", "Current Model",
...
static void selection_changed_callback(GtkTreeSelection *selection,
  ...
  if (gtk_tree_selection_get_selected(selection, &model, &iter)) {
    ...
    gtk_tree_model_get(model, &iter, EV_DOCUMENT_LINKS_COLUMN_LINK, &link, -1);
...
static GtkTreeModel *create_loading_model(void) {
  ...
  retval = (GtkTreeModel *)gtk_list_store_new(
  ...
  gtk_list_store_set(GTK_LIST_STORE(retval), &iter,
                     ...
                     EV_DOCUMENT_LINKS_COLUMN_LINK, NULL, -1);
  g_free(markup);
...
static void print_section_cb(GtkWidget *menuitem, EvSidebarLinks *sidebar) {
  ...
  selection =
      gtk_tree_view_get_selection(GTK_TREE_VIEW(sidebar->priv->tree_view));
  ...
  if (gtk_tree_selection_get_selected(selection, &model, &iter)) {
    ...
    if (gtk_tree_model_iter_next(model, &iter)) {
      gtk_tree_model_get(model, &iter, EV_DOCUMENT_LINKS_COLUMN_LINK, &link,
                         -1);
...
static GtkMenu *build_popup_menu(EvSidebarLinks *sidebar) {
  ...
  g_signal_connect(item, "activate", G_CALLBACK(print_section_cb), sidebar);
...
static void ev_sidebar_links_construct(EvSidebarLinks *ev_sidebar_links) {
  ...
  gtk_tree_view_column_set_attributes(
      GTK_TREE_VIEW_COLUMN(column), renderer, "text",
      EV_DOCUMENT_LINKS_COLUMN_PAGE_LABEL, NULL);
  g_object_set(G_OBJECT(renderer), "style", PANGO_STYLE_ITALIC, "xalign", 1.0,
               NULL);

  g_signal_connect(priv->tree_view, "button_press_event",
                   G_CALLBACK(button_press_cb), ev_sidebar_links);
  g_signal_connect(priv->tree_view, "popup_menu", G_CALLBACK(popup_menu_cb),
                   ev_sidebar_links);
}

static void ev_sidebar_links_init(EvSidebarLinks *ev_sidebar_links) {
  ev_sidebar_links->priv =
      ev_sidebar_links_get_instance_private(ev_sidebar_links);

  gtk_orientable_set_orientation(GTK_ORIENTABLE(ev_sidebar_links),
...
static void ev_sidebar_links_set_current_page(EvSidebarLinks *sidebar_links,
  ...
  if (!gtk_widget_is_visible(GTK_WIDGET(sidebar_links))) return;

  search_data.page = current_page;
  search_data.best_existing = G_MININT;

  path = g_tree_search(sidebar_links->priv->page_link_tree,
                       (GCompareFunc)page_link_tree_search_best_page,
                       &search_data);
  /* No direct hit, try a lookup on the best match. */
  if (!path)
    path = g_tree_lookup(sidebar_links->priv->page_link_tree,
                         GINT_TO_POINTER(search_data.best_existing));

  /* Still no hit, give up. */
  if (!path) return;

  selection = gtk_tree_view_get_selection(
      GTK_TREE_VIEW(sidebar_links->priv->tree_view));

  g_signal_handler_block(selection, sidebar_links->priv->selection_id);
  g_signal_handler_block(sidebar_links->priv->tree_view,
                         sidebar_links->priv->row_activated_id);

  gtk_tree_view_expand_to_path(GTK_TREE_VIEW(sidebar_links->priv->tree_view),
                               path);
  gtk_tree_view_set_cursor(GTK_TREE_VIEW(sidebar_links->priv->tree_view), path,
                           NULL, FALSE);

  g_signal_handler_unblock(selection, sidebar_links->priv->selection_id);
  g_signal_handler_unblock(sidebar_links->priv->tree_view,
                           sidebar_links->priv->row_activated_id);
}
...
static void expand_open_links(GtkTreeView *tree_view, GtkTreeModel *model,
                              GtkTreeIter *parent) {
  GtkTreeIter iter;
  gboolean expand;
  ...
  if (gtk_tree_model_iter_children(model, &iter, parent)) {
    do {
      gtk_tree_model_get(model, &iter, EV_DOCUMENT_LINKS_COLUMN_EXPAND, &expand,
                         -1);
      if (expand) {
        GtkTreePath *path;
...
static gboolean update_page_link_tree_foreach(GtkTreeModel *model,
  ...
  if (!g_tree_lookup(priv->page_link_tree, GINT_TO_POINTER(page)))
    g_tree_insert(priv->page_link_tree, GINT_TO_POINTER(page),
                  gtk_tree_path_copy(path));

  return FALSE;
}

static void ev_sidebar_links_set_links_model(EvSidebarLinks *sidebar_links,
                                             GtkTreeModel *model) {
  EvSidebarLinksPrivate *priv = sidebar_links->priv;

  if (priv->model == model) return;

  if (priv->model) g_object_unref(priv->model);
  priv->model = g_object_ref(model);

  /* Rebuild the binary search tree for finding links on pages. */
  if (priv->page_link_tree) g_tree_unref(priv->page_link_tree);
  priv->page_link_tree = g_tree_new_full(page_link_tree_sort, NULL, NULL,
                                         (GDestroyNotify)gtk_tree_path_free);

  gtk_tree_model_foreach(model, update_page_link_tree_foreach, sidebar_links);
...
static void job_finished_callback(EvJobLinks *job,
  ...
  if (priv->selection_id == 0) {
    priv->selection_id =
        g_signal_connect(selection, "changed",
                         G_CALLBACK(selection_changed_callback), sidebar_links);
  }
  priv->page_changed_id =
      g_signal_connect_swapped(priv->doc_model, "page-changed",
                               G_CALLBACK(update_page_callback), sidebar_links);
  ...
  ev_sidebar_links_set_current_page(
      sidebar_links, ev_document_model_get_page(priv->doc_model));
}

static void ev_sidebar_links_document_changed_cb(
    EvDocumentModel *model, GParamSpec *pspec, EvSidebarLinks *sidebar_links) {
  EvDocument *document = ev_document_model_get_document(model);
  EvSidebarLinksPrivate *priv = sidebar_links->priv;

  if (!EV_IS_DOCUMENT_LINKS(document)) return;
  ...
  if (priv->job) {
    g_signal_handlers_disconnect_by_func(priv->job, job_finished_callback,
                                         sidebar_links);
    g_object_unref(priv->job);
  }

  priv->job = ev_job_links_new(document);
  g_signal_connect(priv->job, "finished", G_CALLBACK(job_finished_callback),
                   sidebar_links);
...
static void ev_sidebar_links_set_model(EvSidebarPage *sidebar_page,
  ...
  if (priv->doc_model == model) return;
