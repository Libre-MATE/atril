...
static void ev_sidebar_dispose(GObject *object) {
...
static void ev_sidebar_select_page(EvSidebar *ev_sidebar, GtkTreeIter *iter) {
  char *title;
  int index;

  gtk_tree_model_get(ev_sidebar->priv->page_model, iter, PAGE_COLUMN_TITLE,
                     &title, PAGE_COLUMN_NOTEBOOK_INDEX, &index, -1);
...
void ev_sidebar_set_page(EvSidebar *ev_sidebar, GtkWidget *main_widget) {
  ...
  while (valid) {
    ...
    gtk_tree_model_get(ev_sidebar->priv->page_model, &iter,
                       PAGE_COLUMN_MAIN_WIDGET, &widget, -1);
...
static void ev_sidebar_set_property(GObject *object, guint prop_id,
...
static GtkWidget *ev_sidebar_get_current_page(EvSidebar *sidebar) {
...
static void ev_sidebar_get_property(GObject *object, guint prop_id,
...
static void ev_sidebar_class_init(EvSidebarClass *ev_sidebar_class) {
  ...
  g_object_class_install_property(
      g_object_class, PROP_CURRENT_PAGE,
      g_param_spec_object("current-page", "Current page",
                          "The currently visible page", GTK_TYPE_WIDGET,
                          G_PARAM_READWRITE));
}
...
static gboolean ev_sidebar_select_button_press_cb(GtkWidget *widget,
  ...
  if (event->button == 1) {
    ...
    gtk_menu_popup_at_widget(GTK_MENU(ev_sidebar->priv->menu), widget,
...
static gboolean ev_sidebar_select_button_key_press_cb(GtkWidget *widget,
                                                      GdkEventKey *event,
                                                      gpointer user_data) {
  EvSidebar *ev_sidebar = EV_SIDEBAR(user_data);
...
static void ev_sidebar_close_clicked_cb(GtkWidget *widget, gpointer user_data) {
  EvSidebar *ev_sidebar = EV_SIDEBAR(user_data);

  gtk_widget_hide(GTK_WIDGET(ev_sidebar));
}

static void ev_sidebar_menu_deactivate_cb(GtkWidget *widget,
                                          gpointer user_data) {
...
static void ev_sidebar_menu_detach_cb(GtkWidget *widget, GtkMenu *menu) {
  EvSidebar *ev_sidebar = EV_SIDEBAR(widget);

  ev_sidebar->priv->menu = NULL;
}

static void ev_sidebar_menu_item_activate_cb(GtkWidget *widget,
                                             gpointer user_data) {
  EvSidebar *ev_sidebar = EV_SIDEBAR(user_data);
  GtkTreeIter iter;
...
static void ev_sidebar_init(EvSidebar *ev_sidebar) {
  GtkWidget *hbox;
  ...
  gtk_orientable_set_orientation(GTK_ORIENTABLE(ev_sidebar),
                                 GTK_ORIENTATION_VERTICAL);

  /* data model */
  ev_sidebar->priv->page_model = (GtkTreeModel *)gtk_list_store_new(
      PAGE_COLUMN_NUM_COLS, G_TYPE_STRING, GTK_TYPE_WIDGET, GTK_TYPE_WIDGET,
      G_TYPE_INT);
  ...
  gtk_button_set_relief(GTK_BUTTON(ev_sidebar->priv->select_button),
                        GTK_RELIEF_NONE);
  g_signal_connect(ev_sidebar->priv->select_button, "button_press_event",
                   G_CALLBACK(ev_sidebar_select_button_press_cb), ev_sidebar);
  ...
  gtk_box_pack_start(GTK_BOX(select_hbox), ev_sidebar->priv->label, FALSE,
                     FALSE, 0);
  ...
  gtk_container_add(GTK_CONTAINER(ev_sidebar->priv->select_button),
                    select_hbox);
  gtk_widget_show(select_hbox);

  gtk_box_pack_start(GTK_BOX(hbox), ev_sidebar->priv->select_button, TRUE, TRUE,
                     0);
  ...
  g_signal_connect(close_button, "clicked",
                   G_CALLBACK(ev_sidebar_close_clicked_cb), ev_sidebar);
  ...
  gtk_menu_attach_to_widget(GTK_MENU(ev_sidebar->priv->menu),
                            GTK_WIDGET(ev_sidebar), ev_sidebar_menu_detach_cb);
  ...
  gtk_box_pack_start(GTK_BOX(ev_sidebar), ev_sidebar->priv->notebook, TRUE,
                     TRUE, 0);
...
void ev_sidebar_add_page(EvSidebar *ev_sidebar, GtkWidget *main_widget) {
  ...
  g_signal_connect(menu_item, "activate",
                   G_CALLBACK(ev_sidebar_menu_item_activate_cb), ev_sidebar);
  gtk_widget_show(menu_item);
  gtk_menu_shell_append(GTK_MENU_SHELL(ev_sidebar->priv->menu), menu_item);

  /* Insert and move to end */
  gtk_list_store_insert_with_values(
      GTK_LIST_STORE(ev_sidebar->priv->page_model), &iter, 0, PAGE_COLUMN_TITLE,
      title, PAGE_COLUMN_MENU_ITEM, menu_item, PAGE_COLUMN_MAIN_WIDGET,
      main_widget, PAGE_COLUMN_NOTEBOOK_INDEX, index, -1);
  ...
  gtk_tree_model_get(ev_sidebar->priv->page_model, &iter, PAGE_COLUMN_TITLE,
                     &label_title, PAGE_COLUMN_NOTEBOOK_INDEX, &index, -1);
...
static void ev_sidebar_document_changed_cb(EvDocumentModel *model,
                                           GParamSpec *pspec,
                                           EvSidebar *sidebar) {
  ...
  for (valid = gtk_tree_model_get_iter_first(priv->page_model, &iter); valid;
       valid = gtk_tree_model_iter_next(priv->page_model, &iter)) {
    GtkWidget *widget;
    GtkWidget *menu_widget;

    gtk_tree_model_get(priv->page_model, &iter, PAGE_COLUMN_MAIN_WIDGET,
                       &widget, PAGE_COLUMN_MENU_ITEM, &menu_widget, -1);
...
void ev_sidebar_set_model(EvSidebar *sidebar, EvDocumentModel *model) {
  ...
  if (model == sidebar->priv->model) return;

  sidebar->priv->model = model;
  g_signal_connect(model, "notify::document",
                   G_CALLBACK(ev_sidebar_document_changed_cb), sidebar);
}
