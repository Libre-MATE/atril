...
static void ev_window_set_icon_from_thumbnail(EvJobThumbnail *job,
                                              EvWindow *ev_window);
static void ev_window_save_job_cb(EvJob *save, EvWindow *window);
...
static void ev_window_cmd_view_fullscreen(GtkAction *action, EvWindow *window);
...
static void ev_window_cmd_view_presentation(GtkAction *action,
                                            EvWindow *window);
static void ev_view_popup_cmd_open_link(GtkAction *action, EvWindow *window);
...
static void ev_view_popup_cmd_save_image_as(GtkAction *action,
                                            EvWindow *window);
static void ev_view_popup_cmd_copy_image(GtkAction *action, EvWindow *window);
...
static void ev_attachment_popup_cmd_save_attachment_as(GtkAction *action,
                                                       EvWindow *window);
static void ev_window_cmd_view_fit_page(GtkAction *action, EvWindow *ev_window);
...
static void ev_window_cmd_edit_find(GtkAction *action, EvWindow *ev_window);
static void find_bar_search_changed_cb(EggFindBar *find_bar, GParamSpec *param,
                                       EvWindow *ev_window);
static void view_external_link_cb(EvWindow *window, EvLinkAction *action);

static void ev_window_load_file_remote(EvWindow *ev_window, GFile *source_file);
...
static gdouble get_monitor_dpi(EvWindow *ev_window) {
  GdkWindow *window;
  GdkMonitor *monitor;
  GdkDisplay *display;
...
static void ev_window_set_action_sensitive(EvWindow *ev_window,
                                           const char *name,
                                           gboolean sensitive) {
  G_GNUC_BEGIN_IGNORE_DEPRECATIONS;
  GtkAction *action =
      gtk_action_group_get_action(ev_window->priv->action_group, name);
...
static void ev_window_setup_action_sensitivity(EvWindow *ev_window) {
  ...
  ev_window_set_action_sensitive(ev_window, "ViewFitPage",
                                 has_pages && !(document->iswebdocument));
  ev_window_set_action_sensitive(ev_window, "ViewFitWidth",
                                 has_pages && !(document->iswebdocument));
...
static void ev_window_update_actions(EvWindow *ev_window) {
  ...
  if (view) {
    ev_window_set_action_sensitive(
        ev_window, "EditCopy", has_pages && ev_view_get_has_selection(view));
  }
#if ENABLE_EPUB
  else if (webview) {
    /*
  ...
  if (view) {
    ev_window_set_action_sensitive(
        ev_window, "F7",
        has_pages && ev_view_supports_caret_navigation(view) &&
            !presentation_mode);
  }
#if ENABLE_EPUB
  else if (webview) {
  ...
  if (has_pages && sizing_mode != EV_SIZING_FIT_WIDTH &&
...
static void set_widget_visibility(GtkWidget *widget, gboolean visible) {
...
static void update_chrome_visibility(EvWindow *window) {
...
static void update_chrome_flag(EvWindow *window, EvChrome flag,
                               gboolean active) {
  EvWindowPrivate *priv = window->priv;
...
static void update_sizing_buttons(EvWindow *window) {
...
static void update_chrome_actions(EvWindow *window) {
  ...
  g_signal_handlers_block_by_func(action, G_CALLBACK(ev_window_view_toolbar_cb),
                                  window);
...
static void ev_window_set_message_area(EvWindow *window, GtkWidget *area) {
  if (window->priv->message_area == area) return;
...
static void ev_window_message_area_response_cb(EvMessageArea *area,
                                               gint response_id,
                                               EvWindow *window) {
  ev_window_set_message_area(window, NULL);
}

static void ev_window_error_message(EvWindow *window, GError *error,
                                    const gchar *format, ...) {
  GtkWidget *area;
  va_list args;
  gchar *msg = NULL;

  if (window->priv->message_area) return;

  va_start(args, format);
  msg = g_strdup_vprintf(format, args);
  va_end(args);

  area = ev_message_area_new(GTK_MESSAGE_ERROR, msg, "gtk-close",
...
static void ev_window_warning_message(EvWindow *window, const gchar *format,
                                      ...) {
  GtkWidget *area;
  va_list args;
  gchar *msg = NULL;
  ...
  area = ev_message_area_new(GTK_MESSAGE_WARNING, msg, "gtk-close",
                             GTK_RESPONSE_CLOSE, NULL);
  g_free(msg);
...
static gboolean show_loading_message_cb(EvWindow *window) {
...
static void ev_window_show_loading_message(EvWindow *window) {
  if (window->priv->loading_message_timeout) return;
  window->priv->loading_message_timeout = g_timeout_add_full(
      G_PRIORITY_LOW, 500, (GSourceFunc)show_loading_message_cb, window, NULL);
}

static void ev_window_hide_loading_message(EvWindow *window) {
...
static gboolean ev_window_find_page_title(GtkTreeModel *tree_model,
                                          GtkTreePath *path, GtkTreeIter *iter,
                                          PageTitleData *data) {
  gchar *page_string;

  gtk_tree_model_get(tree_model, iter, EV_DOCUMENT_LINKS_COLUMN_PAGE_LABEL,
                     &page_string, -1);

  if (!page_string) return FALSE;

  if (!strcmp(page_string, data->page_label)) {
    gtk_tree_model_get(tree_model, iter, EV_DOCUMENT_LINKS_COLUMN_MARKUP,
                       &data->page_title, -1);
    g_free(page_string);
    return TRUE;
  }
...
static gchar *ev_window_get_page_title(EvWindow *window,
                                       const gchar *page_label) {
  if (EV_IS_DOCUMENT_LINKS(window->priv->document) &&
    ...
    if (model) {
      gtk_tree_model_foreach(
          model, (GtkTreeModelForeachFunc)ev_window_find_page_title, &data);
...
static void ev_window_add_history(EvWindow *window, gint page, EvLink *link) {
  ...
  if (window->priv->history == NULL) return;

  if (!EV_IS_DOCUMENT_LINKS(window->priv->document)) return;
...
static void view_caret_cursor_moved_cb(EvView *view, guint page, guint offset,
                                       EvWindow *window) {
  GVariant *position;
  gchar *caret_position;

  if (!window->priv->metadata) return;

  position = g_variant_new("(uu)", page, offset);
  caret_position = g_variant_print(position, FALSE);
  g_variant_unref(position);

  ev_metadata_set_string(window->priv->metadata, "caret-position",
                         caret_position);
  g_free(caret_position);
}

static void view_is_loading_changed_cb(EvView *view, GParamSpec *spec,
                                       EvWindow *window) {
...
static void update_document_mode(EvWindow *window, EvDocumentMode mode) {
...
static void ev_window_init_metadata_with_default_values(EvWindow *window) {
  ...
  if (!ev_metadata_has_key(metadata, "sizing_mode")) {
    ...
    GEnumValue *enum_value =
        g_enum_get_value(g_type_class_peek(EV_TYPE_SIZING_MODE), mode);
...
static void setup_chrome_from_metadata(EvWindow *window) {
  ...
  if (ev_metadata_get_boolean(window->priv->metadata, "show_toolbar",
                              &show_toolbar))
...
static void setup_sidebar_from_metadata(EvWindow *window) {
  gchar *page_id;
  gint sidebar_size;
...
static void setup_model_from_metadata(EvWindow *window) {
  gint page;
  gchar *sizing_mode;
  gdouble zoom;
  gint rotation;
  ...
  if (ev_metadata_get_boolean(window->priv->metadata, "fullscreen",
                              &fullscreen)) {
    if (fullscreen) ev_window_run_fullscreen(window);
  }
}

static void monitor_get_dimesions(EvWindow *ev_window, gint *width,
                                  gint *height) {
  GdkDisplay *display;
...
static void setup_document_from_metadata(EvWindow *window) {
  ...
  if (n_pages == 1) ev_document_model_set_dual_page(window->priv->model, FALSE);
...
static void setup_size_from_metadata(EvWindow *window) {
  gint width;
  gint height;
  gboolean maximized;
  gint x;
...
static void setup_view_from_metadata(EvWindow *window) {
  ...
  if (ev_view_supports_caret_navigation(EV_VIEW(window->priv->view))) {
    ...
    if (ev_metadata_get_string(window->priv->metadata, "caret-position",
                               &caret_position)) {
      GVariant *position;

      position = g_variant_parse(G_VARIANT_TYPE("(uu)"), caret_position, NULL,
                                 NULL, NULL);
      if (position) {
        guint page, offset;
        ...
        ev_view_set_caret_cursor_position(EV_VIEW(window->priv->view), page,
                                          offset);
      }
    }

    if (ev_metadata_get_boolean(window->priv->metadata, "caret-navigation",
                                &caret_navigation))
      ev_view_set_caret_navigation_enabled(EV_VIEW(window->priv->view),
                                           caret_navigation);
  }
#endif
}

static void page_cache_size_changed(GSettings *settings, gchar *key,
                                    EvWindow *ev_window) {
  guint page_cache_mb;
...
static void ev_window_setup_default(EvWindow *ev_window) {
  ...
  ev_document_model_set_continuous(
      model, g_settings_get_boolean(settings, "continuous"));
  ev_document_model_set_dual_page(
      model, g_settings_get_boolean(settings, "dual-page"));
  ev_document_model_set_dual_page_odd_pages_left(
      model, g_settings_get_boolean(settings, "dual-page-odd-left"));
  ev_document_model_set_inverted_colors(
      model, g_settings_get_boolean(settings, "inverted-colors"));
  ev_document_model_set_sizing_mode(
      model, g_settings_get_enum(settings, "sizing-mode"));
...
static void ev_window_clear_thumbnail_job(EvWindow *ev_window) {
...
static void ev_window_refresh_window_thumbnail(EvWindow *ev_window) {
  gdouble page_width;
  gdouble scale;
  gint rotation;
  EvDocument *document = ev_window->priv->document;
...
static void ev_window_setup_toolbar_flags(EvWindow *ev_window) {
  EggTbModelFlags flags =
      egg_toolbars_model_get_flags(ev_window->priv->toolbars_model, 0);
  char *style;
  if (ev_window->priv->interface_settings &&
      (style = g_settings_get_string(ev_window->priv->interface_settings,
                                     MATE_INTERFACE_TB_STYLE))) {
...
static void override_restrictions_changed(GSettings *settings, gchar *key,
                                          EvWindow *ev_window) {
  ev_window_setup_action_sensitivity(ev_window);
}

static void lockdown_changed(GSettings *settings, gchar *key,
                             EvWindow *ev_window) {
  ev_window_setup_action_sensitivity(ev_window);
}
...
static GSettings *ev_window_ensure_settings(EvWindow *ev_window) {
  ...
  if (priv->settings != NULL) return priv->settings;
...
static void interface_changed(GSettings *settings, gchar *key,
                              EvWindow *ev_window) {
  ev_window_setup_toolbar_flags(ev_window);
}

static gboolean ev_window_setup_document(EvWindow *ev_window) {
  ...
  GSettingsSchema *schema_mate_lockdown_schema =
      g_settings_schema_source_lookup(g_settings_schema_source_get_default(),
                                      MATE_LOCKDOWN_SCHEMA, FALSE);
  if (schema_mate_lockdown_schema != NULL) {
    ...
    g_signal_connect(ev_window->priv->lockdown_settings, "changed",
                     G_CALLBACK(lockdown_changed), ev_window);
  }

  ev_window_setup_action_sensitivity(ev_window);

  if (ev_window->priv->history) g_object_unref(ev_window->priv->history);
  ev_window->priv->history = ev_history_new();
  G_GNUC_BEGIN_IGNORE_DEPRECATIONS;
  action = gtk_action_group_get_action(ev_window->priv->action_group,
                                       NAVIGATION_ACTION);
  G_GNUC_END_IGNORE_DEPRECATIONS;
  ev_navigation_action_set_history(EV_NAVIGATION_ACTION(action),
                                   ev_window->priv->history);
...
static void ev_window_set_document(EvWindow *ev_window, EvDocument *document) {
  if (ev_window->priv->document == document) return;

  if (ev_window->priv->document) g_object_unref(ev_window->priv->document);
  ...
  if (document->iswebdocument == TRUE && ev_window->priv->view != NULL) {
    /*We have encountered a web document, replace the atril view with a web
...
static void ev_window_document_changed(EvWindow *ev_window,
                                       gpointer user_data) {
...
static void ev_window_password_view_unlock(EvWindow *ev_window) {
...
static void ev_window_clear_reload_job(EvWindow *ev_window) {
...
static void ev_window_clear_local_uri(EvWindow *ev_window) {
...
static void ev_window_handle_link(EvWindow *ev_window, EvLinkDest *dest) {
  if (ev_window->priv->document &&
      ev_window->priv->document->iswebdocument == TRUE) {
    return;
  }
...
static void ev_window_reload_job_cb(EvJob *job, EvWindow *ev_window) {
  GtkWidget *widget;
 ...
 */
static void ev_window_close_dialogs(EvWindow *ev_window) {
...
static void ev_window_clear_progress_idle(EvWindow *ev_window) {
  if (ev_window->priv->progress_idle > 0)
    g_source_remove(ev_window->priv->progress_idle);
  ev_window->priv->progress_idle = 0;
}

static void reset_progress_idle(EvWindow *ev_window) {
  ev_window->priv->progress_idle = 0;
}

static void ev_window_show_progress_message(EvWindow *ev_window, guint interval,
...
static void ev_window_reset_progress_cancellable(EvWindow *ev_window) {
...
static void ev_window_progress_response_cb(EvProgressMessageArea *area,
...
static gboolean show_loading_progress(EvWindow *ev_window) {
  GtkWidget *area;
  gchar *text;
  gchar *display_name;

  if (ev_window->priv->message_area) return FALSE;
  ...
  area = ev_progress_message_area_new("gtk-open", text, "gtk-close",
                                      GTK_RESPONSE_CLOSE, "gtk-cancel",
...
static GDateTime *_g_file_info_get_modification_date_time(GFileInfo *info) {
  GTimeVal mtime;

  g_file_info_get_modification_time(info, &mtime);
  return g_date_time_new_from_timeval_utc(&mtime);
}
#define g_file_info_get_modification_date_time \
  _g_file_info_get_modification_date_time
#endif

static void ev_window_load_remote_failed(EvWindow *ev_window, GError *error) {
...
static void set_uri_mtime(GFile *source, GAsyncResult *async_result,
                          EvWindow *ev_window) {
  GFileInfo *info;
  gint64 utime = -1;
...
static void mount_volume_ready_cb(GFile *source, GAsyncResult *async_result,
                                  EvWindow *ev_window) {
  GError *error = NULL;
...
static void window_open_file_copy_ready_cb(GFile *source,
...
static void window_open_file_copy_progress_cb(goffset n_bytes,
                                              goffset total_bytes,
                                              EvWindow *ev_window) {
  gchar *status;
  gdouble fraction;

  if (!ev_window->priv->message_area) return;
...
void ev_window_open_uri(EvWindow *ev_window, const char *uri, EvLinkDest *dest,
  ...
  if (ev_window->priv->uri) g_free(ev_window->priv->uri);
  ev_window->priv->uri = g_strdup(uri);

  if (ev_window->priv->metadata) g_object_unref(ev_window->priv->metadata);
  if (ev_window->priv->bookmarks) g_object_unref(ev_window->priv->bookmarks);
  ...
  if (ev_window->priv->search_string) g_free(ev_window->priv->search_string);
  ev_window->priv->search_string =
      search_string ? g_strdup(search_string) : NULL;

  if (ev_window->priv->dest) g_object_unref(ev_window->priv->dest);
...
static void ev_window_reload_local(EvWindow *ev_window) {
  const gchar *uri;
...
static gboolean show_reloading_progress(EvWindow *ev_window) {
  ...
  area = ev_progress_message_area_new("gtk-refresh", text, "gtk-close",
                                      GTK_RESPONSE_CLOSE, "gtk-cancel",
                                      GTK_RESPONSE_CANCEL, NULL);
  g_signal_connect(area, "response", G_CALLBACK(ev_window_progress_response_cb),
                   ev_window);
...
static void reload_remote_copy_progress_cb(goffset n_bytes, goffset total_bytes,
                                           EvWindow *ev_window) {
  gchar *status;
  gdouble fraction;
...
static void query_remote_uri_mtime_cb(GFile *remote, GAsyncResult *async_result,
                                      EvWindow *ev_window) {
  GFileInfo *info;
  GDateTime *mtime;
  GError *error = NULL;
  ...
  if (ev_window->priv->uri_mtime != utime) {
    ...
    target_file = g_file_new_for_uri(ev_window->priv->local_uri);
    g_file_copy_async(
        remote, target_file, G_FILE_COPY_OVERWRITE, G_PRIORITY_DEFAULT,
        ev_window->priv->progress_cancellable,
        (GFileProgressCallback)reload_remote_copy_progress_cb, ev_window,
        (GAsyncReadyCallback)reload_remote_copy_ready_cb, ev_window);
    g_object_unref(target_file);
    ev_window_show_progress_message(ev_window, 1,
...
static void ev_window_reload_remote(EvWindow *ev_window) {
  GFile *remote;
...
static void ev_window_reload_document(EvWindow *ev_window, EvLinkDest *dest) {
  ev_window_clear_reload_job(ev_window);
...
static const gchar *get_settings_key_for_directory(GUserDirectory directory) {
  switch (directory) {
...
static void ev_window_file_chooser_restore_folder(EvWindow *window,
...
static void ev_window_file_chooser_save_folder(EvWindow *window,
                                               GtkFileChooser *file_chooser,
                                               GUserDirectory directory) {
  gchar *uri, *folder;
...
static void file_open_dialog_response_cb(GtkWidget *chooser, gint response_id,
                                         EvWindow *ev_window) {
...
static void ev_window_cmd_file_open(GtkAction *action, EvWindow *window) {
  GtkWidget *chooser;

  chooser = gtk_file_chooser_dialog_new(
      _("Open Document"), GTK_WINDOW(window), GTK_FILE_CHOOSER_ACTION_OPEN,
...
static void ev_window_open_copy_at_dest(EvWindow *window, EvLinkDest *dest) {
...
static void ev_window_cmd_file_open_copy(GtkAction *action, EvWindow *window) {
  ev_window_open_copy_at_dest(window, NULL);
}

static void ev_window_cmd_recent_file_activate(GtkAction *action,
                                               EvWindow *window) {
  GtkRecentInfo *info;
  const gchar *uri;
...
static void ev_window_open_recent_action_item_activated(
    EvOpenRecentAction *action, const gchar *uri, EvWindow *window) {
  ev_application_open_uri_at_dest(EV_APP, uri,
                                  gtk_window_get_screen(GTK_WINDOW(window)),
                                  NULL, 0, NULL, gtk_get_current_event_time());
}
 ...
 */
static gchar *ev_window_get_recent_file_label(gint index,
...
static void ev_window_recent_action_connect_proxy_cb(
    GtkActionGroup *action_group, GtkAction *action, GtkWidget *proxy,
    gpointer data) {
  GtkLabel *label;

  if (!GTK_IS_MENU_ITEM(proxy)) return;
...
static void ev_window_setup_recent(EvWindow *ev_window) {
  GList *items, *l;
  guint n_items = 0;
  const gchar *atril = g_get_application_name();
  static guint i = 0;
  ...
  ev_window->priv->recent_ui_id =
      gtk_ui_manager_new_merge_id(ev_window->priv->ui_manager);
  ...
  G_GNUC_BEGIN_IGNORE_DEPRECATIONS;
  ev_window->priv->recent_action_group =
      gtk_action_group_new("RecentFilesActions");
  G_GNUC_END_IGNORE_DEPRECATIONS;
  ...
  for (l = items; l && l->data; l = g_list_next(l)) {
    ...
    G_GNUC_BEGIN_IGNORE_DEPRECATIONS;
    action = g_object_new(GTK_TYPE_ACTION, "name", action_name, "label", label,
                          "gicon", icon, "always-show-image", TRUE, NULL);
    G_GNUC_END_IGNORE_DEPRECATIONS;
    ...
    G_GNUC_BEGIN_IGNORE_DEPRECATIONS;
    gtk_action_group_add_action(ev_window->priv->recent_action_group, action);
    G_GNUC_END_IGNORE_DEPRECATIONS;
    g_object_unref(action);

    gtk_ui_manager_add_ui(ev_window->priv->ui_manager,
                          ev_window->priv->recent_ui_id,
                          "/MainMenu/FileMenu/RecentFilesMenu", label,
...
static gboolean show_saving_progress(GFile *dst) {
  EvWindow *ev_window;
  GtkWidget *area;
  gchar *text;
  gchar *uri;
  ...
  g_free(uri);
  area = ev_progress_message_area_new("gtk-save", text, "gtk-close",
                                      GTK_RESPONSE_CLOSE, "gtk-cancel",
                                      GTK_RESPONSE_CANCEL, NULL);
  g_signal_connect(area, "response", G_CALLBACK(ev_window_progress_response_cb),
                   ev_window);
...
static void window_save_file_copy_ready_cb(GFile *src,
                                           GAsyncResult *async_result,
                                           GFile *dst) {
  EvWindow *ev_window;
  GError *error = NULL;
  ...
  if (!g_error_matches(error, G_IO_ERROR, G_IO_ERROR_CANCELLED)) {
    ...
    ev_window_error_message(ev_window, error,
                            _("The file could not be saved as “%s”."), name);
    g_free(name);
  }
  ev_tmp_file_unlink(src);
  g_error_free(error);
...
static void window_save_file_copy_progress_cb(goffset n_bytes,
                                              goffset total_bytes, GFile *dst) {
  EvWindow *ev_window;
  EvSaveType save_type;
  gchar *status;
  gdouble fraction;
  ...
  switch (save_type) {
    ...
    case EV_SAVE_IMAGE:
      status =
          g_strdup_printf(_("Uploading image (%d%%)"), (gint)(fraction * 100));
      break;
    default:
      g_assert_not_reached();
  }
...
static void ev_window_save_remote(EvWindow *ev_window, EvSaveType save_type,
                                  GFile *src, GFile *dst) {
  ev_window_reset_progress_cancellable(ev_window);
  g_object_set_data(G_OBJECT(dst), "ev-window", ev_window);
  g_object_set_data(G_OBJECT(dst), "save-type", GINT_TO_POINTER(save_type));
  g_file_copy_async(src, dst, G_FILE_COPY_OVERWRITE, G_PRIORITY_DEFAULT,
                    ev_window->priv->progress_cancellable,
                    (GFileProgressCallback)window_save_file_copy_progress_cb,
                    dst, (GAsyncReadyCallback)window_save_file_copy_ready_cb,
                    dst);
  ev_window->priv->progress_idle = g_timeout_add_seconds_full(
      G_PRIORITY_DEFAULT, 1, (GSourceFunc)show_saving_progress, dst, NULL);
}

static void ev_window_clear_save_job(EvWindow *ev_window) {
...
static void ev_window_save_job_cb(EvJob *job, EvWindow *window) {
  if (ev_job_is_failed(job)) {
...
static void file_save_dialog_response_cb(GtkWidget *fc, gint response_id,
                                         EvWindow *ev_window) {
  gchar *uri;
...
static void ev_window_cmd_save_as(GtkAction *action, EvWindow *ev_window) {
  GtkWidget *fc;
  ...
  fc = gtk_file_chooser_dialog_new(
      _("Save As…"), GTK_WINDOW(ev_window), GTK_FILE_CHOOSER_ACTION_SAVE,
      "gtk-cancel", GTK_RESPONSE_CANCEL, "gtk-save", GTK_RESPONSE_OK, NULL);
  ...
  default_dir =
      g_file_test(documents_dir, G_FILE_TEST_EXISTS | G_FILE_TEST_IS_DIR)
          ? documents_dir
          : g_get_home_dir();
  ...
  gtk_file_chooser_set_current_folder(GTK_FILE_CHOOSER(fc), dest_dir);
  ...
  g_signal_connect(fc, "response", G_CALLBACK(file_save_dialog_response_cb),
                   ev_window);

  gtk_widget_show(fc);
}

static void ev_window_cmd_send_to(GtkAction *action, EvWindow *ev_window) {
  GAppInfo *app_info;
  gchar *command;
  const char *uri;
  char *unescaped_uri;
...
static void save_print_setting_file(GKeyFile *key_file) {
  gchar *filename;
  gchar *data;
  gsize data_length;
  GError *error = NULL;
...
static void ev_window_save_print_settings(EvWindow *window,
                                          GtkPrintSettings *print_settings) {
  GKeyFile *key_file;
  gint i;
...
static void ev_window_load_print_settings_from_metadata(
    EvWindow *window, GtkPrintSettings *print_settings) {
  gint i;
...
static void ev_window_load_print_page_setup_from_metadata(
    EvWindow *window, GtkPageSetup *page_setup) {
  gint int_value;
...
static void ev_window_print_cancel(EvWindow *ev_window) {
  EvPrintOperation *op;

  if (!ev_window->priv->print_queue) return;
...
static void ev_window_print_update_pending_jobs_message(EvWindow *ev_window,
                                                        gint n_jobs) {
  gchar *text = NULL;
  ...
  if (n_jobs > 1) {
    ...
    text =
        g_strdup_printf(g_dngettext(GETTEXT_PACKAGE, "%lu pending job in queue",
                                    "%lu pending jobs in queue", n),
                        n);
  }
...
static gboolean destroy_window(GtkWidget *window) {
  gtk_widget_destroy(window);

  return FALSE;
}

static void ev_window_print_operation_done(EvPrintOperation *op,
...
static void ev_window_print_progress_response_cb(EvProgressMessageArea *area,
                                                 gint response,
                                                 EvWindow *ev_window) {
  if (response == GTK_RESPONSE_CANCEL) {
    EvPrintOperation *op;
...
static void ev_window_print_operation_status_changed(EvPrintOperation *op,
                                                     EvWindow *ev_window) {
  const gchar *status;
  gdouble fraction;
  ...
  if (!ev_window->priv->message_area) {
    ...
    area = ev_progress_message_area_new("gtk-print", text, "gtk-close",
                                        GTK_RESPONSE_CLOSE, "gtk-cancel",
                                        GTK_RESPONSE_CANCEL, NULL);
...
static void ev_window_print_operation_begin_print(EvPrintOperation *op,
                                                  EvWindow *ev_window) {
...
void ev_window_print_range(EvWindow *ev_window, gint first_page,
  ...
  if (ev_window->priv->lockdown_settings)
    ev_print_operation_set_embed_page_setup(
        op, !g_settings_get_boolean(ev_window->priv->lockdown_settings,
...
static void ev_window_print(EvWindow *window) {
  ev_window_print_range(window, 1,
                        ev_document_get_n_pages(window->priv->document));
}

static void ev_window_cmd_file_print(GtkAction *action, EvWindow *ev_window) {
  ev_window_print(ev_window);
}

static void ev_window_cmd_file_properties(GtkAction *action,
                                          EvWindow *ev_window) {
...
static void document_modified_confirmation_dialog_response(
    GtkDialog *dialog, gint response, EvWindow *ev_window) {
...
static gboolean ev_window_check_document_modified(EvWindow *ev_window) {
  ...
  gtk_dialog_add_buttons(GTK_DIALOG(dialog), _("Close _without Saving"),
                         GTK_RESPONSE_NO, "gtk-cancel", GTK_RESPONSE_CANCEL,
                         _("_Save As…"), GTK_RESPONSE_YES, NULL);
...
static void print_jobs_confirmation_dialog_response(GtkDialog *dialog,
                                                    gint response,
                                                    EvWindow *ev_window) {
  gtk_widget_destroy(GTK_WIDGET(dialog));
...
static gboolean ev_window_check_print_queue(EvWindow *ev_window) {
  GtkWidget *dialog;
  gchar *text, *markup;
  gint n_print_jobs;
  ...
  if (n_print_jobs == 0) return FALSE;
  ...
  gtk_dialog_add_buttons(GTK_DIALOG(dialog), _("Cancel _print and Close"),
                         GTK_RESPONSE_NO, "gtk-cancel", GTK_RESPONSE_CANCEL,
                         _("Close _after Printing"), GTK_RESPONSE_YES, NULL);
  gtk_dialog_set_default_response(GTK_DIALOG(dialog), GTK_RESPONSE_YES);
...
static gboolean ev_window_close(EvWindow *ev_window) {
  if (EV_WINDOW_IS_PRESENTATION(ev_window)) {
    gint current_page;
  ...
  if (ev_window_check_document_modified(ev_window)) return FALSE;

  if (ev_window_check_print_queue(ev_window)) return FALSE;

  return TRUE;
}

static void ev_window_cmd_file_close_window(GtkAction *action,
                                            EvWindow *ev_window) {
  if (ev_window_close(ev_window)) gtk_widget_destroy(GTK_WIDGET(ev_window));
}

static void ev_window_cmd_focus_page_selector(GtkAction *act,
                                              EvWindow *window) {
  GtkAction *action;
...
static void ev_window_cmd_scroll_forward(GtkAction *action, EvWindow *window) {
...
static void ev_window_cmd_scroll_backward(GtkAction *action, EvWindow *window) {
  /* If the webview is occupying the window */
  if (window->priv->document && window->priv->document->iswebdocument == TRUE)
    return;
...
static void ev_window_cmd_continuous(GtkAction *action, EvWindow *ev_window) {
  gboolean continuous;
...
static void ev_window_cmd_dual(GtkAction *action, EvWindow *ev_window) {
  gboolean dual_page;
...
static void ev_window_cmd_dual_odd_pages_left(GtkAction *action,
                                              EvWindow *ev_window) {
...
static void ev_window_cmd_view_fit_page(GtkAction *action,
  ...
  if (gtk_toggle_action_get_active(GTK_TOGGLE_ACTION(action))) {
    ev_document_model_set_sizing_mode(ev_window->priv->model,
                                      EV_SIZING_FIT_PAGE);
  } else {
...
static void ev_window_cmd_fit_page(GtkAction *action, EvWindow *ev_window) {
...
static void ev_window_cmd_fit_width(GtkAction *action, EvWindow *ev_window) {
  ev_document_model_set_sizing_mode(ev_window->priv->model,
                                    EV_SIZING_FIT_WIDTH);
}

static void ev_window_cmd_edit_select_all(GtkAction *action,
                                          EvWindow *ev_window) {
  g_return_if_fail(EV_IS_WINDOW(ev_window));
...
static void ev_window_cmd_edit_find(GtkAction *action, EvWindow *ev_window) {
  if (ev_window->priv->document == NULL ||
    ...
    g_error(
        "Find action should be insensitive since document doesn't support "
        "find");
    return;
  }

  if (EV_WINDOW_IS_PRESENTATION(ev_window)) return;
...
static void ev_window_cmd_edit_find_next(GtkAction *action,
                                         EvWindow *ev_window) {
  if (EV_WINDOW_IS_PRESENTATION(ev_window)) return;
...
static void ev_window_cmd_edit_find_previous(GtkAction *action,
                                             EvWindow *ev_window) {
  if (EV_WINDOW_IS_PRESENTATION(ev_window)) return;
...
static void ev_window_cmd_edit_copy(GtkAction *action, EvWindow *ev_window) {
  g_return_if_fail(EV_IS_WINDOW(ev_window));
#if ENABLE_EPUB
...
static void ev_window_sidebar_position_change_cb(GObject *object,
                                                 GParamSpec *pspec,
                                                 EvWindow *ev_window) {
  if (ev_window->priv->metadata && !ev_window_is_empty(ev_window))
...
static void ev_window_update_fullscreen_action(EvWindow *window) {
  GtkAction *action;

  G_GNUC_BEGIN_IGNORE_DEPRECATIONS;
  action =
      gtk_action_group_get_action(window->priv->action_group, "ViewFullscreen");
  g_signal_handlers_block_by_func(
...
static void fullscreen_toolbar_setup_item_properties(GtkUIManager *ui_manager) {
  GtkWidget *item;

  item = gtk_ui_manager_get_widget(ui_manager,
                                   "/FullscreenToolbar/GoPreviousPage");
  ...
  item = gtk_ui_manager_get_widget(ui_manager,
                                   "/FullscreenToolbar/StartPresentation");
  g_object_set(item, "is-important", TRUE, NULL);

  item = gtk_ui_manager_get_widget(ui_manager,
                                   "/FullscreenToolbar/LeaveFullscreen");
  g_object_set(item, "is-important", TRUE, NULL);
}
...
static void ev_window_run_fullscreen(EvWindow *window) {
  ...
  if (ev_document_model_get_fullscreen(window->priv->model)) return;

  if (!window->priv->fullscreen_toolbar) {
    window->priv->fullscreen_toolbar = gtk_ui_manager_get_widget(
        window->priv->ui_manager, "/FullscreenToolbar");
    ...
    gtk_box_pack_start(GTK_BOX(window->priv->main_box),
                       window->priv->fullscreen_toolbar, FALSE, FALSE, 0);
  ...
  g_object_set(G_OBJECT(window->priv->scrolled_window), "shadow-type",
  ...
  if (fullscreen_window) gtk_window_fullscreen(GTK_WINDOW(window));
  if (window->priv->view) {
...
static void ev_window_inhibit_screensaver(EvWindow *window) {
  EvWindowPrivate *priv = window->priv;

  if (priv->presentation_mode_inhibit_id != 0) return;

  priv->presentation_mode_inhibit_id = gtk_application_inhibit(
      GTK_APPLICATION(g_application_get_default()), GTK_WINDOW(window),
      GTK_APPLICATION_INHIBIT_IDLE, _("Running in presentation mode"));
}

static void ev_window_uninhibit_screensaver(EvWindow *window) {
  EvWindowPrivate *priv = window->priv;
...
static void ev_window_update_presentation_action(EvWindow *window) {
  GtkAction *action;
...
static void ev_window_view_presentation_finished(EvWindow *window) {
  ev_window_stop_presentation(window, TRUE);
}

static gboolean ev_window_view_presentation_focus_in(EvWindow *window) {
  ev_window_uninhibit_screensaver(window);

  return FALSE;
}

static gboolean ev_window_view_presentation_focus_out(EvWindow *window) {
  ev_window_uninhibit_screensaver(window);
...
static void ev_window_setup_gtk_settings(EvWindow *window) {
...
static void ev_window_update_max_min_scale(EvWindow *window) {
  ...
  if (!window->priv->document) return;
  ...
  action = gtk_action_group_get_action(window->priv->action_group,
                                       ZOOM_CONTROL_ACTION);
  G_GNUC_END_IGNORE_DEPRECATIONS;
  ephy_zoom_action_set_max_zoom_level(EPHY_ZOOM_ACTION(action),
                                      max_scale * dpi);
...
static void ev_window_screen_changed(GtkWidget *widget, GdkScreen *old_screen) {
  EvWindow *window = EV_WINDOW(widget);
  GdkScreen *screen;

  screen = gtk_widget_get_screen(widget);
  if (screen == old_screen) return;
...
static gboolean ev_window_state_event(GtkWidget *widget,
  ...
  if ((event->changed_mask & GDK_WINDOW_STATE_FULLSCREEN) == 0) return FALSE;
...
static void ev_window_set_page_mode(EvWindow *window,
  ...
  if (window->priv->page_mode == page_mode) return;
...
static void ev_window_cmd_edit_rotate_left(GtkAction *action,
                                           EvWindow *ev_window) {
  gint rotation;
...
static void ev_window_cmd_edit_rotate_right(GtkAction *action,
                                            EvWindow *ev_window) {
  gint rotation;
...
static void ev_window_cmd_view_inverted_colors(GtkAction *action,
                                               EvWindow *ev_window) {
  gboolean inverted_colors =
      ev_document_model_get_inverted_colors(ev_window->priv->model);

  ev_document_model_set_inverted_colors(ev_window->priv->model,
                                        !inverted_colors);
}

static void ev_window_cmd_edit_toolbar_cb(GtkDialog *dialog, gint response,
                                          EvWindow *ev_window) {
...
static void ev_window_cmd_edit_toolbar(GtkAction *action, EvWindow *ev_window) {
  GtkWidget *dialog;
  GtkWidget *editor;
  ...
  dialog = gtk_dialog_new_with_buttons(
      _("Toolbar Editor"), GTK_WINDOW(ev_window),
  ...
  g_signal_connect(dialog, "response",
                   G_CALLBACK(ev_window_cmd_edit_toolbar_cb), ev_window);
  gtk_widget_show_all(dialog);
}

static void ev_window_cmd_edit_save_settings(GtkAction *action,
                                             EvWindow *ev_window) {
  EvWindowPrivate *priv = ev_window->priv;
...
static void ev_window_cmd_view_zoom_in(GtkAction *action, EvWindow *ev_window) {
  g_return_if_fail(EV_IS_WINDOW(ev_window));
...
static void ev_window_cmd_view_zoom_out(GtkAction *action,
                                        EvWindow *ev_window) {
  g_return_if_fail(EV_IS_WINDOW(ev_window));
...
static void ev_window_cmd_view_zoom_reset(GtkAction *action,
                                          EvWindow *ev_window) {
  g_return_if_fail(EV_IS_WINDOW(ev_window));
...
static void ev_window_cmd_go_previous_page(GtkAction *action,
                                           EvWindow *ev_window) {
  g_return_if_fail(EV_IS_WINDOW(ev_window));
#if ENABLE_EPUB
  if (ev_window->priv->document &&
...
static void ev_window_cmd_go_next_page(GtkAction *action, EvWindow *ev_window) {
  g_return_if_fail(EV_IS_WINDOW(ev_window));
#if ENABLE_EPUB
  if (ev_window->priv->document &&
      ev_window->priv->document->iswebdocument == TRUE) {
...
static void ev_window_cmd_go_first_page(GtkAction *action,
                                        EvWindow *ev_window) {
  g_return_if_fail(EV_IS_WINDOW(ev_window));
...
static void ev_window_cmd_go_last_page(GtkAction *action, EvWindow *ev_window) {
  g_return_if_fail(EV_IS_WINDOW(ev_window));
...
static void ev_window_cmd_go_forward(GtkAction *action, EvWindow *ev_window) {
...
static void ev_window_cmd_go_backward(GtkAction *action, EvWindow *ev_window) {
  int current_page;
...
static void ev_window_cmd_bookmark_activate(GtkAction *action,
                                            EvWindow *window) {
...
static void ev_window_setup_bookmarks(EvWindow *window) {
  ...
  if (!window->priv->bookmarks) return;
  ...
  window->priv->bookmarks_ui_id =
      gtk_ui_manager_new_merge_id(window->priv->ui_manager);
  ...
  G_GNUC_BEGIN_IGNORE_DEPRECATIONS;
  window->priv->bookmarks_action_group =
      gtk_action_group_new("BookmarksActions");
  G_GNUC_END_IGNORE_DEPRECATIONS;
  ...
  for (l = items; l && l->data; l = g_list_next(l)) {
    ...
    g_signal_connect(action, "activate",
                     G_CALLBACK(ev_window_cmd_bookmark_activate), window);
    G_GNUC_BEGIN_IGNORE_DEPRECATIONS;
    gtk_action_group_add_action(window->priv->bookmarks_action_group, action);

    action_name = gtk_action_get_name(action);
    action_label = gtk_action_get_label(action);
    G_GNUC_END_IGNORE_DEPRECATIONS;
    gtk_ui_manager_add_ui(
        window->priv->ui_manager, window->priv->bookmarks_ui_id,
        "/MainMenu/BookmarksMenu/BookmarksItems", action_label, action_name,
...
static void ev_window_cmd_bookmarks_add(GtkAction *action, EvWindow *window) {
  EvBookmark bm;
  gchar *page_label;
  gchar *page_title;
  ...
  page_title = ev_window_get_page_title(window, page_label);
  bm.title =
      page_title ? page_title : g_strdup_printf(_("Page %s"), page_label);
  g_free(page_label);
...
static void ev_window_cmd_view_reload(GtkAction *action, EvWindow *ev_window) {
  ev_window_reload_document(ev_window, NULL);
}
...
static void ev_window_cmd_view_autoscroll(GtkAction *action,
                                          EvWindow *ev_window) {
  EvDocument *document = ev_window->priv->document;
...
static void ev_window_cmd_help_contents(GtkAction *action,
                                        EvWindow *ev_window) {
  GError *error = NULL;

  gtk_show_uri_on_window(GTK_WINDOW(ev_window), EV_HELP,
...
static void ev_window_cmd_leave_fullscreen(GtkAction *action,
                                           EvWindow *window) {
  ev_window_stop_fullscreen(window, TRUE);
}

static void ev_window_cmd_start_presentation(GtkAction *action,
                                             EvWindow *window) {
  ev_window_run_presentation(window);
}

static void ev_window_cmd_escape(GtkAction *action, EvWindow *window) {
  GtkWidget *widget;
  ...
  if (widget && gtk_widget_get_ancestor(widget, EGG_TYPE_FIND_BAR)) {
    ...
    if (window->priv->view) gtk_widget_grab_focus(window->priv->view);
#if ENABLE_EPUB
    else
      gtk_widget_grab_focus(window->priv->webview);
#endif
...
static void save_sizing_mode(EvWindow *window) {
  EvSizingMode mode;
  GEnumValue *enum_value;

  if (!window->priv->metadata || ev_window_is_empty(window)) return;
...
static void ev_window_sizing_mode_changed_cb(EvDocumentModel *model,
  ...
  g_object_set(
      ev_window->priv->scrolled_window, "hscrollbar-policy",
      sizing_mode == EV_SIZING_FREE ? GTK_POLICY_AUTOMATIC : GTK_POLICY_NEVER,
...
static void ev_window_zoom_changed_cb(EvDocumentModel *model, GParamSpec *pspec,
  ...
  if (!ev_window->priv->metadata) return;
...
static void ev_window_update_continuous_action(EvWindow *window) {
  GtkAction *action;

  G_GNUC_BEGIN_IGNORE_DEPRECATIONS;
  action =
      gtk_action_group_get_action(window->priv->action_group, "ViewContinuous");
  g_signal_handlers_block_by_func(action, G_CALLBACK(ev_window_cmd_continuous),
                                  window);
  gtk_toggle_action_set_active(
      GTK_TOGGLE_ACTION(action),
...
static void ev_window_update_inverted_colors_action(EvWindow *window) {
  GtkAction *action;
...
static void ev_window_update_dual_page_action(EvWindow *window) {
  GtkAction *action;

  G_GNUC_BEGIN_IGNORE_DEPRECATIONS;
  action = gtk_action_group_get_action(window->priv->action_group, "ViewDual");
  g_signal_handlers_block_by_func(action, G_CALLBACK(ev_window_cmd_dual),
                                  window);
  gtk_toggle_action_set_active(
      GTK_TOGGLE_ACTION(action),
      ev_document_model_get_dual_page(window->priv->model));
  G_GNUC_END_IGNORE_DEPRECATIONS;
  g_signal_handlers_unblock_by_func(action, G_CALLBACK(ev_window_cmd_dual),
                                    window);
}
...
static void ev_window_update_dual_page_odd_pages_left_action(EvWindow *window) {
  GtkAction *action;
...
static void ev_window_dual_mode_odd_pages_left_changed_cb(
    EvDocumentModel *model, GParamSpec *pspec, EvWindow *ev_window) {
...
static char *build_comments_string(EvDocument *document) {
  gchar *comments;
  EvDocumentBackendInfo info;

  if (document && ev_document_get_backend_info(document, &info)) {
    comments =
        g_strdup_printf(_("Atril is a simple multi-page document "
...
static void ev_window_cmd_help_about(GtkAction *action, EvWindow *ev_window) {
  const char *documenters[] = {N_("MATE Documentation Team"),
                               N_("GNOME Documentation Team"), NULL};
  ...
  const char *license[] = {
      ...
      N_("You should have received a copy of the GNU General Public License "
         "along with Atril; if not, write to the Free Software Foundation, "
         "Inc., "
  ...
  bytes = g_resources_lookup_data("/org/mate/atril/shell/atril.about",
  ...
  authors = g_key_file_get_string_list(key_file, ABOUT_GROUP, "Authors",
                                       &n_authors, NULL);

  g_key_file_free(key_file);
  g_bytes_unref(bytes);

  for (i = 0; i < n_authors; ++i) authors[i] = EMAILIFY(authors[i]);
  ...
  gtk_show_about_dialog(GTK_WINDOW(ev_window), "program-name",
                        _("Atril Document Viewer"), "version", VERSION, "title",
                        _("About Atril Document Viewer"), "copyright",
                        _("Copyright \xc2\xa9 1996–2009 The Evince authors\n"
                          "Copyright \xc2\xa9 2012–2021 The MATE developers\n"
                          "Copyright \xc2\xa9 2022 Libre MATE"),
                        "license", license_trans, "website", PACKAGE_URL,
                        "comments", comments, "authors", authors, "documenters",
                        documenters, "translator-credits",
                        _("translator-credits"), "logo-icon-name", "atril",
...
static void ev_window_view_sidebar_cb(GtkAction *action, EvWindow *ev_window) {
  if (EV_WINDOW_IS_PRESENTATION(ev_window)) return;
...
static void ev_window_sidebar_current_page_changed_cb(EvSidebar *ev_sidebar,
                                                      GParamSpec *pspec,
                                                      EvWindow *ev_window) {
  if (ev_window->priv->metadata && !ev_window_is_empty(ev_window)) {
...
static void ev_window_sidebar_visibility_changed_cb(EvSidebar *ev_sidebar,
  ...
  G_GNUC_BEGIN_IGNORE_DEPRECATIONS;
  action =
      gtk_action_group_get_action(ev_window->priv->action_group, "ViewSidebar");
  G_GNUC_END_IGNORE_DEPRECATIONS;
  ...
  if (!EV_WINDOW_IS_PRESENTATION(ev_window)) {
    ...
    if (ev_window->priv->metadata)
      ev_metadata_set_boolean(ev_window->priv->metadata, "sidebar_visibility",
                              visible);
    if (!visible) gtk_widget_grab_focus(ev_window->priv->view);
  }
}

static void view_menu_link_popup(EvWindow *ev_window, EvLink *link) {
  ...
  if (ev_window->priv->link) g_object_unref(ev_window->priv->link);
...
static void view_menu_image_popup(EvWindow *ev_window, EvImage *image) {
  GtkAction *action;
  ...
  if (ev_window->priv->document &&
      ev_window->priv->document->iswebdocument == TRUE)
    return;
  if (ev_window->priv->image) g_object_unref(ev_window->priv->image);
...
static void view_menu_annot_popup(EvWindow *ev_window, EvAnnotation *annot) {
  GtkAction *action;
  ...
  if (ev_window->priv->document &&
      ev_window->priv->document->iswebdocument == TRUE)
    return;
  if (ev_window->priv->annot) g_object_unref(ev_window->priv->annot);
  ...
  if (EV_IS_DOCUMENT_ANNOTATIONS(ev_window->priv->document))
    can_remove_annots = ev_document_annotations_can_remove_annotation(
        EV_DOCUMENT_ANNOTATIONS(ev_window->priv->document));
  ...
  action = gtk_action_group_get_action(
      ev_window->priv->attachment_popup_action_group, "OpenAttachment");
  gtk_action_set_visible(action, show_annot);

  action = gtk_action_group_get_action(
      ev_window->priv->attachment_popup_action_group, "SaveAttachmentAs");
  gtk_action_set_visible(action, show_annot);
  G_GNUC_END_IGNORE_DEPRECATIONS;
}
...
static gboolean view_menu_popup_cb(EvView *view, GList *items,
  ...
  if (!has_link) view_menu_link_popup(ev_window, NULL);
  if (!has_image) view_menu_image_popup(ev_window, NULL);
  if (!has_annot) view_menu_annot_popup(ev_window, NULL);

  gtk_menu_popup_at_pointer(GTK_MENU(ev_window->priv->view_popup), NULL);
  return TRUE;
}

static gboolean attachment_bar_menu_popup_cb(EvSidebarAttachments *attachbar,
                                             GList *attach_list,
                                             EvWindow *ev_window) {
  GtkWidget *popup;
...
static void ev_window_update_find_status_message(EvWindow *ev_window) {
  ...
  if (!ev_window->priv->find_job) return;
  ...
  if (ev_job_is_finished(ev_window->priv->find_job)) {
    ...
    if (ev_job_find_has_results(job_find)) {
      ...
      message = g_strdup_printf(
          g_dngettext(GETTEXT_PACKAGE, "%d found on this page",
                      "%d found on this page", (gulong)n_results),
          n_results);
    } else {
...
static void ev_window_find_job_finished_cb(EvJobFind *job,
                                           EvWindow *ev_window) {
  ev_window_update_find_status_message(ev_window);
}

static void ev_window_find_job_updated_cb(EvJobFind *job, gint page,
                                          EvWindow *ev_window) {
  ev_window_update_actions(ev_window);
#if ENABLE_EPUB
  if (ev_window->priv->document &&
...
static void find_bar_previous_cb(EggFindBar *find_bar, EvWindow *ev_window) {
#if ENABLE_EPUB
  if (ev_window->priv->document &&
      ev_window->priv->document->iswebdocument == TRUE) {
    ev_web_view_find_previous(EV_WEB_VIEW(ev_window->priv->webview));
...
static void find_bar_next_cb(EggFindBar *find_bar, EvWindow *ev_window) {
#if ENABLE_EPUB
  if (ev_window->priv->document &&
      ev_window->priv->document->iswebdocument == TRUE) {
    ev_web_view_find_next(EV_WEB_VIEW(ev_window->priv->webview));
...
static void find_bar_close_cb(EggFindBar *find_bar, EvWindow *ev_window) {
#if ENABLE_EPUB
  if (ev_window->priv->document &&
      ev_window->priv->document->iswebdocument == TRUE) {
...
static void ev_window_search_start(EvWindow *ev_window) {
...
static void find_bar_visibility_changed_cb(EggFindBar *find_bar,
                                           GParamSpec *param,
                                           EvWindow *ev_window) {
  gboolean visible;
...
static void ev_window_drag_data_received(GtkWidget *widget,
...
static void ev_window_set_caret_navigation_enabled(EvWindow *window,
                                                   gboolean enabled) {
  if (window->priv->metadata)
    ev_metadata_set_boolean(window->priv->metadata, "caret-navigation",
                            enabled);

  ev_view_set_caret_navigation_enabled(EV_VIEW(window->priv->view), enabled);
}

static void ev_window_caret_navigation_message_area_response_cb(
    EvMessageArea *area, gint response_id, EvWindow *window) {
  /* Turn the caret navigation mode on */
  if (response_id == GTK_RESPONSE_YES)
    ev_window_set_caret_navigation_enabled(window, TRUE);

  /* Turn the confirmation dialog off if the user has requested not to show it
   * again */
  if (gtk_toggle_button_get_active(
          GTK_TOGGLE_BUTTON(window->priv->ask_caret_navigation_check))) {
    g_settings_set_boolean(ev_window_ensure_settings(window),
...
static void ev_window_cmd_view_toggle_caret_navigation(GtkAction *action,
  ...
  /* Don't ask for user confirmation to turn the caret navigation off when it is
   * active, or to turn it on when the confirmation dialog is not to be shown
   * per settings */
  ...
  message_area = ev_message_area_new(
      GTK_MESSAGE_QUESTION, _("Enable caret navigation?"), "gtk-no",
      GTK_RESPONSE_NO, _("_Enable"), GTK_RESPONSE_YES, NULL);
  ev_message_area_set_secondary_text(
      EV_MESSAGE_AREA(message_area),
  ...
  window->priv->ask_caret_navigation_check =
  ...
  g_signal_connect(
      message_area, "response",
      G_CALLBACK(ev_window_caret_navigation_message_area_response_cb), window);

  gtk_widget_show(message_area);
  ev_window_set_message_area(window, message_area);
}

static void ev_window_dispose(GObject *object) {
  ...
  if (mpkeys)
    g_signal_handlers_disconnect_by_func(
        mpkeys, ev_window_media_player_key_pressed, window);
  ...
  g_clear_object(&priv->view_popup_action_group);
  g_clear_object(&priv->attachment_popup_action_group);
  g_clear_object(&priv->recent_action_group);
  g_clear_object(&priv->bookmarks_action_group);
  ...
  if (priv->default_settings) g_settings_apply(priv->default_settings);
  ...
  if (priv->load_job) ev_window_clear_load_job(window);

  if (priv->reload_job) ev_window_clear_reload_job(window);

  if (priv->save_job) ev_window_clear_save_job(window);

  if (priv->thumbnail_job) ev_window_clear_thumbnail_job(window);

  if (priv->find_job) ev_window_clear_find_job(window);
...
static gboolean ev_window_key_press_event(GtkWidget *widget,
                                          GdkEventKey *event) {
  ...
  if (gtk_window_propagate_key_event(window, event)) return TRUE;

  /* Handle mnemonics and accelerators */
  if (gtk_window_activate_key(window, event)) return TRUE;

  /* Chain up, invokes binding set on window */
  return GTK_WIDGET_CLASS(grand_parent_class)->key_press_event(widget, event);
}

static gboolean ev_window_delete_event(GtkWidget *widget, GdkEventAny *event) {
...
static const GtkActionEntry entries[] = {
    ...
    {"FileProperties", "document-properties", N_("P_roperties"), "<alt>Return",
     NULL, G_CALLBACK(ev_window_cmd_file_properties)},
    ...
    {"EditRotateLeft", EV_STOCK_ROTATE_LEFT, N_("Rotate _Left"),
     "<control>Left", NULL, G_CALLBACK(ev_window_cmd_edit_rotate_left)},
    {"EditRotateRight", EV_STOCK_ROTATE_RIGHT, N_("Rotate _Right"),
    ...
    {"ViewZoomReset", "zoom-original", N_("_Reset Zoom"), "<control>0",
     N_("Reset zoom to 100\%"), G_CALLBACK(ev_window_cmd_view_zoom_reset)},
    {"ViewReload", "view-refresh", N_("_Reload"), "<control>R",
     N_("Reload the document"), G_CALLBACK(ev_window_cmd_view_reload)},
    {"ViewExpandWindow", "zoom-fit-best", N_("_Expand Window to Fit"),
     "<control>e", N_("Expand Window to Fit"),
    ...
    {"GoPreviousPage", "go-up", N_("_Previous Page"), "<control>Page_Up",
     N_("Go to the previous page"), G_CALLBACK(ev_window_cmd_go_previous_page)},
    {"GoNextPage", "go-down", N_("_Next Page"), "<control>Page_Down",
     N_("Go to the next page"), G_CALLBACK(ev_window_cmd_go_next_page)},
    ...
    {"LeaveFullscreen", "view-restore", N_("Leave Fullscreen"), NULL,
     N_("Leave fullscreen mode"), G_CALLBACK(ev_window_cmd_leave_fullscreen)},
    {"StartPresentation", EV_STOCK_RUN_PRESENTATION, N_("Start Presentation"),
     NULL, N_("Start a presentation"),
    ...
    {"p", "go-up", "", "p", NULL, G_CALLBACK(ev_window_cmd_go_previous_page)},
    {"n", "go-down", "", "n", NULL, G_CALLBACK(ev_window_cmd_go_next_page)},
...
static const GtkToggleActionEntry toggle_entries[] = {
    /* View Menu */
    {"ViewToolbar", NULL, N_("_Toolbar"), "t", N_("Show or hide the toolbar"),
    ...
    {"ViewContinuous", EV_STOCK_VIEW_CONTINUOUS, N_("_Continuous"), "c",
     N_("Show the entire document"), G_CALLBACK(ev_window_cmd_continuous),
     TRUE},
    {"ViewDual", EV_STOCK_VIEW_DUAL, N_("_Dual"), "d",
     N_("Show two pages at once"), G_CALLBACK(ev_window_cmd_dual), FALSE},
    ...
    {"ViewInvertedColors", EV_STOCK_INVERTED_COLORS, N_("Inverted _Colors"),
     "<control>I", N_("Show page contents with the colors inverted"),
...
static const GtkActionEntry view_popup_entries[] = {
    /* Links */
    {"OpenLink", "document-open", N_("_Open Link"), NULL, NULL,
     G_CALLBACK(ev_view_popup_cmd_open_link)},
    {"GoLink", "go-next", N_("_Go To"), NULL, NULL,
     G_CALLBACK(ev_view_popup_cmd_open_link)},
    ...
    {"AnnotProperties", NULL, N_("Annotation Properties…"), NULL, NULL,
     G_CALLBACK(ev_view_popup_cmd_annot_properties)},
    {"RemoveAnnot", NULL, N_("Remove Annot…"), NULL, NULL,
     G_CALLBACK(ev_view_popup_cmd_remove_annotation)}};

static const GtkActionEntry attachment_popup_entries[] = {
    {"OpenAttachment", "document-open", N_("_Open Attachment"), NULL, NULL,
     G_CALLBACK(ev_attachment_popup_cmd_open_attachment)},
...
static void activate_link_cb(EvPageAction *page_action, EvLink *link,
                             EvWindow *window) {
  if (window->priv->view) {
    ev_view_handle_link(EV_VIEW(window->priv->view), link);
    gtk_widget_grab_focus(window->priv->view);
...
static void navigation_action_activate_link_cb(EvNavigationAction *action,
                                               EvLink *link, EvWindow *window) {
#if ENABLE_EPUB
  if (window->priv->document && window->priv->document->iswebdocument == TRUE) {
    ev_web_view_handle_link(EV_WEB_VIEW(window->priv->webview), link);
...
static void sidebar_layers_visibility_changed(EvSidebarLayers *layers,
                                              EvWindow *window) {
  if (window->priv->document &&
      window->priv->document->iswebdocument == FALSE) {
...
static void sidebar_annots_annot_activated_cb(
    EvSidebarAnnotations *sidebar_annots, EvMapping *annot_mapping,
    EvWindow *window) {
  if (window->priv->document && window->priv->document->iswebdocument == TRUE)
    return;
...
static void view_annot_added(EvView *view, EvAnnotation *annot,
                             EvWindow *window) {
  ev_sidebar_annotations_annot_added(
...
static void sidebar_annots_annot_add_cancelled(
    EvSidebarAnnotations *sidebar_annots, EvWindow *window) {
  if (window->priv->document && window->priv->document->iswebdocument == TRUE)
    return;
  ev_view_cancel_add_annotation(EV_VIEW(window->priv->view));
}

static void sidebar_bookmarks_add_bookmark(
    EvSidebarBookmarks *sidebar_bookmarks, EvWindow *window) {
...
static void register_custom_actions(EvWindow *window, GtkActionGroup *group) {
  ...
  action =
      g_object_new(EV_TYPE_PAGE_ACTION, "name", PAGE_SELECTOR_ACTION, "label",
                   _("Page"), "tooltip", _("Select Page"), "icon_name",
                   "text-x-generic", "visible_overflown", FALSE, NULL);
  ev_page_action_set_model(EV_PAGE_ACTION(action), window->priv->model);
  g_signal_connect(action, "activate_link", G_CALLBACK(activate_link_cb),
                   window);
  G_GNUC_BEGIN_IGNORE_DEPRECATIONS;
  ...
  action =
      g_object_new(EPHY_TYPE_ZOOM_ACTION, "name", ZOOM_CONTROL_ACTION, "label",
                   _("Zoom"), "stock_id", EV_STOCK_ZOOM, "tooltip",
                   _("Adjust the zoom level"), "zoom", 1.0, NULL);
  g_signal_connect(action, "zoom_to_level", G_CALLBACK(zoom_control_changed_cb),
                   window);
  gtk_action_group_add_action(group, action);
  g_object_unref(action);

  action = g_object_new(EV_TYPE_NAVIGATION_ACTION, "name", NAVIGATION_ACTION,
                        "label", _("Navigation"), "is_important", TRUE,
                        "short_label", _("Back"), "stock_id", "gtk-go-back",
                        /*translators: this is the history action*/
                        "tooltip", _("Move across visited pages"), NULL);
  g_signal_connect(action, "activate_link",
                   G_CALLBACK(navigation_action_activate_link_cb), window);
  gtk_action_group_add_action(group, action);
  g_object_unref(action);

  action = g_object_new(EV_TYPE_OPEN_RECENT_ACTION, "name", "FileOpenRecent",
                        "label", _("_Open…"), "tooltip",
                        _("Open an existing document"), "stock_id", "gtk-open",
                        NULL);
  g_signal_connect(action, "activate", G_CALLBACK(ev_window_cmd_file_open),
                   window);
...
static void set_action_properties(GtkActionGroup *action_group) {
  GtkAction *action;
...
static void sidebar_widget_model_set(EvSidebarLinks *ev_sidebar_links,
...
static void sidebar_page_main_widget_update_cb(GObject *ev_sidebar_page,
                                               GParamSpec *pspec,
                                               EvWindow *ev_window) {
  GtkWidget *widget;
...
static gboolean window_state_event_cb(EvWindow *window,
                                      GdkEventWindowState *event,
                                      gpointer dummy) {
  if (!(event->new_window_state & GDK_WINDOW_STATE_FULLSCREEN)) {
    gboolean maximized;
...
static gboolean window_configure_event_cb(EvWindow *window,
                                          GdkEventConfigure *event,
                                          gpointer dummy) {
  GdkWindowState state;
  gdouble document_width, document_height;

  if (!window->priv->metadata) return FALSE;
...
static void launch_action(EvWindow *window, EvLinkAction *action) {
...
static void launch_external_uri(EvWindow *window, EvLinkAction *action) {
...
static void open_remote_link(EvWindow *window, EvLinkAction *action) {
  gchar *uri;
  gchar *dir;
...
static void do_action_named(EvWindow *window, EvLinkAction *action) {
...
static void ev_view_popup_cmd_copy_link_address(GtkAction *action,
  ...
  ev_view_copy_link_address(EV_VIEW(window->priv->view), ev_action);
}

static GFile *create_file_from_uri_for_format(const gchar *uri,
...
static void image_save_dialog_response_cb(GtkWidget *fc, gint response_id,
                                          EvWindow *ev_window) {
  GFile *target_file;
  gboolean is_native;
  GError *error = NULL;
  ...
  if (is_native) {
    ...
    if (ev_mkstemp("saveimage.XXXXXX", &filename, &error) == -1) goto has_error;
  }

  ev_document_doc_mutex_lock();
  pixbuf = ev_document_images_get_image(
      EV_DOCUMENT_IMAGES(ev_window->priv->document), ev_window->priv->image);
...
has_error:
  ...
  if (!is_native) {
    ...
    ev_window_save_remote(ev_window, EV_SAVE_IMAGE, source_file, target_file);
    g_object_unref(source_file);
  }
...
static void ev_view_popup_cmd_save_image_as(GtkAction *action,
  ...
  if (!window->priv->image) return;

  fc = gtk_file_chooser_dialog_new(
      _("Save Image"), GTK_WINDOW(window), GTK_FILE_CHOOSER_ACTION_SAVE,
      "gtk-cancel", GTK_RESPONSE_CANCEL, "gtk-save", GTK_RESPONSE_OK, NULL);
  ...
  g_signal_connect(fc, "response", G_CALLBACK(image_save_dialog_response_cb),
                   window);
...
static void ev_view_popup_cmd_copy_image(GtkAction *action, EvWindow *window) {
  ...
  ev_document_doc_mutex_lock();
  pixbuf = ev_document_images_get_image(
      EV_DOCUMENT_IMAGES(window->priv->document), window->priv->image);
  ev_document_doc_mutex_unlock();
...
static void ev_view_popup_cmd_annot_properties(GtkAction *action,
  ...
  dialog = EV_ANNOTATION_PROPERTIES_DIALOG(
      ev_annotation_properties_dialog_new_with_annotation(window->priv->annot));
  ...
  if (ev_annotation_set_rgba(annot, &rgba)) mask |= EV_ANNOTATIONS_SAVE_COLOR;
  ...
  if (ev_annotation_markup_set_popup_is_open(EV_ANNOTATION_MARKUP(annot),
                                             popup_is_open))
  ...
  if (mask != EV_ANNOTATIONS_SAVE_NONE) {
    ...
    ev_document_annotations_save_annotation(
        EV_DOCUMENT_ANNOTATIONS(window->priv->document), window->priv->annot,
        mask);
    ev_document_doc_mutex_unlock();
...
static void ev_attachment_popup_cmd_open_attachment(GtkAction *action,
  ...
  if (!window->priv->attach_list) return;
  ...
  for (l = window->priv->attach_list; l && l->data; l = g_list_next(l)) {
    ...
    ev_attachment_open(attachment, screen, gtk_get_current_event_time(),
                       &error);
...
static void attachment_save_dialog_response_cb(GtkWidget *fc, gint response_id,
                                               EvWindow *ev_window) {
  GFile *target_file;
  gchar *uri;
  GList *l;
  ...
  for (l = ev_window->priv->attach_list; l && l->data; l = g_list_next(l)) {
    ...
    if (save_to) ev_attachment_save(attachment, save_to, &error);
    ...
    if (!is_native) {
      ...
      if (is_dir) {
        dest_file =
            g_file_get_child(target_file, ev_attachment_get_name(attachment));
      } else {
        dest_file = g_object_ref(target_file);
      }

      ev_window_save_remote(ev_window, EV_SAVE_ATTACHMENT, save_to, dest_file);
...
static void ev_attachment_popup_cmd_save_attachment_as(GtkAction *action,
  ...
  fc = gtk_file_chooser_dialog_new(
      _("Save Attachment"), GTK_WINDOW(window),
      attachment ? GTK_FILE_CHOOSER_ACTION_SAVE
  ...
  g_signal_connect(fc, "response",
                   G_CALLBACK(attachment_save_dialog_response_cb), window);
...
static EggToolbarsModel *get_toolbars_model(void) {
  EggToolbarsModel *toolbars_model;
  gchar *toolbars_file;
  gint i;
  ...
  toolbars_file = g_build_filename(ev_application_get_dot_dir(EV_APP, FALSE),
                                   "atril_toolbar.xml", NULL);
  egg_toolbars_model_load_names_from_resource(toolbars_model,
                                              TOOLBAR_RESOURCE_PATH);

  if (!egg_toolbars_model_load_toolbars(toolbars_model, toolbars_file)) {
    egg_toolbars_model_load_toolbars_from_resource(toolbars_model,
                                                   TOOLBAR_RESOURCE_PATH);
  ...
  for (i = 0; i < egg_toolbars_model_n_items(toolbars_model, 0); i++) {
    ...
    if (g_ascii_strcasecmp(item, "FileOpen") == 0) {
      ...
      egg_toolbars_model_add_item(toolbars_model, 0, i, "FileOpenRecent");
...
static void ev_window_sync_source(EvWindow *window, EvSourceLink *link) {
  ...
  if (window->priv->skeleton == NULL) return;
  ...
  ev_atril_window_emit_sync_source(window->priv->skeleton, uri_input,
...
static void ev_window_emit_doc_loaded(EvWindow *window) {
  ...
  ev_atril_window_emit_document_loaded(window->priv->skeleton,
                                       window->priv->uri);
}

#ifdef ENABLE_SYNCTEX
static gboolean handle_sync_view_cb(EvAtrilWindow *object,
...
static void ev_window_init(EvWindow *ev_window) {
  ...
  gtk_action_group_add_toggle_actions(action_group, toggle_entries,
                                      G_N_ELEMENTS(toggle_entries), ev_window);
  G_GNUC_END_IGNORE_DEPRECATIONS;
  ...
  gtk_ui_manager_insert_action_group(ev_window->priv->ui_manager, action_group,
                                     0);

  accel_group = gtk_ui_manager_get_accel_group(ev_window->priv->ui_manager);
  ...
  gtk_action_group_add_actions(action_group, view_popup_entries,
                               G_N_ELEMENTS(view_popup_entries), ev_window);
  G_GNUC_END_IGNORE_DEPRECATIONS;
  gtk_ui_manager_insert_action_group(ev_window->priv->ui_manager, action_group,
                                     0);
  ...
  _gtk_css_provider_load_from_resource(
      css_provider, "/org/mate/atril/shell/ui/atril.css", &error);
  g_assert_no_error(error);
  ...
  ev_window->priv->menubar =
      gtk_ui_manager_get_widget(ev_window->priv->ui_manager, "/MainMenu");
  ...
  ev_window->priv->toolbar = GTK_WIDGET(
      g_object_new(EGG_TYPE_EDITABLE_TOOLBAR, "ui-manager",
                   ev_window->priv->ui_manager, "popup-path", "/ToolbarPopup",
                   "model", ev_window->priv->toolbars_model, NULL));

  GSettingsSchema *schema_mate_interface_schema =
      g_settings_schema_source_lookup(g_settings_schema_source_get_default(),
                                      MATE_INTERFACE_SCHEMA, FALSE);
  if (schema_mate_interface_schema != NULL) {
    g_settings_schema_unref(schema_mate_interface_schema);

    if (!ev_window->priv->interface_settings)
      ev_window->priv->interface_settings =
          g_settings_new(MATE_INTERFACE_SCHEMA);
    g_signal_connect(ev_window->priv->interface_settings, "changed",
                     G_CALLBACK(interface_changed), ev_window);
  }
  ev_window_setup_toolbar_flags(ev_window);
  ...
  egg_editable_toolbar_show(EGG_EDITABLE_TOOLBAR(ev_window->priv->toolbar),
                            "DefaultToolBar");
  gtk_box_pack_start(GTK_BOX(ev_window->priv->main_box),
                     ev_window->priv->toolbar, FALSE, FALSE, 0);
  ...
  g_signal_connect(sidebar_widget, "begin_annot_add",
                   G_CALLBACK(sidebar_annots_begin_annot_add), ev_window);
  g_signal_connect(sidebar_widget, "annot_add_cancelled",
                   G_CALLBACK(sidebar_annots_annot_add_cancelled), ev_window);
  gtk_widget_show(sidebar_widget);
  ev_sidebar_add_page(EV_SIDEBAR(ev_window->priv->sidebar), sidebar_widget);
  ...
  ev_window->priv->scrolled_window = GTK_WIDGET(g_object_new(
  ...
  gtk_overlay_add_overlay(GTK_OVERLAY(overlay),
                          ev_window->priv->loading_message);

  gtk_box_pack_start(GTK_BOX(ev_window->priv->view_box), overlay, TRUE, TRUE,
                     0);
...
#if ENABLE_EPUB /* The webview is created in ev_window_set_document only if \
                   the document is a webdocument. */
  ev_window->priv->webview = NULL;
#endif
  ...
  g_signal_connect_object(ev_window->priv->view, "notify::is-loading",
                          G_CALLBACK(view_is_loading_changed_cb), ev_window, 0);
  g_signal_connect_object(ev_window->priv->view, "cursor-moved",
                          G_CALLBACK(view_caret_cursor_moved_cb), ev_window, 0);
#ifdef ENABLE_DBUS
  ...
  /* Popups */
  ev_window->priv->view_popup =
      gtk_ui_manager_get_widget(ev_window->priv->ui_manager, "/DocumentPopup");
  ev_window->priv->link = NULL;

  ev_window->priv->attachment_popup = gtk_ui_manager_get_widget(
      ev_window->priv->ui_manager, "/AttachmentPopup");
  ev_window->priv->attach_list = NULL;
