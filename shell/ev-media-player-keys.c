...
struct _EvMediaPlayerKeysClass {
  ...
  /* Signals */
  void (*key_pressed)(EvMediaPlayerKeys *keys, const gchar *key);
};
...
static void ev_media_player_keys_class_init(EvMediaPlayerKeysClass *klass) {
  ...
  signals[KEY_PRESSED] = g_signal_new(
      ...
      G_STRUCT_OFFSET(EvMediaPlayerKeysClass, key_pressed), NULL, NULL,
      g_cclosure_marshal_VOID__STRING, G_TYPE_NONE, 1, G_TYPE_STRING);
}

static void ev_media_player_keys_update_has_name_owner(
...
static void ev_media_player_keys_grab_keys(EvMediaPlayerKeys *keys) {
   ...
   */
  g_dbus_proxy_call(keys->proxy, "GrabMediaPlayerKeys",
...
static void ev_media_player_keys_release_keys(EvMediaPlayerKeys *keys) {
  ...
  g_dbus_proxy_call(keys->proxy, "ReleaseMediaPlayerKeys",
                    g_variant_new("(s)", "Atril"),
                    G_DBUS_CALL_FLAGS_NO_AUTO_START, -1, NULL, NULL, NULL);
}

static void media_player_key_pressed_cb(GDBusProxy *proxy, gchar *sender_name,
                                        gchar *signal_name,
                                        GVariant *parameters,
                                        gpointer user_data) {
  const char *application, *key;

  if (g_strcmp0(sender_name, SD_NAME) != 0) return;

  if (g_strcmp0(signal_name, "MediaPlayerKeyPressed") != 0) return;

  if (!g_variant_is_of_type(parameters, G_VARIANT_TYPE("(ss)"))) return;
...
static void mediakeys_name_owner_changed(GObject *object, GParamSpec *pspec,
                                         gpointer user_data) {
...
static void mediakeys_service_appeared_cb(GObject *source_object,
  ...
  g_signal_connect(proxy, "notify::g-name-owner",
                   G_CALLBACK(mediakeys_name_owner_changed), keys);
...
static void ev_media_player_keys_init(EvMediaPlayerKeys *keys) {
  g_dbus_proxy_new_for_bus(G_BUS_TYPE_SESSION,
                           ...
                           SD_NAME, SD_OBJECT_PATH, SD_INTERFACE, NULL,
                           mediakeys_service_appeared_cb, keys);
}

void ev_media_player_keys_focused(EvMediaPlayerKeys *keys) {
  if (keys->proxy == NULL) return;
