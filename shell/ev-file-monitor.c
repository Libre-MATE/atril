...
static void ev_file_monitor_changed_cb(GFileMonitor *monitor, GFile *file,
                                       GFile *other_file,
...
static void ev_file_monitor_init(EvFileMonitor *ev_monitor) {
  ev_monitor->priv = ev_file_monitor_get_instance_private(ev_monitor);
}

static void ev_file_monitor_finalize(GObject *object) {
  ...
  if (ev_monitor->priv->monitor) {
    g_signal_handlers_disconnect_by_func(
        ev_monitor->priv->monitor, ev_file_monitor_changed_cb, ev_monitor);
...
static void ev_file_monitor_class_init(EvFileMonitorClass *klass) {
  ...
  /* Signals */
  signals[CHANGED] =
      g_signal_new("changed", EV_TYPE_FILE_MONITOR, G_SIGNAL_RUN_LAST,
                   G_STRUCT_OFFSET(EvFileMonitorClass, changed), NULL, NULL,
                   g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
}

static gboolean timeout_cb(EvFileMonitor *ev_monitor) {
