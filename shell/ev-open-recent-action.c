...
static void recent_chooser_item_activated(GtkRecentChooser *chooser,
                                          GtkAction *action) {
  gchar *uri;
...
static GtkWidget *ev_open_recent_action_create_tool_item(GtkAction *action) {
  GtkWidget *tool_item;
  ...
  g_signal_connect(toolbar_recent_menu, "item_activated",
                   G_CALLBACK(recent_chooser_item_activated), action);
  ...
  gtk_menu_tool_button_set_arrow_tooltip_text(
      GTK_MENU_TOOL_BUTTON(tool_item), _("Open a recently used document"));
...
static void ev_open_recent_action_class_init(EvOpenRecentActionClass *class) {
  ...
  action_signals[ITEM_ACTIVATED] = g_signal_new(
      "item_activated", G_OBJECT_CLASS_TYPE(object_class),
      G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
      G_STRUCT_OFFSET(EvOpenRecentActionClass, item_activated), NULL, NULL,
