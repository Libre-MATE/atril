...
G_DEFINE_TYPE_EXTENDED(
    EvSidebarBookmarks, ev_sidebar_bookmarks, GTK_TYPE_BOX, 0,
...
static gint ev_sidebar_bookmarks_get_selected_page(
    EvSidebarBookmarks *sidebar_bookmarks, GtkTreeSelection *selection) {
  GtkTreeModel *model;
  GtkTreeIter iter;
...
static void ev_bookmarks_popup_cmd_open_bookmark(
    GtkAction *action, EvSidebarBookmarks *sidebar_bookmarks) {
...
static void ev_bookmarks_popup_cmd_rename_bookmark(
    GtkAction *action, EvSidebarBookmarks *sidebar_bookmarks) {
  EvSidebarBookmarksPrivate *priv = sidebar_bookmarks->priv;
  ...
  if (gtk_tree_selection_get_selected(selection, &model, &iter)) {
    ...
    gtk_tree_view_set_cursor(tree_view, path,
                             gtk_tree_view_get_column(tree_view, 0), TRUE);
    gtk_tree_path_free(path);
  }
}

static void ev_bookmarks_popup_cmd_remove_bookmark(
    GtkAction *action, EvSidebarBookmarks *sidebar_bookmarks) {
  EvSidebarBookmarksPrivate *priv = sidebar_bookmarks->priv;
  GtkTreeSelection *selection;
  gint page;
...
static const GtkActionEntry popup_entries[] = {
    {"OpenBookmark", "document-open", N_("_Open Bookmark"), NULL, NULL,
     G_CALLBACK(ev_bookmarks_popup_cmd_open_bookmark)},
    {"RenameBookmark", NULL, N_("_Rename Bookmark"), NULL, NULL,
     G_CALLBACK(ev_bookmarks_popup_cmd_rename_bookmark)},
    {"RemoveBookmark", NULL, N_("_Remove Bookmark"), NULL, NULL,
     G_CALLBACK(ev_bookmarks_popup_cmd_remove_bookmark)}};
...
static void ev_sidebar_bookmarks_update(EvSidebarBookmarks *sidebar_bookmarks) {
  ...
  model =
      GTK_LIST_STORE(gtk_tree_view_get_model(GTK_TREE_VIEW(priv->tree_view)));
  ...
  for (l = items; l; l = g_list_next(l)) {
    ...
    gtk_list_store_set(model, &iter, COLUMN_MARKUP, bm->title, COLUMN_PAGE,
                       bm->page, -1);
  }
  g_list_free(items);
...
static void ev_sidebar_bookmarks_changed(
    EvBookmarks *bookmarks, EvSidebarBookmarks *sidebar_bookmarks) {
  ev_sidebar_bookmarks_update(sidebar_bookmarks);
}

static void ev_sidebar_bookmarks_selection_changed(
    GtkTreeSelection *selection, EvSidebarBookmarks *sidebar_bookmarks) {
  EvSidebarBookmarksPrivate *priv = sidebar_bookmarks->priv;
  gint page;
...
static void ev_sidebar_bookmarks_add_clicked(
    GtkWidget *button, EvSidebarBookmarks *sidebar_bookmarks) {
...
static void ev_sidebar_bookmarks_del_clicked(
    GtkWidget *button, EvSidebarBookmarks *sidebar_bookmarks) {
  EvSidebarBookmarksPrivate *priv = sidebar_bookmarks->priv;
  GtkTreeSelection *selection;
  gint page;
...
static void ev_sidebar_bookmarks_bookmark_renamed(
    GtkCellRendererText *renderer, const gchar *path_string,
    const gchar *new_text, EvSidebarBookmarks *sidebar_bookmarks) {
  EvSidebarBookmarksPrivate *priv = sidebar_bookmarks->priv;
...
static gboolean ev_sidebar_bookmarks_query_tooltip(
    GtkWidget *widget, gint x, gint y, gboolean keyboard_tip,
    GtkTooltip *tooltip, EvSidebarBookmarks *sidebar_bookmarks) {
  EvSidebarBookmarksPrivate *priv = sidebar_bookmarks->priv;
  GtkTreeModel *model;
  GtkTreeIter iter;
  ...
  model = gtk_tree_view_get_model(GTK_TREE_VIEW(priv->tree_view));
  if (!gtk_tree_view_get_tooltip_context(GTK_TREE_VIEW(priv->tree_view), &x, &y,
  ...
  gtk_tree_view_set_tooltip_row(GTK_TREE_VIEW(priv->tree_view), tooltip, path);
  gtk_tree_path_free(path);

  return TRUE;
}

static gboolean ev_sidebar_bookmarks_popup_menu_show(
    EvSidebarBookmarks *sidebar_bookmarks, gint x, gint y,
  ...
  if (keyboard_mode) {
    if (!gtk_tree_selection_get_selected(selection, NULL, NULL)) return FALSE;
  } else {
    GtkTreePath *path;

    if (!gtk_tree_view_get_path_at_pos(tree_view, x, y, &path, NULL, NULL,
                                       NULL))
      return FALSE;
  ...
  if (!priv->popup)
    priv->popup =
        gtk_ui_manager_get_widget(priv->ui_manager, "/BookmarksPopup");

  gtk_menu_popup_at_pointer(GTK_MENU(priv->popup), NULL);
  return TRUE;
}

static gboolean ev_sidebar_bookmarks_button_press(
    GtkWidget *widget, GdkEventButton *event,
    EvSidebarBookmarks *sidebar_bookmarks) {
  if (event->button != 3) return FALSE;

  return ev_sidebar_bookmarks_popup_menu_show(sidebar_bookmarks, event->x,
                                              event->y, FALSE);
}
...
static void ev_sidebar_bookmarks_init(EvSidebarBookmarks *sidebar_bookmarks) {
  ...
  g_object_set(renderer, "ellipsize", PANGO_ELLIPSIZE_END, "editable", TRUE,
               NULL);
  g_signal_connect(renderer, "edited",
                   G_CALLBACK(ev_sidebar_bookmarks_bookmark_renamed),
                   sidebar_bookmarks);
  gtk_tree_view_insert_column_with_attributes(GTK_TREE_VIEW(priv->tree_view), 0,
                                              NULL, renderer, "markup",
                                              COLUMN_MARKUP, NULL);
  ...
  gtk_action_group_add_actions(priv->action_group, popup_entries,
                               G_N_ELEMENTS(popup_entries), sidebar_bookmarks);
...
static void ev_sidebar_bookmarks_get_property(GObject *object, guint prop_id,
                                              GValue *value,
                                              GParamSpec *pspec) {
  EvSidebarBookmarks *sidebar_bookmarks;
...
static void ev_sidebar_bookmarks_class_init(EvSidebarBookmarksClass *klass) {
  ...
  signals[ADD_BOOKMARK] = g_signal_new(
      "add-bookmark", G_TYPE_FROM_CLASS(g_object_class),
      G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
      G_STRUCT_OFFSET(EvSidebarBookmarksClass, add_bookmark), NULL, NULL,
...
void ev_sidebar_bookmarks_set_bookmarks(EvSidebarBookmarks *sidebar_bookmarks,
  ...
  if (priv->bookmarks == bookmarks) return;

  if (priv->bookmarks) g_object_unref(priv->bookmarks);
  priv->bookmarks = g_object_ref(bookmarks);
  g_signal_connect(priv->bookmarks, "changed",
                   G_CALLBACK(ev_sidebar_bookmarks_changed), sidebar_bookmarks);
...
static gboolean ev_sidebar_bookmarks_support_document(
    EvSidebarPage *sidebar_page, EvDocument *document) {
  return TRUE;
}

static const gchar *ev_sidebar_bookmarks_get_label(
    EvSidebarPage *sidebar_page) {
  return _("Bookmarks");
}

static void ev_sidebar_bookmarks_page_iface_init(
