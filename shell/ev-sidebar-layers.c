...
G_DEFINE_TYPE_EXTENDED(EvSidebarLayers, ev_sidebar_layers, GTK_TYPE_BOX, 0,
                       G_ADD_PRIVATE(EvSidebarLayers) G_IMPLEMENT_INTERFACE(
                           EV_TYPE_SIDEBAR_PAGE,
                           ev_sidebar_layers_page_iface_init))

static void ev_sidebar_layers_dispose(GObject *object) {
...
static void ev_sidebar_layers_get_property(GObject *object, guint prop_id,
...
static GtkTreeModel *ev_sidebar_layers_create_loading_model(void) {
  ...
  retval = (GtkTreeModel *)gtk_list_store_new(
      EV_DOCUMENT_LAYERS_N_COLUMNS, G_TYPE_STRING, G_TYPE_OBJECT,
  ...
  gtk_list_store_set(GTK_LIST_STORE(retval), &iter,
                     ...
                     EV_DOCUMENT_LAYERS_COLUMN_LAYER, NULL, -1);
  g_free(markup);

  return retval;
}

static gboolean update_kids(GtkTreeModel *model, GtkTreePath *path,
                            GtkTreeIter *iter, GtkTreeIter *parent) {
  if (gtk_tree_store_is_ancestor(GTK_TREE_STORE(model), parent, iter)) {
    gboolean visible;

    gtk_tree_model_get(model, parent, EV_DOCUMENT_LAYERS_COLUMN_VISIBLE,
                       &visible, -1);
    gtk_tree_store_set(GTK_TREE_STORE(model), iter,
                       EV_DOCUMENT_LAYERS_COLUMN_ENABLED, visible, -1);
  }

  return FALSE;
}

static gboolean clear_rb_group(GtkTreeModel *model, GtkTreePath *path,
                               GtkTreeIter *iter, gint *rb_group) {
  gint group;

  gtk_tree_model_get(model, iter, EV_DOCUMENT_LAYERS_COLUMN_RBGROUP, &group,
                     -1);

  if (group == *rb_group) {
    gtk_tree_store_set(GTK_TREE_STORE(model), iter,
                       EV_DOCUMENT_LAYERS_COLUMN_VISIBLE, FALSE, -1);
  }

  return FALSE;
}

static void ev_sidebar_layers_visibility_changed(GtkCellRendererToggle *cell,
                                                 gchar *path_str,
  ...
  gtk_tree_model_get(model, &iter, EV_DOCUMENT_LAYERS_COLUMN_VISIBLE, &visible,
                     EV_DOCUMENT_LAYERS_COLUMN_LAYER, &layer, -1);
  ...
  if (visible) {
    ...
    if (rb_group) {
      gtk_tree_model_foreach(model, (GtkTreeModelForeachFunc)clear_rb_group,
                             &rb_group);
    }
  } else {
  ...
  gtk_tree_store_set(GTK_TREE_STORE(model), &iter,
                     EV_DOCUMENT_LAYERS_COLUMN_VISIBLE, visible, -1);
...
static GtkTreeView *ev_sidebar_layers_create_tree_view(
    EvSidebarLayers *ev_layers) {
  GtkTreeView *tree_view;
  ...
  gtk_tree_view_column_set_attributes(
      column, renderer, "active", EV_DOCUMENT_LAYERS_COLUMN_VISIBLE,
      "activatable", EV_DOCUMENT_LAYERS_COLUMN_ENABLED, "visible",
      EV_DOCUMENT_LAYERS_COLUMN_SHOWTOGGLE, "sensitive",
      EV_DOCUMENT_LAYERS_COLUMN_ENABLED, NULL);
  g_object_set(G_OBJECT(renderer), "xpad", 4, "ypad", 0, NULL);
  ...
  gtk_tree_view_column_set_attributes(
      column, renderer, "markup", EV_DOCUMENT_LAYERS_COLUMN_TITLE, "sensitive",
      EV_DOCUMENT_LAYERS_COLUMN_ENABLED, NULL);
...
static void ev_sidebar_layers_init(EvSidebarLayers *ev_layers) {
  ...
  gtk_orientable_set_orientation(GTK_ORIENTABLE(ev_layers),
...
static void ev_sidebar_layers_class_init(
    EvSidebarLayersClass *ev_layers_class) {
  ...
  signals[LAYERS_VISIBILITY_CHANGED] = g_signal_new(
      "layers_visibility_changed", G_TYPE_FROM_CLASS(g_object_class),
      G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
      G_STRUCT_OFFSET(EvSidebarLayersClass, layers_visibility_changed), NULL,
...
static void update_layers_state(GtkTreeModel *model, GtkTreeIter *iter,
  ...
  do {
    ...
    if (layer) {
      ...
      layer_visible =
          ev_document_layers_layer_is_visible(document_layers, layer);
...
void ev_sidebar_layers_update_layers_state(EvSidebarLayers *sidebar_layers) {
  ...
  model =
      gtk_tree_view_get_model(GTK_TREE_VIEW(sidebar_layers->priv->tree_view));
...
static gboolean ev_sidebar_layers_support_document(EvSidebarPage *sidebar_page,
                                                   EvDocument *document) {
