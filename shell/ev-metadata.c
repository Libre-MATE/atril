...
static void ev_metadata_load(EvMetadata *metadata) {
  ...
  for (i = 0; attrs[i]; i++) {
    ...
    if (!g_str_has_prefix(attrs[i], EV_METADATA_NAMESPACE)) continue;

    if (!g_file_info_get_attribute_data(info, attrs[i], &type, &value, NULL)) {
      continue;
    }
...
static void metadata_set_callback(GObject *file, GAsyncResult *result,
                                  EvMetadata *metadata) {
  GError *error = NULL;
...
gboolean ev_metadata_set_string(EvMetadata *metadata, const gchar *key,
  ...
  if (value) {
    ...
    g_file_info_set_attribute(info, gio_key, G_FILE_ATTRIBUTE_TYPE_INVALID,
                              NULL);
  }
  g_free(gio_key);

  g_file_set_attributes_async(metadata->file, info, 0, G_PRIORITY_DEFAULT, NULL,
...
gboolean ev_metadata_get_int(EvMetadata *metadata, const gchar *key,
  ...
  if (!ev_metadata_get_string(metadata, key, &string_value)) return FALSE;

  int_value = g_ascii_strtoull(string_value, &endptr, 0);
  if (int_value == 0 && string_value == endptr) return FALSE;
...
gboolean ev_metadata_get_double(EvMetadata *metadata, const gchar *key,
  ...
  if (double_value == 0. && string_value == endptr) return FALSE;
...
gboolean ev_metadata_get_boolean(EvMetadata *metadata, const gchar *key,
  ...
  if (!ev_metadata_get_int(metadata, key, &int_value)) return FALSE;
