...
static const BadTitleEntry bad_extensions[] = {{EV_BACKEND_PS, ".dvi"},
                                               {EV_BACKEND_PDF, ".doc"},
                                               {EV_BACKEND_PDF, ".dvi"},
                                               {EV_BACKEND_PDF, ".indd"},
                                               {EV_BACKEND_PDF, ".rtf"}};
...
static void ev_window_title_sanitize_title(EvWindowTitle *window_title,
  ...
  backend =
      ev_backends_manager_get_document_module_name(window_title->document);
  ...
  for (i = 0; i < G_N_ELEMENTS(bad_extensions); i++) {
    if (g_ascii_strcasecmp(bad_extensions[i].backend, backend) == 0 &&
      ...
      new_title =
          g_strndup(*title, strlen(*title) - strlen(bad_extensions[i].text));
      g_free(*title);
      *title = new_title;
...
static void ev_window_title_update(EvWindowTitle *window_title) {
  ...
  if (window_title->document != NULL) {
    ...
    if (doc_title != NULL) {
      ...
      if (doc_title[0] != '\0' && g_utf8_validate(doc_title, -1, NULL)) {
