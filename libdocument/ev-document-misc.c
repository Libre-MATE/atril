 ...
 * source_pixbuf.
 */
static GdkPixbuf *create_thumbnail_frame(int width, int height,
  ...
  if (source_pixbuf) g_return_val_if_fail(GDK_IS_PIXBUF(source_pixbuf), NULL);
  ...
  retval =
      gdk_pixbuf_new(GDK_COLORSPACE_RGB, TRUE, 8, width_r + 4, height_r + 4);
  ...
  if (source_pixbuf)
    gdk_pixbuf_copy_area(source_pixbuf, 0, 0, width_r, height_r, retval, 1, 1);
  /* Add the corner */
...
void ev_document_misc_paint_one_page(cairo_t *cr, GtkWidget *widget,
  ...
  cairo_rectangle(cr, area->x + area->width - border->right,
                  area->y + border->right - border->left, border->right,
                  area->height - border->right + border->left);
  cairo_rectangle(cr, area->x + border->bottom - border->top,
                  area->y + area->height - border->bottom,
                  area->width - border->bottom + border->top, border->bottom);
  cairo_fill(cr);
  ...
  cairo_rectangle(cr, area->x + border->left, area->y + border->top,
...
cairo_surface_t *ev_document_misc_surface_from_pixbuf(GdkPixbuf *pixbuf) {
  ...
  surface = cairo_image_surface_create(
      gdk_pixbuf_get_has_alpha(pixbuf) ? CAIRO_FORMAT_ARGB32
                                       : CAIRO_FORMAT_RGB24,
...
cairo_surface_t *ev_document_misc_surface_rotate_and_scale(
  ...
  if (dest_width == width && dest_height == height && dest_rotation == 0) {
  ...
  new_surface = cairo_surface_create_similar(
      surface, cairo_surface_get_content(surface), new_width, new_height);
  ...
  if (dest_width != width || dest_height != height) {
    ...
    cairo_scale(cr, (gdouble)dest_width / width, (gdouble)dest_height / height);
  }
...
gdouble ev_document_misc_get_monitor_dpi(GdkMonitor *monitor) {
  ...
  /*diagonal in inches*/
  di = hypot(gdk_monitor_get_width_mm(monitor),
...
void ev_document_misc_get_pointer_position(GtkWidget *widget, gint *x,
                                           gint *y) {
  GdkSeat *seat;
  ...
  gdk_window_get_device_position(gtk_widget_get_window(widget), device_pointer,
                                 x, y, NULL);

  if (gtk_widget_get_has_window(widget)) return;
