...
#define ev_debug_message(section, format, args...) \
  G_STMT_START {}                                  \
  G_STMT_END
#define ev_profiler_start(format, args...) \
  G_STMT_START {}                          \
  G_STMT_END
#define ev_profiler_stop(format, args...) \
  G_STMT_START {}                         \
  G_STMT_END
...
static void ev_debug_message(EvDebugSection section, const gchar *file,
 ...
 * sections.
 */
typedef enum { EV_NO_DEBUG = 0, EV_DEBUG_JOBS = 1 << 0 } EvDebugSection;
 ...
 * sections.
 */
typedef enum { EV_NO_PROFILE = 0, EV_PROFILE_JOBS = 1 << 0 } EvProfileSection;

void _ev_debug_init(void);
void _ev_debug_shutdown(void);

void ev_debug_message(EvDebugSection section, const gchar *file, gint line,
                      const gchar *function, const gchar *format, ...)
    G_GNUC_PRINTF(5, 6);
void ev_profiler_start(EvProfileSection section, const gchar *format, ...)
    G_GNUC_PRINTF(2, 3);
void ev_profiler_stop(EvProfileSection section, const gchar *format, ...)
    G_GNUC_PRINTF(2, 3);
