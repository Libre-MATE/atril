...
static void ev_link_get_property(GObject *object, guint prop_id, GValue *value,
...
static void ev_link_set_property(GObject *object, guint prop_id,
...
static void ev_link_class_init(EvLinkClass *ev_window_class) {
  ...
  g_object_class_install_property(
      g_object_class, PROP_TITLE,
      g_param_spec_string("title", "Link Title", "The link title", NULL,
                          G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY));
  g_object_class_install_property(
      g_object_class, PROP_ACTION,
      g_param_spec_pointer("action", "Link Action", "The link action",
                           G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY));
}

EvLink *ev_link_new(const char *title, EvLinkAction *action) {
  return EV_LINK(
      g_object_new(EV_TYPE_LINK, "title", title, "action", action, NULL));
}
