...
gdouble ev_link_dest_get_top(EvLinkDest *self, gboolean *change_top) {
  ...
  if (change_top) *change_top = (self->priv->change & EV_DEST_CHANGE_TOP);
...
gdouble ev_link_dest_get_left(EvLinkDest *self, gboolean *change_left) {
  ...
  if (change_left) *change_left = (self->priv->change & EV_DEST_CHANGE_LEFT);
...
gdouble ev_link_dest_get_zoom(EvLinkDest *self, gboolean *change_zoom) {
  ...
  if (change_zoom) *change_zoom = (self->priv->change & EV_DEST_CHANGE_ZOOM);
...
static void ev_link_dest_get_property(GObject *object, guint prop_id,
...
static void ev_link_dest_set_property(GObject *object, guint prop_id,
                                      const GValue *value,
...
static void ev_link_dest_class_init(EvLinkDestClass *ev_link_dest_class) {
  ...
  g_object_class_install_property(
      g_object_class, PROP_TYPE,
      g_param_spec_enum("type", "Dest Type", "The destination type",
                        EV_TYPE_LINK_DEST_TYPE, EV_LINK_DEST_TYPE_UNKNOWN,
                        G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY));
  g_object_class_install_property(
      g_object_class, PROP_PAGE,
      g_param_spec_int("page", "Dest Page", "The destination page", -1,
                       G_MAXINT, 0,
                       G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY));
  g_object_class_install_property(
      g_object_class, PROP_LEFT,
      g_param_spec_double("left", "Left coordinate", "The left coordinate",
                          -G_MAXDOUBLE, G_MAXDOUBLE, 0,
                          G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY));
  g_object_class_install_property(
      g_object_class, PROP_TOP,
      g_param_spec_double("top", "Top coordinate", "The top coordinate",
                          -G_MAXDOUBLE, G_MAXDOUBLE, 0,
                          G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY));
  g_object_class_install_property(
      g_object_class, PROP_BOTTOM,
      g_param_spec_double("bottom", "Bottom coordinate",
                          "The bottom coordinate", -G_MAXDOUBLE, G_MAXDOUBLE, 0,
                          G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY));
  g_object_class_install_property(
      g_object_class, PROP_RIGHT,
      g_param_spec_double("right", "Right coordinate", "The right coordinate",
                          -G_MAXDOUBLE, G_MAXDOUBLE, 0,
                          G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY));

  g_object_class_install_property(
      g_object_class, PROP_ZOOM,
      g_param_spec_double("zoom", "Zoom", "Zoom", 0, G_MAXDOUBLE, 0,
                          G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY));
  g_object_class_install_property(
      g_object_class, PROP_CHANGE,
      g_param_spec_uint(
          "change", "Change", "Whether top, left, and zoom should be changed",
          0, G_MAXUINT, 0, G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY));
  g_object_class_install_property(
      g_object_class, PROP_NAMED,
      g_param_spec_string("named", "Named destination", "The named destination",
                          NULL, G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY));
  g_object_class_install_property(
      g_object_class, PROP_PAGE_LABEL,
      g_param_spec_string("page_label", "Label of the page",
                          "The label of the destination page", NULL,
                          G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY));
}

EvLinkDest *ev_link_dest_new_page(gint page) {
  return EV_LINK_DEST(g_object_new(EV_TYPE_LINK_DEST, "page", page, "type",
                                   EV_LINK_DEST_TYPE_PAGE, NULL));
}

EvLinkDest *ev_link_dest_new_xyz(gint page, gdouble left, gdouble top,
                                 gdouble zoom, gboolean change_left,
                                 gboolean change_top, gboolean change_zoom) {
  EvDestChange change = 0;

  if (change_left) change |= EV_DEST_CHANGE_LEFT;
  if (change_top) change |= EV_DEST_CHANGE_TOP;
  if (change_zoom) change |= EV_DEST_CHANGE_ZOOM;

  return EV_LINK_DEST(g_object_new(EV_TYPE_LINK_DEST, "page", page, "type",
                                   EV_LINK_DEST_TYPE_XYZ, "left", left, "top",
                                   top, "zoom", zoom, "change", change, NULL));
}

EvLinkDest *ev_link_dest_new_fit(gint page) {
  return EV_LINK_DEST(g_object_new(EV_TYPE_LINK_DEST, "page", page, "type",
                                   EV_LINK_DEST_TYPE_FIT, NULL));
}
...
EvLinkDest *ev_link_dest_new_fith(gint page, gdouble top, gboolean change_top) {
  ...
  return EV_LINK_DEST(g_object_new(EV_TYPE_LINK_DEST, "page", page, "type",
                                   EV_LINK_DEST_TYPE_FITH, "top", top, "change",
                                   change, NULL));
}
...
EvLinkDest *ev_link_dest_new_fitv(gint page, gdouble left,
  ...
  return EV_LINK_DEST(g_object_new(EV_TYPE_LINK_DEST, "page", page, "type",
                                   EV_LINK_DEST_TYPE_FITV, "left", left,
                                   "change", change, NULL));
}

EvLinkDest *ev_link_dest_new_fitr(gint page, gdouble left, gdouble bottom,
                                  gdouble right, gdouble top) {
  EvDestChange change = EV_DEST_CHANGE_TOP | EV_DEST_CHANGE_LEFT;

  return EV_LINK_DEST(g_object_new(EV_TYPE_LINK_DEST, "page", page, "type",
                                   EV_LINK_DEST_TYPE_FITR, "left", left,
                                   "bottom", bottom, "right", right, "top", top,
                                   "change", change, NULL));
}

EvLinkDest *ev_link_dest_new_named(const gchar *named_dest) {
  return EV_LINK_DEST(g_object_new(EV_TYPE_LINK_DEST, "named", named_dest,
                                   "type", EV_LINK_DEST_TYPE_NAMED, NULL));
}

EvLinkDest *ev_link_dest_new_page_label(const gchar *page_label) {
  return EV_LINK_DEST(g_object_new(EV_TYPE_LINK_DEST, "page_label", page_label,
                                   "type", EV_LINK_DEST_TYPE_PAGE_LABEL, NULL));
}

EvLinkDest *ev_link_dest_new_hlink(const gchar *hlink, gint page) {
  return EV_LINK_DEST(g_object_new(EV_TYPE_LINK_DEST, "named", hlink, "page",
                                   page, "type", EV_LINK_DEST_TYPE_HLINK,
                                   NULL));
}
