...
GQuark ev_attachment_error_quark(void) {
  ...
  if (error_quark == 0)
    error_quark = g_quark_from_static_string("ev-attachment-error-quark");
...
static void ev_attachment_set_property(GObject *object, guint prop_id,
                                       const GValue *value,
  ...
  switch (prop_id) {
    ...
    case PROP_DATA:
      ...
      attachment->priv->mime_type = g_content_type_guess(
          attachment->priv->name, (guchar *)attachment->priv->data,
          attachment->priv->size, NULL);
      break;
    default:
...
static void ev_attachment_class_init(EvAttachmentClass *klass) {
  ...
  g_object_class_install_property(
      ...
      g_param_spec_string("name", "Name", "The attachment name", NULL,
                          G_PARAM_WRITABLE | G_PARAM_CONSTRUCT_ONLY));
  g_object_class_install_property(
      g_object_class, PROP_DESCRIPTION,
      g_param_spec_string("description", "Description",
                          "The attachment description", NULL,
                          G_PARAM_WRITABLE | G_PARAM_CONSTRUCT_ONLY));
  g_object_class_install_property(
      g_object_class, PROP_MTIME,
      g_param_spec_ulong("mtime", "ModifiedTime",
                         "The attachment modification date", 0, G_MAXULONG, 0,
                         G_PARAM_WRITABLE | G_PARAM_CONSTRUCT_ONLY));
  g_object_class_install_property(
      g_object_class, PROP_CTIME,
      g_param_spec_ulong("ctime", "CreationTime",
                         "The attachment creation date", 0, G_MAXULONG, 0,
                         G_PARAM_WRITABLE | G_PARAM_CONSTRUCT_ONLY));
  g_object_class_install_property(
      g_object_class, PROP_SIZE,
      g_param_spec_uint("size", "Size", "The attachment size", 0, G_MAXUINT, 0,
                        G_PARAM_WRITABLE | G_PARAM_CONSTRUCT_ONLY));
  g_object_class_install_property(
      g_object_class, PROP_DATA,
      g_param_spec_pointer("data", "Data", "The attachment data",
                           G_PARAM_WRITABLE | G_PARAM_CONSTRUCT_ONLY));
...
EvAttachment *ev_attachment_new(const gchar *name, const gchar *description,
  ...
  attachment = g_object_new(EV_TYPE_ATTACHMENT, "name", name, "description",
                            description, "mtime", mtime, "ctime", ctime, "size",
                            size, "data", data, NULL);
...
gboolean ev_attachment_save(EvAttachment *attachment, GFile *file,
  ...
  if (output_stream == NULL) {
    ...
    g_set_error(error, EV_ATTACHMENT_ERROR, ioerror->code,
                _("Couldn't save attachment “%s”: %s"), uri, ioerror->message);
  ...
  written_bytes = g_output_stream_write(G_OUTPUT_STREAM(output_stream),
                                        ...
                                        attachment->priv->size, NULL, &ioerror);
  if (written_bytes == -1) {
    char *uri;
...
static gboolean ev_attachment_launch_app(EvAttachment *attachment,
  ...
  result = g_app_info_launch(attachment->priv->app, files,
                             G_APP_LAUNCH_CONTEXT(context), &ioerror);
  g_object_unref(context);

  if (!result) {
    g_set_error(error, EV_ATTACHMENT_ERROR, (gint)result,
                _("Couldn't open attachment “%s”: %s"), attachment->priv->name,
                ioerror->message);
...
gboolean ev_attachment_open(EvAttachment *attachment, GdkScreen *screen,
  ...
  if (!attachment->priv->app) {
    app_info =
        g_app_info_get_default_for_type(attachment->priv->mime_type, FALSE);
    attachment->priv->app = app_info;
  }

  if (!attachment->priv->app) {
    g_set_error(error, EV_ATTACHMENT_ERROR, 0,
                _("Couldn't open attachment “%s”"), attachment->priv->name);

    return FALSE;
  }

  if (attachment->priv->tmp_file) {
    retval = ev_attachment_launch_app(attachment, screen, timestamp, error);
  } else {
    char *basename;
    char *template;
    GFile *file;
