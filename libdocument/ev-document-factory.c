...
static gboolean mime_type_supported_by_gdk_pixbuf(const gchar *mime_type) {
  GList *mime_types;
  GList *list;
...
static EvCompressionType get_compression_from_mime_type(
    const gchar *mime_type) {
  gchar type[3];
  gchar *p;

  if (!(p = g_strrstr(mime_type, "/"))) return EV_COMPRESSION_NONE;
 ...
 */
static EvDocument *get_document_from_uri(const char *uri, gboolean fast,
  ...
  *compression = EV_COMPRESSION_NONE;
  ...
  if (document == NULL) {
    ...
    if (content_type) mime_desc = g_content_type_get_description(content_type);
...
EvDocument *ev_document_factory_get_document(const char *uri, GError **error) {
  ...
  if (document != NULL) {
    ...
    if (uri_unc) {
      g_object_set_data_full(G_OBJECT(document), "uri-uncompressed", uri_unc,
    ...
    if (result == FALSE || err) {
      if (err && g_error_matches(err, EV_DOCUMENT_ERROR,
                                 EV_DOCUMENT_ERROR_ENCRYPTED)) {
        g_propagate_error(error, err);
        return document;
      }
  ...
  if (result == FALSE) {
    if (err == NULL) {
       ...
       */
      g_set_error_literal(&err, EV_DOCUMENT_ERROR, EV_DOCUMENT_ERROR_INVALID,
                          _("Unknown MIME Type"));
    } else if (g_error_matches(err, EV_DOCUMENT_ERROR,
                               EV_DOCUMENT_ERROR_ENCRYPTED)) {
      g_propagate_error(error, err);
      return document;
    }
...
static void file_filter_add_mime_types(EvTypeInfo *info,
 ...
 *
 * If @document is %NULL, adds a #GtkFileFilter for each document type that
 atril
 * can handle.
 */
void ev_document_factory_add_filters(GtkWidget *chooser, EvDocument *document) {
  ...
  gtk_file_chooser_set_filter(GTK_FILE_CHOOSER(chooser), document == NULL
                                                             ? document_filter
                                                             : default_filter);
}
