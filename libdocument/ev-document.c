...
static void _ev_document_get_page_size(EvDocument *document, EvPage *page,
...
GQuark ev_document_error_quark(void) {
  static GQuark q = 0;
  if (q == 0) q = g_quark_from_static_string("ev-document-error-quark");
...
void ev_document_doc_mutex_lock(void) { g_mutex_lock(&ev_doc_mutex); }

void ev_document_doc_mutex_unlock(void) { g_mutex_unlock(&ev_doc_mutex); }

gboolean ev_document_doc_mutex_trylock(void) {
  return g_mutex_trylock(&ev_doc_mutex);
}

void ev_document_fc_mutex_lock(void) { g_mutex_lock(&ev_fc_mutex); }

void ev_document_fc_mutex_unlock(void) { g_mutex_unlock(&ev_fc_mutex); }
 ...
 */
gboolean ev_document_load(EvDocument *document, const char *uri,
                          GError **error) {
   ...
   * and webkit, support for any other web document types can be added
   * similarly.
   */

  if (!g_strcmp0(ev_file_get_mime_type(uri, TRUE, &err),
  ...
  if (!retval) {
    if (err) {
      ...
      g_warning(
          "%s::EvDocument::load returned FALSE but did not fill in @error; fix "
          "the backend!\n",
    ...
    for (i = 0; i < priv->n_pages; i++) {
      /*
       * Since there is no sense of paging in an ePub,it makes no sense to have
       * pages sizes. We are however geeneralising the scenario by considering
       * epub as a type of web document.
      ...
      } else if (priv->uniform && (priv->uniform_width != page_width ||
                                   priv->uniform_height != page_height)) {
      ...
      if (!priv->uniform) {
        ...
        if (page_width > priv->max_width) priv->max_width = page_width;
        if (page_width < priv->min_width) priv->min_width = page_width;

        if (page_height > priv->max_height) priv->max_height = page_height;
        if (page_height < priv->min_height) priv->min_height = page_height;
      }
      ...
      if (page_label) {
        ...
        priv->max_label = MAX(priv->max_label, g_utf8_strlen(page_label, 256));
      }
 ...
 */
gboolean ev_document_save(EvDocument *document, const char *uri,
                          GError **error) {
  EvDocumentClass *klass = EV_DOCUMENT_GET_CLASS(document);
 ...
 *
 * Returns: A pointer to the EvSourceLink structure that holds the result. @NULL
 * if synctex is not enabled for the document or no result is found. The
 * EvSourceLink pointer should be freed with g_free after it is used.
 */
EvSourceLink *ev_document_synctex_backward_search(EvDocument *document,
                                                  gint page_index, gfloat x,
                                                  gfloat y) {
  ...
  if (synctex_edit_query(scanner, page_index + 1, x, y) > 0) {
    ...
    if (node != NULL) {
      ...
      if (filename) {
        result = ev_source_link_new(filename, synctex_node_line(node),
...
void ev_document_get_page_size(EvDocument *document, gint page_index,
                               double *width, double *height) {
  g_return_if_fail(EV_IS_DOCUMENT(document));
  g_return_if_fail(page_index >= 0 || page_index < document->priv->n_pages);
  if (document->iswebdocument == TRUE) {
    if (width) *width = document->priv->uniform_width;
    if (height) *height = document->priv->uniform_height;
  } else {
    if (width)
...
gchar *ev_document_get_page_label(EvDocument *document, gint page_index) {
  g_return_val_if_fail(EV_IS_DOCUMENT(document), NULL);
  g_return_val_if_fail(page_index >= 0 || page_index < document->priv->n_pages,
                       NULL);

  return (document->priv->page_labels &&
          document->priv->page_labels[page_index])
...
gboolean ev_document_get_backend_info(EvDocument *document,
  ...
  EvDocumentClass *klass = EV_DOCUMENT_GET_CLASS(document);
  if (klass->get_backend_info == NULL) return FALSE;
...
void ev_document_get_max_page_size(EvDocument *document, gdouble *width,
                                   gdouble *height) {
  g_return_if_fail(EV_IS_DOCUMENT(document));

  if (width) *width = document->priv->max_width;
  if (height) *height = document->priv->max_height;
}

void ev_document_get_min_page_size(EvDocument *document, gdouble *width,
                                   gdouble *height) {
  g_return_if_fail(EV_IS_DOCUMENT(document));

  if (width) *width = document->priv->min_width;
  if (height) *height = document->priv->min_height;
}
...
/* EvSourceLink */
G_DEFINE_BOXED_TYPE(EvSourceLink, ev_source_link, ev_source_link_copy,
                    ev_source_link_free)
...
/* EvDocumentInfo */
G_DEFINE_BOXED_TYPE(EvDocumentInfo, ev_document_info, ev_document_info_copy,
...
/* EvDocumentLicense */
G_DEFINE_BOXED_TYPE(EvDocumentLicense, ev_document_license,
                    ev_document_license_copy, ev_document_license_free)
...
EvDocumentLicense *ev_document_license_copy(EvDocumentLicense *license) {
  ...
  if (license->text) new_license->text = g_strdup(license->text);
  if (license->uri) new_license->uri = g_strdup(license->uri);
...
/* EvRectangle */
G_DEFINE_BOXED_TYPE(EvRectangle, ev_rectangle, ev_rectangle_copy,
                    ev_rectangle_free)
...
void ev_rectangle_free(EvRectangle *rectangle) { g_free(rectangle); }
...
gint ev_rect_cmp(EvRectangle *a, EvRectangle *b) {
  ...
  return !((ABS(a->x1 - b->x1) < EPSILON) && (ABS(a->y1 - b->y1) < EPSILON) &&
           (ABS(a->x2 - b->x2) < EPSILON) && (ABS(a->y2 - b->y2) < EPSILON));
}
