...
G_DEFINE_TYPE_WITH_PRIVATE(EvTransitionEffect, ev_transition_effect,
                           G_TYPE_OBJECT)

static void ev_transition_effect_set_property(GObject *object, guint prop_id,
                                              const GValue *value,
                                              GParamSpec *pspec) {
  EvTransitionEffectPrivate *priv;

  priv =
      ev_transition_effect_get_instance_private(EV_TRANSITION_EFFECT(object));
...
static void ev_transition_effect_get_property(GObject *object, guint prop_id,
                                              GValue *value,
                                              GParamSpec *pspec) {
  EvTransitionEffectPrivate *priv;
...
static void ev_transition_effect_class_init(EvTransitionEffectClass *klass) {
  ...
  g_object_class_install_property(
      ...
      g_param_spec_enum("type", "Effect type", "Page transition effect type",
                        EV_TYPE_TRANSITION_EFFECT_TYPE,
                        EV_TRANSITION_EFFECT_REPLACE, G_PARAM_READWRITE));
  g_object_class_install_property(
      object_class, PROP_ALIGNMENT,
      g_param_spec_enum("alignment", "Effect alignment",
                        "Alignment for the effect",
                        EV_TYPE_TRANSITION_EFFECT_ALIGNMENT,
                        EV_TRANSITION_ALIGNMENT_HORIZONTAL, G_PARAM_READWRITE));
  g_object_class_install_property(
      object_class, PROP_DIRECTION,
      g_param_spec_enum("direction", "Effect direction",
                        "Direction for the effect",
                        EV_TYPE_TRANSITION_EFFECT_DIRECTION,
                        EV_TRANSITION_DIRECTION_INWARD, G_PARAM_READWRITE));
  g_object_class_install_property(
      object_class, PROP_DURATION,
      g_param_spec_int("duration", "Effect duration",
                       "Effect duration in seconds", 0, G_MAXINT, 0,
                       G_PARAM_READWRITE));
  g_object_class_install_property(
      object_class, PROP_ANGLE,
      g_param_spec_int("angle", "Effect angle",
                       "Effect angle in degrees, counted "
                       "counterclockwise from left to right",
                       0, 360, 0, G_PARAM_READWRITE));
  g_object_class_install_property(
      object_class, PROP_SCALE,
      g_param_spec_double("scale", "Effect scale",
                          "Scale at which the effect is applied", 0., 1., 1.,
                          G_PARAM_READWRITE));
  g_object_class_install_property(
      object_class, PROP_RECTANGULAR,
      g_param_spec_boolean("rectangular", "Rectangular area",
                           "Whether the covered area is rectangular", FALSE,
                           G_PARAM_READWRITE));
}
...
EvTransitionEffect *ev_transition_effect_new(EvTransitionEffectType type,
  ...
  object = g_object_new(EV_TYPE_TRANSITION_EFFECT, "type", type, NULL);
