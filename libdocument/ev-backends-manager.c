...
static EvBackendInfo *ev_backends_manager_load_backend(const gchar *file) {
  ...
  if (!info->module_name) {
    g_warning("Bad atril backend file %s: Could not find 'Module'", file);
  ...
  info->mime_types = g_key_file_get_string_list(backend_file, EV_BACKENDS_GROUP,
                                                "MimeType", NULL, NULL);
  if (!info->mime_types) {
    g_warning("Bad atril backend file %s: Could not find 'MimeType'", file);
    ev_backend_info_free(info);
    g_key_file_free(backend_file);
...
static gboolean ev_backends_manager_load(void) {
  ...
  while ((dirent = g_dir_read_name(dir))) {
    ...
    if (!g_str_has_suffix(dirent, EV_BACKENDS_EXTENSION)) continue;
...
gboolean _ev_backends_manager_init(void) {
  if (ev_backends_list) return TRUE;
...
static EvBackendInfo *ev_backends_manager_get_backend_info(
  ...
  for (l = ev_backends_list; l; l = g_list_next(l)) {
    ...
    while ((mime = info->mime_types[i++])) {
      if (g_ascii_strcasecmp(mime, mime_type) == 0) return info;
    }
  }
...
const gchar *ev_backends_manager_get_document_module_name(
    EvDocument *document) {
  EvBackendInfo *info;
