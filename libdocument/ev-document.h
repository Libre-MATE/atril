...
#define EV_DOCUMENT(o) \
  (G_TYPE_CHECK_INSTANCE_CAST((o), EV_TYPE_DOCUMENT, EvDocument))
#define EV_DOCUMENT_CLASS(k) \
  (G_TYPE_CHECK_CLASS_CAST((k), EV_TYPE_DOCUMENT, EvDocumentClass))
...
#define EV_DOCUMENT_GET_CLASS(inst) \
  (G_TYPE_INSTANCE_GET_CLASS((inst), EV_TYPE_DOCUMENT, EvDocumentClass))
...
struct _EvDocument {
  ...
  /*
   * Since we can only access the members of this structure from the window
   * frontend, we need a flag to detemine whether to replace the atril-view with
   * a web-view.
   */
...
struct _EvDocumentClass {
  ...
  void (*get_page_size)(EvDocument *document, EvPage *page, double *width,
                        double *height);
 ...
 * (EV_TYPE_DOCUMENT_THUMBNAILS, pdf_document_document_thumbnails_iface_init));
 */
#define EV_BACKEND_IMPLEMENT_INTERFACE(TYPE_IFACE, iface_init)        \
  {                                                                   \
    const GInterfaceInfo g_implement_interface_info = {               \
        (GInterfaceInitFunc)iface_init, NULL, NULL};                  \
    g_type_module_add_interface(module, g_define_type_id, TYPE_IFACE, \
...
#define EV_BACKEND_REGISTER_WITH_CODE(BackendName, backend_name, CODE)     \
  ...
  static GType g_define_type_id = 0;                                       \
                                                                           \
  GType backend_name##_get_type(void) { return g_define_type_id; }         \
                                                                           \
  ...
  G_MODULE_EXPORT GType register_atril_backend(GTypeModule *module) {      \
    const GTypeInfo our_info = {                                           \
        ...
        (GInstanceInitFunc)backend_name##_init};                           \
                                                                           \
    g_define_type_id = g_type_module_register_type(                        \
        module, EV_TYPE_DOCUMENT, #BackendName, &our_info, (GTypeFlags)0); \
                                                                           \
    CODE                                                                   \
                                                                           \
