...
static void ev_annotation_text_markup_iface_init(
    EvAnnotationMarkupInterface *iface);
static void ev_annotation_attachment_markup_iface_init(
    EvAnnotationMarkupInterface *iface);
...
G_DEFINE_TYPE_WITH_CODE(EvAnnotationText, ev_annotation_text,
                        EV_TYPE_ANNOTATION, {
                          G_IMPLEMENT_INTERFACE(
                              EV_TYPE_ANNOTATION_MARKUP,
                              ev_annotation_text_markup_iface_init);
                        });
G_DEFINE_TYPE_WITH_CODE(EvAnnotationAttachment, ev_annotation_attachment,
                        EV_TYPE_ANNOTATION, {
                          G_IMPLEMENT_INTERFACE(
                              EV_TYPE_ANNOTATION_MARKUP,
...
static void ev_annotation_set_property(GObject *object, guint prop_id,
...
static void ev_annotation_get_property(GObject *object, guint prop_id,
...
static void ev_annotation_class_init(EvAnnotationClass *klass) {
  ...
  g_object_class_install_property(
      g_object_class, PROP_ANNOT_PAGE,
      g_param_spec_object("page", "Page", "The page wehere the annotation is",
                          EV_TYPE_PAGE,
                          G_PARAM_WRITABLE | G_PARAM_CONSTRUCT_ONLY));
  g_object_class_install_property(
      g_object_class, PROP_ANNOT_CONTENTS,
      g_param_spec_string("contents", "Contents", "The annotation contents",
                          NULL, G_PARAM_READWRITE));
  g_object_class_install_property(
      g_object_class, PROP_ANNOT_NAME,
      g_param_spec_string("name", "Name", "The annotation unique name", NULL,
                          G_PARAM_READWRITE));
  g_object_class_install_property(
      g_object_class, PROP_ANNOT_MODIFIED,
      g_param_spec_string("modified", "Modified",
                          "Last modified date as string", NULL,
                          G_PARAM_READWRITE));
  /**
   * EvAnnotation:color:
   *
   ...
   */
  g_object_class_install_property(
      g_object_class, PROP_ANNOT_COLOR,
      g_param_spec_pointer("color", "Color", "The annotation color",
                           G_PARAM_READWRITE));
  ...
  g_object_class_install_property(
      g_object_class, PROP_ANNOT_COLOR,
      g_param_spec_boxed("rgba", NULL, NULL, GDK_TYPE_RGBA,
...
gboolean ev_annotation_set_contents(EvAnnotation *annot,
  ...
  if (g_strcmp0(annot->contents, contents) == 0) return FALSE;

  if (annot->contents) g_free(annot->contents);
...
gboolean ev_annotation_set_name(EvAnnotation *annot, const gchar *name) {
  ...
  if (g_strcmp0(annot->name, name) == 0) return FALSE;

  if (annot->name) g_free(annot->name);
 ...
 *
 * Returns: %TRUE if the last modification date has been updated, %FALSE
 * otherwise.
 */
gboolean ev_annotation_set_modified(EvAnnotation *annot,
  ...
  if (g_strcmp0(annot->modified, modified) == 0) return FALSE;

  if (annot->modified) g_free(annot->modified);
...
gboolean ev_annotation_set_color(EvAnnotation *annot, const GdkColor *color) {
  ...
  if (color == NULL ||
      ((color->red == annot_color.red) && (color->green == annot_color.green) &&
...
gboolean ev_annotation_set_rgba(EvAnnotation *annot, const GdkRGBA *rgba) {
  ...
  if (gdk_rgba_equal(rgba, &annot->rgba)) return FALSE;
...
static void ev_annotation_markup_default_init(
    EvAnnotationMarkupInterface *iface) {
  ...
  if (!initialized) {
    g_object_interface_install_property(
        iface,
        g_param_spec_string("label", "Label", "Label of the markup annotation",
                            NULL, G_PARAM_READWRITE));
    g_object_interface_install_property(
        iface, g_param_spec_double("opacity", "Opacity",
                                   "Opacity of the markup annotation", 0,
                                   G_MAXDOUBLE, 1., G_PARAM_READWRITE));
    g_object_interface_install_property(
        iface, g_param_spec_boolean("has_popup", "Has popup",
                                    "Whether the markup annotation has "
                                    "a popup window associated",
                                    TRUE, G_PARAM_READWRITE));
    g_object_interface_install_property(
        iface, g_param_spec_boxed("rectangle", "Rectangle",
                                  "The Rectangle of the popup associated "
                                  "to the markup annotation",
                                  EV_TYPE_RECTANGLE, G_PARAM_READWRITE));
    g_object_interface_install_property(
        iface, g_param_spec_boolean("popup_is_open", "PopupIsOpen",
...
static EvAnnotationMarkupProps *ev_annotation_markup_get_properties(
  ...
  if (!props) {
    ...
    g_object_set_qdata_full(G_OBJECT(markup), props_key, props,
...
static void ev_annotation_markup_set_property(GObject *object, guint prop_id,
                                              const GValue *value,
                                              GParamSpec *pspec) {
  ...
  switch (prop_id) {
    ...
    case PROP_MARKUP_POPUP_IS_OPEN:
      ev_annotation_markup_set_popup_is_open(markup,
...
static void ev_annotation_markup_get_property(GObject *object, guint prop_id,
                                              GValue *value,
                                              GParamSpec *pspec) {
  EvAnnotationMarkupProps *props;
...
static void ev_annotation_markup_class_install_properties(GObjectClass *klass) {
  ...
  g_object_class_override_property(klass, PROP_MARKUP_POPUP_IS_OPEN,
                                   "popup_is_open");
}
...
gboolean ev_annotation_markup_set_label(EvAnnotationMarkup *markup,
  ...
  props = ev_annotation_markup_get_properties(markup);
  if (g_strcmp0(props->label, label) == 0) return FALSE;

  if (props->label) g_free(props->label);
...
gboolean ev_annotation_markup_set_opacity(EvAnnotationMarkup *markup,
  ...
  props = ev_annotation_markup_get_properties(markup);
  if (props->opacity == opacity) return FALSE;
...
gboolean ev_annotation_markup_set_has_popup(EvAnnotationMarkup *markup,
  ...
  props = ev_annotation_markup_get_properties(markup);
  if (props->has_popup == has_popup) return FALSE;
...
gboolean ev_annotation_markup_set_rectangle(EvAnnotationMarkup *markup,
  ...
  if (props->rectangle.x1 == ev_rect->x1 &&
      ...
      props->rectangle.x2 == ev_rect->x2 && props->rectangle.y2 == ev_rect->y2)
    return FALSE;
...
gboolean ev_annotation_markup_set_popup_is_open(EvAnnotationMarkup *markup,
  ...
  props = ev_annotation_markup_get_properties(markup);
  if (props->popup_is_open == is_open) return FALSE;
...
static void ev_annotation_text_set_property(GObject *object, guint prop_id,
                                            const GValue *value,
                                            GParamSpec *pspec) {
...
static void ev_annotation_text_get_property(GObject *object, guint prop_id,
                                            GValue *value, GParamSpec *pspec) {
  EvAnnotationText *annot = EV_ANNOTATION_TEXT(object);
...
static void ev_annotation_text_class_init(EvAnnotationTextClass *klass) {
  ...
  g_object_class_install_property(
      g_object_class, PROP_TEXT_ICON,
      g_param_spec_enum("icon", "Icon", "The icon fo the text annotation",
                        EV_TYPE_ANNOTATION_TEXT_ICON,
                        EV_ANNOTATION_TEXT_ICON_NOTE, G_PARAM_READWRITE));
  g_object_class_install_property(
      g_object_class, PROP_TEXT_IS_OPEN,
      g_param_spec_boolean("is_open", "IsOpen",
                           "Whether text annot is initially open", FALSE,
                           G_PARAM_READWRITE));
}
...
EvAnnotation *ev_annotation_text_new(EvPage *page) {
  return EV_ANNOTATION(
      g_object_new(EV_TYPE_ANNOTATION_TEXT, "page", page, NULL));
}
...
gboolean ev_annotation_text_set_icon(EvAnnotationText *text,
  ...
  if (text->icon == icon) return FALSE;
...
gboolean ev_annotation_text_set_is_open(EvAnnotationText *text,
  ...
  if (text->is_open == is_open) return FALSE;
...
static void ev_annotation_attachment_class_init(
    EvAnnotationAttachmentClass *klass) {
  GObjectClass *g_object_class = G_OBJECT_CLASS(klass);
  ...
  g_object_class_install_property(
      ...
      g_param_spec_object(
          "attachment", "Attachment", "The attachment of the annotation",
          EV_TYPE_ATTACHMENT, G_PARAM_CONSTRUCT | G_PARAM_READWRITE));
}
...
EvAnnotation *ev_annotation_attachment_new(EvPage *page,
  ...
  return EV_ANNOTATION(g_object_new(EV_TYPE_ANNOTATION_ATTACHMENT, "page", page,
...
gboolean ev_annotation_attachment_set_attachment(EvAnnotationAttachment *annot,
  ...
  if (annot->attachment == attachment) return FALSE;

  if (annot->attachment) g_object_unref(annot->attachment);
