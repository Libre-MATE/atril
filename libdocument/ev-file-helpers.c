 ...
 */
static gboolean _ev_dir_ensure_exists(const gchar *dir, int mode,
                                      GError **error) {
  int errsv;
  char *display_name;
  ...
  errno = 0;
  if (g_mkdir_with_parents(dir, mode) == 0) return TRUE;

  errsv = errno;
  if (errsv == EEXIST && g_file_test(dir, G_FILE_TEST_IS_DIR)) return TRUE;

  display_name = g_filename_display_name(dir);
  g_set_error(error, G_IO_ERROR, g_io_error_from_errno(errsv),
              "Failed to create directory '%s': %s", display_name,
              g_strerror(errsv));
  g_free(display_name);
...
static const char *_ev_tmp_dir(GError **error) {
  ...
  if (!_ev_dir_ensure_exists(tmp_dir, 0700, error)) return NULL;
...
void _ev_file_helpers_shutdown(void) {
  if (tmp_dir != NULL) g_rmdir(tmp_dir);
...
/**
 * ev_mkstemp:
 * @template: a template string; must contain 'XXXXXX', but not necessarily as a
 * suffix
...
int ev_mkstemp(const char *template, char **file_name, GError **error) {
  ...
  if ((tmp = _ev_tmp_dir(error)) == NULL) return -1;
  ...
  if (fd == -1) {
    ...
    g_set_error(error, G_IO_ERROR, g_io_error_from_errno(errsv),
                _("Failed to create a temporary file: %s"), g_strerror(errsv));
 ...
 * @error: a location to store a #GError
 *
 * Creates a temp #GFile in the atril temp directory. See ev_mkstemp() for more
 * information.
 *
 * Returns: a newly allocated #GFile for the newly created temp file name, or
 * %NULL on error with @error filled in
 */
GFile *ev_mkstemp_file(const char *template, GError **error) {
  ...
  g_object_set_data_full(G_OBJECT(file), "ev-mkstemp-fd", GINT_TO_POINTER(fd),
                         (GDestroyNotify)close_fd_cb);
...
gchar *ev_mkdtemp(const char *template, GError **error) {
  ...
  if ((tmp = _ev_tmp_dir(error)) == NULL) return NULL;
...
static gchar *get_mime_type_from_uri(const gchar *uri, GError **error) {
  ...
  file = g_file_new_for_uri(uri);
  file_info = g_file_query_info(file, G_FILE_ATTRIBUTE_STANDARD_CONTENT_TYPE, 0,
                                NULL, error);
  g_object_unref(file);
...
static gchar *get_mime_type_from_data(const gchar *uri, GError **error) {
  ...
  size_read = g_input_stream_read(G_INPUT_STREAM(input_stream), buffer,
...
static const char *compressor_cmds[] = {NULL, "bzip2", "gzip", "xz"};

#define N_ARGS 4
#define BUFFER_SIZE 1024

static gchar *compression_run(const gchar *uri, EvCompressionType type,
  ...
  if (type == EV_COMPRESSION_NONE) return NULL;
  ...
  if (g_spawn_async_with_pipes(NULL, argv, NULL, G_SPAWN_STDERR_TO_DEV_NULL,
                               NULL, NULL, NULL, NULL, &pout, NULL, &err)) {
    GIOChannel *in, *out;
    ...
    do {
      read_st =
          g_io_channel_read_chars(in, buf, BUFFER_SIZE, &bytes_read, error);
      if (read_st == G_IO_STATUS_NORMAL) {
        write_st = g_io_channel_write_chars(out, buf, bytes_read,
                                            &bytes_written, error);
        if (write_st == G_IO_STATUS_ERROR) break;
