...
static void ev_page_action_widget_set_current_page(
    EvPageActionWidget *action_widget, gint page) {
  if (page >= 0) {
    gchar *page_label;

    gtk_entry_set_width_chars(
        GTK_ENTRY(action_widget->entry),
        CLAMP(ev_document_get_max_label_len(action_widget->document), 6, 12));
...
static void page_changed_cb(EvDocumentModel *model, gint old_page,
                            gint new_page, EvPageActionWidget *action_widget) {
...
static gboolean page_scroll_cb(EvPageActionWidget *action_widget,
  ...
  if ((event->direction == GDK_SCROLL_DOWN) &&
    ...
    pageno++;
  if ((event->direction == GDK_SCROLL_UP) && (pageno > 0)) pageno--;
...
static void ev_page_action_widget_init(EvPageActionWidget *action_widget) {
  ...
  gtk_box_pack_start(GTK_BOX(hbox), action_widget->label, FALSE, FALSE, 0);
...
static void ev_page_action_widget_document_changed_cb(
  ...
  if (action_widget->document) g_object_unref(action_widget->document);
  action_widget->document = document;

#if GLIB_CHECK_VERSION(2, 62, 0)
  g_clear_signal_handler(&action_widget->signal_id, action_widget->doc_model);
#else
...
#endif
  action_widget->signal_id =
      g_signal_connect_object(action_widget->doc_model, "page-changed",
                              G_CALLBACK(page_changed_cb), action_widget, 0);
...
static void ev_page_action_widget_class_init(EvPageActionWidgetClass *class) {
  GObjectClass *object_class = G_OBJECT_CLASS(class);

  object_class->finalize = ev_page_action_widget_finalize;

  widget_signals[WIDGET_ACTIVATE_LINK] = g_signal_new(
      "activate_link", G_OBJECT_CLASS_TYPE(object_class),
      G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
      G_STRUCT_OFFSET(EvPageActionWidgetClass, activate_link), NULL, NULL,
...
static gboolean match_selected_cb(GtkEntryCompletion *completion,
  ...
  gtk_tree_model_get(filter_model, filter_iter, 0, &iter, -1);
  gtk_tree_model_get(proxy->model, iter, EV_DOCUMENT_LINKS_COLUMN_LINK, &link,
                     -1);
...
static gboolean match_completion(GtkEntryCompletion *completion,
                                 const gchar *key, GtkTreeIter *filter_iter,
                                 EvPageActionWidget *proxy) {
  EvLink *link;
  GtkTreeIter *iter;
  const gchar *text = NULL;

  gtk_tree_model_get(gtk_entry_completion_get_model(completion), filter_iter, 0,
                     &iter, -1);
  gtk_tree_model_get(proxy->model, iter, EV_DOCUMENT_LINKS_COLUMN_LINK, &link,
                     -1);
  ...
  if (text && key) {
    ...
    if (strstr(case_normalized_text, case_normalized_key)) retval = TRUE;
...
static gboolean build_new_tree_cb(GtkTreeModel *model, GtkTreePath *path,
  ...
  if (type == EV_LINK_ACTION_TYPE_GOTO_DEST) {
    ...
    gtk_list_store_set(GTK_LIST_STORE(filter_model), &filter_iter, 0, iter, -1);
  }
...
static GtkTreeModel *get_filter_model_from_model(GtkTreeModel *model) {
  ...
  if (filter_model == NULL) {
    ...
    gtk_tree_model_foreach(model, build_new_tree_cb, filter_model);
    g_object_set_data_full(G_OBJECT(model), EPA_FILTER_MODEL_DATA, filter_model,
                           g_object_unref);
  }
...
void ev_page_action_widget_update_links_model(EvPageActionWidget *proxy,
  ...
  g_object_set(G_OBJECT(completion), "popup-set-width", FALSE, "model",
               filter_model, NULL);

  g_signal_connect(completion, "match-selected", G_CALLBACK(match_selected_cb),
                   proxy);
  gtk_entry_completion_set_match_func(
      completion, (GtkEntryCompletionMatchFunc)match_completion, proxy, NULL);
  ...
  gtk_cell_layout_set_cell_data_func(
      GTK_CELL_LAYOUT(completion), renderer,
      (GtkCellLayoutDataFunc)display_completion_text, proxy, NULL);
