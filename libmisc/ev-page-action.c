...
static void update_model(EvPageAction *page, GParamSpec *pspec,
                         EvPageActionWidget *proxy) {
  ev_page_action_widget_update_links_model(proxy, page->priv->model);
}

static void activate_link_cb(EvPageActionWidget *proxy, EvLink *link,
                             EvPageAction *action) {
...
static void connect_proxy(GtkAction *action, GtkWidget *proxy) {
  ...
  if (GTK_IS_TOOL_ITEM(proxy)) {
    ev_page_action_widget_set_model(EV_PAGE_ACTION_WIDGET(proxy),
                                    page->priv->doc_model);
    g_signal_connect(proxy, "activate_link", G_CALLBACK(activate_link_cb),
                     action);
    g_signal_connect_object(action, "notify::model", G_CALLBACK(update_model),
                            proxy, 0);
  }
...
static void ev_page_action_set_property(GObject *object, guint prop_id,
                                        const GValue *value,
                                        GParamSpec *pspec) {
  EvPageAction *page = EV_PAGE_ACTION(object);
...
static void ev_page_action_get_property(GObject *object, guint prop_id,
...
void ev_page_action_set_model(EvPageAction *page, EvDocumentModel *model) {
  ...
  if (page->priv->doc_model == model) return;
...
void ev_page_action_set_links_model(EvPageAction *page,
  ...
  if (page->priv->model == links_model) return;

  if (page->priv->model) g_object_unref(page->priv->model);
...
static void ev_page_action_init(EvPageAction *page) {
...
static void ev_page_action_class_init(EvPageActionClass *class) {
  ...
  signals[ACTIVATE_LINK] = g_signal_new(
      "activate_link", G_OBJECT_CLASS_TYPE(object_class),
      G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
      G_STRUCT_OFFSET(EvPageActionClass, activate_link), NULL, NULL,
      g_cclosure_marshal_VOID__OBJECT, G_TYPE_NONE, 1, G_TYPE_OBJECT);

  g_object_class_install_property(
      object_class, PROP_MODEL,
      g_param_spec_object("model", "Model", "Current Links Model",
