...
enum { PROP_0, PROP_UI_MANAGER, PROP_TOOLBARS_MODEL };
...
static gint compare_items(gconstpointer a, gconstpointer b) {
  ...
  char *key1 = g_object_get_data(G_OBJECT(item1), "egg-collate-key");
  char *key2 = g_object_get_data(G_OBJECT(item2), "egg-collate-key");

  return strcmp(key1, key2);
}

static GtkAction *find_action(EggToolbarEditor *t, const char *name) {
  GList *l;
...
static void egg_toolbar_editor_set_ui_manager(EggToolbarEditor *t,
                                              GtkUIManager *manager) {
...
static void item_added_or_removed_cb(EggToolbarsModel *model, int tpos,
                                     int ipos, EggToolbarEditor *editor) {
...
static void egg_toolbar_editor_disconnect_model(EggToolbarEditor *t) {
...
void egg_toolbar_editor_set_model(EggToolbarEditor *t,
  ...
  priv->sig_handlers[SIGNAL_HANDLER_ITEM_ADDED] = g_signal_connect_object(
      model, "item_added", G_CALLBACK(item_added_or_removed_cb), t, 0);
  priv->sig_handlers[SIGNAL_HANDLER_ITEM_REMOVED] = g_signal_connect_object(
      model, "item_removed", G_CALLBACK(item_added_or_removed_cb), t, 0);
  priv->sig_handlers[SIGNAL_HANDLER_TOOLBAR_REMOVED] = g_signal_connect_object(
      model, "toolbar_removed", G_CALLBACK(toolbar_removed_cb), t, 0);
}

static void egg_toolbar_editor_set_property(GObject *object, guint prop_id,
                                            const GValue *value,
                                            GParamSpec *pspec) {
...
static void egg_toolbar_editor_get_property(GObject *object, guint prop_id,
...
static void egg_toolbar_editor_class_init(EggToolbarEditorClass *klass) {
  ...
  g_object_class_install_property(
      ...
      g_param_spec_object(
          "ui-manager", "UI-Manager", "UI Manager", GTK_TYPE_UI_MANAGER,
          G_PARAM_READWRITE | G_PARAM_STATIC_NAME | G_PARAM_STATIC_NICK |
              G_PARAM_STATIC_BLURB | G_PARAM_CONSTRUCT_ONLY));
  g_object_class_install_property(
      object_class, PROP_TOOLBARS_MODEL,
      g_param_spec_object(
          "model", "Model", "Toolbars Model", EGG_TYPE_TOOLBARS_MODEL,
          G_PARAM_READWRITE | G_PARAM_STATIC_NAME | G_PARAM_STATIC_NICK |
              G_PARAM_STATIC_BLURB | G_PARAM_CONSTRUCT));
...
GtkWidget *egg_toolbar_editor_new(GtkUIManager *manager,
                                  EggToolbarsModel *model) {
  return GTK_WIDGET(g_object_new(EGG_TYPE_TOOLBAR_EDITOR, "ui-manager", manager,
                                 "model", model, NULL));
}
...
static void drag_data_get_cb(GtkWidget *widget, GdkDragContext *context,
                             ...
                             guint32 time, EggToolbarEditor *editor) {
  const char *target;
...
static gchar *elide_underscores(const gchar *original) {
  gchar *q, *result;
  const gchar *p;
...
static void set_drag_cursor(GtkWidget *widget) {
  GdkCursor *cursor;
  GdkScreen *screen;
...
static void event_box_realize_cb(GtkWidget *widget, GtkImage *icon) {
  GtkImageType type;
  ...
  if (type == GTK_IMAGE_STOCK) {
    ...
    pixbuf = gtk_icon_theme_load_icon(icon_theme, icon_name, MIN(width, height),
                                      0, NULL);
    if (G_UNLIKELY(!pixbuf)) return;
...
static GtkWidget *editor_create_item(EggToolbarEditor *editor, GtkImage *icon,
  ...
  gtk_drag_source_set(event_box, GDK_BUTTON1_MASK, source_drag_types,
                      G_N_ELEMENTS(source_drag_types), action);
  g_signal_connect(event_box, "drag_data_get", G_CALLBACK(drag_data_get_cb),
                   editor);
  g_signal_connect_after(event_box, "realize", G_CALLBACK(event_box_realize_cb),
                         icon);

  if (action == GDK_ACTION_MOVE) {
    g_signal_connect(event_box, "drag_begin", G_CALLBACK(drag_begin_cb), NULL);
    g_signal_connect(event_box, "drag_end", G_CALLBACK(drag_end_cb), NULL);
  }
...
static GtkWidget *editor_create_item_from_name(EggToolbarEditor *editor,
                                               const char *name,
  ...
  if (strcmp(name, "_separator") == 0) {
    ...
    item =
        editor_create_item(editor, GTK_IMAGE(icon), short_label, drag_action);
  } else {
    GtkAction *action;
    GtkWidget *icon;
  ...
  g_object_set_data_full(G_OBJECT(item), "egg-item-name", (gpointer)item_name,
                         g_free);

  return item;
}

static gint append_grid(GtkGrid *grid, GList *items, gint y, gint width) {
  if (items != NULL) {
    gint x = 0;
    GtkWidget *item;
...
static void update_editor_sheet(EggToolbarEditor *editor) {
  ...
  gtk_drag_dest_set(grid, GTK_DEST_DEFAULT_ALL, dest_drag_types,
...
static void setup_editor(EggToolbarEditor *editor) {
  GtkWidget *scrolled_window;
