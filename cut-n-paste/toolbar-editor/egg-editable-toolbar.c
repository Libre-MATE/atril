...
enum { ACTION_REQUEST, LAST_SIGNAL };
...
G_DEFINE_TYPE_WITH_PRIVATE(EggEditableToolbar, egg_editable_toolbar,
                           GTK_TYPE_BOX);

static int get_dock_position(EggEditableToolbar *etoolbar, GtkWidget *dock) {
  GList *l;
  int result;
...
static int get_toolbar_position(EggEditableToolbar *etoolbar,
                                GtkWidget *toolbar) {
  return get_dock_position(etoolbar, gtk_widget_get_parent(toolbar));
}

static int get_n_toolbars(EggEditableToolbar *etoolbar) {
  GList *l;
  int result;
...
static GtkWidget *get_dock_nth(EggEditableToolbar *etoolbar, int position) {
  GList *l;
  GtkWidget *result;
...
static GtkWidget *get_toolbar_nth(EggEditableToolbar *etoolbar, int position) {
  GList *l;
  GtkWidget *dock;
  GtkWidget *result;
...
static GtkAction *find_action(EggEditableToolbar *etoolbar, const char *name) {
  GList *l;
...
static void drag_data_delete_cb(GtkWidget *widget, GdkDragContext *context,
  ...
  pos = gtk_toolbar_get_item_index(GTK_TOOLBAR(parent), GTK_TOOL_ITEM(widget));
...
static void drag_data_get_cb(GtkWidget *widget, GdkDragContext *context,
                             ...
                             guint32 time, EggEditableToolbar *etoolbar) {
  EggToolbarsModel *model;
  const char *name;
  char *data;
  GdkAtom target;
  ...
  if (name == NULL) {
    name = g_object_get_data(G_OBJECT(gtk_widget_get_parent(widget)),
                             EGG_ITEM_NAME);
  ...
  if (data != NULL) {
    gtk_selection_data_set(selection_data, target, 8, (unsigned char *)data,
                           strlen(data));
    g_free(data);
  }
}

static void move_item_cb(GtkAction *action, EggEditableToolbar *etoolbar) {
  GtkWidget *toolitem = gtk_widget_get_ancestor(
  ...
  gdk_event_get_root_coords(realevent, &event->motion.x_root,
                            &event->motion.y_root);

  gtk_drag_begin_with_coordinates(toolitem, list, GDK_ACTION_MOVE, 1, event,
                                  event->motion.x, event->motion.y);
  gdk_event_free(event);
  gtk_target_list_unref(list);
}

static void remove_item_cb(GtkAction *action, EggEditableToolbar *etoolbar) {
  GtkWidget *toolitem = gtk_widget_get_ancestor(
      egg_editable_toolbar_get_selected(etoolbar), GTK_TYPE_TOOL_ITEM);
  ...
  toolbar_pos = get_toolbar_position(etoolbar, parent);
  pos =
      gtk_toolbar_get_item_index(GTK_TOOLBAR(parent), GTK_TOOL_ITEM(toolitem));
...
static void remove_toolbar_cb(GtkAction *action, EggEditableToolbar *etoolbar) {
...
static void popup_context_deactivate(GtkMenuShell *menu,
                                     EggEditableToolbar *etoolbar) {
  egg_editable_toolbar_set_selected(etoolbar, NULL);
  g_object_notify(G_OBJECT(etoolbar), "selected");
}

static void popup_context_menu_cb(GtkWidget *toolbar, gint x, gint y,
                                  gint button_number,
                                  EggEditableToolbar *etoolbar) {
  if (etoolbar->priv->popup_path != NULL) {
    ...
    g_signal_connect_object(menu, "selection-done",
                            G_CALLBACK(popup_context_deactivate), etoolbar, 0);
  }
}

static gboolean edit_mode_button_press_event_cb(GtkWidget *widget,
...
static void configure_item_sensitivity(GtkToolItem *item,
                                       EggEditableToolbar *etoolbar) {
  GtkAction *action;
  char *name;
  ...
  gtk_tool_item_set_use_drag_window(item, (etoolbar->priv->edit_mode > 0) ||
                                              GTK_IS_SEPARATOR_TOOL_ITEM(item));
}

static void configure_item_window_drag(GtkToolItem *item,
                                       EggEditableToolbar *etoolbar) {
  if (etoolbar->priv->edit_mode > 0) {
    ...
    g_signal_handlers_disconnect_by_func(
        item, G_CALLBACK(edit_mode_button_press_event_cb), NULL);
  }
}

static void configure_item_cursor(GtkToolItem *item,
                                  EggEditableToolbar *etoolbar) {
  ...
  if (window != NULL) {
    if (priv->edit_mode > 0) {
      ...
      cursor =
          gdk_cursor_new_for_display(gdk_screen_get_display(screen), GDK_HAND2);
      ...
      if (GTK_IS_SEPARATOR_TOOL_ITEM(item)) {
        ...
        if (action) {
          g_object_get(action, "icon-name", &icon_name, "stock-id", &stock_id,
                       NULL);
        }
        if (icon_name) {
          GdkScreen *screen;
          ...
          if (!gtk_icon_size_lookup(GTK_ICON_SIZE_LARGE_TOOLBAR, &width,
                                    &height)) {
            width = height = 24;
          }
...
static void configure_item_tooltip(GtkToolItem *item) {
  GtkAction *action;
...
static void connect_widget_signals(GtkWidget *proxy,
                                   EggEditableToolbar *etoolbar) {
  ...
  if (GTK_IS_TOOL_ITEM(proxy)) {
    g_signal_connect_object(proxy, "drag_begin", G_CALLBACK(drag_begin_cb),
                            etoolbar, 0);
    g_signal_connect_object(proxy, "drag_end", G_CALLBACK(drag_end_cb),
                            etoolbar, 0);
  ...
  if (GTK_IS_BUTTON(proxy) || GTK_IS_TOOL_ITEM(proxy)) {
    g_signal_connect_object(proxy, "button-press-event",
                            G_CALLBACK(button_press_event_cb), etoolbar, 0);
  }
}

static void action_sensitive_cb(GtkAction *action, GParamSpec *pspec,
                                GtkToolItem *item) {
  EggEditableToolbar *etoolbar;
...
static GtkToolItem *create_item_from_action(EggEditableToolbar *etoolbar,
                                            const char *name) {
  GtkToolItem *item;
  ...
  g_object_set_data_full(G_OBJECT(item), EGG_ITEM_NAME, g_strdup(name), g_free);

  return item;
}

static GtkToolItem *create_item_from_position(EggEditableToolbar *etoolbar,
                                              int toolbar_position,
                                              int position) {
  GtkToolItem *item;
  const char *name;

  name = egg_toolbars_model_item_nth(etoolbar->priv->model, toolbar_position,
                                     position);
...
static void toolbar_drag_data_received_cb(GtkToolbar *toolbar,
  ...
  if (gtk_selection_data_get_length(selection_data) >= 0) {
    ...
    name =
        egg_toolbars_model_get_name(etoolbar->priv->model, type, data, FALSE);
    if (name != NULL) {
      used = ((egg_toolbars_model_get_name_flags(etoolbar->priv->model, name) &
  ...
  if (etoolbar->priv->dnd_pending > 0) {
    ...
    if (name == NULL && gtk_selection_data_get_length(selection_data) >= 0) {
      name =
          egg_toolbars_model_get_name(etoolbar->priv->model, type, data, TRUE);
    }
...
static gboolean toolbar_drag_drop_cb(GtkToolbar *toolbar,
...
static gboolean toolbar_drag_motion_cb(GtkToolbar *toolbar,
                                       GdkDragContext *context, gint x, gint y,
                                       guint time,
                                       EggEditableToolbar *etoolbar) {
  GdkAtom target =
...
static void toolbar_drag_leave_cb(GtkToolbar *toolbar, GdkDragContext *context,
                                  guint time, EggEditableToolbar *etoolbar) {
  gtk_toolbar_set_drop_highlight_item(toolbar, NULL, 0);
...
static void configure_drag_dest(EggEditableToolbar *etoolbar,
                                GtkToolbar *toolbar) {
  ...
  gtk_drag_dest_set(GTK_WIDGET(toolbar), 0, dest_drag_types,
...
static void toggled_visibility_cb(GtkToggleAction *action,
                                  EggEditableToolbar *etoolbar) {
  EggEditableToolbarPrivate *priv = etoolbar->priv;
  GtkWidget *dock;
  ...
  for (i = 0; i < priv->visibility_actions->len; i++)
    if (g_ptr_array_index(priv->visibility_actions, i) == action) break;
...
static void toolbar_visibility_refresh(EggEditableToolbar *etoolbar) {
  EggEditableToolbarPrivate *priv = etoolbar->priv;
  ...
  for (i = 0; i < n_toolbars; i++) {
    ...
    if (i >= priv->visibility_actions->len) {
      ...
      g_signal_connect_object(action, "toggled",
                              G_CALLBACK(toggled_visibility_cb), etoolbar, 0);
...
static GtkWidget *create_dock(EggEditableToolbar *etoolbar) {
  ...
  g_signal_connect(toolbar, "drag_drop", G_CALLBACK(toolbar_drag_drop_cb),
                   etoolbar);
  g_signal_connect(toolbar, "drag_motion", G_CALLBACK(toolbar_drag_motion_cb),
                   etoolbar);
  g_signal_connect(toolbar, "drag_leave", G_CALLBACK(toolbar_drag_leave_cb),
                   etoolbar);
...
static void set_fixed_style(EggEditableToolbar *t, GtkToolbarStyle style) {
  g_return_if_fail(GTK_IS_TOOLBAR(t->priv->fixed_toolbar));
  gtk_toolbar_set_style(
      GTK_TOOLBAR(t->priv->fixed_toolbar),
      style == GTK_TOOLBAR_ICONS ? GTK_TOOLBAR_BOTH_HORIZ : style);
}

static void unset_fixed_style(EggEditableToolbar *t) {
  g_return_if_fail(GTK_IS_TOOLBAR(t->priv->fixed_toolbar));
  gtk_toolbar_unset_style(GTK_TOOLBAR(t->priv->fixed_toolbar));
}

static void toolbar_changed_cb(EggToolbarsModel *model, int position,
                               EggEditableToolbar *etoolbar) {
  GtkWidget *toolbar;
  EggTbModelFlags flags;
...
static void unparent_fixed(EggEditableToolbar *etoolbar) {
...
static void update_fixed(EggEditableToolbar *etoolbar) {
  GtkWidget *toolbar, *dock;
...
static void toolbar_added_cb(EggToolbarsModel *model, int position,
                             EggEditableToolbar *etoolbar) {
  GtkWidget *dock;
...
static void toolbar_removed_cb(EggToolbarsModel *model, int position,
                               EggEditableToolbar *etoolbar) {
  GtkWidget *dock;
...
static void item_added_cb(EggToolbarsModel *model, int tpos, int ipos,
                          EggEditableToolbar *etoolbar) {
  GtkWidget *dock;
  GtkWidget *toolbar;
  GtkToolItem *item;
...
static void item_removed_cb(EggToolbarsModel *model, int toolbar_position,
  ...
  item = GTK_WIDGET(gtk_toolbar_get_nth_item(GTK_TOOLBAR(toolbar), position));
...
static void egg_editable_toolbar_build(EggEditableToolbar *etoolbar) {
...
static void egg_editable_toolbar_disconnect_model(EggEditableToolbar *toolbar) {
  EggToolbarsModel *model = toolbar->priv->model;

  g_signal_handlers_disconnect_by_func(model, G_CALLBACK(item_added_cb),
                                       toolbar);
  g_signal_handlers_disconnect_by_func(model, G_CALLBACK(item_removed_cb),
                                       toolbar);
  g_signal_handlers_disconnect_by_func(model, G_CALLBACK(toolbar_added_cb),
                                       toolbar);
  g_signal_handlers_disconnect_by_func(model, G_CALLBACK(toolbar_removed_cb),
                                       toolbar);
  g_signal_handlers_disconnect_by_func(model, G_CALLBACK(toolbar_changed_cb),
                                       toolbar);
}

static void egg_editable_toolbar_deconstruct(EggEditableToolbar *toolbar) {
  EggToolbarsModel *model = toolbar->priv->model;
  GList *children;
...
void egg_editable_toolbar_set_model(EggEditableToolbar *etoolbar,
  ...
  g_signal_connect(model, "item_added", G_CALLBACK(item_added_cb), etoolbar);
  g_signal_connect(model, "item_removed", G_CALLBACK(item_removed_cb),
                   etoolbar);
  g_signal_connect(model, "toolbar_added", G_CALLBACK(toolbar_added_cb),
                   etoolbar);
  g_signal_connect(model, "toolbar_removed", G_CALLBACK(toolbar_removed_cb),
                   etoolbar);
  g_signal_connect(model, "toolbar_changed", G_CALLBACK(toolbar_changed_cb),
                   etoolbar);
}
...
static void egg_editable_toolbar_dispose(GObject *object) {
  ...
  if (priv->model) egg_editable_toolbar_disconnect_model(etoolbar);
...
static void egg_editable_toolbar_set_ui_manager(EggEditableToolbar *etoolbar,
                                                GtkUIManager *manager) {
...
void egg_editable_toolbar_set_selected(EggEditableToolbar *etoolbar,
  ...
  toolbar = (widget != NULL) ? gtk_widget_get_ancestor(widget, GTK_TYPE_TOOLBAR)
                             : NULL;
  toolitem = (widget != NULL)
                 ? gtk_widget_get_ancestor(widget, GTK_TYPE_TOOL_ITEM)
                 : NULL;

  if (toolbar != NULL) {
    gint tpos = get_toolbar_position(etoolbar, toolbar);
    editable = ((egg_toolbars_model_get_flags(etoolbar->priv->model, tpos) &
  ...
  gtk_action_set_visible(find_action(etoolbar, "RemoveToolbar"),
                         (toolbar != NULL) && (etoolbar->priv->edit_mode > 0));
...
static void set_edit_mode(EggEditableToolbar *etoolbar, gboolean mode) {
  EggEditableToolbarPrivate *priv = etoolbar->priv;
  int i, l, n_items;
...
static void egg_editable_toolbar_set_property(GObject *object, guint prop_id,
                                              const GValue *value,
                                              GParamSpec *pspec) {
  EggEditableToolbar *etoolbar = EGG_EDITABLE_TOOLBAR(object);
...
static void egg_editable_toolbar_get_property(GObject *object, guint prop_id,
                                              GValue *value,
                                              GParamSpec *pspec) {
  EggEditableToolbar *etoolbar = EGG_EDITABLE_TOOLBAR(object);
...
static void egg_editable_toolbar_class_init(EggEditableToolbarClass *klass) {
  ...
  egg_editable_toolbar_signals[ACTION_REQUEST] = g_signal_new(
      "action_request", G_OBJECT_CLASS_TYPE(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(EggEditableToolbarClass, action_request), NULL, NULL,
      g_cclosure_marshal_VOID__STRING, G_TYPE_NONE, 1, G_TYPE_STRING);

  g_object_class_install_property(
      object_class, PROP_UI_MANAGER,
      g_param_spec_object("ui-manager", "UI-Mmanager", "UI Manager",
                          GTK_TYPE_UI_MANAGER,
                          G_PARAM_READWRITE | G_PARAM_STATIC_NAME |
                              G_PARAM_STATIC_NICK | G_PARAM_STATIC_BLURB));
  g_object_class_install_property(
      object_class, PROP_TOOLBARS_MODEL,
      g_param_spec_object("model", "Model", "Toolbars Model",
                          EGG_TYPE_TOOLBARS_MODEL,
                          G_PARAM_READWRITE | G_PARAM_STATIC_NAME |
                              G_PARAM_STATIC_NICK | G_PARAM_STATIC_BLURB));
  g_object_class_install_property(
      object_class, PROP_SELECTED,
      g_param_spec_object("selected", "Selected", "Selected toolitem",
                          GTK_TYPE_TOOL_ITEM,
                          G_PARAM_READABLE | G_PARAM_STATIC_NAME |
                              G_PARAM_STATIC_NICK | G_PARAM_STATIC_BLURB));

  g_object_class_install_property(
      object_class, PROP_POPUP_PATH,
      g_param_spec_string("popup-path", "popup-path", "popup-path", NULL,
                          G_PARAM_READWRITE | G_PARAM_STATIC_NAME |
                              G_PARAM_STATIC_NICK | G_PARAM_STATIC_BLURB));

  g_object_class_install_property(
      object_class, PROP_EDIT_MODE,
      g_param_spec_boolean("edit-mode", "Edit-Mode", "Edit Mode", FALSE,
                           G_PARAM_READWRITE | G_PARAM_STATIC_NAME |
                               G_PARAM_STATIC_NICK | G_PARAM_STATIC_BLURB));
}
...
GtkWidget *egg_editable_toolbar_new(GtkUIManager *manager,
  ...
  return GTK_WIDGET(g_object_new(EGG_TYPE_EDITABLE_TOOLBAR, "ui-manager",
                                 manager, "popup-path", popup_path, NULL));
}
...
GtkWidget *egg_editable_toolbar_new_with_model(GtkUIManager *manager,
                                               ...
                                               const char *popup_path) {
  return GTK_WIDGET(g_object_new(EGG_TYPE_EDITABLE_TOOLBAR, "ui-manager",
                                 manager, "model", model, "popup-path",
                                 popup_path, NULL));
}
...
void egg_editable_toolbar_add_visibility(EggEditableToolbar *etoolbar,
                                         const char *path) {
  etoolbar->priv->visibility_paths =
      g_list_prepend(etoolbar->priv->visibility_paths, g_strdup(path));
}

void egg_editable_toolbar_show(EggEditableToolbar *etoolbar, const char *name) {
  EggEditableToolbarPrivate *priv = etoolbar->priv;
...
void egg_editable_toolbar_hide(EggEditableToolbar *etoolbar, const char *name) {
  EggEditableToolbarPrivate *priv = etoolbar->priv;
  EggToolbarsModel *model = priv->model;
  int i, n_toolbars;
 ...
 */
static GdkPixbuf *new_pixbuf_from_widget(GtkWidget *widget) {
  GtkWidget *window;
  GdkPixbuf *pixbuf;
  gint icon_height;

  if (!gtk_icon_size_lookup(GTK_ICON_SIZE_LARGE_TOOLBAR, NULL, &icon_height)) {
...
static void update_separator_image(GtkImage *image) {
  GdkPixbuf *pixbuf = new_separator_pixbuf();
  gtk_image_set_from_pixbuf(GTK_IMAGE(image), pixbuf);
  g_object_unref(pixbuf);
}

static gboolean style_set_cb(GtkWidget *widget, GtkStyle *previous_style,
                             GtkImage *image) {
