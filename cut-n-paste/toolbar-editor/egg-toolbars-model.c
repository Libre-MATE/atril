...
static xmlDocPtr egg_toolbars_model_to_xml(EggToolbarsModel *model) {
  ...
  for (l1 = tl->children; l1 != NULL; l1 = l1->next) {
    ...
    xmlSetProp(tnode, (const xmlChar *)"hidden",
               (toolbar->flags & EGG_TB_MODEL_HIDDEN)
                   ? (const xmlChar *)"true"
                   : (const xmlChar *)"false");
    xmlSetProp(tnode, (const xmlChar *)"editable",
               (toolbar->flags & EGG_TB_MODEL_NOT_EDITABLE)
                   ? (const xmlChar *)"false"
                   : (const xmlChar *)"true");
    ...
    for (l2 = l1->children; l2 != NULL; l2 = l2->next) {
      ...
      for (l3 = model->priv->types; l3 != NULL; l3 = l3->next) {
        ...
        if (type->get_name != NULL && type->get_data != NULL) {
          ...
          if (tmp != NULL) {
            dnode = xmlNewTextChild(node, NULL, (const xmlChar *)"data",
                                    (const xmlChar *)tmp);
            g_free(tmp);
...
static gboolean safe_save_xml(const char *xml_file, xmlDocPtr doc) {
  char *tmp_file;
  char *old_file;
  gboolean old_exist;
...
static gboolean is_unique(EggToolbarsModel *model, EggToolbarsItem *idata) {
  EggToolbarsItem *idata2;
  GNode *toolbar, *item;

  for (toolbar = g_node_first_child(model->priv->toolbars); toolbar != NULL;
       toolbar = g_node_next_sibling(toolbar)) {
    for (item = g_node_first_child(toolbar); item != NULL;
         item = g_node_next_sibling(item)) {
      idata2 = item->data;
...
static GNode *toolbar_node_new(const char *name) {
...
static GNode *item_node_new(const char *name, EggToolbarsModel *model) {
  ...
  if ((flags & EGG_TB_MODEL_NAME_INFINITE) == 0)
    g_hash_table_insert(model->priv->flags, g_strdup(item->name),
                        GINT_TO_POINTER(flags | EGG_TB_MODEL_NAME_USED));

  return g_node_new(item);
}

static void item_node_free(GNode *item_node, EggToolbarsModel *model) {
...
static void toolbar_node_free(GNode *toolbar_node, EggToolbarsModel *model) {
...
void egg_toolbars_model_set_flags(EggToolbarsModel *model, int toolbar_position,
                                  EggTbModelFlags flags) {
  GNode *toolbar_node;
  EggToolbarsToolbar *toolbar;
  ...
  g_signal_emit(G_OBJECT(model), signals[TOOLBAR_CHANGED], 0, toolbar_position);
}

char *egg_toolbars_model_get_data(EggToolbarsModel *model, GdkAtom type,
                                  const char *name) {
...
char *egg_toolbars_model_get_name(EggToolbarsModel *model, GdkAtom type,
  ...
  if (create) {
    for (l = model->priv->types; name == NULL && l != NULL; l = l->next) {
      t = l->data;
      if (t->type == type && t->new_name != NULL) name = t->new_name(t, data);
    }
    ...
    for (l = model->priv->types; name == NULL && l != NULL; l = l->next) {
      t = l->data;
      if (t->type == type && t->get_name != NULL) name = t->get_name(t, data);
    }
...
static gboolean impl_add_item(EggToolbarsModel *model, int toolbar_position,
  ...
  g_signal_emit(G_OBJECT(model), signals[ITEM_ADDED], 0, toolbar_position,
                real_position);
...
int egg_toolbars_model_add_toolbar(EggToolbarsModel *model, int position,
                                   const char *name) {
  GNode *node;
  int real_position;
...
static char *parse_data_list(EggToolbarsModel *model, xmlNodePtr child,
                             gboolean create) {
  char *name = NULL;
  while (child && name == NULL) {
    if (xmlStrEqual(child->name, (const xmlChar *)"data")) {
      ...
      if (type != NULL) {
        ...
        name = egg_toolbars_model_get_name(model, atom, (const char *)data,
                                           create);
      }
...
static void parse_item_list(EggToolbarsModel *model, xmlNodePtr child,
                            int position) {
  while (child) {
    if (xmlStrEqual(child->name, (const xmlChar *)"toolitem")) {
      ...
      if (name == NULL) {
        ...
        name =
            egg_toolbars_model_get_name(model, atom, (const char *)data, FALSE);
        if (name == NULL) {
          name = egg_toolbars_model_get_name(model, atom, (const char *)data,
                                             TRUE);
        }
...
static void parse_toolbars(EggToolbarsModel *model, xmlNodePtr child) {
  while (child) {
...
gboolean egg_toolbars_model_load_toolbars_from_resource(EggToolbarsModel *model,
  ...
  if (doc == NULL) g_error("Failed to load XML data from resource %s", path);

  root = xmlDocGetRootElement(doc);
  if (root == NULL) g_error("Failed to get root element");
  parse_toolbars(model, root->children);
...
static void parse_available_list(EggToolbarsModel *model, xmlNodePtr child) {
  gint flags;
  ...
  while (child) {
    if (xmlStrEqual(child->name, (const xmlChar *)"toolitem")) {
      ...
      flags = egg_toolbars_model_get_name_flags(model, (const char *)name);
      egg_toolbars_model_set_name_flags(model, (const char *)name,
                                        flags | EGG_TB_MODEL_NAME_KNOWN);
      xmlFree(name);
    }
    child = child->next;
  }
...
static void parse_names(EggToolbarsModel *model, xmlNodePtr child) {
  while (child) {
...
static void egg_toolbars_model_class_init(EggToolbarsModelClass *klass) {
  ...
  signals[ITEM_ADDED] = g_signal_new(
      "item_added", G_OBJECT_CLASS_TYPE(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(EggToolbarsModelClass, item_added), NULL, NULL,
      _egg_marshal_VOID__INT_INT, G_TYPE_NONE, 2, G_TYPE_INT, G_TYPE_INT);
  signals[TOOLBAR_ADDED] = g_signal_new(
      "toolbar_added", G_OBJECT_CLASS_TYPE(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(EggToolbarsModelClass, toolbar_added), NULL, NULL,
      g_cclosure_marshal_VOID__INT, G_TYPE_NONE, 1, G_TYPE_INT);
  signals[ITEM_REMOVED] = g_signal_new(
      "item_removed", G_OBJECT_CLASS_TYPE(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(EggToolbarsModelClass, item_removed), NULL, NULL,
      _egg_marshal_VOID__INT_INT, G_TYPE_NONE, 2, G_TYPE_INT, G_TYPE_INT);
  signals[TOOLBAR_REMOVED] = g_signal_new(
      "toolbar_removed", G_OBJECT_CLASS_TYPE(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(EggToolbarsModelClass, toolbar_removed), NULL, NULL,
      g_cclosure_marshal_VOID__INT, G_TYPE_NONE, 1, G_TYPE_INT);
  signals[TOOLBAR_CHANGED] = g_signal_new(
      "toolbar_changed", G_OBJECT_CLASS_TYPE(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(EggToolbarsModelClass, toolbar_changed), NULL, NULL,
      g_cclosure_marshal_VOID__INT, G_TYPE_NONE, 1, G_TYPE_INT);
}
...
static void egg_toolbars_model_init(EggToolbarsModel *model) {
  ...
  egg_toolbars_model_set_name_flags(
      model, "_separator",
      EGG_TB_MODEL_NAME_KNOWN | EGG_TB_MODEL_NAME_INFINITE);
}
...
void egg_toolbars_model_remove_toolbar(EggToolbarsModel *model, int position) {
  GNode *node;
  EggTbModelFlags flags;
...
void egg_toolbars_model_remove_item(EggToolbarsModel *model,
  ...
  g_signal_emit(G_OBJECT(model), signals[ITEM_REMOVED], 0, toolbar_position,
                position);
}

void egg_toolbars_model_move_item(EggToolbarsModel *model, int toolbar_position,
                                  int position, int new_toolbar_position,
                                  int new_position) {
  ...
  g_signal_emit(G_OBJECT(model), signals[ITEM_ADDED], 0, new_toolbar_position,
                new_position);
}

void egg_toolbars_model_delete_item(EggToolbarsModel *model, const char *name) {
  ...
  while (toolbar != NULL) {
    ...
    while (item != NULL) {
      ...
      if (strcmp(idata->name, name) == 0) {
        ...
        g_signal_emit(G_OBJECT(model), signals[ITEM_REMOVED], 0, tpos, ipos);
      } else {
        ipos++;
      }
...
int egg_toolbars_model_n_items(EggToolbarsModel *model, int toolbar_position) {
  GNode *toolbar;
...
static void fill_avail_array(gpointer key, gpointer value, GPtrArray *array) {
