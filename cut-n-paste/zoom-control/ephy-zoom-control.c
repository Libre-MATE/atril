...
enum { COL_TEXT, COL_IS_SEP };

enum { PROP_0, PROP_ZOOM, PROP_MIN_ZOOM, PROP_MAX_ZOOM };

enum { ZOOM_TO_LEVEL_SIGNAL, LAST_SIGNAL };

static guint signals[LAST_SIGNAL] = {0};

G_DEFINE_TYPE_WITH_PRIVATE(EphyZoomControl, ephy_zoom_control,
                           GTK_TYPE_TOOL_ITEM)

static void combo_changed_cb(GtkComboBox *combo, EphyZoomControl *control) {
  gint index;
  float zoom;
...
static void sync_zoom_cb(EphyZoomControl *control, GParamSpec *pspec,
                         gpointer data) {
...
static void sync_zoom_max_min_cb(EphyZoomControl *control, GParamSpec *pspec,
                                 gpointer data) {
  EphyZoomControlPrivate *p = control->priv;
  ...
  for (i = 0; i < n_zoom_levels; i++) {
    if (zoom_levels[i].level > 0) {
      if (zoom_levels[i].level < p->min_zoom) continue;

      if (zoom_levels[i].level > p->max_zoom) break;
    }

    gtk_list_store_append(model, &iter);

    if (zoom_levels[i].name != NULL) {
      gtk_list_store_set(model, &iter, COL_TEXT, _(zoom_levels[i].name), -1);
    } else {
      gtk_list_store_set(model, &iter, COL_IS_SEP, zoom_levels[i].name == NULL,
                         -1);
    }
  }
...
static gboolean row_is_separator(GtkTreeModel *model, GtkTreeIter *iter,
                                 gpointer data) {
  gboolean is_sep;
...
static void ephy_zoom_control_init(EphyZoomControl *control) {
  ...
  for (i = 0; i < n_zoom_levels; i++) {
    ...
    if (zoom_levels[i].name != NULL) {
      gtk_list_store_set(store, &iter, COL_TEXT, _(zoom_levels[i].name), -1);
    } else {
      gtk_list_store_set(store, &iter, COL_IS_SEP, zoom_levels[i].name == NULL,
                         -1);
    }
  }
  ...
  gtk_cell_layout_set_attributes(GTK_CELL_LAYOUT(p->combo), renderer, "text",
                                 COL_TEXT, NULL);
  gtk_combo_box_set_row_separator_func(
      p->combo, (GtkTreeViewRowSeparatorFunc)row_is_separator, NULL, NULL);
  ...
  g_signal_connect_object(control, "notify::zoom", G_CALLBACK(sync_zoom_cb),
                          NULL, 0);
...
static void ephy_zoom_control_set_property(GObject *object, guint prop_id,
                                           const GValue *value,
                                           GParamSpec *pspec) {
...
static void ephy_zoom_control_get_property(GObject *object, guint prop_id,
...
static void ephy_zoom_control_class_init(EphyZoomControlClass *klass) {
  ...
  g_object_class_install_property(
      ...
      g_param_spec_float("zoom", "Zoom", "Zoom level to display in the item.",
                         ZOOM_MINIMAL, ZOOM_MAXIMAL, 1.0, G_PARAM_READWRITE));
  g_object_class_install_property(
      object_class, PROP_MIN_ZOOM,
      g_param_spec_float("min-zoom", "MinZoom", "The minimum zoom",
                         ZOOM_MINIMAL, ZOOM_MAXIMAL, ZOOM_MINIMAL,
                         G_PARAM_READWRITE));
  g_object_class_install_property(
      object_class, PROP_MAX_ZOOM,
      g_param_spec_float("max-zoom", "MaxZoom", "The maximum zoom",
                         ZOOM_MINIMAL, ZOOM_MAXIMAL, ZOOM_MAXIMAL,
                         G_PARAM_READWRITE));

  signals[ZOOM_TO_LEVEL_SIGNAL] = g_signal_new(
      "zoom_to_level", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(EphyZoomControlClass, zoom_to_level), NULL, NULL,
