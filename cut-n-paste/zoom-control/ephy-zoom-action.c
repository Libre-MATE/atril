...
static void zoom_to_level_cb(EphyZoomControl *control, float zoom,
                             EphyZoomAction *action) {
  g_signal_emit(action, signals[ZOOM_TO_LEVEL_SIGNAL], 0, zoom);
}

static void sync_zoom_cb(GtkAction *action, GParamSpec *pspec,
                         GtkWidget *proxy) {
...
static void sync_min_zoom_cb(GtkAction *action, GParamSpec *pspec,
                             GtkWidget *proxy) {
  EphyZoomAction *zoom_action = EPHY_ZOOM_ACTION(action);
...
static void sync_max_zoom_cb(GtkAction *action, GParamSpec *pspec,
                             GtkWidget *proxy) {
  EphyZoomAction *zoom_action = EPHY_ZOOM_ACTION(action);
...
static void connect_proxy(GtkAction *action, GtkWidget *proxy) {
  if (EPHY_IS_ZOOM_CONTROL(proxy)) {
    g_signal_connect_object(action, "notify::zoom", G_CALLBACK(sync_zoom_cb),
                            proxy, 0);
    ...
    g_signal_connect(proxy, "zoom_to_level", G_CALLBACK(zoom_to_level_cb),
                     action);
  }
  G_GNUC_BEGIN_IGNORE_DEPRECATIONS;
...
static void proxy_menu_activate_cb(GtkMenuItem *menu_item,
                                   EphyZoomAction *action) {
  gint index;
  float zoom;
...
static GtkWidget *create_menu_item(GtkAction *action) {
  ...
  for (i = 0; i < n_zoom_levels; i++) {
    if (zoom_levels[i].level == EPHY_ZOOM_SEPARATOR) {
      ...
      menu_item =
          gtk_radio_menu_item_new_with_label(group, _(zoom_levels[i].name));
  ...
  G_GNUC_BEGIN_IGNORE_DEPRECATIONS;
  menu_item =
      GTK_ACTION_CLASS(ephy_zoom_action_parent_class)->create_menu_item(action);
  G_GNUC_END_IGNORE_DEPRECATIONS;
...
static void ephy_zoom_action_set_property(GObject *object, guint prop_id,
                                          const GValue *value,
                                          GParamSpec *pspec) {
  EphyZoomAction *action;
...
static void ephy_zoom_action_get_property(GObject *object, guint prop_id,
...
static void ephy_zoom_action_class_init(EphyZoomActionClass *class) {
  ...
  g_object_class_install_property(
      ...
      g_param_spec_float("zoom", "Zoom", "Zoom", ZOOM_MINIMAL, ZOOM_MAXIMAL,
  ...
  signals[ZOOM_TO_LEVEL_SIGNAL] = g_signal_new(
      "zoom_to_level", G_OBJECT_CLASS_TYPE(object_class), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(EphyZoomActionClass, zoom_to_level), NULL, NULL,
...
void ephy_zoom_action_set_min_zoom_level(EphyZoomAction *action, float zoom) {
  g_return_if_fail(EPHY_IS_ZOOM_ACTION(action));
...
void ephy_zoom_action_set_max_zoom_level(EphyZoomAction *action, float zoom) {
  g_return_if_fail(EPHY_IS_ZOOM_ACTION(action));
